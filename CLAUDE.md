...
- **Run bare `cargo`/`cargo fmt`/`cargo clippy` — never an explicit `+toolchain`, except a DECLARED cross-toolchain comparison (say so at the call site); otherwise it silently overrides the pin below.** → [`run-bare-cargo-the-toolchain-is-pinned`](docs/method-rules.md#run-bare-cargo-the-toolchain-is-pinned)
- **`rust-toolchain.toml` is the single source of truth; an explicit `+toolchain` overrides it (rustup precedence 1 over 4)** — silently pinning you to a stale compiler the day the file bumps. → [`run-bare-cargo-the-toolchain-is-pinned`](docs/method-rules.md#run-bare-cargo-the-toolchain-is-pinned)
- **Confirm `rust-toolchain.toml` is on disk before trusting bare `cargo`** — a checkout that predates it defaults to nightly, not the pin. → [`run-bare-cargo-the-toolchain-is-pinned`](docs/method-rules.md#run-bare-cargo-the-toolchain-is-pinned)
...
- **A struct-field add needs `--all-targets` on every CONSTRUCTING crate** — find them by grepping the struct name, not by intuition. → [`a-struct-field-add-breaks-constructing-crates`](docs/method-rules.md#a-struct-field-add-breaks-constructing-crates)
- **Unless every field is private** — then external construction is impossible by Rust's visibility rules, and the defining crate's own gate covers the change completely. → [`a-struct-field-add-breaks-constructing-crates`](docs/method-rules.md#a-struct-field-add-breaks-constructing-crates)
