...
- **Never panic on production paths.** `Result` from day one. (Standing violations to fix: `Tensor::from_*` `.expect()` at `fuel-graph/src/lib.rs:~2256`; the three panicking `decompose`s — `nf4_matmul.rs:120`, `flash_attn`, `selective_scan` — which are recipe-principle bugs, not a permanent category.)
