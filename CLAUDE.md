...
- **CUDA builds:** read `Cargo.lock` for baracuda's version (the manifest's `alpha.78` is a caret minimum); build through `vcvarsall amd64` (`C:\Program Files\Microsoft Visual Studio\18\Community\VC\Auxiliary\Build\vcvarsall.bat`; harvest its env in a throwaway `cmd /c "vcvarsall amd64 && set"` — never on a Git-Bash command line, which mangles `/c` and the quotes — and never hand-set `NVCC_CCBIN`); set **`BARACUDA_FORGE_THREADS=6` on any `--features cuda` OR `--workspace` build** (the forge runs unconditionally); prepend `C:\Program Files\NVIDIA\CUDNN\v9.23\bin\13.3\x64` to PATH to launch cuda test exes (else `0xc0000135`); take a build slot through `scripts/cuda-build.ps1`. Grep baracuda-kernels-sys, not the plan facade, for kernels. → [`the-baracuda-dependency-and-the-cuda-build-recipe`](docs/method-rules.md#the-baracuda-dependency-and-the-cuda-build-recipe)
