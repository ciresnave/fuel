...
2. **fuel-core / fuel-core-types retirement → `fuel-ir` + `fuel-hardware`** (foundation refactor; scoped, sibling-safe). `fuel-core-types` (the vocabulary) → `fuel-ir`; hardware discovery (the `SystemTopology` split: discovery vs dispatch-overlay) → a new `fuel-hardware` crate; the dispatch overlay stays in `fuel-dispatch`. Sequenced inside the cleanup (Step B0) because both names collide on crates.io before publish. See the memory `fuel-core-retirement`.
