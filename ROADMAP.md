...
## Current frontier (last full pass 2026-08-26; item 11 added 2026-09-02; item 13 added 2026-09-08)

> ⚠️⚠️ **DO NOT RENUMBER THIS LIST AND DO NOT REMOVE AN ITEM FROM IT. THE NUMBERS ARE LOAD-BEARING CITATIONS.**
>
> Measured at `785b6ecc` with `git grep -lIE 'ROADMAP item [0-9]+' -- '*.md' '*.rs'`: **31 files, 90 citations.** By item: 8×76, 9×7, 12×3, 7×2, 10×2. The bulk is `ROADMAP item 8 (II)` across ~24 model files in `fuel-transformers/src`, plus `fuel-model-core` (lib + registry test), three `docs/architecture/` files, `docs/restructure-migration-design.md` and `docs/gaps.md`.
>
> **Five items below are marked SHIPPED / RECORD / STRUCK and four say "MOVE TO ...". Taking that literally removes entries from a NUMBERED list, which renumbers every later item: 2,3,5,6,7,8,11 become 1..7 and item 8 becomes item 6 — repointing seventy-six citations at the wrong item.**
>
> ⚠️⚠️ **AND THREE OF THE FIVE ITEMS MARKED FOR REMOVAL ARE THEMSELVES CITED: items 9, 10 and 12 carry 12 citations between them.** Moving those does not merely renumber their neighbours — it breaks the references pointing AT them.
>
> ⚠️ **The citations live in CODE COMMENTS, so nothing checks them and nothing would fail.** A wrong item number does not dangle — it points at a REAL item that is not the one meant, and reads as correct. Same asymmetry as a stale line number: a wrong path fails loudly, a wrong number misleads quietly.
>
> **So a marked item STAYS IN PLACE, keeping its number, and its marker IS the whole remedy.** The marker-at-the-opener already stops mis-dispatch — that was the fix and it works. If this list is ever genuinely reorganised, the numbers must be retired as citation anchors FIRST (give each item a stable slug, migrate the citing files), and only then may the list be re-ordered.
>
> *Stated with the command that produced it because the figure is not stable under rephrasing: a looser pattern that also matches "ROADMAP **frontier** item N" finds one more file. Citations here are prose and have no canonical form — which is the deeper reason numbers are a poor anchor, and why a slug migration would have to enumerate the PHRASINGS, not just the numbers.*
>
> *Found 2026-09-06 by the architect while about to execute the audit's own instruction. The instruction was not careless: it prescribed a reorganisation without checking whether anything addressed the list by position, and nothing in this file recorded that anything does.*
...
8. **⚠️ OPENER CORRECTED 2026-09-05 (ROADMAP currency audit, verified at `921e3a87`): the headline below is STALE — `fuel-model-core` EXISTS (`trait Model`, `AutoModel::from_path`, an `inventory` registry; built at `96e1c5de`), and the body already supersedes the "does not exist" claim at the `292ef3df` marker further down. Read from that marker, not from the opening sentence. LIVE SCOPE per the 2026-09-02 ruling: (I) the registry is NOT the seam; (II) CONFIG-FROM-PATH is the work — batches 1+2 shipped (#84/#94), a ~12-model tail remains (Fuel 1). This correction sits at the opener because a reader dispatches off the opener and a list renders the title.** **The model-construction layer is missing, and every model pays for it — measured 2026-08-27, surfaced by CireSnave asking how a developer loads a NEW model.** **02-layers v0.5 ratifies exactly this layer as `fuel-model-core` (the `Model` trait, the `model_type`/`general.architecture` → builder registry, `AutoModel::from_path`, link-time distributed registration). THE CRATE DOES NOT EXIST**, and neither does any part of what it specifies. **Measured at `90cb5265`, with controls:** `trait Model` **0** · `AutoModel` **0** · registry / `inventory` / `linkme` **0** *(control: 100 files contain `trait `, so the query finds traits where they are)*. **What exists instead is the hand-rolled cross-product:** **399** distinct `*Weights` structs · **148** distinct `*Config` structs · **27** separate definitions of `from_hf_json_str`, all in `fuel-core`. **`LlamaFullConfig` + `build_llama3_model` are not an EXAMPLE of the pattern — they ARE the pattern, 148 times over.** ⚠️⚠️ **SUPERSEDED 2026-09-02 AT `292ef3df` — THE CENTRAL CLAIM ABOVE IS NOW FALSE AND EVERY NUMBER IN IT IS STALE. `fuel-model-core` EXISTS** (built at `96e1c5de`): `trait Model`, `AutoModel::from_path`, and an `inventory` registry are all present. **The premise moved TWICE — the crate was built, and Stage 2 then relocated the duplication it was written about.** Re-measured at head, constructs named: **`pub struct *Weights` DEFINITIONS 439** (was 399) — **`pub struct *Config` DEFINITIONS 169** (was 148) — **`fn from_hf_json_str` DEFINITIONS 34** (was 27), of which **30 are now in `fuel-transformers` and 4 in `fuel-core`**, because Stage 2 moved 146 model files out. **A number attached to `fuel-core` in the sentence above is a number about a crate that no longer holds the subject.** ⚠️ **AND THE CRATE EXISTING IS NOT THE CRATE WORKING: `impl Model for` yields **3 TYPES, AND THEY ARE SCAFFOLDING BY MEASUREMENT RATHER THAN BY CHARACTERISATION: `LocalModel` (`fuel-model-core/tests/registry.rs:43`), `FixtureModel` (`fuel-model-fixture/src/lib.rs:26`), `UmbrellaModel` (`fuel-model-umbrella-fixture/src/lib.rs:29`)** — **one lives in a `tests/` directory and two in crates whose names END IN `-fixture`.** **ZERO production models implement it and the zoo is not wired.** **TRIAL, run by Fuel 1 2026-09-02 (throwaway branch, nothing landed): `impl Model` + a `build(&ModelSpec)` for Qwen2, trait and model unmodified. IT COMPILES CLEAN, AND THAT IS THE TRAP RATHER THAN THE RESULT.** ⚠️ **`trait Model` is ONE method — `architecture(&self) -> &str` — so the impl is a hardcoded string literal and it compiles for ANY model. A test that supplies the value it claims to check tells you nothing about what production computes.** ⚠️⚠️ **THE LOAD-BEARING FINDING: `build()` COMPILES AND IS SEMANTICALLY BROKEN.** `ModelSpec` is `{path, architecture}` only. `Qwen2Config` derives `Debug, Clone, PartialEq` — **no `Deserialize`, no `from_hf_json_str`** — and is constructible ONLY from hardcoded presets (`qwen2_7b()`). **So the builder cannot parse `spec.path/config.json`; it is forced to a preset, IGNORING THE ARTIFACT IT WAS HANDED — correct for a 7B checkpoint and wrong for every other Qwen2. It is GREEN AND UNABLE TO HONOUR ITS OWN INPUT.** *(Confirmed independently; note a path glob `*qwen2*` hits `lazy_qwen2_moe.rs`, a SIBLING that does have a parser, and reads as a refutation. Control: bert returns 4 hits.)* **NON-UNIFORM: ~30 of 146 models have a parser; ~116 including Qwen2 do not.** ⚠️⚠️ **AND THE FACT THAT DECIDES THE PROGRAM — FUEL HAS TWO MODEL TRAITS WITH OPPOSITE DEFECTS.** `fuel-model-core::Model` is **UNIVERSAL AND EMPTY** (1 method, 3 fixture impls). **`fuel-inference::multi_session::DecodeModel` (`:79`) is CAPABLE AND NARROW** — it carries `n_layers`, `layer_state_specs`, **`forward_with_kv_context_persistent`**, `supports_batched_decode`, `build_batched_decode_logits` — and **`impl DecodeModel for` yields 5 TYPES against a zoo of 434.** **`Box<dyn Model>` is INFERENCE-DEAD: `forward` lives on the concrete type by design (*"forward IS the architecture"*), so `AutoModel::from_path` returns a handle you can only ask `architecture()` of, and running it requires a downcast — which is the match statement the registry was meant to remove, relocated into a type.** ⚠️ **Universal-and-capable is NOT AVAILABLE, because model inputs genuinely vary across encoder / vision / multimodal. That is a constraint, not a gap.** **RULED 2026-09-02. (I) IS NOT THE SEAM — item 8 no longer proposes wiring the zoo to it.** **(II) CONFIG-FROM-PATH IS THE WORK AND IT STANDS ALONE:** the `from_hf_json_str` convention is proven, the work is mechanical and gateable, and the value does not depend on `Model` existing. ⚠️⚠️ **NUMBERS UPDATED 2026-09-05 — THE "PROVEN ON 30, REMAINING ~116" PAIR PREDATES THE SWEEP AND BOTH HALVES WERE WRONG ABOUT THE POPULATION.** **Measured at `28b2a43c`: 27 models carry `fn from_hf_json_str`; the gate's `NOT_YET_PARSED` holds 12; `STRUCTURAL_EXEMPT` holds 2.** The "~116" ranged over every model file rather than the config-bearing subset — **Fuel 1 enumerated 123 config-bearing models, of which most carry no kv field and are OUT OF THE DOMAIN, not pending.** ⚠️ **SHIPPED: batch 1 (`mistral`, `mixtral`, `stablelm`, `qwen3` — #84, four shapes chosen to stress different parts of the template) and batch 2 (11 models including `glm4` — #94).** The template held unchanged across dense, MoE, partial-rotary and explicit-`head_dim` shapes. ⚠️⚠️ **THE REMAINING 12 ARE A SEPARATE PROGRAM, AND MEASURING THEM CONTRADICTS BOTH EARLIER CHARACTERISATIONS.** They were handed on as *"complex tail, each needs model-specific care, not the mechanical template"*. **Measured at `28b2a43c`, config STRUCTURE says otherwise: ELEVEN OF TWELVE ARE FLAT CONFIGS of 12-19 fields — the same structural shape as the 27 already parsed.** Only **`voxtral`** carries NESTED sub-configs and therefore a genuinely different parse shape; **`z_image`** is a size outlier at 35 fields and still flat. **All 12 have `vocab_size` and a kv field, so all 12 are IN the domain** *(the `gemma4_vision` exclusion — `vocab_size` 0, a ViT tower whose config is a sub-object of a multimodal parent — reaches none of them)*. ⚠️ **SO THE PER-MODEL WORK IS NOT STRUCTURAL AND THAT IS THE SCOPING FACT: it is identifying ARCHITECTURE-DEPENDENT FIELD DEFAULTS, which is not statically measurable and is why the tail cannot be swept blind.** **The worked example is `glm4`: HF's `glm` configs OMIT `partial_rotary_factor` and mean 0.5, while Fuel's doc comment claimed 1.0 *"(default for GLM-4-9B)"* — false about the model it named, and the artefact that made a one-line falsehood look like a two-horned dilemma.** **Each remaining model needs that question asked of its own fields; the PARSING is the same work as before.** **Ordering when it is taken: the ten flat non-outliers first (the template applies unchanged), `z_image` next on size alone, `voxtral` LAST because a nested sub-config is the only structural novelty in the set.** ⚠️ **NOT STARTED, UNALLOCATED, AND NOBODY IS BLOCKED ON IT — recorded here so a named successor does not read as a handled one.** **Build it as a capability of THE CONFIG TYPES, never as a service to a registry, so it survives whatever happens to (I).** **(III) `WeightSource` sequences behind (II).** **The born-red is Fuel 1's own broken case: a builder that ignores `spec.path` and returns a 7B preset must FAIL ON VALUE against a real non-7B `config.json`** — a POSITIVE golden, not a `≠ preset` relative oracle, since the latter passes for garbage that merely differs. **Assert `num_key_value_heads` explicitly: a parser defaulting it to `num_attention_heads` yields a structurally different model while every derived accessor still returns something plausible.** **EMPIRICAL POSITIVES, measured rather than assumed:** `Model: Send + Sync` **HOLDS** for a Tensor-holding model — the uncertain break did not occur; and the **orphan rule forces the impl into the model's own crate**, so wiring lands in `fuel-transformers` or a per-model leaf, never a standalone wiring crate (`E0117`). **NOT VERIFIED and stated as such: the bert (parser-present) case, and encoder / vision / multimodal / GGUF fit.** **The `96e1c5de` linkage defect — registration needs a link-forcing line — is orthogonal and stands.** ⚠️⚠️ **RE-SCOPED 2026-09-05 BY MEASURING THE ARTIFACTS. THE SCOPING ABOVE IS WRONG AND THE ERROR IS IN WHICH CONSTRUCT IT COUNTED.** **It says ELEVEN OF TWELVE are flat and the template applies unchanged. Measured against the real HF `config.json` for every one: THREE of twelve are cleanly parseable.** ⚠️⚠️ **THE MEASURED CONSTRUCT WAS `pub` FIELDS IN A RUST STRUCT. THE CONSUMED CONSTRUCT IS KEYS IN A `config.json`. THEY ARE DIFFERENT OBJECTS, AND ONE WAS USED TO LICENSE CLAIMS ABOUT THE OTHER — IN A SCOPING DOCUMENT A LANE THEN WORKED FROM.** **STRUCT-FLATNESS IS A POOR PREDICTOR OF ARTIFACT-PARSEABILITY: 3 of 12.** **SHIPPED, and this is the whole mechanical tail: `qwen3_moe`, `gemma3`, `gemma`. Gate NOT_YET_PARSED 12 — 9.** ⚠️ **THE OTHER NINE ARE FOUR DISPOSITIONS, NOT A REMAINDER, and each was measured against its real artifact:** **OUT OF SCOPE, NO ARTIFACT (3) — `metavoice` (its entire `config.json` is `{"model_name": "metavoice-1B-v0.1"}`), `z_image` (a DIFFUSION PIPELINE: `transformer/` `text_encoder/` `vae/` `scheduler/` subdirs and a `model_index.json`, no top-level config; it entered the cluster via a `TextEncoderConfig` Qwen3 backbone), `gemma4_text` (no public config exists).** **A parser here would be a guard with nothing behind it and a born-red would test our own fixture. Same shape as `gemma4_vision`: in the domain by predicate, out of it in fact. DO NOT SYNTHESISE ARTIFACTS.** **HELD, PORT-INCOMPLETE (1) — `granitemoehybrid`. The real config is 35 Mamba + 5 attention layers and the port PANICS on Mamba (*"Reserved for future Mamba expansion"*), so a parser would produce a `Config` the model cannot run on 87% of its layers.** **The blocker is the PORT, not the parser; parsing it builds a door into a room that does not exist, and the green would read as capability.** **NESTED ARTIFACT (3) — `qwen3_vl_text` (all text fields under a `text_config` sub-object), `paddleocr_vl_text` (`mrope_section` under `rope_scaling`), `voxtral`. PARSEABLE, but they need a nested-config shape the template does not have — a template EXTENSION, not an application, and it needs its own validation batch exactly as batch 1 did.** **KEY/SHAPE MISMATCH (2) — `lfm2` (`conv_L_cache` not `conv_kernel_size`, `block_ff_dim` not `intermediate_size`, `full_attn_idxs` not `block_types`, and no `head_dim` key at all) and `recurrent_gemma` (`_block_types` is a 3-element PATTERN where the struct wants a per-layer Vec). Each needs a per-model MAPPER, which is semantic translation rather than parsing, and each mapper is a claim about HF's format needing its own golden.** ⚠️ **SO: the mechanical sweep is COMPLETE at 3. The remainder is THREE follow-on programs plus FOUR permanent exclusions — NOT "nine models left in this sweep", which is how the earlier number would be read.** **The follow-ons are UNSCOPED and UNALLOCATED; nobody is blocked on them.** **Build it as a capability of THE CONFIG TYPES, never as a service to a registry, so it survives whatever happens to (I).** **(III) `WeightSource` sequences behind (II).** **The born-red is Fuel 1's own broken case: a builder that ignores `spec.path` and returns a 7B preset must FAIL ON VALUE against a real non-7B `config.json`** — a POSITIVE golden, not a `≠ preset` relative oracle, since the latter passes for garbage that merely differs. **Assert `num_key_value_heads` explicitly: a parser defaulting it to `num_attention_heads` yields a structurally different model while every derived accessor still returns something plausible.** **EMPIRICAL POSITIVES, measured rather than assumed:** `Model: Send + Sync` **HOLDS** for a Tensor-holding model — the uncertain break did not occur; and the **orphan rule forces the impl into the model's own crate**, so wiring lands in `fuel-transformers` or a per-model leaf, never a standalone wiring crate (`E0117`). **NOT VERIFIED and stated as such: the bert (parser-present) case, and encoder / vision / multimodal / GGUF fit.** **The `96e1c5de` linkage defect — registration needs a link-forcing line — is orthogonal and stands.**
...
11. **The crate restructure — Stage 1 SHIPPED 2026-09-02, ⚠️⚠️ **STAGE 2 LANDED 2026-09-02 (#50) — THIS LINE READ "IN FLIGHT" UNTIL 2026-09-03 AND WAS STALE FOR THE WHOLE OF THAT WINDOW.** **Measured at `origin/main` `2bde6d35`: 146 `lazy_*` files under `fuel-transformers/src/models/`, and exactly ONE (`lazy_latent_cache`) left in `fuel-core/src/` — which is the `147 − 1 = 146` carve-out this item itself predicts, so the item contains its own confirmation.** *(Control: 153 `.rs` files under `fuel-transformers/src/`, so the query sees files where they are.)*; and this program was MISSING from this frontier entirely until now, which is an instance of the very class items 9 and 10 catalogue.** The design is [`docs/restructure-migration-design.md`](docs/restructure-migration-design.md); §5 pins the four-stage ordering, and 02-layers v0.5 already ratified the destination, so nothing here re-opens it. **Recorded because ROADMAP is *the path*: a multi-stage structural program with a landed stage and no line on the frontier is doc-vs-code drift of exactly the kind item 10 was filed to catalogue.**
...
12. **⚠️ STRUCK 2026-09-05 (ROADMAP currency audit, verified at `921e3a87`) — RESOLVED, no live work remains.** The cargo cycle is broken (`fuel-transformers/Cargo.toml` has no `fuel` facade dep; Stage 2 landed as **#50** and the facade now re-exports the model zoo — see `fuel-core/Cargo.toml:21-27`), and the layer-violation remainder is empty: at head only `fuel-onnx` and `fuel-tensor-tools` still carry a `fuel =` facade dep, both **leaf consumers** ("consuming the facade is what the facade is FOR"); the other six cleared. Body kept below as the audit record. ~~TEN crates depend on the CONSUMER FACADE — THREE of them are LAYER VIOLATIONS, TWO cannot be classified because the layer model never placed them, and one of them is a hard cargo CYCLE that blocks Stage 2 — measured 2026-09-02 at `a744417e`.~~ The facade `fuel` exists so *consumers* have one import. **A Models-tier or Foundation-adjacent crate depending on it is a layering inversion**, and `fuel-transformers` depending on `fuel` makes it **structurally impossible for the facade to re-export the 147 models Stage 2 is about to move into it** — `facade → fuel-transformers → facade`. **No re-export trick avoids it; a `pub use` needs the dependency edge.**
...
13. **`fuel-cognitive` — a consumer-side crate for cognitive-science research, and the three MECHANISMS it is gated behind (2026-09-08).** **CireSnave, 2026-09-08: *"Get all of it worth building written into our plans where it should be. Since I, personally, am looking forward to performing cognitive research using Fuel, this is something I think we should eventually build."*** Provenance: a cognitive-science researcher was asked to evaluate Fuel's potential for cognitive research and read `docs/architecture/` end-to-end, producing an 11-section technical proposal. This item records what survived triage, what did not, and **the order** — because the crate is the LAST step and the proposal presents it as the first.
    **THE PRECEDENT IS REAL AND MEASURED, so this is not a new category.** `fuel-inference` (16 `.rs`) and `fuel-training` (3 `.rs`) are both `[workspace.members]` AND `default-members` AND path deps, and both declare `//! **Layer**: … | **Stability**: experimental` at `src/lib.rs:4`. **They are consumer-side POLICY shipped inside Fuel with sane defaults, which is exactly the shape asked for.** The seam is unchanged: Fuel is mechanism, the consumer crate is policy; a separate layered crate SEPARATES those, it does not merge them.
    ⚠️ **DO NOT BUILD THE FACADE FIRST. A facade over absent mechanisms is a crate of `todo!()`,** and the proposal's own §10 assumes machinery that does not exist. **Sequenced work, in dependency order:**
    **(a) MULTI-CARRY `Op::Scan` — the highest-leverage item, and it is Fuel's OWN deferral, not an outside ask.** `fuel-graph/src/lib.rs`'s doc comment reads *"Single carry tensor in v1"*; the researcher independently named it *"the single highest-leverage primitive for cognitive modeling"*. **Two derivations, no contact, opposite directions.** Tracked as **GAP-303, which is THREE rows**: the `fuel-graph` change (`n_carries: usize`, symmetric with the existing `n_xs` — a FIELD add, not a variant add, so the EXHAUSTIVE-BY-DESIGN workspace rule does not apply and Metal is a measured non-participant); the pre-existing bundle-composer / N-slot realization gap; and ⚠️ **`fuel-kernel-seam-types`'s canonical body, where `n_carries` is FORCED BY INJECTIVITY** — `canonical.rs` records KISS-OPS-6.19, in which an arm reading too few attrs made `Gather` on axis 2 and axis 0 emit IDENTICAL bytes, and a 1-carry and 3-carry `Scan` would collapse the same way. **That third row forces a version bump on `fuel-kernel-seam-types`.** ⚠️ **BARACUDA'S ANSWER TO THE PRE-LAND ASK, measured on their side at `a5b40a4f` and recorded here so nobody re-derives it: they emit ZERO `OpTag::Scan` bodies** *(control: `Add`/`Div`/`Gelu`/`Gt`/`Iota`/`Mul`/`Relu`/`Where` DO appear, so the query discriminates)*, **so the emitted-body blast radius is ZERO and this is Fuel-internal.** **They identify kernels by hashing the `Debug` representation of the seam tree rather than parsing a canonical body.** ⚠️ **A FIRST READING OF THAT — that a Rust-field add would silently re-key every baracuda JIT id — WAS RETRACTED BY BARACUDA ON MEASUREMENT AND IS RECORDED HERE SO IT IS NOT RE-DERIVED: their registry is an in-memory, per-synthesizer, take-once map with no persistence, so an id is minted and consumed inside ONE process. There is no cross-run cache, nothing to invalidate, and the functional impact of a `Debug` change on their side is NIL.** ⚠️ **WHAT IS LIVE IS SHARPER AND IS OUR SHAPE AGAIN: that id's INJECTIVITY is load-bearing by their own doc — a collision silently overwrites a not-yet-taken kernel — and it rests on `Debug` being injective, which `Debug` promises nothing about. Measured HERE: `fuel-kernel-seam-types` has ZERO manual `Debug` impls** *(control: 9 `#[derive(…Debug…)]`)*, **so every field prints and it is injective TODAY BY ACCIDENT, with nothing asserting it.** **And the filable item is neither of those: `OpAttrs` derives `Clone, Debug, Default, PartialEq` and NOT `Hash`/`Eq`, while its sibling `OpTag` derives both — so hashing a rendered string is a WORKAROUND FOR A MISSING TRAIT IMPL, and the workaround is what kept the absence unfileable. A future `fuel-kernel-seam-types` should consider `Hash` + `Eq` on `OpAttrs`; small, not urgent, NOT a GAP-303 dependency.** **The axis they asked for is a caret-incompatible bump of `fuel-kernel-seam-types`, which their CI compiles and tests under `--features seam` — an axis they cannot miss, rather than an FKC-side pin they would have to learn to read.** ⚠️ **AND A NAME COLLISION TO CARRY INTO ANY SIZING: baracuda has `Access::Scan` / `Schedule::Scan` / `emit_scan` (27 hits) — that is a PREFIX-SCAN (cumsum over a contiguous axis), an unrelated construct with no carries. Grepping their tree for `Scan` finds a live implementation and reads as seam involvement.**
    **(b) A STRICT REPRODUCIBILITY MODE.** The substrate exists — `bit_stable_on_same_hardware` (257 `*.rs` occurrences) and `PrecisionGuarantee` (407) — so the ask is a *policy* surface that refuses any non-bit-stable arm rather than new numerics. **Cheapest of the three and the one a researcher notices first.**
    **(c) EXTENSIBLE `Judge` COST DIMENSIONS.** `Judge` is real (435 `*.rs` occurrences); the ask is that a consumer add a cost dimension without patching Fuel. **Design it as a mechanism (an open dimension set) so the cognitive-specific dimensions live in `fuel-cognitive`, not here.**
    ⚠️ **DECLINED, WITH THE REASON, SO NOBODY RE-DERIVES THEM. Measured at `218b4395` with `git grep -c -I -w` over `*.rs` and `docs/*.md`; controls `NodeHandle` 713 `.rs` and `FusedOpRegistry` 27 `.rs` are nonzero, so the zeros below are ABSENCE and not a broken query.** The *programmable optimization rule engine* is built on `OptimizationMap` (**0 `.rs` / 23 `docs/*.md`**) and `DecompositionMap` (**0 / 20**); *cognitive chunk discovery from pattern harvest* is built on `FusionMissRecord` (**0 / 34**). **Those three are DOC-ONLY NAMES — the mechanisms exist under other names, so this is drift, and it is tracked as GAP-302.** Separately, `StrategySpace`, `Intervention` and `CalibrationEngine` measure **0 `.rs` AND 0 `.md`** — they are the proposal's own inventions rather than anything Fuel ever claimed, and they are declined on merit, not on drift. **The two categories have different remedies and must not be collapsed.**
    ⚠️⚠️ **THE FINDING THAT OUTLIVES THIS ITEM, AND IT IS WHY GAP-302 IS TIER B RATHER THAN HYGIENE: THE RESEARCHER'S ACCURACY TRACKED EXACTLY WHETHER A DOC CLAIM HAD CODE BEHIND IT.** Everything they said about `Op::Scan` (221 `.rs` occurrences), `Op::Branch`, `PrecisionGuarantee` and `bit_stable_on_same_hardware` was CORRECT; **every error they made was in the doc-only layer.** **A backticked name in a constitution-tier document reads as an API, and nothing in the text marks one as unimplemented** — so this cost an outsider real design work, silently, and it will recur. **The remedy is a two-direction gate, not a sweep: a sweep fixes the names and leaves the class intact.**
    **GATING, stated so it cannot be read as sequencing advice: (a) needs the Baracuda agreement before ANY of its three rows land. (b) and (c) are independent of (a) and of each other. The `fuel-cognitive` crate itself is gated behind all three, and lands marked `**Stability**: experimental` like its two siblings.** **No consumer is not a reason to skip a capability — it IS a reason to sequence it behind things with consumers, and this item is sequenced accordingly.**
...
- **Recipe-grammar convergence — Increment A SHIPPED (2026-07-16); shape-oracle SHIPPED as
  ...
  *(Update 2026-07-23: the four **source-op leaf BYTE ARMS** — `const{bits}` with the MBZ
  ...
  declines THREE of them (`Const`/`RuntimeScalar`/`ReducedCount`) as honest misses and
  RECONSTRUCTS `ScanPlaceholder` via an explicit arm, so the remaining leaves' graph
  wiring stays slices 2–5 work.)* See
...
**⚠️ STALE-NOTE (2026-09-05 currency audit, verified at `921e3a87`): the gate named below is RESOLVED — the *data-determined* half of symbolic extents (`DynScalar`/`SymEnv`) has SHIPPED, so SSM/MoE/MLA no longer gate on it. Low risk (a note is not schedulable — nobody picks up a note); one-line update, not a strike.** **Keystone:** SSM decode, MoE sparsity, and MLA cache all gate on the *data-determined*
...
1. **Clause↔test traceability + build-fail gate — ⚠️ STILL-LEADS, ADOPT THE MECHANISM NOT THE NUMBER (2026-09-05, KISS confirms, measured at KISS `f43a9bef`): `kiss_trace.py` + `COVERAGE_FLOOR.tsv` + a blocking traceability job are live, but a clause can name exactly ONE test, so the figure measures NAMING not COVERAGE and diverges BOTH ways (under-credits a clause covered by tests it cannot name; over-credits one named by a test that cannot fail). Adopt the wiring; do not treat the number as coverage.** — give every FKC §10 rule + FDX V1–V21
...
4. **Determinism-class-selected verify comparators — ⚠️ LIVE, with a caveat (2026-09-05, KISS #421, measured at KISS `f43a9bef`): KISS defines the COMPARATOR and NOT the METRIC ("ULP" appears 161× and how ULP distance is computed is never defined; three defensible metrics disagree on `+0.0` vs `-0.0`). Verbatim from the KISS architect, true at KISS `f43a9bef`: "you will be adopting kiss-ref's `ulp_distance` by default, because it is the one that shipped — know that you are CHOOSING it rather than INHERITING a standard." If KISS later defines the metric, that quote goes wrong, which is why the ref is attached.** — select the verify comparator from
...
5. **Oracle independence + edge-case corpus in verification — ⚠️ STILL-LEADS IN TEXT (2026-09-05, KISS confirms, measured at KISS `f43a9bef`).** — require the differential
...
6. **⚠️ BLOCKED-ON-EXTERNAL — DO NOT ADOPT AS WRITTEN (2026-09-05, KISS #414, measured at KISS `f43a9bef`).** The axis IS specified (§6.8-0004 mandates importing `MathPrecision` from KISS-Ops and FORBIDS modelling it as a dtype) — but `kiss-ops-vocab 0.1.0` contains ZERO `MathPrecision`, while a same-named DTYPE-shaped one IS published in `unpopped-vocab`, is the obvious `use`, and COMPILES; adopting this as written lands on the forbidden enum with no diagnostic (Baracuda is in that state today). Fuel is currently clean but by LUCK, not a guard — we carry `GemMathPrecision {BitStable, ReducedMantissa}` (behaviour-shaped, `structure_key_derive.rs:74`) and depend on `kiss-ops-vocab` not `unpopped-vocab`; that goes silent the day someone adds `unpopped-vocab` for an unrelated reason. **`MathPrecision` reduced-mantissa axis** — an orthogonal `{bit-stable,
