...
use fuel::quantized::GgmlType;
use fuel::{CpuStorage, Device, Layout, Module, Result, Shape, Tensor, D};
...
impl fuel::CustomOp1 for Im2Col {
...
impl Benchmark for QMatMul {
    type PreProcessData = (fuel::quantized::QMatMul, Tensor);
    type RunResult = Tensor;
    fn preprocess() -> Result<Self::PreProcessData> {
        let zeros = vec![fuel::quantized::k_quants::BlockQ4_0::zeros(); 4096 * 11008 / 32];
        let mm = fuel::quantized::QTensor::new(
            fuel::quantized::QStorage::Cpu(Box::new(zeros)),
            (4096, 11008),
        )?;
        let mm = fuel::quantized::QMatMul::from_qtensor(mm)?;
...
impl Benchmark for Softmax {
    ...
    fn run_one(d: &Self::PreProcessData) -> Result<Self::RunResult> {
        fuel_nn::ops::softmax(d, D::Minus1)
    }
...
impl Benchmark for SoftmaxLastDim {
    ...
    fn run_one(d: &Self::PreProcessData) -> Result<Self::RunResult> {
        fuel_nn::ops::softmax_last_dim(d)
    }
