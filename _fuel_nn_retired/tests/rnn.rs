...
use fuel::{test_utils::to_vec2_round, DType, Device, Result, Tensor};
use fuel_nn::RNN;
...
fn lstm() -> Result<()> {
    ...
    .into_iter()
    .collect();
    let vb = fuel_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let lstm = fuel_nn::lstm(2, 3, Default::default(), vb)?;
...
fn gru() -> Result<()> {
    ...
    .collect();
    let vb = fuel_nn::VarBuilder::from_tensors(tensors, DType::F32, cpu);
    let gru = fuel_nn::gru(2, 3, Default::default(), vb)?;
