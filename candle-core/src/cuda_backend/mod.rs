//! Thin delegation layer to the `candle-cuda` crate.
//!
//! All CUDA types, kernel implementations, and device management live in
//! `candle_cuda`. This module re-exports them and provides the
//! [`BackendStorage`] and [`BackendDevice`] trait implementations that
//! candle-core requires.

// Re-export everything from candle-cuda so that existing code using
...
// `CudaStorage` (defined in candle-cuda).  Rust's method resolution prefers
// inherent methods over trait methods, so `self.foo()` inside this trait impl
// always resolves to the candle-cuda inherent impl — no infinite recursion.
...
impl BackendDevice for CudaDevice {
    ...
    fn storage_from_slice<T: crate::WithDType>(&self, data: &[T]) -> Result<CudaStorage> {
