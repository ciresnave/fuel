//! Thin delegation layer to the `candle-metal` crate.
//!
//! All Metal types, kernel implementations, and device management live in
//! `candle_metal`. This module re-exports them and provides the
//! [`BackendStorage`] and [`BackendDevice`] trait implementations that
//! candle-core requires.

// Re-export everything from candle-metal so that existing code using
...
// `MetalStorage` (defined in candle-metal).  Rust's method resolution prefers
// inherent methods over trait methods, so `self.foo()` inside this trait impl
// always resolves to the candle-metal inherent impl — no infinite recursion.
