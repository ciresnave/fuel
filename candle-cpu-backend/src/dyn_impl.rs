...
//! that implement the object-safe `DynBackend*` traits from `candle-core-types`.
//!
//! These newtypes exist because of Rust's orphan rule: both `DynBackendStorage`
//! and `CpuStorage` are defined in `candle-core-types`, but the impl needs access
//! to the computation infrastructure in this crate (`candle-cpu-backend`). The
...
use candle_core_types::cpu::erf;
use candle_core_types::dyn_backend::{DynBackendDevice, DynBackendStorage};
use candle_core_types::op::{BinaryOp, CmpOp, ReduceOp, UnaryOp};
use candle_core_types::{CpuStorage, DType, DeviceLocation, Error, Layout, Result,
                         Scalar, Shape};
use float8::F8E4M3;
use half::{bf16, f16};
...
/// `candle-core-types`, so their impl must live in that crate or in a crate
...
// ---------------------------------------------------------------------------
// Copy helpers (same as in candle-core/src/cpu_backend/mod.rs)
// ---------------------------------------------------------------------------
...
impl DynBackendStorage for CpuBackendStorage {
    ...
    fn to_dtype_dyn(&self, layout: &Layout, dtype: DType) -> Result<Box<dyn DynBackendStorage>> {
        ...
        //
        // We replicate the approach from candle-core's BackendStorage::to_dtype:
    ...
    match dtype {
        DType::BF16 => {
            let uniform = rand::distr::Uniform::new(bf16::from_f64(min), bf16::from_f64(max))
                .map_err(candle_core_types::Error::wrap)?;
...
fn cpu_const_set(storage: &mut CpuStorage, s: Scalar, l: &Layout) -> Result<()> {
    use candle_core_types::Scalar as S;
    fn set<T: candle_core_types::WithDType + Copy>(src: &mut [T], l: &Layout, s: T) {
        match l.strided_blocks() {
            candle_core_types::StridedBlocks::SingleBlock { start_offset, len } => {
                src[start_offset..start_offset + len].fill(s)
            }
            candle_core_types::StridedBlocks::MultipleBlocks { block_start_index, block_len: 1 } => {
                for src_index in block_start_index {
                    src[src_index] = s;
                }
            }
            candle_core_types::StridedBlocks::MultipleBlocks { block_start_index, block_len } => {
                for src_index in block_start_index {
...
/// `BackendStorage::to_dtype` from candle-core's cpu_backend.
