...
//!
//! Same orphan-rule motivation as `candle-cpu-backend`'s `CpuBackendStorage`:
//! both the traits and inner types live in `candle-core-types`, so the impl must
...
use candle_core_types::{CpuStorage, DType, DeviceLocation, Error, Layout, Result, Scalar, Shape};
...
impl Map1 for UnaryKernel {
    fn f<T: DeviceRepr + candle_core_types::WithDType + ValidAsZeroBits>(
        &self,
        src: &CudaSlice<T>,
        dev: &CudaDevice,
        layout: &Layout,
