use candle_core_types::{DType, Layout};
...
impl From<CudaError> for candle_core_types::Error {
    fn from(val: CudaError) -> Self {
        candle_core_types::Error::cuda(val)
    }
}

pub trait WrapErr<O> {
    fn w(self) -> std::result::Result<O, candle_core_types::Error>;
}

impl<O, E: Into<CudaError>> WrapErr<O> for std::result::Result<O, E> {
    fn w(self) -> std::result::Result<O, candle_core_types::Error> {
        self.map_err(|e| candle_core_types::Error::cuda(e.into()))
    }
}
