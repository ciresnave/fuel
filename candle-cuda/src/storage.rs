...
use candle_core_types::{CpuStorage, DType, Layout, Result};
...
fn push_scalar_arg<'a>(scalar: &'a candle_core_types::scalar::Scalar, builder: &mut cudarc::driver::LaunchArgs<'a>) {
    use candle_core_types::scalar::Scalar;
    match scalar {
...
struct Conv1D<'a>(&'a candle_core_types::conv::ParamsConv1D);
impl Map2 for Conv1D<'_> {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let ds = if dims.len() == 3 {
            ...
            candle_core_types::bail!("unexpected input shape for conv1d {dims:?}")
        };
...
struct Conv2D<'a>(&'a candle_core_types::conv::ParamsConv2D);
impl Map2 for Conv2D<'_> {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let ds = if dims.len() == 4 {
            ...
            candle_core_types::bail!("unexpected input shape for conv2d {dims:?}")
        };
        let ds = dev.clone_htod(&ds)?;
        let mut builder = func.builder();
...
struct ConvTranspose1D<'a>(&'a candle_core_types::conv::ParamsConvTranspose1D);
impl Map2 for ConvTranspose1D<'_> {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let ds = if dims.len() == 3 {
            ...
            candle_core_types::bail!("unexpected input shape for conv_transpose1d {dims:?}")
        };
        let ds = dev.clone_htod(&ds)?;
        let mut builder = func.builder();
        barg!(builder, el);
...
struct ConvTranspose2D<'a>(&'a candle_core_types::conv::ParamsConvTranspose2D);
impl Map2 for ConvTranspose2D<'_> {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let ds = if dims.len() == 4 {
            ...
            candle_core_types::bail!("unexpected input shape for conv_transpose2d {dims:?}")
        };
        let ds = dev.clone_htod(&ds)?;
        let mut builder = func.builder();
        barg!(builder, el);
...
impl Map1 for Pool2D {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        ...
        let ds = if dims.len() == 4 {
            ...
            candle_core_types::bail!("unexpected input shape for pool {dims:?}")
        };
        let el = shape.elem_count();
...
impl<U: candle_core_types::op::BinaryOpT> Map2 for U {
    fn f<T: DeviceRepr + WithDType + ValidAsZeroBits>(
        &self,
        lhs: &CudaSlice<T>,
        lhs_l: &Layout,
...
impl CudaStorage {
    ...
    pub fn const_set(&mut self, s: candle_core_types::scalar::Scalar, layout: &Layout) -> Result<()> {
        let dev = &self.device;
        let shape = layout.shape();
        let dims = shape.dims();
        let el_count = shape.elem_count();
    ...
    pub fn conv1d(
        ...
        kernel: &Self,
        kernel_l: &Layout,
        params: &candle_core_types::conv::ParamsConv1D,
    ) -> Result<Self> {
    ...
    pub fn conv_transpose1d(
        ...
        kernel: &Self,
        kernel_l: &Layout,
        params: &candle_core_types::conv::ParamsConvTranspose1D,
    ) -> Result<Self> {
    ...
    pub fn conv2d(
        ...
        kernel: &Self,
        kernel_l: &Layout,
        params: &candle_core_types::conv::ParamsConv2D,
    ) -> Result<Self> {
    ...
    pub fn conv_transpose2d(
        ...
        kernel: &Self,
        kernel_l: &Layout,
        params: &candle_core_types::conv::ParamsConvTranspose2D,
    ) -> Result<Self> {
        let device = self.device().clone();
        let slice =
    ...
    pub fn upsample_nearest1d(&self, _: &Layout, _out_sz: usize) -> Result<Self> {
        candle_core_types::bail!("upsample-nearest1d is not supported on cuda")
    }
