...
//!
//! Same orphan-rule motivation as `candle-cuda`'s `CudaBackendStorage`:
...
use candle_core_types::{CpuStorage, DType, DeviceLocation, Layout, Result, Scalar, Shape};
use std::any::Any;
use std::sync::Arc;
...
fn downcast(s: &dyn DynBackendStorage) -> Result<&MetalBackendStorage> {
    s.as_any()
        ...
        .ok_or_else(|| {
            candle_core_types::Error::DeviceMismatchBinaryOp {
