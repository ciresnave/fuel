...
//!
use candle_core_types::conv::{
    ParamsConv1D, ParamsConv2D, ParamsConvTranspose1D, ParamsConvTranspose2D,
};
use candle_core_types::op::{BinaryOpT, CmpOp, ReduceOp, UnaryOpT};
use candle_core_types::{CpuStorage, CpuStorageRef, DType, Error, Layout, Result, Shape};
...
impl From<MetalError> for candle_core_types::Error {
    fn from(val: MetalError) -> Self {
        candle_core_types::Error::metal(val)
    }
}
...
impl MetalStorage {
    ...
    pub fn to_cpu_storage(&self) -> Result<CpuStorage> {
        match self.dtype {
            ...
            DType::F6E2M3 | DType::F6E3M2 | DType::F4 | DType::F8E8M0 => Err(
                candle_core_types::Error::UnsupportedDTypeForOp(self.dtype, "to_cpu_storage").bt(),
            ),
        }
    }
    ...
    pub fn affine(&self, layout: &Layout, mul: f64, add: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            let name = match self.dtype {
                ...
                dtype => {
                    candle_core_types::bail!("Metal contiguous affine {dtype:?} not implemented")
                }
            };
            ...
            let name = match self.dtype {
                ...
                dtype => candle_core_types::bail!("Metal strided affine {dtype:?} not implemented"),
            };
    ...
    pub fn powf(&self, layout: &Layout, pow: f64) -> Result<Self> {
        let device = self.device().clone();
        ...
        if layout.is_contiguous() {
            let name = match self.dtype {
                ...
                dtype => {
                    candle_core_types::bail!("Metal contiguous powf {dtype:?} not implemented")
                }
            };
            ...
            let name = match self.dtype {
                ...
                dtype => candle_core_types::bail!("Metal strided powf {dtype:?} not implemented"),
            };
    ...
    pub fn elu(&self, layout: &Layout, alpha: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            let name = match self.dtype {
                ...
                DType::F16 => "elu_f16",
                DType::BF16 => "elu_bf16",
                dtype => candle_core_types::bail!("Metal contiguous elu {dtype:?} not implemented"),
            };
            ...
            let name = match self.dtype {
                ...
                dtype => candle_core_types::bail!("Metal strided elu {dtype:?} not implemented"),
            };
    ...
    pub fn reduce_op(&self, op: ReduceOp, layout: &Layout, sum_dims: &[usize]) -> Result<Self> {
        ...
        if layout.is_contiguous() && reduction_shape.is_contiguous(&stride) {
            let (name, check_empty, return_index) = match (op, self.dtype) {
                ...
                (k, dtype) => {
                    candle_core_types::bail!(
                        "Metal contiguous reduce op {k:?} {dtype:?} not implemented"
                    )
                }
            };
            if check_empty && layout.shape().elem_count() == 0 {
                Err(candle_core_types::Error::EmptyTensor { op: "reduce" }.bt())?
            }
        ...
        let (name, check_empty, return_index) = match (op, self.dtype) {
            ...
            (k, dtype) => {
                candle_core_types::bail!("Metal strided reduce op {k:?} {dtype:?} not implemented")
            }
        };
        if check_empty && layout.shape().elem_count() == 0 {
            Err(candle_core_types::Error::EmptyTensor { op: "reduce" }.bt())?
    ...
    pub fn const_set(&mut self, s: candle_core_types::Scalar, l: &Layout) -> Result<()> {
        use candle_core_types::Scalar;
        fn set<
            S: candle_core_types::dtype::WithDType + candle_metal_kernels::utils::EncoderParam,
        >(
            ...
            if l.is_contiguous() {
                ...
                let kernel_name = match dtype {
                    ...
                    DType::F8E4M3 => candle_core_types::bail!("unsupported const-set f8e4m3"),
                    DType::F64 => candle_core_types::bail!("unsupported const-set f64"),
                    DType::F4
                    | DType::F6E2M3
                    | DType::F6E3M2
                    | DType::F8E8M0
        ...
        match (self.dtype, s) {
            ...
            _ => candle_core_types::bail!("dtype mismatch, expected {:?}, got {:?}", self.dtype, s),
        }
    }
    ...
    pub fn unary_impl<B: UnaryOpT>(&self, layout: &Layout) -> Result<Self> {
    ...
    pub fn unary(&self, op: &'static str, layout: &Layout) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            let kernel_name = match (op, dtype) {
                ...
                (name, dtype) => {
                    candle_core_types::bail!("Metal strided unary {name} {dtype:?} not implemented")
                }
            };
    ...
    pub fn where_cond(
        ...
        let name = match (self.dtype, t.dtype()) {
            ...
            (left, right) => {
                candle_core_types::bail!("Metal where_cond {left:?} {right:?} not implemented")
            }
        };
        let src = buffer_o(&self.buffer, layout, self.dtype);
    ...
    pub fn conv1d(
        ...
        let name = match self.dtype {
            ...
            dtype => candle_core_types::bail!("Metal conv1d {dtype:?} not implemented"),
        };
        let src = buffer_o(&self.buffer, layout, self.dtype);
    ...
    pub fn conv_transpose1d(
        ...
        let buffer = if USE_COL2IM_CONV1D_TR && can_use_col2im {
            ...
            let name = match self.dtype {
                ...
                dtype => candle_core_types::bail!("metal col2im1d {dtype:?} not implemented"),
            };
            let col = {
            ...
            let name = match self.dtype {
                ...
                dtype => {
                    candle_core_types::bail!("Metal conv_transpose1d {dtype:?} not implemented")
                }
            };
    ...
    pub fn conv2d(
        ...
        let name = match self.dtype {
            ...
            dtype => candle_core_types::bail!("Metal conv2d {dtype:?} not implemented"),
        };
        let src = buffer_o(&self.buffer, layout, self.dtype);
    ...
    pub fn conv_transpose2d(
        ...
        let name = match self.dtype {
            ...
            dtype => candle_core_types::bail!("Metal conv_transpose2d {dtype:?} not implemented"),
        };
    ...
    pub fn avg_pool2d(
        ...
        let name = match self.dtype {
            ...
            dtype => candle_core_types::bail!("Metal avg_pool2d {dtype:?} not implemented"),
        };
    ...
    pub fn max_pool2d(
        ...
        let name = match self.dtype {
            ...
            dtype => candle_core_types::bail!("Metal max_pool2d {dtype:?} not implemented"),
        };
        let out_w = (width - w_k) / w_stride + 1;
        let out_h = (height - h_k) / h_stride + 1;
        let dst_el = out_w * out_h * b_size * channels;
    ...
    pub fn upsample_nearest1d(&self, _: &Layout, _: usize) -> Result<Self> {
        candle_core_types::bail!("Metal upsample_nearest1d not implemented")
    }
    ...
    pub fn upsample_nearest2d(&self, inp_l: &Layout, out_w: usize, out_h: usize) -> Result<Self> {
        ...
        let strides = inp_l.stride();
        if dims.len() != 4 {
            candle_core_types::bail!("unexpected input shape for upsample {dims:?}")
        }
        let name = match self.dtype {
            ...
            dtype => candle_core_types::bail!("Metal upsample_nearest2d {dtype:?} not implemented"),
        };
    ...
    pub fn upsample_bilinear2d(
        ...
        if dims.len() != 4 {
            candle_core_types::bail!("unexpected input shape for upsample_bilinear2d {dims:?}")
        }
        ...
        let name = match self.dtype {
            ...
            dtype => {
                candle_core_types::bail!("Metal upsample_bilinear2d {dtype:?} not implemented")
            }
        };
    ...
    pub fn gather(&self, src_l: &Layout, ids: &Self, ids_l: &Layout, dim: usize) -> Result<Self> {
        if !ids_l.is_contiguous() {
            return Err(candle_core_types::Error::RequiresContiguous { op: "gather" }.bt());
        };
        ...
        let name = match (ids.dtype, self.dtype) {
            ...
            (left, right) => {
                candle_core_types::bail!("Metal gather {left:?} {right:?} not implemented")
            }
        };
        let encoder = self.device.command_encoder()?;
    ...
    pub fn scatter_set(
        ...
        if !l.is_contiguous() || !ids_l.is_contiguous() || !src_l.is_contiguous() {
            return Err(candle_core_types::Error::RequiresContiguous { op: "scatter" }.bt());
        };
        let name = match (ids.dtype, self.dtype) {
    ...
    pub fn scatter_add_set(
        ...
        if !l.is_contiguous() || !ids_l.is_contiguous() || !src_l.is_contiguous() {
            return Err(candle_core_types::Error::RequiresContiguous { op: "scatter-add" }.bt());
        };
        let name = match (ids.dtype, self.dtype) {
    ...
    pub fn index_select(
        ...
        if !ids_l.is_contiguous() {
            candle_core_types::bail!("Metal index_select requires contiguous ids")
        }
    ...
    pub fn index_add(
        ...
        if !ids_l.is_contiguous() || !src_l.is_contiguous() {
            return Err(candle_core_types::Error::RequiresContiguous { op: "index-add" }.bt());
        };
        let name = match (ids.dtype, self.dtype) {
    ...
    pub fn copy2d(
        ...
        if src_s == d2 && dst_s == d2 {
            ...
            let kernel_name = match self.dtype {
                ...
                dtype => candle_core_types::bail!("Metal copy2d {dtype:?} not implemented"),
            };
            let encoder = self.device.command_encoder()?;
    ...
    pub fn copy_strided_src(
        ...
        if src_l.is_contiguous() && self.dtype == dst.dtype() {
            ...
            let kernel_name = match self.dtype {
                ...
                dtype => candle_core_types::bail!("Metal copy_strided {dtype:?} not implemented"),
            };
...
impl MetalDevice {
    ...
    pub fn location(&self) -> candle_core_types::DeviceLocation {
        candle_core_types::DeviceLocation::Metal {
    ...
    pub unsafe fn alloc_uninit(&self, shape: &Shape, dtype: DType) -> Result<MetalStorage> {
    ...
    pub fn zeros_impl(&self, shape: &Shape, dtype: DType) -> Result<MetalStorage> {
    ...
    pub fn storage_from_slice<T: candle_core_types::dtype::WithDType>(
        &self,
        s: &[T],
    ) -> Result<MetalStorage> {
    ...
    pub fn rand_uniform(
        ...
        let name = match dtype {
            ...
            dtype => candle_core_types::bail!("rand_uniform not implemented for {dtype:?}"),
        };
