...
- **Lazy-only execution; no eager mode.** Eager hides the DAG and prevents optimization. Fuel's only execution path is lazy + explicit `.realize()`. The performance target is "lazy-realize is fast enough that an eager mode would be a regression" — not "eager is forbidden because we say so." JAX has demonstrated this idiom is learnable at scale.
