...
**Status**: v0.3 (draft, 2026-06-20). Minor: the closed-Op-enum non-goal gains the trusted/untrusted nuance and the "backends never decide" commitment is tied to the JIT loop, per the 2026-06-20 adaptive-runtime-fusion decision ([10-decisions-log](10-decisions-log.md)).
...
**Backends advertise; they don't decide.** Every backend (CPU, CUDA, Vulkan, Metal, AOCL, MKL, future ones) reports its kernels, its capabilities, its measured cost data, and its current telemetry to the optimizer. Backends never choose between alternatives at the strategic level — they execute what the optimizer hands them. Strategic decisions (placement, routing, fusion, dtype choice, tolerance budget consumption) live in fuel-core. Tactical decisions internal to a single kernel call (which CUDA tile shape, which BLAS variant for a chosen backend) may live in the backend, with cost feedback flowing up. The adaptive runtime-fusion JIT loop (2026-06-20, G7) preserves this exactly: **Fuel is the strategist** — it chooses *which* sub-base-map region to fuse and *when* — while **a backend is the synthesizer** that builds the best kernel for the Fuel-chosen region. Fuel choosing the region *is* the fusion decision; there is no backend-side opportunity-finding, so the loop is not backend-internal fusion. See [10-decisions-log](10-decisions-log.md).
...
- **Not framework-agnostic dispatch.** Fuel doesn't try to be a generic DAG executor that works with arbitrary user-defined ops at runtime. The primitive Op enum is closed and small (build-time-closed; no opaque / `Custom` node), and *untrusted* user-installable ops/rules stay out. The freeze does **not** extend to *trusted, Fuel-orchestrated, cost-gated* registration of new **fused-op** identities — a fused op is a composition of existing primitives (a recipe), not a new primitive, so registering one via the declarative form on Fuel's own schedule (Tier 2) is in-bounds. The kernel binding table (implementations) is already runtime-extensible (Tier 1). The boundary is trust + provenance, not "runtime" per se; see [09-non-goals](09-non-goals.md) and the 2026-06-20 adaptive-fusion decision (G3/G4) in [10-decisions-log](10-decisions-log.md).
