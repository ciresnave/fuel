...
> **As-built note (2026-07-29): `fuel-nn` is a planned crate, not a shipped one.** The diagram above describes the target layering. There is no `fuel-nn` directory and no mention of it in the workspace manifest; the NN surface currently lives in `fuel-core` as `lazy_nn_*` modules — `fuel_core::lazy_nn_varbuilder::LazyVarBuilder`, `fuel_core::lazy_nn_varmap::LazyVarMap`, plus `lazy_nn_loss` / `lazy_nn_optim` / `lazy_nn_dropout` / `lazy_nn_gru` / `lazy_nn_prelu` / `lazy_nn_one_hot` / `lazy_nn_conv_transpose`. Extracting it is a step of the `fuel-core` retirement program. Recorded because a consumer reading this diagram wrote `use fuel_nn::VarBuilder` and hit a wall (the Lightbulb port, 2026-07-29); the same stale reference appears in `fuel-inference/src/lib.rs`'s header.
