...
This is **load-bearing for optimization itself, not only for JIT/telemetry**: because optimization *is* lower-to-base-map-then-find-the-best-cover, an op that won't decompose **breaks the optimizer** (it cannot be lowered, so it cannot participate in re-fusion or cost comparison). A fused op's recipe therefore **always ships with the op** — never deferred "until intermediates fit," which would strand an opaque island in the base map. The three formerly-flagged self-returning decomposes were resolved 2026-07-03 (see [10-decisions-log §2026-07-03](10-decisions-log.md)): `nf4_matmul` and `flash_attn`'s concrete-`k_len` decode now carry total primitive recipes; `flash_attn`'s **symbolic** `k_len` was left the sole remaining documented basis gap (a `DynScalar`-length slice is a missing primitive), and `selective_scan` was held as the twin gap pending a higher-order SSM `Scan` primitive (G3). **`selective_scan` (and its Mamba-2 sibling `ssd_chunk_scan`) closed that twin gap 2026-07-15** (see [10-decisions-log §2026-07-15](10-decisions-log.md)): `Op::Scan` ([03-ir](03-ir.md#higher-order-primitives-opscan)) now exists as a build-time primitive, so both `decompose` to a genuine `Op::Scan` recipe instead of self-returning. `decompose` is therefore now total over genuine primitives for the **whole** registered fused-op set; `flash_attn`'s symbolic-`k_len` decode remains the **one** separately-open, never-crash surfaced basis gap. (The recipe is the *math* definition; the kernel is a faster, numerically-close implementation governed by the FKC `precision` tolerance — e.g. flash-attn's online softmax vs its materialized `softmax(QKᵀ)·V` recipe.)

**⚠️ WHAT FIND-THE-BEST-COVER CAN PRODUCE — a bound on "re-fusion" that this section asserted without stating (added 2026-08-13, measured, not inferred).** A `SubgraphPattern` match rewrites through `FusionRule::rewrite`, which pushes **exactly one node kind**: `Op::Fused(self.id, params)`. **The cover step therefore produces fused ops and nothing else — it can never rewrite a subgraph back into a PRIMITIVE.** The consequence is not obvious and is easy to design against by accident: **once a lowering dissolves a primitive into a subgraph, that primitive's own registered kernels become unreachable on that path**, because no pattern can reconstitute the primitive the kernels are bound to. Recovering them requires **re-registering the operation as a fused op** (with its `pattern` and `decompose`, per the recipe principle) and binding the existing kernels to *that* — which is available and architecturally clean, but is a permanent addition to the registry surface rather than a free consequence of "the cover step will pick it up." **Worked example: GAP-183 lowered `Cast(float → Bool)` to `Ne(x, x·0)` — correctly, since `decompose` must be total — which bypassed CPU's registered single-pass `cast_*_to_bool` kernels. GAP-187 proposed recovering them with a re-fuse pattern; the pattern is expressible (`PatternNode::Bind` supports the shared operand), but there is nowhere for it to land, so the row was closed WON'T-DO with the doubled pass accepted as a measured cost of totality.** **The rule to carry into any future lowering: decide whether the primitive's kernels matter BEFORE dissolving it, because the cover step will not give them back.** (Totality itself is unaffected — `lower_to_base_map` still dissolves fused ops, so a backend without a fused kernel gets the base map and its own primitives; only *recovery of a specific primitive's kernel* is out of reach.)
