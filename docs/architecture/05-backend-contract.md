...
> ⚠️⚠️ **STRUCK 2026-08-28 — `fuel-reference-backend` DOES NOT EXIST, AND THIS IS THE MOST DIRECTLY ACTIONABLE DOC INJURY IN THE CORPUS.** Measured at head: **no `fuel-reference-backend` directory, ZERO manifest references, and every `fuel_reference_backend::` occurrence in `*.rs` is inside a COMMENT** (`fuel-core/tests/flash_attn_oracle.rs`, `fuel-cuda-backend/src/lib.rs`). *(Positive control: the same query finds `fuel_correctness_fixtures` in 3 files, so the zero is real absence and not a broken query.)*
> 
> **WHY THIS ONE IS WORSE THAN AN ABSENT TRAIT NAME: it is a FULLY-QUALIFIED CALL WITH AN ARGUMENT, so it reads as COPY-PASTEABLE.** An absent trait is something a reader must go looking for; this tells them *"you can still invoke it"* and hands them the line. **The `use fuel_nn::VarBuilder` injury in its most actionable form.**
> 
> ⚠️ **AND THIS SECTION CONTRADICTS ITSELF SIXTEEN LINES APART.** Above, correctly: *"The `fuel-reference-backend` crate, as of v0.4 (2026-06-07), **no longer participates** in the dispatch system. `BackendId::Reference` is removed, `ReferenceFactory` is removed…"* Here: *"**the crate remains** as a test-oracle utility."* **A PARTIAL RETIREMENT WHERE THE DOC KEPT THE CONSOLATION PRIZE AND THE CRATE WENT ANYWAY** — and the careful, correct retirement note above is precisely what makes a reader trust this sentence.
> 
> **THE CAPABILITY IS REAL AND THE SHAPE MOVED — same as `BackendIdentity` above.** `realize_f32` exists at head as a **trait method** on **`pub trait Realizer`** (`fuel-core/src/factories.rs:53`, *"object-safe realize seam used by judge.rs"*), not as a free function in a reference crate. It is reachable as **`fuel::factories::Realizer`** by module path — `pub mod factories` at `fuel-core/src/lib.rs:260`, **with no root re-export**, so a reader checking only `lib.rs` would wrongly conclude it is internal.
> 
> **OPEN QUESTION, NOT RESOLVED HERE:** `fuel-core/tests/flash_attn_oracle.rs` still cites `fuel_reference_backend::attention::attention_naive` in comments describing its oracle. **Whether that test's oracle was re-homed or silently orphaned is a real question underneath this doc defect**, and it is a code question rather than a doc one. ROADMAP item 9.

Fixture fast-path (live as of v0.4, 2026-06-08): the Judge optionally loads pre-validated correctness fixtures (`fuel-correctness-fixtures` crate) via `Judge::with_fixtures_from(path)`. When a fixture exists for an `(op, dtype, size_class)` cell, the Judge skips inline pairwise consensus and instead populates each `ProfileEntry::max_rel_error` from `validate_against_fixture`'s verdict — `0.0` on a clean match, the reported `rel_err` on `OutOfTolerance`, and `f32::INFINITY` on `LengthMismatch` / `NonFinite`. Cells with no matching fixture continue to use the pairwise-consensus path described above. This gives single-backend systems the same correctness signal that multi-backend systems get from inline consensus, without needing peer backends locally. Fixtures are captured on multi-backend rigs by the `fuel-capture-fixtures` binary, distributed as JSON files (`v1/<dtype>/<op>.json` under a fixture root), and loaded by consumers at Judge construction time.
...
> ⚠️⚠️ **AS-BUILT 2026-08-28 — NO `trait BackendIdentity` EXISTS, AND THIS IS THE MANDATORY TIER, WHICH MAKES IT THE HIGHEST-SEVERITY DOC DEFECT IN THE CORPUS.** **The CAPABILITY is real; the SHAPE below is not.** Measured at head: **`backend_id` has ZERO `fn` definitions — it is a STRUCT FIELD** (`BackendCapabilities.backend_id`), so identity is carried as DATA, never as a trait method; **`device_location`/`same_device` exist as INHERENT METHODS on concrete types** (`fuel-core/src/device.rs:327`, `fuel-cuda-backend/src/device.rs:591`, `fuel-backend-contract/src/storage.rs:293`) plus `same_device_dyn` on `DynBackendDevice`, **not on any shared trait.** The `pub trait Backend*` set at head is `BackendCapabilityProvider`/`BackendFactory`/`BackendProbe`/`BackendRuntime`/`BackendStorage`/`BackendStreams`. 
> 
> **WHY THIS ONE MATTERS MOST: this document is what EXTERNAL BACKEND AUTHORS ARE POINTED AT, and this heading sits under *Tier 1 — Mandatory*, which the doc says is *"the picker's load-bearing surface."* Someone implementing to it searches for `trait BackendIdentity` to `impl`, finds nothing, and CANNOT TELL whether they have missed a mandatory requirement or the doc is stale.** That is the `use fuel_nn::VarBuilder` injury recorded in `02-layers` — a consumer following the document into a wall — **except in the MANDATORY tier of the backend contract.** 
> 
> **DISPOSITION: category 4 (doc fix), NOT new work.** The Tier-1 identity requirement is genuine and is satisfied today by `BackendCapabilities.backend_id` plus the inherent `same_device`/`device_location`. **This section must be rewritten to describe what a backend ACTUALLY provides.** Marked rather than rewritten tonight because the rewrite is a real design statement about where identity belongs, not a typo fix. ROADMAP item 9.
...
> ⚠️ **AS-BUILT 2026-08-28 — GENUINELY UNBUILT. FILE AS WORK.** Measured at head: **ZERO `fn` definitions for either declared method (`register_pressure_callback`, `unregister_pressure_callback`)**, not renamed, not restructured, not abbreviated. **This heading plus its ```rust``` block is the strongest existence claim a document can make short of code — it READS AS AN INTERFACE DEFINITION, so nobody re-derives it — and nothing compiles it.** **Tier 2 (conditional), so nowhere near Tier-1 urgency** — file at its own tier's priority. **ROADMAP item 9; ITEM 9's *"DO NOT FILE AS NEW WORK"* RULING WAS WRONG FOR THIS TRAIT and is corrected there.**
...
> ⚠️ **AS-BUILT 2026-08-28 — GENUINELY UNBUILT. FILE AS WORK.** Measured at head: **ZERO `fn` definitions for all four declared methods (`device_name`, `driver_version`, `peak_throughput`, `peak_memory_bandwidth`)**, not renamed, not restructured, not abbreviated. **This heading plus its ```rust``` block is the strongest existence claim a document can make short of code — it READS AS AN INTERFACE DEFINITION, so nobody re-derives it — and nothing compiles it.** **Tier 3 (optional)** — file at its own tier's priority. **ROADMAP item 9; ITEM 9's *"DO NOT FILE AS NEW WORK"* RULING WAS WRONG FOR THIS TRAIT and is corrected there.**
