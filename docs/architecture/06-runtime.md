...
- **fuel-dispatch** — `PipelinedExecutor` (the single executor on every realize entry: the work-item producer + the executor loop), the ranker/selector chain (the runtime selector / Picker 2), dispatch. (The former `fuel-graph-router` and `fuel-graph-executor` are retired — see the executor-unification program. **`PlanStore` is likewise gone and was listed here as current until 2026-08-28**: Phase A deleted the identity-keyed plan memoization together with the legacy `compile_plan`-drives-dispatch route, and the code's own comments record it — `fuel-core/src/planner.rs`, `fuel-dispatch/src/optimize.rs`. [02-layers](02-layers.md) struck the same claim in its v0.7; **this copy survived because the fix was applied where the claim was noticed, not everywhere it lived**.)
