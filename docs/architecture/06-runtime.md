...
**Status**: v1.1 (2026-06-11). v1.1 changes (per the load-time-planning decision, [04-optimization §Load-time incremental planning](04-optimization.md#load-time-incremental-planning--cost-based-placement-v04)): the optimization producer starts at **model load** (graph-construction time), not at the first `realize()`; `realize()` becomes wait-for-plan-coverage + dispatch. The plan doubles as the weight-prefetch schedule — with mmap'd weights, page-in, planning, and execution pipeline against each other; nothing waits for "the model to finish loading." v1.0 changes (preserved): (1) atomic-swap commit is per-decision-point, not whole-DAG — re-optimization preserves cached alternatives and merges them with locally-discovered ones, atomically swapping each decision point's alternative set as the optimizer reaches it; (2) re-optimization scope is computed from the trigger using per-decision-point dependency records — most triggers (device removed, kernel updated, profile data refined) affect a small subset of decision points, not the whole graph; (3) optimization cache is mmap'd at startup with lazy KernelRef resolution — startup is near-instant for cache hits, only the cache header is touched before the first realize. v0.2 added "Background re-optimization" and "Local Judge baseline initialization" sections.
