...
What is **landed today**: the picker resolves one arm per branch, once per realize, keyed on **live per-tier free memory** (a VRAM-pressure guard), the Judge-measured rank, and the static cost winner; under no pressure it degrades to arm-0 and realize is unchanged. What is **the architecture's intent but not yet built**: selecting arms by **live device load — queue depth and stream utilization** — and moving the pick from a once-per-realize bridge step into the executor, so it re-picks as load shifts during dispatch. The `slot availability, queue depth` telemetry named in the resolution sketch above is the target signal set; the queue-depth signal and the executor-side arm selection are the near-term step, not the current state. The free-memory-pressure path is the one live runtime signal the picker consumes today.
