...
**Residual gap surfaced, not closed**: `OpKind::ReluInplace`'s CUDA binding (`unary_inplace_kernel!(unary_inplace_relu_*, unary_relu_*, ...)` in the same file) still reuses the NaN-**scrubbing** `unary_relu_*` stem — it was out of this rebind's explicit scope (`OpKind::ReluElementwise` only) and has no live parity test yet. CPU's `ReluInplace` already propagates (it inherits the CPU `Relu` blanket from the entry above). Flagged in-source next to `unary_inplace_relu_f32` and in `docs/kernel-contracts/dispatch/elementwise-unary.fkc.md`'s `relu` section; a follow-up should rebind it the same way and add a dispatch-level live test mirroring `cuda_relu_propagates_nan_f32`.
