...
**Implications going forward**: (1) 4b-δ's specific placement blocker (CUDA matmul refused by `BitStablePreferenceFilter`) is resolved — verified by re-running `cargo test -p fuel-dispatch --features cuda --lib` (633 passed, 0 failed, no regressions from the flip) and the `register_baracuda_binds_group6_matmul_facades_from_contract` contract-import test. (2) `fuel-core`'s `forward_with_kv_context_captured_matches_persistent` (the task 4b-δ end-to-end gate) STILL FAILS after this flip — but at a *different*, unrelated node: `capture_decode: graph has a cross-device Op::Copy at node NodeId(161) (target Cpu)`, traced (temporary diagnostic, reverted, not committed) to a `BroadcastTo` of a small `[128]`-element `Const` being placed on CPU — nothing to do with `MatMul`/`gemm_dense`. This is a separate, legitimate cost-based tiny-op placement decision (the same phenomenon flagged in the 2026-07-08 NaN-conventions entry above: "cost-based placement can route a tiny elementwise op to CPU"), not a defect in this audit's finding — see the audit report for the full trace. Unblocking 4b-δ end-to-end needs a follow-up task addressing that separate placement decision (or single-device-forcing capture-scope logic), which is out of this audit's scope per its brief ("do not modify fuel-core to force it to pass"). (3) The uncommitted `fuel-core/src/lazy.rs` fixture-size bump (`vocab_size`/`dim`/`ffn_dim` etc., pre-existing in this worktree from task 4b-ζ, intended to rule out cost-based CPU placement by making every op's CUDA cost estimate uniformly win) is LEFT AS-IS, not reverted — it isolates the audited-flag variable from the separate tiny-`BroadcastTo` placement issue found in (2); reverting it would reintroduce a confound this audit didn't need to re-litigate.
