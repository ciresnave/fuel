...
- **JIT-registered fused ops + their kernels** (the kernel cache): the artifacts the idle-time JIT-fusion loop produces (per the 2026-06-20 decision, [10-decisions-log](10-decisions-log.md) G4). A Fuel-orchestrated, cost-gated runtime fusion registers a **new fused-op identity** as a **declarative recipe** (`decompose` + `pattern` + shape/dtype/cost carried as *data*) plus the synthesized kernel and its FKC `PrecisionGuarantee`. These are cacheable so a future load reuses a kernel already synthesized for this hardware. The registry is **append-only with stable, never-reused `FusedOpId`s**, so a cached recipe id remains valid across loads; growth is bounded by the [kernel-cache pruning policy](#kernel-cache-pruning-prune-rarely-loses-everywhere-drive-space-capped). A kernel for an *existing* op identity needs no new identity — it is a binding-table entry (Tier 1), cached the same way kernel binaries already are (per [04-optimization §Plan-fragment memoization](04-optimization.md#plan-fragment-memoization-repeated-structure)).
...
- **Architecture version** (the registry's schema; the `FusedOpEntry` shape; the rule schema) *(**corrected 2026-08-28** — this said `OpEntry`, which does not exist. Measured: **`FusedOpEntry` in `fuel-graph/src/registry.rs`, 32 occurrences in code**, published as **`fuel_graph::registry::FusedOpEntry` by MODULE PATH ONLY — `pub mod registry` with no root re-export — and consumed by 6 files outside `fuel-graph`**, so it is a real external surface that a reader checking `lib.rs` alone would wrongly call internal. `RuntimeFusedOpEntry` also exists but has 0 external users and is not what this line means.)*. Bumps with material architecture changes. Mismatch → invalidate.
...
The `hf://` and `github://` URI schemes are sugar over the underlying transport. **INTENDED SURFACE, NOT AS-BUILT:** `fuel-loaders` is to use the existing `hf-hub` Rust crate for HF Hub, with GitHub as HTTPS GET on `raw.githubusercontent.com`.

> ⚠️ **AS-BUILT CORRECTION 2026-08-28 — THIS SENTENCE WAS IN THE PRESENT TENSE AND DESCRIBED A CRATE THAT DOES NOT EXIST.** Measured at head: **`fuel-loaders` ABSENT** *(controls: `fuel-formats`, `fuel-transformers`, `fuel-model-core` all PRESENT)*. **The CAPABILITY is real and SCATTERED; the ABSTRACTION and the URI schemes are not** — `hf-hub` is a workspace dependency consumed **directly** by `fuel-core`, `fuel-datasets` and `fuel-examples`, while `hf://` appears in **1** file, `github://` in **0**, and `raw.githubusercontent` in **1** *(control: `safetensors`, 246 files — so the near-zeros are real absence, not a broken query)*. **A reader has no cue to doubt a present-tense sentence, so this is the STRONGER form of the defect than a future-tense promise** — and it is the same injury `02-layers` records as the corpus's only measured consumer cost, where someone wrote `use fuel_nn::VarBuilder` against a crate that did not exist. **The crate is a genuine open question and stays unallocated (ROADMAP item 10(e)); the false present tense is not, and is corrected here on sight.**
...
## Kernel-cache pruning: prune rarely, loses-everywhere, drive-space-capped

The JIT-fusion loop (per [06-runtime §Background re-optimization](06-runtime.md#background-re-optimization)) synthesizes kernels over time, so the on-disk kernel cache grows. The pruning policy is the **canonical home** for that growth (per the 2026-06-20 decision, [10-decisions-log](10-decisions-log.md) G8), and it is deliberately conservative:

- **Prune rarely — likely not at all initially.** Synthesizing a kernel cost real idle-time work; discarding one risks paying that cost again. The default is to keep.
- **Gate eviction behind a developer-set max-kernel-drive-space cap.** Eviction runs **only under space pressure** — a kernel is never discarded while there is room. The cap is a developer knob, not an automatic clock.
- **Evict only kernels that lose across *every* model.** A kernel that "loses no matter which model considers it" is a never-useful proof, and is the only safe first eviction target. A kernel that is currently **shadowed** (it would lose under the present cover, but might win under a different cover, in a different model, or after a cost shift) **stays** — "winning" is relative to the current kernel set, so shadowing is not loss.
- **Never prune on a single model's losses.** One model preferring another kernel says nothing about the kernel's value to other models; single-model loss is not an eviction signal.

So pruning is the rare, space-pressure-only, loses-everywhere eviction of kernels from the cache — never a routine garbage-collect, and never a judgment made from one model's cover. This keeps the JIT loop's append-only `FusedOpId` registry (see [Artifacts and their lifecycles](#artifacts-and-their-lifecycles)) bounded without throwing away kernels that have not been proven worthless.

## Captured-run replay as a persistence artifact

A repeated plan fragment is the natural unit for **record-once-replay execution** (per [04-optimization §Plan-fragment memoization](04-optimization.md#plan-fragment-memoization-repeated-structure)): a CUDA Graph / pre-recorded Vulkan command buffer captured once and replayed per instance with rebased operand pointers. Persisting these captures is the **cheaper step below a megakernel** (per the 2026-06-20 decision, [10-decisions-log](10-decisions-log.md) G6): it captures most of the launch-overhead win — removing per-op submission cost across a whole run — *without* fusing compute, so it avoids the megakernel's register-allocation and fixed-launch-geometry penalties. A captured run is keyed like other plan fragments (structural hash + hardware fingerprint) and invalidates on the same fields as the optimization cache; it is the persistence analogue of the run-dispatch unit [06-runtime](06-runtime.md#dispatch-runs-not-single-ops) describes.
...
- [06-runtime](06-runtime.md) — cache load and re-resolve on startup; the idle-time JIT-fusion loop that produces the kernel-cache artifacts; run dispatch (the unit captured-run replay records).
...
- [08-pattern-harvest](08-pattern-harvest.md) — sibling community-sharing infrastructure; the missing-fusion telemetry that orders the JIT loop.
- [10-decisions-log §2026-06-20](10-decisions-log.md) — the adaptive-runtime-fusion decision this version's v1.3 adds (kernel-cache pruning G8, JIT-registered fused-op artifacts G4, captured-run replay G6).
