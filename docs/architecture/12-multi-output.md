...
**Status**: v1.2 (2026-07-15). Captures the Option-C design that lets a single graph node produce N logically independent outputs without growing the `DynBackendStorage` trait surface or forcing backends to learn "multi-output kernel" semantics. Landed across 6 sessions; the consumer migrations and the autograd scatter kernel light up downstream. **v1.2 (minor) closes the recipe obligation for the two SSM multi-output ops**: `selective_scan` and `ssd_chunk_scan` now `decompose` to `Op::Scan` ([03-ir](03-ir.md#higher-order-primitives-opscan), G3 closed 2026-07-15, [10-decisions-log](10-decisions-log.md)) — a 2-slot bundle preserved through decompose for the primary `y` (slot 0); the `last_state` (slot 1) view carried through the *decomposed* graph is a documented, never-crash Phase-2 wiring item, not a violation of the obligation below (see [§The recipe obligation](#the-recipe-obligation-mandatory)). v1.1 (minor) had added the **recipe obligation** to the multi-output authoring contract — every multi-output fused op MUST also ship a total, never-panic `decompose` + a re-fusion `pattern` (2026-06-20, G1/G2, [10-decisions-log](10-decisions-log.md)); at the time, the `selective_scan` panic-in-`decompose` was exactly the bug this ruled out (that panic was later found to have already been converted to a self-return, per [10-decisions-log §2026-07-03](10-decisions-log.md), before being resolved for real by v1.2 above).
...
`selective_scan` and `ssd_chunk_scan` now satisfy this obligation (2026-07-15, [10-decisions-log](10-decisions-log.md)): both `decompose` to an `Op::Scan` body ([03-ir](03-ir.md#higher-order-primitives-opscan)) instead of self-returning — a real recipe over a genuine primitive, not a bug distinguished only by basis membership. The decompose preserves the 2-slot bundle for the primary output (`y`, slot 0); the secondary slot (`last_state`) view carried through the *decomposed* (not fused) graph fails loudly today rather than silently — the kernel-less `Op::Scan` makes any realize-through-decompose of slot 1 a typed error — and wiring or dropping that view is a named Phase-2 item, not an open recipe-obligation violation. See [03-ir](03-ir.md) and [04-optimization](04-optimization.md) for the recipe principle, the DecompositionMap/OptimizationMap derivation, and the total-`decompose` contract.
