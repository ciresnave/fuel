...
Graph import produces a graph regardless of whether it matches a known architecture — an arbitrary ONNX file is just a DAG, and fuel can run it as-is. But an imported graph can additionally be **pattern-matched against the registered architectures** ("this is a Llama") and swapped for the optimized parametric version. ⚠️⚠️ **UNBUILT AS OF `4ff1fa85` — `can` HERE IS ASPIRATIONAL, NOT A CAPABILITY THE READER HAS.** **Zero functions map an imported graph to an architecture name: no `recogni*`, `identify_arch`, `detect_arch`, `infer_arch` or `arch_from_graph` definition exists in `*.rs`, and a widened sweep over every `fn` whose name suggests architecture classification finds nothing that takes a graph.** ⚠️ **POSITIVE CONTROLS, since this is an absence claim: both INGREDIENTS exist and the query would find anything joining them** — graph pattern-matching is defined in 2 files (`fn pattern*`, the fusion path) and the `inventory` architecture registry in 4. **The absence is of the JOIN.** ⚠️ **AND THE NEAR-MISS, NAMED SO IT IS NOT MISTAKEN FOR A REFUTATION: `fuel-core/src/quantized/arch.rs:94` defines `pub fn detect_from_gguf(content: &Content) -> Architecture`. That READS A DECLARED LABEL** — GGUF's `general.architecture` metadata string — **which is exactly what an arbitrary imported ONNX DAG ARRIVES WITHOUT.** **Reading a name from a header and inferring one from topology are different problems, and only the second is what this sentence promises.** ⚠️ **IT ALSO COULD NOT WORK YET IF WRITTEN: it matches against THE REGISTERED ARCHITECTURES, and the registry holds three fixture models and zero production ones ([02-layers](02-layers.md) v0.14).** **A recognizer over an empty registry recognises nothing.** That recognizer is a *model-tier* concern (it depends on both interchange and the zoo) and lives in `fuel-model-core`; it does not invert the layering, because the model tier already sits above interchange.
...
- **Apache Arrow `Tensor`** — *tensor*-level, not model-level; **backlog**. It carries tensor data, not a graph, so it is out of scope for *model* interchange — but a candidate **leaf I/O format** for the columnar / data-engineering ecosystem (Arrow Flight distributed loading, polars/DuckDB feature pipelines, columnar feature stores). Crucially it is the **host/serialization** boundary, *complementary* to **DLPack/FDX** which owns the **device/kernel** zero-copy boundary (`docs/specs/dlpack-extension.md`) — not a competitor. Sequence behind a real consumer; lean on the existing DLPack↔Arrow-device-interface bridge first (Arrow's `ArrowDeviceArray` and DLPack interoperate, so FDX support already gives partial Arrow reach). Borrow Arrow's sparse layouts (COO/CSR/CSF) / `dim_names` as references only. See ROADMAP deferred backlog.
