...
The worked case: an inference host's eviction policies were not uniformly available. Attention-driven policies (H2O heavy-hitter, R-KV importance) need the attention probabilities; recency, sink-window/StreamingLLM and span-level eviction need no such observation. A consumer choosing the fused arm for throughput would silently lose half its policy catalogue — and the failure mode is not an error but **a policy starved of input**: it runs, and scores nothing.

> **The motivating instance is largely retired (measured 2026-07-30) — the obligation is not.** A backend built the second output; attention-driven eviction and FlashAttention **compose** rather than exclude (see [C-6 §Measured](#the-arm-question-was-measured-and-they-compose)). So H2O and R-KV are no longer arm-gated in practice.
>
> **This clause stands anyway, and the distinction matters:** the obligation is *"where an arm constraint gates a capability, say so at constraint-admission time"*. That it took a kernel and a benchmark to dissolve **this** instance is the argument for the clause, not against it — for the months the gating was real, nothing in the contract would have told a consumer its policy set had quietly halved. Keep the rule; the example is now historical.
...
#### The arm question was measured, and they compose

**[measured 2026-07-30 — backend-reported, not reproduced here]** A backend built the second output as three opt-in, nullptr-guarded configs (zero cost when off), deterministic throughout — fixed-order online-softmax rescale, **no FP atomics**, which was the non-negotiable. f16, B=1 H=32 D=128, best-of-3 × 400 on-device:

| | K=2048 | K=4096 | K=8192 |
| --- | --- | --- | --- |
| per-head `a[B,Hq,Sk]` | +5.0% | +4.2% | +4.6% |
| head-mean on top | +6.0% | +1.0% | +0.5% (flat ~3µs) |

**A heavy-hitter observer pays ~4–5% of decode.** So the earlier framing — that attention-driven eviction and FlashAttention are *mutually exclusive*, forcing a per-deployment arm choice — **is wrong. They compose.**

**And the cost is self-limiting, which is the part worth carrying into the clause.** Attention-driven eviction exists to keep KV *short*, and the expensive case is exactly the regime it evicts you out of: the same baselines price K=8192 at 1.36 ms against K=2048 at 65.5 µs — **~20× cheaper**. A session held at 2K instead of 8K saves ~1.3 ms/step and pays ~2.8 µs for the statistic that made the decision: **roughly 0.2% of what the eviction it enables saves.** You never pay the high percentage *and* stay in the expensive regime — you pay it briefly on the way down, then a lower one on a kernel an order of magnitude cheaper. **The cost removes its own worst case.**

**What survives as a genuine C-5 constraint** is much narrower: a deployment that cannot afford ~4–5% of decode, or one needing per-head rather than head-mean granularity, still chooses. A real trade — just not "you cannot have both".

**Two caveats kept attached:**

- **Decode only.** Prefill is unmeasured and *deliberately* unbuilt: an eviction-set experiment found 90–99% overlap with the un-normalized proxy, and prefill's contribution decays to 0.6% after 100 tokens at decay = 0.95 — not worth 1.5–2× on the prefill softmax.
- **A fused path would collapse this to ~baseline** (fold fixed-h-order accumulation into the combine kernel; no `[H,Sk]` materialization, no cross-block atomic). The design is banked and **deliberately not built** — there is no live GPU decode workload to A/B it against yet, and building it now would optimise for a consumer that does not exist. The current ~4–5% is partly "unread but materialized": `a_mean` reduces *over* the stored per-head tensor, so `[H,Sk]` is written even when only the mean is read.

**Robust across the GQA axis — no shape qualifier needed [measured 2026-07-30].** The obvious reopener was aggressive GQA / MQA: the `a` store scales with `H_q` while the baseline KV read scales with `H_kv`, so as `H_kv` falls the overhead's *share* should grow. That was **pre-registered as a prediction and then falsified by measurement** (`H_q=32, D=128, H_kv ∈ {32,8,4,1}`, f16, best-of-3 ×400, two runs):

| | H_kv=32 | H_kv=8 | H_kv=4 | H_kv=1 |
| --- | --- | --- | --- | --- |
| Sk=2048 | +5.3% | +4.5% | +4.4% | **+3.6%** |
| Sk=4096 | +3.9% | +5.1% | +5.3% | **+5.2%** |

A tight **~3.5–5.5% band, flat-to-*decreasing* toward MQA**. No blowup at `H_kv=1`.

**Why the prediction failed is the reusable part.** The numerator was right — the `a` store is ∝ `H_q` and flat in absolute µs. **The denominator was wrong**: `baseline ∝ H_kv` holds only while the K/V working set is DRAM-bound. Below `H_kv ≈ 8` the distinct K/V set becomes **L2-resident** (32 Q-head blocks all reading the same few K/V heads), effective bandwidth caps, and the baseline **plateaus** (~44 µs @2048, ~74 µs @4096) instead of shrinking. The denominator saturates, so the ratio flattens. **The same mechanism predicted to make MQA pathological is what protects it.**

**Caveat kept rather than smoothed:** the `H_kv=32 / Sk=2048` cell jitters run-to-run (+12.5% → +5.3%) with the baseline rock-stable and only the `+a` leg noisy — it sits at the DRAM-saturation knee. **Read that cell as a band, not a point.** Everything else reproduced tight.

**Consequence:** the fused path's deferral now stands purely on *"no GPU decode workload to A/B against"*, not on a shape argument — because the shape argument turned out not to exist.

**[note] None of this was reachable by reading.** The question went from *"attention isn't observable on the fused arm"* → KISS reframing it as a recipe with a second output → a kernel that exists → a number that retires most of a contract constraint → a pre-registered falsification that removes its last shape qualifier.

**[method] Two cost predictions from algebra were wrong here, both from an idealised memory model** (once reading "per-head falls out free" as bandwidth-free when it meant contention-free; once modelling a baseline as pure DRAM traffic with no cache term). The right conclusion is *not* "stop predicting": a falsifiable mechanism with a **named failure point** is what let the measurement mean something. A vague "some percent" would have taught nothing when measured, and would have left the MQA question permanently open.
