...
**[note] None of this was reachable by reading.** The question went from *"attention isn't observable on the fused arm"* → KISS reframing it as a recipe with a second output → a kernel that exists and a number that retires most of a contract constraint.
