...
| Gap | Status | What's needed | Home |
| --- | --- | --- | --- |
| **MLA decode-time compressed KV cache** | Partial (prefill-only) | Cache only the compressed latent + `k_pe` during decode instead of recomputing full attention. The MLA port is `batch==1`, no-cache (`lazy_deepseek2.rs:44-48`). | orphan → this doc |
| **KV-cache container generalization** | Absent | Both `LazyKvCache` and `InferenceContext::KvCache` hardwire a symmetric **K/V pair** shaped by `n_kv_heads`/`head_dim`. Caching a single latent (MLA) or a single retained projection (two-projection) needs a cache holding an arbitrary latent tensor. **This is the structural blocker** for latent/pruned KV compression. | orphan (source `lazy_kv_cache.rs`, `inference_context.rs:173`) → this doc |
