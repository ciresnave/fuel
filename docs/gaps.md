...
| GAP-028 | fuel-core `rebind_and_realize_prebuilt` / `_paged_prebuilt` | C | **Perf follow-up to GAP-014, NOT a correctness gap.** A KV-allocation swap now correctly invalidates the held plan, so a slot-pooled server pays one plan REBUILD per admitted request. Re-binding the `kv_nodes` entries in the `base_cache` clone — the same mechanism the per-token data Consts already use — would make a swap free and keep the 223× reuse across requests. Requires proving the layouts match; conservative rebuild shipped first because it is what every other validity-key field already does. | OPEN |
