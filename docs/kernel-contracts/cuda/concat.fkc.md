...
# fuel-cuda-backend - N-ary concatenation (Concat; binding [T,T]) kernel contracts

CUDA (baracuda) N-ary via N-1 chained baracuda concat2; the dispatch binding is [T,T]; OpParams::Concat.. Each section binds one concrete `OpKind` and fans its operand(s)
...
## concat  (Concat - {F32, F16, BF16, F64}, strided+broadcast)

out=concat(inputs) along axis Backs `OpKind::Concat`. N-ary via N-1 chained baracuda concat2; the dispatch binding is [T,T]; OpParams::Concat. Output: fresh, contiguous, no aliasing.

```fkc
kernel: concat
op_kind: Concat
blurb: "out=concat(inputs) along axis (CUDA/baracuda) {F32, F16, BF16, F64}; strided+broadcast; per-(op,dtype)."
backend: Cuda
kernel_source: "baracuda"
entry_point: "fuel_cuda_backend::fkc::concat"
