...
# fuel-cuda-backend - single-axis inclusive prefix sum (cumsum) kernel contracts

CUDA (baracuda) axis from OpParams::CumSum; reverse-cumsum expressed upstream as Flip->CumSum->Flip.. Each section binds one concrete `OpKind` and fans its operand(s)
...
## cumsum  (CumSum - {F32, F16, BF16, F64}, strided+broadcast)

inclusive prefix sum along the axis Backs `OpKind::CumSum`. axis from OpParams::CumSum; reverse-cumsum expressed upstream as Flip->CumSum->Flip. Output: fresh, contiguous, no aliasing.

```fkc
kernel: cumsum
op_kind: CumSum
blurb: "inclusive prefix sum along the axis (CUDA/baracuda) {F32, F16, BF16, F64}; strided+broadcast; per-(op,dtype)."
backend: Cuda
kernel_source: "baracuda"
entry_point: "fuel_cuda_backend::fkc::cumsum"
...
precision:
  ...
  max_absolute: ~
  audited: false
  notes: "inclusive prefix sum along the axis; author-declared UNAUDITED seed (byte-for-byte the deleted plain register default); pointwise, bit-stable same hardware."
