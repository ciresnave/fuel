...
# fuel-cuda-backend - single-axis reverse (flip) kernel contracts

CUDA (baracuda) axis from OpParams::Flip; wrapper picks contig rank-3 reshape vs strided rank-N walk.. Each section binds one concrete `OpKind` and fans its operand(s)
...
## flip  (Flip - {F32, F16, BF16, F64}, strided+broadcast)

reverse along the flip axis Backs `OpKind::Flip`. axis from OpParams::Flip; wrapper picks contig rank-3 reshape vs strided rank-N walk. Output: fresh, contiguous, no aliasing.

```fkc
kernel: flip
op_kind: Flip
blurb: "reverse along the flip axis (CUDA/baracuda) {F32, F16, BF16, F64}; strided+broadcast; per-(op,dtype)."
backend: Cuda
kernel_source: "baracuda"
entry_point: "fuel_cuda_backend::fkc::flip"
