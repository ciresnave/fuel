...
# fuel-cuda-backend - multi-dim padding (Pad) kernel contracts

CUDA (baracuda) Constant/Reflect/Replicate modes on OpParams::Pad (mode_tag), not the key; contiguous.. Each section binds one concrete `OpKind` and fans its operand(s)
...
## pad  (Pad - {F32, F16, BF16, F64}, contiguous-only)

out=pad(in, padding, mode) Backs `OpKind::Pad`. Constant/Reflect/Replicate modes on OpParams::Pad (mode_tag), not the key; contiguous. Output: fresh, contiguous, no aliasing.

```fkc
kernel: pad
op_kind: Pad
blurb: "out=pad(in, padding, mode) (CUDA/baracuda) {F32, F16, BF16, F64}; contiguous-only; per-(op,dtype)."
backend: Cuda
kernel_source: "baracuda"
entry_point: "fuel_cuda_backend::fkc::pad"
