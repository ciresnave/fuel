...
# fuel-cuda-backend - integer-exponent power (x^n) kernel contracts

CUDA (baracuda) y=x^n by power-of-squaring on device (correct on negative bases); exp rides OpParams::PowI { exp }.. Each section binds one concrete `OpKind` and fans its operand(s)
...
## powi  (PowIElementwise - {F32, F64, F16, BF16}, strided+broadcast)

out[i]=in[i]^exp Backs `OpKind::PowIElementwise`. y=x^n by power-of-squaring on device (correct on negative bases); exp rides OpParams::PowI { exp }. Output: fresh, contiguous, no aliasing.

```fkc
kernel: powi
op_kind: PowIElementwise
blurb: "out[i]=in[i]^exp (CUDA/baracuda) {F32, F64, F16, BF16}; strided+broadcast; per-(op,dtype)."
backend: Cuda
kernel_source: "baracuda"
entry_point: "fuel_cuda_backend::fkc::powi"
