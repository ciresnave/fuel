...
# fuel-cuda-backend - integer-exponent power backward kernel contracts

CUDA (baracuda) Single-launch grad_x for x^n (x, upstream) -> grad_x; same exp via OpParams::PowI { exp }.. Each section binds one concrete `OpKind` and fans its operand(s)
...
## powi_backward  (PowIElementwiseBackward - {F32, F64, F16, BF16}, strided+broadcast)

grad_x = upstream * n * x^(n-1) Backs `OpKind::PowIElementwiseBackward`. Single-launch grad_x for x^n (x, upstream) -> grad_x; same exp via OpParams::PowI { exp }. Output: fresh, contiguous, no aliasing.

```fkc
kernel: powi_backward
op_kind: PowIElementwiseBackward
blurb: "grad_x = upstream * n * x^(n-1) (CUDA/baracuda) {F32, F64, F16, BF16}; strided+broadcast; per-(op,dtype)."
backend: Cuda
kernel_source: "baracuda"
entry_point: "fuel_cuda_backend::fkc::powi_backward"
