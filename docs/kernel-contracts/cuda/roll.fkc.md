...
# fuel-cuda-backend - single-axis cyclic shift (roll) kernel contracts

CUDA (baracuda) axis + shift from OpParams::Roll.. Each section binds one concrete `OpKind` and fans its operand(s)
...
## roll  (Roll - {F32, F16, BF16, F64}, strided+broadcast)

cyclic shift along the roll axis Backs `OpKind::Roll`. axis + shift from OpParams::Roll. Output: fresh, contiguous, no aliasing.

```fkc
kernel: roll
op_kind: Roll
blurb: "cyclic shift along the roll axis (CUDA/baracuda) {F32, F16, BF16, F64}; strided+broadcast; per-(op,dtype)."
backend: Cuda
kernel_source: "baracuda"
entry_point: "fuel_cuda_backend::fkc::roll"
