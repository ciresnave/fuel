...
`x.max(0.0)`, NaN-as-missing (`f32::max`, `max(NaN,0)=0`); contiguous-only. CUDA
(`baracuda::unary::relu_*`, strided) f32/f64/bf16/f16. Vulkan (`unary::relu_f32`, strided,
`VULKAN_FLOAT_POINTWISE_PRECISION`) f32 (+f16/f64 gated). One cheap branchless op per element;
bandwidth-bound.

```fkc
kernel: relu
op_kind: ReluElementwise
blurb: "Elementwise ReLU max(0, x); CPU contiguous, CUDA/Vulkan strided; half via f32; multi-backend siblings."
backend: Cpu
...
precision:
  ...
  max_absolute: ~
  audited: false
  notes: "max(0, x); exact for f32/f64. bf16/f16 widen to f32 then narrow. NaN-as-missing (f32::max). CPU bit-stable; Vulkan VULKAN_FLOAT_POINTWISE_PRECISION."
