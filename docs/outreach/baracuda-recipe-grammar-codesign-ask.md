...
**Reply positions:** Q1 structured node-map · Q2 flat-DAG (agree) · Q3 canonicalization = Fuel's `base_map_hash` (offered) · Q4 adopt §6.19 canonical-serialization on `OpAttrs` · Q5 **higher-order op-as-argument form + Fuel COMMITS to building the backing `Op::Scan`/`Op::Reduce` primitive now** (CireSnave chose "build now, not behind a consumer" — closes the `selective_scan`/`ssd_chunk_scan` G3 gaps, makes `decompose` total) · Q6 resolve-to-base-map + numeric (Baracuda emits named ops, Fuel resolves) · Q7 structural DAG dtype-agnostic, precision/NaN → C2 · cap `SEAM_CAP_RECIPE_IMPORT` (KISS FEAT range). **New workstream:** `Op::Scan` primitive gets its own design→plan→build ahead of the `emit` full-parity convergence step.
