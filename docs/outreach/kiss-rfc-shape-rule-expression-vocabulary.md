...
**Affects (corrected — see banner):** KISS-Ops **§6.20** (new — shape-expression vocabulary + shape oracle) and KISS-Contract **§6.4-0011** (new — the Interface⟷shape-rule tie, companion to the §6.4-0006 value oracle); §6.19 supplies the canonical serialization; §6.4-0009 is the **op_dag node schema** (not a "recipe node schema"). *(Original line read "Kernel-Contract §5 (`return` / `OutputDesc.shape_rule`) … §6.4-0009 recipe node schema" — but `OutputDesc.shape_rule` is a Fuel FKC field, not a KISS §5 field, and KISS §5 is Conventions.)*
