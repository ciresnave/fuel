...
- **Self-describing Storage** — steps 1–4 SHIPPED 2026-06-19 (`SType`/`Encoding`/`ScaleSpec` in
  ...
  consumer; step-7 gate green. **The one remaining tie-in (workstream E, NOT step 4):** call
  `view_with_quant` from the live `nf4_matmul` dispatch — gated on the comm-layer being wired into
  the executor. See `self-describing-storage-plan.md` Progress header.
