...
5. **KISC framing is the *serialization* half of this convergence (user decision, 2026-07-14):** the single unified recipe expression (item 4) needs a single **frame** to travel in. Adopt **KISC** — the self-delimiting kernel-import frame (`KISC` magic + version + `len` + `crc32`, hard-reject, per-document isolation) — as the **single import frame for all kernel data, the in-repo `.fkc.md` corpus included**, per `docs/outreach/baracuda-kisc-framing-reply.md`: an outer envelope around the existing `parse.rs` (additive; parser survives as the inner-body reader), `len`/`crc32` **build-stamped** (xtask/build.rs), one kernel per document, a `{abort_batch | isolate}` failure knob (local build fail-fast, wire isolate), and a cap bit in the **KISS FEAT range** co-allocated with Baracuda. This is the **ingestion service's real front door**: Spec B ([[spec-b-ingestion-complete]]) is the consumer; KISC + `parse.rs` → `CandidateKernel` is how a real (non-fixture) candidate arrives to be verified (Increment 1) and adopted. Designed **together** with the representation unification (item 4) in the convergence spec — not a separate track. Closes the KISS §2.B / RFC #23 (reference wire codec) gap.
