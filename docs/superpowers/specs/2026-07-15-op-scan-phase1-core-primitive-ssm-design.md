...
Handle each `_ => None`/`false`/`Transient` no-op site deliberately, not by default: `op_to_op_kind` ([pipelined.rs:3120](fuel-dispatch/src/pipelined.rs#L3120)) — **no native Scan kernel in Phase 1**, so no dispatch entry (Scan reaches a backend only via decompose/unroll); `op_to_tag` ([jit.rs:22](fuel-graph/src/jit.rs#L22)) — document as "not a region node; its decomposition is," following the `Fused` precedent; the backward walk ([lib.rs:7145](fuel-graph/src/lib.rs#L7145)) — **`Op::Scan` = a clean `NotDifferentiable` panic** (mirroring the `QMatMul`/`PagedAttn` precedent in that infallible `-> GradMap` walk), `Op::ScanPlaceholder` = an inert drop. **Correction (plan-material verified, 2026-07-15):** `BackwardKind` is *not consulted* anywhere — the backward walk is a hand-written exhaustive `match op` that panics for unwired ops, and there is no generic "differentiate the decomposition" mechanism to hook into. So Phase 2's differentiability is not "set `BackwardKind::Decompose`" — it requires *building* the decompose-backward plumbing (unroll → autograd over primitives) and wiring the `Op::Scan` backward-walk arm to it. SSM fused ops keep `NotDifferentiable` in Phase 1; `infer_storage_class`/`destructive_input`/`is_view_op`/`try_simplify` — safe conservative defaults, likely no edit, each confirmed not-silently-wrong. The `Op::Branch` reachability fixpoint (`effective_roots`, [lib.rs:2187](fuel-graph/src/lib.rs#L2187)) is extended to `Scan` bodies if a body can nest inside another Branch/Scan, so reachability doesn't miss live scan bodies.
