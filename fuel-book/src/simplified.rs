//! #A simplified example in Rust of training a neural network and then using it based on the Fuel Framework by Hugging Face.
...
use fuel::{DType, Result, Tensor, D, Device};
use fuel_nn::{loss, ops, Linear, Module, VarBuilder, VarMap, Optimizer};
...
impl MultiLevelPerceptron {
    fn new(vs: VarBuilder) -> Result<Self> {
        let ln1 = fuel_nn::linear(VOTE_DIM, LAYER1_OUT_SIZE, vs.pp("ln1"))?;
        let ln2 = fuel_nn::linear(LAYER1_OUT_SIZE, LAYER2_OUT_SIZE, vs.pp("ln2"))?;
        let ln3 = fuel_nn::linear(LAYER2_OUT_SIZE, RESULTS + 1, vs.pp("ln3"))?;
        Ok(Self { ln1, ln2, ln3 })
    }
