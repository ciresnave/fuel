...
thread_local! {
    static FUEL_GRAD_DO_NOT_DETACH: bool = {
        match std::env::var("FUEL_GRAD_DO_NOT_DETACH") {
            Ok(s) => {
                !s.is_empty() && s != "0"
            },
            Err(_) => false,
...
impl Tensor {
    ...
    pub fn backward(&self) -> Result<GradStore> {
        ...
        for node in sorted_nodes.iter() {
            ...
            let grad = grads
                .remove(node)
                .expect("fuel internal error - grad not populated");
            // https://github.com/huggingface/fuel/issues/1241
            ...
            let do_not_detach = FUEL_GRAD_DO_NOT_DETACH.with(|b| *b);
...
///
/// ```rust
/// use fuel_core::{Tensor, Device, Var};
...
impl GradStore {
    ...
    ///
    /// ```rust
    /// use fuel_core::{Device, Tensor, Var};
    /// let x = Var::new(1f32, &Device::cpu())?;
    /// let y = x.sqr()?;
    /// let mut grads = y.backward()?;
