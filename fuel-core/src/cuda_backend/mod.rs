...
//! a [`fuel_graph_cuda::CudaDevice`] into a [`crate::Device`].
//!
//! After step B1 of the backend extraction, all CUDA logic lives in
//! `fuel-graph-cuda`; this module owns only the `From<CudaDevice> for Device`
...
#[cfg(feature = "cuda")]
impl From<fuel_graph_cuda::CudaDevice> for Device {
    fn from(dev: fuel_graph_cuda::CudaDevice) -> Self {
...
pub fn new_device(ordinal: usize) -> Result<Device> {
    Ok(fuel_graph_cuda::CudaDevice::new(ordinal)?.into())
}
...
pub fn new_device_with_stream(ordinal: usize) -> Result<Device> {
    Ok(fuel_graph_cuda::CudaDevice::new_with_stream(ordinal)?.into())
}
...
/// not a CUDA device.
#[cfg(feature = "cuda")]
pub fn as_device(device: &Device) -> Result<&fuel_graph_cuda::CudaDevice> {
    device
        .inner
        .as_any()
        .downcast_ref::<fuel_graph_cuda::CudaDevice>()
