...
mod tests {
    ...
    // `same_geometry_different_weights_do_not_share_a_plan` moved to
    // `fuel-model-llama/tests/decode_shape_geometry.rs` (fuel-core dissolution):
    // it constructs a real `LlamaModel`, which now lives in `fuel-model-llama`, a
    // crate that depends on `fuel-core` -- keeping the test here as a
    // [dev-dependencies] consumer hits Cargo's diamond-dependency limitation
    // (two non-unifying copies of `fuel_core` in the same test binary's
    // dependency graph, one being tested and one `fuel-model-llama` depends on).
    // Moving the test to `fuel-model-llama`'s own integration tests avoids the
    // diamond entirely: there `fuel_core` is an ordinary single dependency.
}
