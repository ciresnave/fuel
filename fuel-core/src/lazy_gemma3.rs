...
impl Gemma3Model {
    ...
    fn run_backbone(&self, tokens: &[u32], start_pos: usize) -> Result<LazyTensor> {
        ...
        let rope_cos_g = h.const_f32_like(cos_g, rope_shape.clone());
        let rope_sin_g = h.const_f32_like(sin_g, rope_shape.clone());
...
fn apply_offset_rms_norm(
    ...
    assert_eq!(
        gain.len(), dim,
        "apply_offset_rms_norm: gain length {} != dim {dim}", gain.len(),
    );
...
fn opt_bias(
    ...
    match bias {
        ...
        Some(b) => {
            assert_eq!(
                b.len(), last_dim,
                "opt_bias: bias length {} != last_dim {last_dim}", b.len(),
            );
