...
impl MixtralModel {
    ...
    fn run_backbone_embeds(&self, embeds: &Tensor, start_pos: usize) -> Result<Tensor> {
        ...
        if seq == 0 {
            return Err(
                crate::Error::Msg("MixtralModel::forward_embeds: seq must be > 0".into()).bt(),
            );
        }
...
mod tests {
    ...
    fn tiny_weights(cfg: &MixtralConfig) -> MixtralWeights {
        ...
        let layers: Vec<MixtralLayerWeights> = (0..cfg.num_hidden_layers)
            .map(|_| {
                ...
                let experts: Vec<MixtralExpertWeights> = (0..cfg.num_local_experts)
                    .map(|_| MixtralExpertWeights {
