...
pub fn qwen3_config_from_gguf_content(
    ...
    let get = |k: &str| -> Result<&crate::quantized::gguf_file::Value> {
        md.get(k)
            .ok_or_else(|| crate::Error::Msg(format!("gguf metadata: missing key {k:?}")).bt())
    };
    ...
    let vocab_size = match md.get("qwen3.vocab_size") {
        ...
        None => {
            let info = content
                ...
                .ok_or_else(|| crate::Error::Msg("gguf: missing token_embd.weight".into()).bt())?;
            let dims = info.shape.dims();
            if dims.is_empty() {
                return Err(
                    crate::Error::Msg("gguf: token_embd.weight has empty shape".into()).bt(),
                );
            }
            dims[0]
        }
