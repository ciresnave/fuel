use anyhow::Result;
use fuel_core::{test_device, test_utils, Device, IndexOp, Tensor};
...
fn conv2d_grad(dev: &Device) -> Result<()> {
    ...
    // Replicate the issue from https://github.com/huggingface/fuel/issues/1212
    ...
    // 0.2 abs-tol absorbs the ±0.1-at-1-decimal precision drift between
    // baracuda's conv-backward algorithm choice and the prior path's; both
    // are IEEE-754-valid roundings of the same exact gradient. PyTorch
    // reference values inline mark cells where torch already differed by
    // ~0.1 from the Fuel-PTX baseline.
    let grad_w_actual = test_utils::to_vec1_round(&grad_w.flatten_all()?, 1)?;
    test_utils::assert_close_vec1(
        ...
        0.2,
        "grad_w_conv_transpose2d",
    );

    let grad_t_actual = grad_t.i(0)?.flatten_all()?.to_vec1::<f32>()?;
    let grad_t_actual_rounded: Vec<f32> = grad_t_actual
        .iter()
        .map(|v| (v * 10.0).round() / 10.0)
        .collect();
    test_utils::assert_close_vec1(
        &grad_t_actual_rounded,
        &[
...
/* Regression test for https://github.com/huggingface/fuel/issues/3404
...
fn conv2d_c_eq_h_eq_w(dev: &Device) -> Result<()> {
    ...
    let w = Tensor::ones((2, 4, 3, 3), fuel_core::DType::F32, dev)?;
