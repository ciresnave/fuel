    ...
    R: ReduceOp<T>,
{
    ...
    for (in_flat, &v) in in_view.iter().take(in_elems).enumerate() {
        ...
        acc[out_flat] = R::fold(acc[out_flat], v);
    }
    for (slot, &a) in out_view.iter_mut().zip(&acc) {
        *slot = R::finalize(a, count);
    }
