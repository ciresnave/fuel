...
impl LinkRegistry for CudaLinkRegistry {
    ...
    fn resolve_fused(&self, _symbol: &str) -> Option<KernelRef> {
        ...
        // primitive `op_kind`. (A `FusedOps::ROPE` fused contract was added
        // and reverted on 2026-07-13; see `baracuda_dispatch.rs`'s "FUSED
        // FusedOps::ROPE … REVERTED" note for why baracuda's interleaved
        // `rope_apply` can't back Fuel's rotate-half ROPE.)
        None
    }
