...
use crate::fkc::cost_expr::{CostNode, parse_expr as parse_cost_expr};
...
pub enum AxisIndex {
    FromStart(usize),
    FromEnd(usize),
} // dim[2]=FromStart(2); dim[-1]=FromEnd(1)
...
pub struct ShapeConstraint {
    ...
    pub freetext: Vec<String>,
}

fn parse_axis(s: &str) -> Option<AxisIndex> {
    let s = s.trim();
    if let Some(n) = s.strip_prefix('-') {
        n.trim().parse::<usize>().ok().map(AxisIndex::FromEnd)
    } else {
        s.parse::<usize>().ok().map(AxisIndex::FromStart)
    }
}
...
pub fn parse_rank_spec(s: &str) -> Option<RankSpec> {
    let s = s.trim();
    if s == "any" {
        return Some(RankSpec::Any);
    }
    if let Ok(n) = s.parse::<usize>() {
        return Some(RankSpec::Exact(n));
    }
    if let Some((lo, hi)) = s.split_once("..=") {
        return Some(RankSpec::Range {
            lo: lo.trim().parse().ok()?,
            hi: Some(hi.trim().parse().ok()?),
        });
    }
...
pub fn parse_shape_constraint(
    ...
    for seg_raw in raw.split(';') {
        ...
        if let Some(r) = seg.strip_prefix("same_as=") {
            out.atoms.push(ShapeAtom::SameAs(r.trim().into()));
            continue;
        }
        if let Some(r) = seg.strip_prefix("same_rank=") {
            out.atoms.push(ShapeAtom::SameRank(r.trim().into()));
            continue;
        }
        if let Some(r) = seg.strip_prefix("broadcast_to=") {
            out.atoms.push(ShapeAtom::BroadcastTo(r.trim().into()));
            continue;
        }
        if let Some(r) = seg.strip_prefix("last_dim_eq=") {
            out.atoms.push(ShapeAtom::LastDimEq(r.trim().into()));
            continue;
        }
        if let Some(r) = seg.strip_prefix("rank=") {
            // COMMITTED keyword
            out.atoms
                .push(ShapeAtom::Rank(parse_rank_spec(r).ok_or_else(unparse)?));
            continue;
        }
        if seg.starts_with("divisible(") {
            // COMMITTED: require close paren
            let inner = seg
            ...
            out.atoms.push(ShapeAtom::Divisible { lhs, rhs });
            continue;
        }
        if seg.starts_with("capacity_ge(") {
            // COMMITTED: require close paren
            let inner = seg
                .strip_prefix("capacity_ge(")
                .and_then(|s| s.strip_suffix(')'))
                .ok_or_else(unparse)?;
            let (a, b) = split_two_args(inner).ok_or_else(unparse)?;
            let axis = a
                ...
                .and_then(parse_axis)
                .ok_or_else(unparse)?;
            out.atoms.push(ShapeAtom::CapacityGe {
                axis,
                sym: b.trim().to_string(),
            });
            continue;
        }
...
fn seed_axis(profile: SeedProfile, axis: usize, rank: usize) -> i64 {
    if profile.odd_last && rank > 0 && axis == rank - 1 {
        3
    } else {
        profile.base
    }
...
fn as_dim_ref(node: &CostNode) -> Option<(Option<String>, AxisIndex)> {
    if let CostNode::Index { base, index } = node {
        let axis = match &**index {
            ...
            CostNode::Neg(inner) => {
                if let CostNode::Lit(v) = &**inner {
                    AxisIndex::FromEnd(*v as usize)
                } else {
                    return None;
                }
            }
        ...
        if let CostNode::Sym(s) = &**base {
            return Some(if let Some(r) = s.strip_suffix(".dim") {
                (Some(r.to_string()), axis)
            } else if s == "dim" {
                (None, axis)
            } else {
                return None;
...
fn axis_to_index(axis: AxisIndex, rank: usize) -> Option<usize> {
    match axis {
        AxisIndex::FromStart(i) => Some(i),
        AxisIndex::FromEnd(n) => rank.checked_sub(n),
    }
}
...
fn eval_dim_expr(
    ...
    match node {
        ...
        CostNode::Bin { op, lhs, rhs } => {
            let (l, r) = (
                eval_dim_expr(lhs, s, ranks, self_role)?,
                eval_dim_expr(rhs, s, ranks, self_role)?,
            );
...
fn warn(section: &str, message: String) -> ImportWarning {
    ImportWarning {
        section: section.into(),
        message,
    }
...
fn dep_sources(atoms: &[ShapeAtom], input_roles: &HashSet<String>) -> Vec<String> {
    fn collect(n: &CostNode, roles: &HashSet<String>, out: &mut Vec<String>) {
        ...
        match n {
            CostNode::Bin { lhs, rhs, .. } => {
                ...
                collect(rhs, roles, out);
            }
            CostNode::Neg(i) => collect(i, roles, out),
            CostNode::Index { base, index } => {
                collect(base, roles, out);
                collect(index, roles, out);
            }
            CostNode::Call { args, .. } => {
                for a in args {
                    collect(a, roles, out);
    ...
    for a in atoms {
        match a {
            ShapeAtom::SameAs(r) | ShapeAtom::BroadcastTo(r) | ShapeAtom::LastDimEq(r)
                if input_roles.contains(r) =>
            {
                deps.push(r.clone())
            }
            ShapeAtom::DimEq { expr, .. } => collect(expr, input_roles, &mut deps),
            ShapeAtom::Divisible { lhs, rhs } => {
                collect(lhs, input_roles, &mut deps);
                collect(rhs, input_roles, &mut deps);
            }
            _ => {}
        }
    }
...
fn topo_order(
    ...
    if out.len() < order_in.len() {
        ...
        w.push(warn(
            section,
            format!("shape_constraint dependency cycle among {residual:?}; using seed shapes"),
        ));
        for r in order_in {
            if !out.contains(r) {
                out.push(r.clone());
...
fn apply_atom(
    ...
    match atom {
        ...
        ShapeAtom::SameAs(src) | ShapeAtom::BroadcastTo(src) => match s.dims.get(src).cloned() {
            ...
            None => w.push(warn(
                section,
                format!("operand `{self_role}` references unknown role `{src}`; using seed shape"),
            )),
        },
        ShapeAtom::LastDimEq(src) => match s.dims.get(src).and_then(|d| d.last().copied()) {
            ...
            None => w.push(warn(
                section,
                format!(
                    "operand `{self_role}` last_dim_eq references unknown role `{src}`; using seed"
                ),
            )),
        },
        ShapeAtom::DimEq { axis, expr } => match eval_dim_expr(expr, s, ranks, self_role) {
            ...
            None => w.push(warn(
                section,
                format!("operand `{self_role}` dim rule unresolved; using seed"),
            )),
        },
        ShapeAtom::Divisible { lhs, rhs } => {
            if let (Some((role, axis)), Some(v)) =
                (as_dim_ref(lhs), eval_dim_expr(rhs, s, ranks, self_role))
            {
                if v > 0 {
...
pub fn solve_probe_shapes(
    ...
    for (i, d) in inputs.iter().enumerate() {
        ...
        for seg in &sc.freetext {
            warnings.push(warn(
                section,
                format!("operand `{operand}` shape_constraint free text: {seg}"),
            ));
        }
        ...
        if d.rank.is_some() && rank_spec.is_none() {
            warnings.push(warn(
                section,
                format!("operand `{operand}` rank field is malformed; defaulting to rank 4"),
            ));
        }
    ...
    for profile in PROFILES {
        ...
        let mut s = Solve {
            dims: HashMap::with_capacity(inputs.len()),
            sym: HashMap::new(),
            base: profile.base,
        };
        for key in &keys {
        ...
        for ((d, role), key) in inputs.iter().zip(roles.iter()).zip(keys.iter()) {
            let dims: Vec<usize> = s
                .dims
                .get(key)
                .map(|v| v.iter().map(|&x| x.max(1) as usize).collect())
                .unwrap_or_default();
...
mod tests {
    ...
    fn rank_any_defaults_to_4_and_open_range_uses_lo() {
        ...
        assert_eq!(
            solve_probe_shapes(&[any], "s", &mut Vec::new()).unwrap()[0][0]
                .1
                .rank(),
            4
        );
        ...
        assert_eq!(
            solve_probe_shapes(&[open], "s", &mut Vec::new()).unwrap()[0][0]
                .1
                .rank(),
            2
        );
    ...
    fn parse_covers_vocab_freetext_and_rejects_malformed_vocab() {
        assert_eq!(
            parse_shape_constraint("same_as=k", "s", "v").unwrap().atoms,
            vec![ShapeAtom::SameAs("k".into())]
        );
        assert_eq!(
            parse_shape_constraint("same_rank=k", "s", "v")
                .unwrap()
                .atoms,
            vec![ShapeAtom::SameRank("k".into())]
        );
        assert_eq!(
            parse_shape_constraint("broadcast_to=x", "s", "v")
                .unwrap()
                .atoms,
            vec![ShapeAtom::BroadcastTo("x".into())]
        );
        assert_eq!(
            parse_shape_constraint("last_dim_eq=x", "s", "v")
                .unwrap()
                .atoms,
            vec![ShapeAtom::LastDimEq("x".into())]
        );
        assert_eq!(
            parse_shape_constraint("rank=4", "s", "x").unwrap().atoms,
            vec![ShapeAtom::Rank(RankSpec::Exact(4))]
        );
        assert_eq!(
            parse_shape_constraint("rank=2..=4", "s", "x")
                .unwrap()
                .atoms,
        ...
        assert_eq!(
            parse_shape_constraint("rank=any", "s", "x").unwrap().atoms,
            vec![ShapeAtom::Rank(RankSpec::Any)]
        );
        assert_eq!(
            parse_shape_constraint("rank=2..", "s", "x").unwrap().atoms,
        ...
        assert!(matches!(
            parse_shape_constraint("divisible(q.dim[2], k.dim[2])", "f", "k")
                .unwrap()
                .atoms[0],
            ShapeAtom::Divisible { .. }
        ));
        assert!(matches!(
            parse_shape_constraint("capacity_ge(dim[0], seqlen)", "f", "kv")
                .unwrap()
                .atoms[0],
            ShapeAtom::CapacityGe { .. }
        ));
        ...
        assert!(matches!(
            parse_shape_constraint("rank=banana", "s", "x").unwrap_err(),
    ...
    fn bare_symbol_is_shared_mutably_across_operands() {
        ...
        assert_eq!(
            bk, 8,
            "b's last axis must observe the SHARED, bumped k (=8), not a fresh k=2"
        );
    }
    ...
    fn dim_expr_overflow_degrades_without_panic() {
        ...
        assert!(
            result.is_ok(),
            "overflow must degrade gracefully, not panic or error"
        );
        assert!(
            !w.is_empty(),
            "the unresolved (overflowed) atom should surface an ImportWarning"
        );
    }
    ...
    fn pathological_int_min_div_and_neg_degrade_without_panic() {
        ...
        assert!(
            !wa.is_empty(),
            "the unresolved (MIN/-1) atom should surface an ImportWarning"
        );
        ...
        assert!(
            !wb.is_empty(),
            "the unresolved (Neg MIN) atom should surface an ImportWarning"
        );
    ...
    fn two_unnamed_operands_are_not_aliased() {
        ...
        let a = crate::fkc::schema::TensorDesc {
            name: None,
            optional: false,
            dtypes: vec!["F32".into()],
            dtype_class: None,
            layout: None,
            rank: Some(serde_yaml_ng::Value::Number(2u64.into())),
            shape_constraint: None,
            fdx: None,
            device: None,
            substrate: None,
        ...
        assert_eq!(
            combos[0][0].1.rank(),
            2,
            "first unnamed operand keeps rank 2"
        );
        assert_eq!(
            combos[0][1].1.rank(),
            3,
            "second unnamed operand keeps rank 3, not aliased to 2"
        );
    }
    ...
    fn dependency_ordering_resolves_and_cycles_fall_back_with_warning() {
        ...
        assert_eq!(
            combos[0].iter().find(|(r, _, _)| r == "v").unwrap().1,
            Shape::from_dims(&[8, 2])
        );
        ...
        assert!(
            w.iter().any(|x| x.message.to_lowercase().contains("cycle")),
            "warns: {w:?}"
        );
    }
    ...
    fn double_reference_to_one_predecessor_is_not_a_false_cycle() {
        ...
        assert!(
            !w.iter().any(|m| m.message.to_lowercase().contains("cycle")),
        ...
        assert_eq!(
            x0, 10,
            "x.dim0 = rounded k.dim0(8) + k.dim1(2); proves k-before-x ordering"
        );
    }
    ...
    fn duplicate_operand_name_emits_a_warning() {
        ...
        assert!(
            w.iter().any(|m| m.message.contains("dup")
                && m.message.to_lowercase().contains("more than once")),
    ...
    fn dtype_pick_and_soft_fallback_warnings() {
        ...
        assert_eq!(
            solve_probe_shapes(&[c], "s", &mut Vec::new()).unwrap()[0][0].2,
            DType::BF16
        );
    ...
    fn malformed_rank_field_warns_and_defaults_to_rank_4() {
        ...
        assert_eq!(
            combos[0][0].1.rank(),
            4,
            "malformed rank field still defaults to rank 4"
        );
        assert!(
            w.iter()
                .any(|x| x.message.contains("weird") && x.message.to_lowercase().contains("rank")),
