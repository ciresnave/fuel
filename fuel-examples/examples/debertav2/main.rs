...
use fuel_nn::ops::softmax;
use fuel_nn::VarBuilder;
use fuel_transformers::models::debertav2::{Config as DebertaV2Config, DebertaV2NERModel};
use fuel_transformers::models::debertav2::{DebertaV2SeqClassificationModel, Id2Label};
use fuel_transformers::models::debertav2::{NERItem, TextClassificationItem};
...
impl Args {
    fn build_model_and_tokenizer(
        ...
        let mut tokenizer = Tokenizer::from_file(tokenizer_filename)
            .map_err(|e| fuel::Error::Msg(format!("Tokenizer error: {e}")))?;
        tokenizer.with_padding(Some(PaddingParams::default()));

        let vb = if self.use_pth {
            VarBuilder::from_pth(
                &weights_filename,
                fuel_transformers::models::debertav2::DTYPE,
                &device,
            )?
        } else {
            unsafe {
...
) -> impl Fn(F) -> Result<(), fuel::Error>
where
    F: Fn(&Tensor, Tensor, Tensor) -> Result<(), fuel::Error>,
{
    move |code: F| -> Result<(), fuel::Error> {
