...
fn main() -> Result<()> {
    ...
    let filenames: Vec<std::path::PathBuf> = match args.weights {
        Some(files) => files
            .split(',')
    ...
    let helium_cfg: HeliumConfig = helium_config_from_hf_json_str(&config_json)?;

    // HeliumWeights uses the shared `LayerWeights` shape that LlamaWeights
    // also uses, so we can reuse the LLaMA safetensors loader and lift
    // the result.
    ...
    let eos_token_id: Option<u32> = parse_eos_token_id(&config_json);
    let bos_token_id: Option<u32> = parse_bos_token_id(&config_json);
    ...
    for index in 0..args.sample_len {
        ...
        let vocab_size = helium_cfg.vocab_size;
        let seq = tokens.len();
        ...
        generated_tokens += 1;
        if Some(next_token) == eos_token_id || Some(next_token) == bos_token_id {
            break;
        }
...
fn helium_config_from_hf_json_str(json: &str) -> Result<HeliumConfig> {
    ...
    let rms_norm_eps = get_f64("rms_norm_eps").unwrap_or(1e-8);
    let rope_theta = get_f64("rope_theta").unwrap_or(100_000.0);
