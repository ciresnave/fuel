...
use fuel::{bail, DType, Device, Result, Tensor};
use fuel_transformers::models::llava::{
...
impl ImageProcessor {
    pub fn from_pretrained(clip_id: &str) -> Result<Self> {
        let api = Api::new().map_err(|e| fuel::Error::Msg(e.to_string()))?;
        let api = api.model(clip_id.to_string());
        let config_filename = api
            .get("preprocessor_config.json")
            .map_err(|e| fuel::Error::Msg(e.to_string()))?;
        let image_processor =
            serde_json::from_slice(&std::fs::read(config_filename).map_err(fuel::Error::Io)?)
                .map_err(|e| fuel::Error::Msg(e.to_string()))?;
        Ok(image_processor)
    }
