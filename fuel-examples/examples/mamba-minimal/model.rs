...
use fuel_transformers::models::with_tracing::{linear, linear_no_bias, Linear};
...
impl MambaBlock {
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let conv_cfg = fuel_nn::Conv1dConfig {
            groups: d_inner,
            padding: d_conv - 1,
            ..Default::default()
        };
        let conv1d = fuel_nn::conv1d(d_inner, d_inner, d_conv, conv_cfg, vb.pp("conv1d"))?;
    ...
    fn ssm(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let a = self.a_log.to_dtype(fuel::DType::F32)?.exp()?.neg()?;
        let d = self.d.to_dtype(fuel::DType::F32)?;
...
impl Module for MambaBlock {
    ...
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let xs = xs
            ...
            .t()?;
        let xs = fuel_nn::ops::silu(&xs)?;
        let ys = (self.ssm(&xs)? * fuel_nn::ops::silu(res))?;
