...
use fuel_transformers::models::encodec;
use fuel_transformers::models::metavoice::{adapters, gpt, tokenizers, transformer};
use fuel_transformers::models::quantized_metavoice::transformer as qtransformer;
...
fn main() -> Result<()> {
    ...
    let api = Api::new()?;
    let repo = api.model("lmz/fuel-metavoice".to_string());
    ...
    for logits in logits.iter() {
        ...
        for step in 0..seq_len {
            ...
            let prs = fuel_nn::ops::softmax_last_dim(logits)?.to_vec1::<f32>()?;
    ...
    let pcm = fuel_examples::audio::normalize_loudness(&pcm, 24_000, true)?;
