...
use fuel_transformers::models::mimi::{Config, Model};
        ...
        Action::AudioToCode | Action::AudioToAudio => {
            ...
            match args.streaming {
                Some(chunk_size) => {
                    ...
                    Tensor::cat(&code_chunks, fuel::D::Minus1)?
                }
                None => {
                    let pcm_len = pcm.len();
    ...
    match args.action {
        ...
        Action::AudioToAudio | Action::CodeToAudio => {
            let pcm = match args.streaming {
                Some(chunk_size) => {
                    let seq_len = codes.dim(fuel::D::Minus1)?;
                    ...
                    for chunk_start in (0..seq_len).step_by(chunk_size) {
                        ...
                        let codes = codes.narrow(fuel::D::Minus1, chunk_start, chunk_len)?;
                        let pcm = model.decode_step(&codes.into())?;
                        if let Some(pcm) = pcm.as_option() {
                            pcm_chunks.push(pcm.clone())
                        }
                    }
                    Tensor::cat(&pcm_chunks, fuel::D::Minus1)?
                }
