...
use fuel_nn::{loss, ops, Conv2d, Linear, Module, ModuleT, Optimizer, VarBuilder, VarMap};

const IMAGE_DIM: usize = 784;
const LABELS: usize = 10;

fn linear_z(in_dim: usize, out_dim: usize, vs: VarBuilder) -> Result<Linear> {
    let ws = vs.get_with_hints((out_dim, in_dim), "weight", fuel_nn::init::ZERO)?;
    let bs = vs.get_with_hints(out_dim, "bias", fuel_nn::init::ZERO)?;
...
impl Model for Mlp {
    fn new(vs: VarBuilder) -> Result<Self> {
        let ln1 = fuel_nn::linear(IMAGE_DIM, 100, vs.pp("ln1"))?;
        let ln2 = fuel_nn::linear(100, LABELS, vs.pp("ln2"))?;
        Ok(Self { ln1, ln2 })
    }
...
impl ConvNet {
    fn new(vs: VarBuilder) -> Result<Self> {
        let conv1 = fuel_nn::conv2d(1, 32, 5, Default::default(), vs.pp("c1"))?;
        let conv2 = fuel_nn::conv2d(32, 64, 5, Default::default(), vs.pp("c2"))?;
        let fc1 = fuel_nn::linear(1024, 1024, vs.pp("fc1"))?;
        let fc2 = fuel_nn::linear(1024, LABELS, vs.pp("fc2"))?;
        let dropout = fuel_nn::Dropout::new(0.5);
        Ok(Self {
            conv1,
            conv2,
            fc1,
...
fn training_loop_cnn(
    ...
    let dev = fuel::Device::cuda_if_available(0)?;
    ...
    let adamw_params = fuel_nn::ParamsAdamW {
        lr: args.learning_rate,
        ..Default::default()
    };
    let mut opt = fuel_nn::AdamW::new(varmap.all_vars(), adamw_params)?;
...
fn training_loop<M: Model>(
    ...
    let mut sgd = fuel_nn::SGD::new(varmap.all_vars(), args.learning_rate)?;
    let test_images = m.test_images.to_device(&dev)?;
    let test_labels = m.test_labels.to_dtype(DType::U32)?.to_device(&dev)?;
    for epoch in 1..=args.epochs {
...
pub fn main() -> anyhow::Result<()> {
    ...
    let m = if let Some(directory) = args.local_mnist {
        fuel_datasets::vision::mnist::load_dir(directory)?
    } else {
