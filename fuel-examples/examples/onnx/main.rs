...
pub fn main() -> anyhow::Result<()> {
    ...
    let image = match args.which {
        Which::SqueezeNet | Which::EfficientNet => {
            fuel_examples::imagenet::load_image224(&args.image)?
        }
        Which::EsrGan => fuel_examples::imagenet::load_image_with_std_mean(
            &args.image,
            128,
            &[0.0f32, 0.0, 0.0],
            &[1.0f32, 1.0, 1.0],
    ...
    let model = fuel_onnx::read_file(model)?;
    let graph = model.graph.as_ref().unwrap();
    let mut inputs = std::collections::HashMap::new();
    inputs.insert(graph.input[0].name.to_string(), image.unsqueeze(0)?);
    let mut outputs = fuel_onnx::simple_eval(&model, inputs)?;
    let output = outputs.remove(&graph.output[0].name).unwrap();
    let prs = match args.which {
        Which::SqueezeNet => fuel_nn::ops::softmax(&output, D::Minus1)?,
    ...
    match args.which {
        ...
        Which::EsrGan => {
            let max_pixel_val = fuel::Tensor::try_from(255.0f32)?
                .to_device(prs.device())?
                .broadcast_as(prs.shape())?;
            let out = (prs * max_pixel_val)?.i(0)?.to_dtype(fuel::DType::U8)?;
