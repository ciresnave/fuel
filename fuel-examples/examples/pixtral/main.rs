...
use fuel_transformers::models::pixtral::{vision_model, Config, Model};
...
fn main() -> Result<()> {
    ...
    let image = if args.image.ends_with(".safetensors") {
        match fuel::safetensors::load(&args.image, &device)?.remove("img") {
            None => anyhow::bail!("no img tensor in {}", args.image),
            Some(v) => v,
        }
    } else {
        fuel_examples::imagenet::load_image_with_std_mean(
            &args.image,
            1024,
