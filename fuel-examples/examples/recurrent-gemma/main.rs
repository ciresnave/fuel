...
use fuel_transformers::models::quantized_recurrent_gemma::Model as QModel;
use fuel_transformers::models::recurrent_gemma::{Config, Model as BModel};
...
impl TextGeneration {
    ...
    fn new(
        ...
        let sampling = match temp {
            None => fuel_transformers::generation::Sampling::ArgMax,
            Some(temperature) => match top_p {
                None => fuel_transformers::generation::Sampling::TopK {
                    temperature,
                    k: top_k,
                },
                Some(top_p) => fuel_transformers::generation::Sampling::TopKThenTopP {
                    temperature,
                    k: top_k,
                    p: top_p,
                },
    ...
    let filenames = match args.weight_files {
        ...
        None => {
            if args.quantized {
                ...
                let filename = api.model("lmz/fuel-gemma".to_string()).get(filename)?;
                vec![filename]
            } else {
