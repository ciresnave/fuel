...
use fuel_transformers::models::segformer::{
...
fn get_vb_and_config(
    ...
    let vb =
        unsafe { VarBuilder::from_mmaped_safetensors(&[model_file], fuel::DType::F32, device)? };
...
fn classification_task(args: ClassificationArgs, device: &Device) -> anyhow::Result<()> {
    ...
    let classification = fuel_nn::ops::softmax_last_dim(&classification)?;
