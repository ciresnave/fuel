...
XL using Rust and [fuel](https://github.com/huggingface/fuel).

The `stable-diffusion` example is a conversion of
[diffusers-rs](https://github.com/LaurentMazare/diffusers-rs) using fuel
...
`FUEL_FLASH_ATTN_BUILD_DIR` environment variable to something like
`/home/user/.fuel` to ensures that the compilation artifacts are properly
cached.
