...
use fuel::lazy_stablelm::{StableLmConfig, StableLmLayerWeights, StableLmModel, StableLmWeights};
...
fn main() -> Result<()> {
    ...
    let filenames: Vec<std::path::PathBuf> = match args.weight_files.as_ref() {
        ...
        None => match args.which {
            Which::V1Orig | Which::V2 | Which::V2Zephyr => vec![repo.get("model.safetensors")?],
...
fn stablelm_config_from_hf_json_str(json: &str, which: Which) -> Result<StableLmConfig> {
    ...
    let use_qkv_bias =
        get_bool("use_qkv_bias").unwrap_or(matches!(which, Which::V2 | Which::V2Zephyr));
    Ok(StableLmConfig {
        vocab_size,
        hidden_size,
        intermediate_size,
