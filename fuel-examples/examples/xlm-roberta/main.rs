...
use fuel_transformers::models::xlm_roberta::{
...
fn main() -> Result<()> {
    ...
    let vb = if weights_filename.ends_with("model.safetensors") {
        unsafe {
            VarBuilder::from_mmaped_safetensors(&[weights_filename], fuel::DType::F16, &device)
                .unwrap()
        }
    } else {
        println!("Loading weights from pytorch_model.bin");
        VarBuilder::from_pth(&weights_filename, fuel::DType::F16, &device).unwrap()
    };
    tokenizer
    ...
    match args.task {
        ...
        Task::Reranker => {
            ...
            let output = fuel_nn::ops::sigmoid(&output)?.t().unwrap();
            let ranks = output
