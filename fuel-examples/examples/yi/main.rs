...
fn main() -> Result<()> {
    ...
    let llama_cfg = LlamaConfig {
        vocab_size: yi_cfg.vocab_size,
        dim: yi_cfg.hidden_size,
        n_layers: yi_cfg.num_hidden_layers,
        n_heads: yi_cfg.num_attention_heads,
        n_kv_heads: yi_cfg.num_key_value_heads,
        head_dim: yi_cfg.head_dim,
        ffn_dim: yi_cfg.intermediate_size,
        norm_eps: yi_cfg.rms_norm_eps,
        rope_base: yi_cfg.rope_theta,
    };
    ...
    for index in 0..args.sample_len {
        ...
        if let Some(t) = tok_stream.next_token(next)? {
            print!("{}", t.replace("<|im_end|>", "\n"));
            std::io::stdout().flush()?;
        }
    }
    let dt = start_gen.elapsed();
    if let Some(r) = tok_stream.decode_rest().map_err(E::msg)? {
        print!("{r}");
    }
    println!(
...
fn yi_config_from_hf_json_str(json: &str, which: Which) -> Result<YiConfig> {
    ...
    let (dh, di, dl, dh2, dkv, dhead) = match which {
        Which::L6b => (4096, 11_008, 32, 32, 4, 128),
        Which::L34b => (7168, 20_480, 60, 56, 8, 128),
    };
    Ok(YiConfig {
...
fn parse_eos_token_id(j: &str) -> Option<u32> {
