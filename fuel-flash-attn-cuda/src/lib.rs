...
impl FlashAttn {
    ...
    >(
        ...
        let q = q.slice(q_l.start_offset()..q.len());
        let k = k.slice(k_l.start_offset()..k.len());
        let v = v.slice(v_l.start_offset()..v.len());
        ...
        if q_stride[q_rank - 1] != 1 {
            fuel::bail!("the last dim of q must be contiguous {q_stride:?}")
        }
        if k_stride[k_rank - 1] != 1 {
            fuel::bail!("the last dim of k must be contiguous {k_stride:?}")
        }
        if v_stride[v_rank - 1] != 1 {
            fuel::bail!("the last dim of v must be contiguous {v_stride:?}")
        }
        ...
        if expected_kv != k_l.shape().dims4()? {
            fuel::bail!("shape mismatch q {:?} and k {:?}", q_l.shape(), k_l.shape())
        }
        if expected_kv != v_l.shape().dims4()? {
            fuel::bail!("shape mismatch q {:?} and v {:?}", q_l.shape(), v_l.shape())
        }
        if head_size_og > 512 {
            fuel::bail!("only supports head dimension at most 512 (got {head_size_og})")
        }
        if head_size_og % 8 != 0 {
            // TODO: Handle head sizes that are not a multiple of 8 via some padding.
            fuel::bail!("only supports head sizes that are a multiple of 8 (got {head_size_og})")
        }
        if num_heads % num_heads_k != 0 {
            fuel::bail!("number of k/v heads {num_heads_k} must divide number of heads in query {num_heads}")
        }
        ...
        unsafe {
            let q_ptr = q.as_raw().0;
            let k_ptr = k.as_raw().0;
            let v_ptr = v.as_raw().0;
            let dst_ptr = dst.as_raw().0;
            let softmax_lse_ptr = softmax_lse.as_raw().0;
            ffi::run_mha(
        ...
        let dst = fuel::CudaStorage::wrap_cuda_slice(dst, dev.clone());
        Ok((dst, out_shape))
    }
}

impl fuel::CustomOp3 for FlashAttn {
    ...
    fn fwd(
        ...
        if s1.as_any().is::<CpuBackendStorage>() {
            fuel::bail!("no cpu support for flash-attn");
        }
        let q = s1
            .as_any()
            .downcast_ref::<CudaStorage>()
            .ok_or_else(|| fuel::Error::Msg("flash-attn requires CUDA storage".into()).bt())?;
        let k = s2
            .as_any()
            .downcast_ref::<CudaStorage>()
            .ok_or_else(|| fuel::Error::Msg("flash-attn requires CUDA storage".into()).bt())?;
        ...
        let (out, shape) = match q.dtype() {
            ...
            dt => fuel::bail!("flash-attn is only supported for f16/bf16 ({dt:?})"),
        }?;
...
impl FlashAttnVarLen {
    ...
    >(
        ...
        let seqlens_k = match seqlens_k_layout.contiguous_offsets() {
            ...
            None => fuel::bail!("seqlens_k has to be contiguous"),
        };
        ...
        if nseqlens_q < 2 {
            fuel::bail!("seqlens_q should have a len >= 2 {nseqlens_q}")
        }
        let nseqlens_k = seqlens_k_layout.shape().dims1()?;
        if nseqlens_k != nseqlens_q {
            fuel::bail!("seqlens_q and seqlens_k should have the same number of elements {nseqlens_q} <> {nseqlens_k}")
        }
        ...
        unsafe {
            ...
            let dst_ptr = dst.as_raw().0;
            let softmax_lse_ptr = softmax_lse.as_raw().0;
            let seqlens_q_ptr = seqlens_q.as_raw().0;
            let seqlens_k_ptr = seqlens_k.as_raw().0;
            ffi::run_mha(
                q_ptr as *const core::ffi::c_void,
                k_ptr as *const core::ffi::c_void,
                v_ptr as *const core::ffi::c_void,
...
impl fuel::CustomOp3 for FlashAttnVarLen {
    fn name(&self) -> &'static str {
        "flash-attn-varlen"
    }
    ...
    fn fwd(
        ...
        if s1.as_any().is::<CpuBackendStorage>() {
            fuel::bail!("no cpu support for flash-attn-varlen");
        }
        let q = s1
            .as_any()
            .downcast_ref::<CudaStorage>()
            .ok_or_else(|| fuel::Error::Msg("flash-attn-varlen requires CUDA storage".into()).bt())?;
        let k = s2
            .as_any()
            .downcast_ref::<CudaStorage>()
            .ok_or_else(|| fuel::Error::Msg("flash-attn-varlen requires CUDA storage".into()).bt())?;
        ...
        let (out, shape) = match q.dtype() {
            fuel::DType::F16 => self.cuda_fwd_t::<f16>(q, l1, k, l2, v, l3, false),
            fuel::DType::BF16 => self.cuda_fwd_t::<bf16>(q, l1, k, l2, v, l3, true),
            dt => fuel::bail!("flash-attn-varlen is only supported for f16/bf16 ({dt:?})"),
        }?;
        Ok((Box::new(out), shape))
    }
}
