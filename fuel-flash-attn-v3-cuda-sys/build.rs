...
// adapted from https://github.com/huggingface/fuel-flash-attn-v1 , Oliver Dehaene
// adapted further in 2025 by Eric Buehler for fuel repo.
//
...
fn main() -> Result<()> {
    ...
    println!("cargo:rerun-if-env-changed=FUEL_NVCC_CCBIN");
    ...
    let build_dir = match std::env::var("FUEL_FLASH_ATTN_BUILD_DIR") {
        Err(_) => out_dir.clone(),
        Ok(build_dir) => {
