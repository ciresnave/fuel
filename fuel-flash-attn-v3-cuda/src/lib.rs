...
//               2025 adjusted by Eric Buehler for fuel repo.
...
impl FlashAttn {
    ...
    >(
        ...
        if !(head_size_og == 512
            ...
            || head_size_og == 64)
        {
            fuel::bail!("only supports head dimension 64, 128, 256 and 512 (got {head_size_og})")
        }
        if head_size_og % 8 != 0 {
...
impl fuel::CustomOp3 for FlashAttn {
    ...
    fn fwd(
        ...
        if s1.as_any().is::<CpuBackendStorage>() {
            fuel::bail!("no cpu support for flash-attn-v3");
        }
        let q = s1
            .as_any()
...
impl fuel::CustomOp3 for FlashAttnVarLen {
    ...
    fn fwd(
        ...
        if s1.as_any().is::<CpuBackendStorage>() {
            fuel::bail!("no cpu support for flash-attn-v3-varlen");
        }
        let q = s1
            .as_any()
            .downcast_ref::<CudaStorage>()
