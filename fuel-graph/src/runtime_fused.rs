...
fn validate_node(node: &PatternNode, n_binds: usize) -> Result<(), RuntimeFusedError> {
    match node {
        PatternNode::Op {
            ...
            let probed;
            let attrs: &OpAttrs = if has_rel_attrs(attrs) {
...
fn emit<'r>(
    ...
    match node {
        ...
        PatternNode::Op {
            ...
            let filled;
            let attrs: &OpAttrs =
                if attrs.scalars.is_empty() && arity > 0 && attrs.scalar_rel.is_none() {
...
mod tests {
    ...
    fn tanh_mul_scalar_slot_region() -> PatternNode {
        PatternNode::Op {
            ...
            operands: vec![PatternNode::Op {
                op: OpTag::MulScalar,
                attrs: Box::new(OpAttrs::default()), // empty scalars = an open slot
                operands: vec![PatternNode::Bind { index: 0 }],
            }],
        }
    }
