...
// (h_q in group, q_i):
//   1. Cooperative softmax recompute → P[qi, :].
//   2. Cooperative dP recompute       → dp[kj] = dO[qi, :] · V[kj, :].
//   3. row_dot = Σ_kj P[kj] · dp[kj]  (cooperative reduction).
//   4. dS[k_j] = (dp[k_j] - row_dot) · P[k_j]   (just our column).
//   5. Each active thread (tid < D) accumulates
//      dK[k_j, tid] += scale · dS[k_j] · Q[qi, tid].
// Final write to global at the end.
...
layout(set = 0, binding = 5, std430) buffer dKBuf { float dK[]; };
...
shared float scores[MAX_SK];   // P (after stage 1)
...
void main() {
    ...
    for (uint g = 0u; g < groups; ++g) {
        ...
        for (uint qi = 0u; qi < p.Sq; ++qi) {
            ...
            for (uint kk = tid; kk < p.Sk; kk += TPB) {
                scores[kk] *= inv_sum;     // P[qi, kk]
            }
            barrier();

            // Stage 4: dp[kk] = dO[qi, :] · V[kk, :].
            for (uint kk = tid; kk < p.Sk; kk += TPB) {
                float acc = 0.0;
                for (uint dx = 0u; dx < p.D; ++dx) {
                    acc += dO[do_row_off + dx] * V[v_off + kk * p.D + dx];
                }
                dp[kk] = acc;
            }
            barrier();

            // Stage 5: row_dot = Σ_kk P[kk] · dp[kk].
            float local_dot = 0.0;
            for (uint kk = tid; kk < p.Sk; kk += TPB) {
                local_dot += scores[kk] * dp[kk];
            }
            ...
            // Stage 6: dS[k_j] = (dp[k_j] - row_dot) · P[k_j].
            // Only this WG's column is needed. Compute once per WG
            // in thread 0, broadcast via shared mem.
            if (tid == 0u) {
                row_sum_shared = (dp[kj] - row_dot) * scores[kj];
            }
            barrier();
            float ds_kj = row_sum_shared;     // reuse the slot

            // Stage 7: accumulate dK[k_j, tid] += scale · dS · Q[qi, tid].
            if (active_thread && ds_kj != 0.0) {
                dk_accum += p.softmax_scale * ds_kj * Q[q_row_off + tid];
            }
            barrier();
        }
    }
