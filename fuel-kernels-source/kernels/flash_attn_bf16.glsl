...
// FlashAttention forward, bf16. Sibling of flash_attn_f32 — same
// naive single-pass algorithm, same shared-memory score-row
// materialization. Inputs / outputs are bf16 (stored as uint16_t);
// scores, softmax, output reduction all carry f32 accumulators
// (matches CPU's flash_attn_bf16 reference: "math at f32, narrow
// on output store").
...
layout(set = 0, binding = 0, std430) readonly buffer QBuf { uint16_t Q[]; };
layout(set = 0, binding = 1, std430) readonly buffer KBuf { uint16_t K[]; };
layout(set = 0, binding = 2, std430) readonly buffer VBuf { uint16_t V[]; };
layout(set = 0, binding = 3, std430) buffer OBuf { uint16_t O[]; };
layout(set = 0, binding = 4, std430) readonly buffer ABuf { uint16_t alibi_slopes[]; };
...
void main() {
    ...
    float alibi_h = (p.use_alibi != 0u) ? bf16_u16_to_f32(alibi_slopes[hi]) : 0.0;
    ...
    for (uint kj = tid; kj < p.Sk; kj += TPB) {
        ...
        for (uint dx = 0u; dx < p.D; ++dx) {
            float qx = bf16_u16_to_f32(Q[q_row_off + dx]);
            float kx = bf16_u16_to_f32(K[k_off + kj * p.D + dx]);
            acc += qx * kx;
        }
    ...
    for (uint dx = tid; dx < p.D; dx += TPB) {
        ...
        for (uint kj = 0u; kj < p.Sk; ++kj) {
            float vd = bf16_u16_to_f32(V[v_off + kj * p.D + dx]);
            acc += scores[kj] * vd;
        }
        O[o_row_off + dx] = uint16_t(f32_to_bf16_bits(acc * inv_sum));
    }
}
