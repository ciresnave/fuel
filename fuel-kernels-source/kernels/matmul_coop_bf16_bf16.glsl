...
// A: [M, K] bf16 (packed as u16 in storage)
// B: [K, N] bf16 (packed as u16 in storage)
// C: [M, N] f32
//
// Sibling of matmul_coop.glsl (which does f32 × bf16 → f32). Both
// sides downcast bf16 → f16 on shared-mem load and use the coop[3]
// tile (A=f16, B=f16, C=f32, R=f32) for the actual matmul.
// f32 accumulator means the only precision loss is the bf16→f16
// downcast on the inputs.
//
...
// bf16 (stored as u16) → f16 via bf16→f32→f16. Lossy in mantissa
// (bf16 has 7 bits, f16 has 10), but the f32 accumulator absorbs
// any drift from the K-axis reduction.
