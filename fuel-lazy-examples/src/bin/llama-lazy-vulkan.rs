...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    eprintln!("---");
    // Stream through generate_streaming_with_kv_context: a
    // pre-allocated `KvCache` keeps K/V on the GPU between decode
    // steps (Op::WriteSlice in-graph mutation) — no D2H/H2D
