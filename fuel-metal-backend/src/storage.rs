...
pub enum MetalError {
    ...
    Message(String),
    #[error(transparent)]
    KernelError(#[from] fuel_metal_kernels::MetalKernelError),
    #[error("{0:?}")]
    LockError(LockError),
...
impl MetalStorage {
    ...
    pub fn reduce_op(&self, op: ReduceOp, layout: &Layout, sum_dims: &[usize]) -> Result<Self> {
        ...
        if layout.is_contiguous() && reduction_shape.is_contiguous(&stride) {
            ...
            let src = buffer_o(&self.buffer, layout, self.dtype);
            fuel_metal_kernels::call_reduce_contiguous(
                &device.device,
                &encoder,
                &device.kernels,
                name,
        ...
        encoder.set_label("reduce");
        let src = buffer_o(&self.buffer, layout, self.dtype);
        fuel_metal_kernels::call_reduce_strided(
            &device.device,
            &encoder,
            &device.kernels,
            name,
    ...
    pub fn const_set(&mut self, s: fuel_ir::Scalar, l: &Layout) -> Result<()> {
        ...
        >(
            ...
            if l.is_contiguous() {
                ...
                fuel_metal_kernels::call_const_set_contiguous(
                    &device.device,
                    &encoder,
                    &device.kernels,
                    kernel_name,
                ...
                fuel_metal_kernels::call_const_set_strided(
                    &device.device,
                    &encoder,
                    &device.kernels,
                    kernel_name,
    ...
    pub fn where_cond(
        ...
        fuel_metal_kernels::call_where_cond(
            &device.device,
            &encoder,
            &device.kernels,
            name,
    ...
    pub fn conv_transpose1d(
        ...
        let buffer = if USE_COL2IM_CONV1D_TR && can_use_col2im {
            ...
            fuel_metal_kernels::call_col2im1d(
                &self.device.device,
                &encoder,
                &self.device.kernels,
                name,
    ...
    pub fn avg_pool2d(
        ...
        fuel_metal_kernels::call_pool2d(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    pub fn upsample_nearest2d(&self, inp_l: &Layout, out_w: usize, out_h: usize) -> Result<Self> {
        ...
        fuel_metal_kernels::call_upsample_nearest_2d(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    pub fn upsample_bilinear2d(
        ...
        let src = buffer_o(&self.buffer, inp_l, self.dtype);
        fuel_metal_kernels::call_upsample_bilinear_2d(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    pub fn gather(&self, src_l: &Layout, ids: &Self, ids_l: &Layout, dim: usize) -> Result<Self> {
        ...
        fuel_metal_kernels::call_gather(
            &device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    pub fn scatter_set(
        ...
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        fuel_metal_kernels::call_scatter(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    pub fn index_select(
        ...
        let src = buffer_o(&self.buffer, src_l, dtype);
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        fuel_metal_kernels::call_index_select(
            &device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    pub fn index_add(
        ...
        let src = buffer_o(&src.buffer, src_l, src.dtype);
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        fuel_metal_kernels::call_index_add(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            name,
    ...
    pub fn matmul(
        ...
        let dtype = match self.dtype {
            DType::F32 => fuel_metal_kernels::GemmDType::F32,
            DType::F16 => fuel_metal_kernels::GemmDType::F16,
            DType::BF16 => fuel_metal_kernels::GemmDType::BF16,
            dtype => {
                return Err(
        ...
        fuel_metal_kernels::call_mlx_gemm(
            &self.device.device,
            &encoder,
            &self.device.kernels,
            dtype,
    ...
    pub fn copy2d(
        ...
        if src_s == d2 && dst_s == d2 {
            ...
            let kernel_name = match self.dtype {
                DType::F32 => fuel_metal_kernels::copy2d::FLOAT,
                DType::F16 => fuel_metal_kernels::copy2d::HALF,
                DType::BF16 => fuel_metal_kernels::copy2d::BFLOAT,
            ...
            fuel_metal_kernels::call_copy2d(
                &self.device.device,
                &encoder,
                &self.device.kernels,
                kernel_name,
    ...
    pub fn copy_strided_src(
        ...
        if src_l.is_contiguous() && self.dtype == dst.dtype() {
            ...
            let kernel_name = match self.dtype {
                DType::F32 => fuel_metal_kernels::unary::strided::copy::FLOAT,
                DType::F16 => fuel_metal_kernels::unary::strided::copy::HALF,
                DType::BF16 => fuel_metal_kernels::unary::strided::copy::BFLOAT,
...
impl MetalStorage {
    ...
    pub fn binary(
        ...
        let buffer = if lhs_contiguous && rhs_contiguous {
            ...
            let buffer = device.new_buffer(el_count, dtype, op)?;
            fuel_metal_kernels::call_binary_contiguous(
                &device.device,
                &encoder,
                &device.kernels,
                kernel,
            ...
            let buffer = device.new_buffer(el_count, dtype, op)?;
            fuel_metal_kernels::call_binary_strided(
                &device.device,
                &encoder,
                &device.kernels,
                kernel,
...
impl MetalDevice {
    ...
    pub fn rand_uniform(
        ...
        fuel_metal_kernels::call_random_uniform(
            &self.device,
            &encoder,
            &self.kernels,
            name,
    ...
    pub fn rand_normal(
        ...
        fuel_metal_kernels::call_random_normal(
            &self.device,
            &encoder,
            &self.kernels,
            name,
