...
impl MoeRouter {
    ...
    pub fn route(&self, xs: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        for _ in 0..self.top_k {
            ...
            let neg_inf =
                idx_col.const_f32_like(Arc::from(vec![MASK_NEG; n]), Shape::from_dims(&[n, 1]));
...
impl MoeExpert {
    ...
    pub fn forward_dyn_m(&self, xs: &Tensor, count: fuel_ir::DynScalar) -> Result<Tensor> {
        ...
        let g = self
            ...
            .silu();
        let u = self.up.weight().apply_linear_dyn_m(
            xs,
            self.hidden_size,
            self.intermediate_size,
            count,
...
mod tests {
    ...
    fn moe_router_outputs_top_k_indices_and_weights_sum_to_one() {
        ...
        for t in 0..n {
            ...
            for v in row_idx {
                assert!((*v as usize) < num_experts, "row {t}: idx {v} out of range");
            }
    ...
    fn moe_layer_with_uniform_router_averages_experts() {
        ...
        for (i, (a, e)) in layer_out.iter().zip(expected.iter()).enumerate() {
            assert!((a - e).abs() < 1e-4, "out[{i}] expected {e}, got {a}",);
        }
    }
}
