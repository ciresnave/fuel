...
//!
//! A [`VarBuilder`] is the primary mechanism for loading model weights in fuel. It provides
...
//! ```
use crate::VarMap;
use fuel::{safetensors::Load, DType, Device, Error, Result, Shape, Tensor};
...
impl SimpleBackend for HashMap<String, Tensor> {
    fn get(
        ...
        let tensor = self
            ...
            .clone();
        if tensor.shape() != &s {
            Err(fuel::Error::UnexpectedShape {
...
impl SimpleBackend for VarMap {
    ...
    fn get_unchecked(&self, _name: &str, _dtype: DType, _dev: &Device) -> Result<Tensor> {
        fuel::bail!("`get_unchecked` does not make sense for `VarMap`, use `get`.");
    }
...
impl SimpleBackend for fuel::npy::NpzTensors {
    fn get(
        &self,
        s: Shape,
        path: &str,
...
impl SimpleBackend for fuel::pickle::PthTensors {
    fn get(
        &self,
        s: Shape,
        path: &str,
...
impl SimpleBackend for fuel::safetensors::MmapedSafetensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl SimpleBackend for fuel::safetensors::BufferedSafetensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl SimpleBackend for fuel::safetensors::SliceSafetensors<'_> {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl<'a> VarBuilder<'a> {
    ...
    pub unsafe fn from_mmaped_safetensors<P: AsRef<std::path::Path>>(
        ...
        let tensors = unsafe { fuel::safetensors::MmapedSafetensors::multi(paths)? };
    ...
    pub fn from_buffered_safetensors(data: Vec<u8>, dtype: DType, dev: &Device) -> Result<Self> {
        let tensors = fuel::safetensors::BufferedSafetensors::new(data)?;
        Ok(Self::from_backend(Box::new(tensors), dtype, dev.clone()))
    }
    ...
    pub fn from_slice_safetensors(data: &'a [u8], dtype: DType, dev: &Device) -> Result<Self> {
        let tensors = fuel::safetensors::SliceSafetensors::new(data)?;
        Ok(Self::from_backend(Box::new(tensors), dtype, dev.clone()))
    }
    ...
    pub fn from_npz<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let npz = fuel::npy::NpzTensors::new(p)?;
    ...
    pub fn from_pth<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let pth = fuel::pickle::PthTensors::new(p, None)?;
    ...
    pub fn from_pth_with_state<P: AsRef<std::path::Path>>(
        ...
        let pth = fuel::pickle::PthTensors::new(p, Some(state_key))?;
        Ok(Self::from_backend(Box::new(pth), dtype, dev.clone()))
    }
    ...
    /// .into_iter()
    /// .collect();
    /// let vb = fuel_nn::VarBuilder::from_tensors(tensors, DType::F32, &Device::Cpu);
...
pub struct ShardedSafeTensors(fuel::safetensors::MmapedSafetensors);
...
impl Backend for ShardedSafeTensors {
    ...
    fn get(
        ...
        let iterator = if dim == 0 {
            ...
            fuel::bail!("Get sharded on dimensions != 0 or 1")
        };
    ...
    fn get_unchecked(&self, _name: &str, _dtype: DType, _dev: &Device) -> Result<Tensor> {
        fuel::bail!("`get_unchecked` does not make sense for `ShardedSafeTensors`, use `get`.");
    }
