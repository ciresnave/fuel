...
use fuel::test_utils::{to_vec0_round, to_vec2_round};

use anyhow::Result;
use fuel::{DType, Device, Tensor, Var};
use fuel_nn::{AdamW, Linear, Module, Optimizer, ParamsAdamW, SGD};
...
fn sgd_linear_regression() -> Result<()> {
    ...
    let gen_model = Linear::new(w_gen, Some(b_gen));
    let sample_xs = Tensor::new(&[[2f32, 1.], [7., 4.], [-4., 12.], [5., 8.]], &Device::Cpu)?;
    let sample_ys = gen_model.forward(&sample_xs)?;
...
fn adamw_linear_regression_varmap() -> Result<()> {
    ...
    let mut var_map = fuel_nn::VarMap::new();
