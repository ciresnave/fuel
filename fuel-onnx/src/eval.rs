...
/// ```no_run
/// use fuel::Tensor;
/// use fuel_onnx::eval::Value;
///
/// // Value is just a type alias for Tensor.
/// let v: Value = Tensor::zeros((2, 3), fuel::DType::F32, &fuel::Device::cpu())?;
/// # Ok::<(), fuel::Error>(())
/// ```
pub type Value = Tensor;

/// Converts an ONNX [`DataType`] to the corresponding fuel [`DType`], if supported.
///
/// Returns `None` for ONNX data types that have no fuel equivalent.
///
/// # Example
///
/// ```no_run
/// use fuel_onnx::onnx::tensor_proto::DataType;
/// use fuel_onnx::eval::dtype;
/// use fuel::DType;
///
...
impl Attr for str {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<&Self> {
        std::str::from_utf8(&attr.s).map_err(fuel::Error::wrap)
    }
}
...
impl Attr for GraphProto {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<&Self> {
        attr.g
            .as_ref()
            .ok_or_else(|| fuel::Error::Msg("attribute does not contain graph".to_string()))
    }
}
...
impl AttrOwned for Vec<String> {
    ...
    fn get(attr: &onnx::AttributeProto) -> Result<Self> {
        ...
        for bytes in attr.strings.iter() {
            let s = String::from_utf8(bytes.clone()).map_err(fuel::Error::wrap)?;
            ret.push(s);
        }
        Ok(ret)
    }
...
/// Converts an ONNX [`TensorProto`] initializer into a fuel [`Tensor`].
///
/// # Example
///
/// ```no_run
/// use fuel_onnx::{read_file, eval::get_tensor};
///
...
///
/// let model = read_file("model.onnx")?;
/// let input = Tensor::zeros((1, 3, 224, 224), fuel::DType::F32, &fuel::Device::cpu())?;
    ...
    for node in graph.node.iter() {
        ...
        match node.op_type.as_str() {
            ...
            "Gather" => {
                ...
                // tensor directly, but fuel does not support tensor indexing at the moment, so
...
fn to_scalar_flexible<T: fuel::WithDType>(t: &Tensor) -> Result<T> {
...
fn to_vec0_flexible<T: fuel::WithDType>(t: &Tensor) -> Result<T> {
    if t.rank() > 0 && t.elem_count() == 1 {
