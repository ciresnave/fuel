...
fn logical_ops_on_zero_one_masks() -> Result<()> {
    ...
    assert_close(&run(&m, std::slice::from_ref(&a))?, &[1.0, 1.0, 0.0, 0.0], "Not");
    Ok(())
}
...
fn clip_from_attributes_and_from_inputs() -> Result<()> {
    ...
    assert_close(&run(&m, std::slice::from_ref(&x))?, &[-1.0, 0.0, 1.0], "Clip attrs");
...
fn shape_and_size_report_metadata() -> Result<()> {
    ...
    assert_close(&run(&m, std::slice::from_ref(&x))?, &[2.0, 3.0, 4.0], "Shape");
