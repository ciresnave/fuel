...
use ::fuel::{quantized::QTensor, DType, Device, Module, Tensor, WithDType};
...
fn actual_index(t: &Tensor, dim: usize, index: i64) -> ::fuel::Result<usize> {
    let dim = t.dim(dim)?;
    if 0 <= index {
        let index = index as usize;
        if dim <= index {
            ::fuel::bail!("index {index} is too large for tensor dimension {dim}")
        }
        Ok(index)
    } else {
        if (dim as i64) < -index {
            ::fuel::bail!("index {index} is too low for tensor dimension {dim}")
        }
        Ok((dim as i64 + index) as usize)
    }
}

fn actual_dim(t: &Tensor, dim: i64) -> ::fuel::Result<usize> {
    let rank = t.rank();
    if 0 <= dim {
        let dim = dim as usize;
        if rank <= dim {
            ::fuel::bail!("dimension index {dim} is too large for tensor rank {rank}")
        }
        Ok(dim)
    } else {
        if (rank as i64) < -dim {
            ::fuel::bail!("dimension index {dim} is too low for tensor rank {rank}")
        }
        Ok((rank as i64 + dim) as usize)
    }
}

// TODO: Something similar to this should probably be a part of fuel core.
trait MapDType {
    type Output;
...
impl PyTensor {
    ...
    /// Converts fuel's tensor to pytorch's tensor
    /// &RETURNS&: torch.Tensor
    fn to_torch(&self, py: Python<'_>) -> PyResult<Py<PyAny>> {
        let fuel_values = self.values(py)?;
    ...
    fn __richcmp__(&self, rhs: &Bound<PyAny>, op: CompareOp) -> PyResult<Self> {
        ...
        if let Ok(rhs) = rhs.extract::<PyTensor>() {
            if self.0.shape() == rhs.0.shape() {
                ...
                // We broadcast manually here because `fuel.cmp` does not support automatic broadcasting
...
impl PyQTensor {
    ...
    fn matmul_t(&self, lhs: &PyTensor) -> PyResult<PyTensor> {
        let qmatmul = ::fuel::quantized::QMatMul::from_arc(self.0.clone()).map_err(wrap_err)?;
...
fn save_safetensors(
    ...
    ::fuel::safetensors::save(&tensors, path).map_err(wrap_err)
}
...
fn load_ggml<'py>(
    ...
    let device = device.unwrap_or(PyDevice::Cpu).as_device()?;
    let ggml =
        ::fuel::quantized::ggml_file::Content::read(&mut file, &device).map_err(wrap_err)?;
    let tensors = ggml
        .tensors
        .into_iter()
...
fn cuda_is_available() -> bool {
    ::fuel::utils::cuda_is_available()
}

#[pyfunction]
/// Returns true if fuel was compiled with 'accelerate' support.
/// &RETURNS&: bool
fn has_accelerate() -> bool {
    ::fuel::utils::has_accelerate()
}

#[pyfunction]
/// Returns true if fuel was compiled with MKL support.
/// &RETURNS&: bool
fn has_mkl() -> bool {
    ::fuel::utils::has_mkl()
}
...
#[pyfunction]
/// Returns the number of threads used by the fuel.
/// &RETURNS&: int
fn get_num_threads() -> usize {
    ::fuel::utils::get_num_threads()
}

fn fuel_utils(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
fn fuel_functional_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
#[cfg(feature = "onnx")]
fn fuel_onnx_m(_py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
...
#[pymodule]
fn fuel(py: Python<'_>, m: &Bound<'_, PyModule>) -> PyResult<()> {
    let utils = PyModule::new(py, "utils")?;
    fuel_utils(py, &utils)?;
    m.add_submodule(&utils)?;
    let nn = PyModule::new(py, "functional")?;
    fuel_functional_m(py, &nn)?;
    m.add_submodule(&nn)?;
    #[cfg(feature = "onnx")]
    {
