...
fn baracuda_clamp_f32() {
    ...
    let out = run_clamp(
        ...
        DType::F32,
        4,
        fuel_storage::baracuda_dispatch::clamp::clamp_f32,
        &input,
        0.0,
        2.0,
    );
...
fn baracuda_clamp_f64() {
    ...
    let out = run_clamp(
        ...
        DType::F64,
        8,
        fuel_storage::baracuda_dispatch::clamp::clamp_f64,
        &input,
        0.0,
        2.0,
    );
...
#[test]
fn dual_register_appends_clamp_alternative() {
    let mut table = KernelBindingTable::new();
    register_cuda_kernels(&mut table);
    let before = table
        .lookup_alternatives(
    ...
    let after = table
        ...
        .len();
    assert_eq!(before, 1, "PTX-only registers one F32 clamp");
    assert_eq!(after, 2, "baracuda registers a second F32 clamp alternative");
}
