use fuel::quantized::{gguf_file, GgmlDType, QTensor};
...
fn run_print(
    ...
    if full {
        fuel::display::set_print_options_full();
    }
    if let Some(line_width) = line_width {
        fuel::display::set_line_width(line_width)
    }
    ...
    match format {
        Format::Npz => {
            let tensors = fuel::npy::NpzTensors::new(file)?;
        ...
        Format::Safetensors => {
            use fuel::safetensors::Load;
            let tensors = unsafe { fuel::safetensors::MmapedSafetensors::new(file)? };
        ...
        Format::Pth => {
            let pth_file = fuel::pickle::PthTensors::new(file, None)?;
            let names = if names.is_empty() {
                pth_file
                    .tensor_infos()
                    .keys()
        ...
        Format::Pickle => {
            fuel::bail!("pickle format is not supported for print")
        }
        Format::Ggml => {
            let mut file = std::fs::File::open(file)?;
            let content = fuel::quantized::ggml_file::Content::read(&mut file, device)?;
            let names = if names.is_empty() {
    ...
    match format {
        ...
        Format::Safetensors => {
            ...
            for (name, view) in tensors.iter() {
                ...
                let dtype = match fuel::DType::try_from(dtype) {
        ...
        Format::Pth => {
            let mut tensors = fuel::pickle::read_pth_tensor_info(file, verbose, None)?;
        ...
        Format::Pickle => {
            ...
            let mut stack = fuel::pickle::Stack::empty();
...
fn run_dequantize(
    ...
    fuel::safetensors::save(&tensors, out_file)?;
    Ok(())
}
...
fn run_quantize(
    ...
    if in_files.is_empty() {
        fuel::bail!("no specified input files")
    }
    if let Some(extension) = out_file.extension() {
        if extension == "safetensors" {
            fuel::bail!("the generated file cannot use the safetensors extension")
        }
    }
    ...
    if in_files.len() != 1 {
        fuel::bail!("only a single in-file can be used when quantizing gguf files")
    }
