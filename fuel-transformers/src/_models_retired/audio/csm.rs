...
//!
//! See: [Conversational Speech Model](https://www.sesame.com/research/crossing_the_uncanny_valley_of_voice)
//!
...
use fuel_nn::{embedding, kv_cache::KvCache, linear_b, Embedding, Linear, RmsNorm, VarBuilder};
use std::collections::HashMap;
use std::sync::Arc;

/// Model size variant for the backbone and decoder Llama sub-models.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::csm::Flavor;
/// let flavor = Flavor::Llama1B;
/// ```
...
/// Top-level CSM model configuration.
///
/// Specifies the number of audio codebooks, vocabulary sizes, and which
/// Llama size [`Flavor`] to use for the backbone and decoder.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::csm::{Config, Flavor};
/// let cfg = Config {
...
/// Internal Llama model configuration used by the CSM backbone and decoder.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::csm::{Flavor, LlamaConfig};
/// let cfg = LlamaConfig::from_flavor(Flavor::Llama1B);
/// ```
#[allow(unused)]
#[derive(Debug, Clone)]
...
impl LlamaConfig {
    /// Return the pre-set configuration for the given model [`Flavor`].
    ///
    /// # Example
    ///
    /// ```
...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb_qkv(
        ...
        let q_embed = fuel_nn::rotary_emb::rope_i(q, &cos, &sin)?;
        let k_embed = fuel_nn::rotary_emb::rope_i(k, &cos, &sin)?;
        Ok((q_embed, k_embed))
    }
}
...
/// Llama transformer block used internally by the CSM backbone and decoder.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::csm::{Flavor, LlamaConfig, LlamaModel};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = LlamaConfig::from_flavor(Flavor::Llama1B);
/// let model = LlamaModel::new(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
...
impl LlamaModel {
    /// Construct a `LlamaModel` from `cfg`, loading weights from `vb`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Clear the KV cache of every layer.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run a forward pass and return per-position hidden states.
    ///
    /// `xs` has shape `(batch, seq_len, embed_dim)`; returns shape `(batch, 1, embed_dim)` (last position).
    ///
    /// # Example
    ///
    /// ```no_run
...
/// Complete CSM model: Llama backbone and decoder with audio and text embeddings.
///
/// The backbone encodes combined audio/text token sequences; the decoder 
/// auto-regressively predicts each RVQ codebook layer from the backbone output.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::csm::{Config, Model};
/// # use fuel_nn::VarBuilder;
...
impl Model {
    /// Construct the full CSM model from `cfg`, loading weights from `vb`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Clear KV caches in both the backbone and decoder.
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use fuel_transformers::models::csm::Model;
    ...
    /// Generate one audio frame (one RVQ code per codebook) auto-regressively.
    ///
    /// `tokens` has shape `(batch, seq_len, audio_num_codebooks + 1)` with interleaved
    /// audio and text tokens; `tokens_mask` is a matching binary mask.
    /// Returns a `Vec<u32>` of length `audio_num_codebooks` with the sampled codes.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Build a `(tokens, mask)` pair for one audio frame.
    ///
    /// `frame` must contain exactly `audio_num_codebooks` token ids.
    /// Returns tensors of shape `(1, 1, audio_num_codebooks + 1)`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// let frame = vec![0u32; 32];
    /// let (tokens, mask) = model.audio_tokens_and_mask(frame)?;
    /// # Ok::<(), fuel::Error>(())
    /// ```
    ...
    /// Build a `(tokens, mask)` pair for a text token sequence.
    ///
    /// Each input token id becomes a `(1, 1, audio_num_codebooks + 1)` frame with the
    /// text token in the last slot; audio slots are zeroed.
    /// Returns tensors of shape `(1, ids.len(), audio_num_codebooks + 1)`.
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use fuel_transformers::models::csm::Model;
    /// # let model: Model = unimplemented!();
    /// let ids = &[128000u32, 9906, 13];
    /// let (tokens, mask) = model.text_tokens_and_mask(ids)?;
    /// # Ok::<(), fuel::Error>(())
    /// ```
