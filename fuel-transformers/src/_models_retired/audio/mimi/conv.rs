...
use fuel::{Module, Result, StreamTensor, StreamingModule, Tensor, D};
use fuel_nn::{Conv1d, VarBuilder};

#[allow(clippy::enum_variant_names)]
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
/// Normalization applied to convolution weights in Mimi.
pub enum Norm {
    WeightNorm,
    SpectralNorm,
    TimeGroupNorm,
...
/// Padding strategy for causal / non-causal convolutions.
#[derive(Debug, Copy, Clone, PartialEq, Eq)]
pub enum PadMode {
    Constant,
    Reflect,
...
/// 1-D convolution with optional weight-norm or time-group-norm applied at load time.
#[derive(Debug, Clone)]
...
impl NormConv1d {
    ...
    /// Build a [`NormConv1d`] with the requested normalization.
    pub fn new(
        in_c: usize,
        out_c: usize,
        k_size: usize,
        ...
        norm: Option<Norm>,
        bias: bool,
        cfg: fuel_nn::Conv1dConfig,
        vb: VarBuilder,
    ) -> Result<Self> {
        let conv = match norm {
            None | Some(Norm::TimeGroupNorm) => {
                if bias {
                    fuel_nn::conv1d(in_c, out_c, k_size, cfg, vb.pp("conv"))?
                } else {
                    fuel_nn::conv1d_no_bias(in_c, out_c, k_size, cfg, vb.pp("conv"))?
                }
            }
            Some(Norm::WeightNorm) => {
                conv1d_weight_norm(in_c, out_c, k_size, bias, cfg, vb.pp("conv"))?
            }
            Some(Norm::SpectralNorm) => fuel::bail!("SpectralNorm is not supported yet."),
        };
        let norm = match norm {
            None | Some(Norm::WeightNorm) | Some(Norm::SpectralNorm) => None,
            Some(Norm::TimeGroupNorm) => {
                if causal {
                    fuel::bail!("GroupNorm doesn't support causal evaluation.")
                }
                let norm = fuel_nn::group_norm(1, out_c, 1e-5, vb.pp("norm"))?;
                Some(norm)
            }
        };
        Ok(Self {
...
/// Transposed 1-D convolution with optional weight-norm or time-group-norm.
#[derive(Debug, Clone)]
...
impl NormConvTranspose1d {
    #[allow(clippy::too_many_arguments)]
    /// Build a [`NormConvTranspose1d`] layer with the requested normalization.
    pub fn new(
        in_c: usize,
        out_c: usize,
        k_size: usize,
...
fn unpad1d(xs: &Tensor, unpad_l: usize, unpad_r: usize) -> Result<Tensor> {
    ...
    if len < unpad_l + unpad_r {
        fuel::bail!("unpad1d: tensor len {len} is too low, {unpad_l} + {unpad_r}")
    }
    xs.narrow(D::Minus1, unpad_l, len - (unpad_l + unpad_r))
}

/// Streaming-capable 1-D convolution with causal or symmetric padding.
///
/// Maintains a left-context buffer between streaming steps so the model
/// can process audio one frame at a time without recomputing history.
#[derive(Debug, Clone)]
...
impl StreamableConv1d {
    #[allow(clippy::too_many_arguments)]
    /// Build a [`StreamableConv1d`] layer.
    pub fn new(
        in_c: usize,
        out_c: usize,
        k_size: usize,
        ...
        if k_size < stride {
            fuel::bail!("kernel-size {k_size} is smaller than stride {stride}")
        }
        Ok(Self {
            conv,
            causal,
...
/// Streaming-capable transposed 1-D convolution for upsampling.
#[derive(Debug, Clone)]
...
impl StreamableConvTranspose1d {
    #[allow(clippy::too_many_arguments)]
    /// Build a [`StreamableConvTranspose1d`] layer.
    pub fn new(
        in_c: usize,
        out_c: usize,
        k_size: usize,
...
/// Learnable downsampling block: a single strided [`StreamableConv1d`].
#[derive(Debug, Clone)]
...
impl ConvDownsample1d {
    /// Build a downsampler with the given `stride`, operating on `dim` channels.
    pub fn new(
        stride: usize,
        dim: usize,
        causal: bool,
        ...
        if !learnt {
            fuel::bail!("only learnt=true is supported")
        }
...
/// Learnable upsampling block: a single strided [`StreamableConvTranspose1d`].
#[derive(Debug, Clone)]
...
impl ConvTrUpsample1d {
    /// Build an upsampler with the given `stride`, operating on `dim` channels.
    pub fn new(
        stride: usize,
        dim: usize,
        causal: bool,
...
mod tests {
    ...
    fn run_conv1d(
        ...
        if diff > 1e-5 {
            ...
            println!("{ys}");
            println!("{ys_steps}");
            fuel::bail!("larger diff than expected {diff}")
        }
        Ok(())
    }
