...
use fuel_nn::{Conv1d, Conv1dConfig, LayerNorm, Module};
...
/// GGUF-quantised Whisper audio encoder (CNN + transformer).
#[derive(Debug, Clone)]
...
impl AudioEncoder {
    ...
    /// Reset the KV cache in all encoder transformer blocks.
...
/// GGUF-quantised Whisper autoregressive text decoder.
#[derive(Debug, Clone)]
...
impl TextDecoder {
    ...
    /// Run one decoder step; flushes KV cache when `flush_kv_cache` is true.
    ...
    /// Reset the KV cache in all decoder transformer blocks.
    pub fn reset_kv_cache(&mut self) {
        for block in self.blocks.iter_mut() {
            block.reset_kv_cache();
        }
...
/// GGUF-quantised full Whisper model combining an audio encoder and a text decoder.
#[derive(Debug, Clone)]
pub struct Whisper {
...
impl Whisper {
    /// Load the quantised Whisper model from a `VarBuilder` and config.
