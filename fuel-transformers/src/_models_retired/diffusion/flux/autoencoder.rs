...
impl Config {
    ...
    /// Returns the autoencoder config for FLUX.1-dev.
    pub fn dev() -> Self {
        Self {
            resolution: 256,
            in_channels: 3,
    ...
    /// Returns the autoencoder config for FLUX.1-schnell.
    pub fn schnell() -> Self {
        Self {
            resolution: 256,
            in_channels: 3,
...
fn scaled_dot_product_attention(q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
    ...
    fuel_nn::ops::softmax_last_dim(&attn_weights)?.matmul(v)
}
...
impl fuel::Module for AttnBlock {
...
impl fuel::Module for ResnetBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let h = xs
            .apply(&self.norm1)?
            .apply(&fuel_nn::Activation::Swish)?
            .apply(&self.conv1)?
            .apply(&self.norm2)?
            .apply(&fuel_nn::Activation::Swish)?
            .apply(&self.conv2)?;
...
impl fuel::Module for Downsample {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
/// VAE encoder that maps images to a latent distribution.
#[derive(Debug, Clone)]
pub struct Encoder {
    conv_in: Conv2d,
...
impl Encoder {
    /// Build the VAE encoder from config.
...
impl fuel_nn::Module for Encoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
/// VAE decoder that maps latents back to pixel space.
#[derive(Debug, Clone)]
pub struct Decoder {
    conv_in: Conv2d,
    mid_block_1: ResnetBlock,
...
impl Decoder {
    /// Build the VAE decoder from config.
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let conv_cfg = fuel_nn::Conv2dConfig {
            padding: 1,
            ..Default::default()
...
impl fuel_nn::Module for Decoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
/// Diagonal Gaussian reparameterisation module (samples or takes the mean).
#[derive(Debug, Clone)]
...
impl DiagonalGaussian {
    /// Build the sampler; set `sample = true` to draw a sample, `false` to return the mean.
    pub fn new(sample: bool, chunk_dim: usize) -> Result<Self> {
        Ok(Self { sample, chunk_dim })
    }
}

impl fuel_nn::Module for DiagonalGaussian {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
...
/// Full FLUX VAE (encoder + diagonal Gaussian + decoder).
#[derive(Debug, Clone)]
...
impl AutoEncoder {
    /// Build the full autoencoder (encoder + Gaussian sampler + decoder) from config.
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
    ...
    /// Encode an image tensor to a scaled, shifted latent.
    pub fn encode(&self, xs: &Tensor) -> Result<Tensor> {
        let z = xs.apply(&self.encoder)?.apply(&self.reg)?;
        (z - self.shift_factor)? * self.scale_factor
    }
    /// Decode a scaled latent back to image space.
...
impl fuel::Module for AutoEncoder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
