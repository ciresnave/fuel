use fuel::{DType, IndexOp, Result, Tensor, D};
use fuel_nn::{LayerNorm, Linear, RmsNorm, VarBuilder};

// https://github.com/black-forest-labs/flux/blob/727e3a71faf37390f318cf9434f0939653302b60/src/flux/model.py#L12
/// Configuration for the Flux rectified-flow transformer.
#[derive(Debug, Clone)]
pub struct Config {
...
impl Config {
    ...
    /// Returns the Flux config for FLUX.1-dev (12B, guidance-distilled).
    pub fn dev() -> Self {
        Self {
            in_channels: 64,
            vec_in_dim: 768,
    ...
    /// Returns the Flux config for FLUX.1-schnell (12B, distillation-free).
    pub fn schnell() -> Self {
        Self {
            in_channels: 64,
            vec_in_dim: 768,
...
fn scaled_dot_product_attention(q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
    ...
    let attn_scores = fuel_nn::ops::softmax_last_dim(&attn_weights)?.matmul(&v)?;
...
pub(crate) fn timestep_embedding(t: &Tensor, dim: usize, dtype: DType) -> Result<Tensor> {
    ...
    if dim % 2 == 1 {
        fuel::bail!("{dim} is odd")
    }
    let dev = t.device();
    let half = dim / 2;
    let t = (t * TIME_FACTOR)?;
    let arange = Tensor::arange(0, half as u32, dev)?.to_dtype(fuel::DType::F32)?;
...
/// N-dimensional rotary position embedder for Flux.
#[derive(Debug, Clone)]
pub struct EmbedNd {
    #[allow(unused)]
    dim: usize,
...
impl EmbedNd {
    /// Build the N-dim RoPE embedder with given dimension, theta, and per-axis dims.
...
/// Two-layer SiLU MLP embedder for conditioning vectors.
#[derive(Debug, Clone)]
...
impl MlpEmbedder {
    fn new(in_sz: usize, h_sz: usize, vb: VarBuilder) -> Result<Self> {
        let in_layer = fuel_nn::linear(in_sz, h_sz, vb.pp("in_layer"))?;
        let out_layer = fuel_nn::linear(h_sz, h_sz, vb.pp("out_layer"))?;
        Ok(Self {
            in_layer,
            out_layer,
        })
...
impl fuel::Module for MlpEmbedder {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        xs.apply(&self.in_layer)?.silu()?.apply(&self.out_layer)
    }
}

/// Per-head RMS normalisation applied to Q and K before attention.
#[derive(Debug, Clone)]
pub struct QkNorm {
    query_norm: RmsNorm,
    key_norm: RmsNorm,
...
impl Modulation1 {
    fn new(dim: usize, vb: VarBuilder) -> Result<Self> {
        let lin = fuel_nn::linear(dim, 3 * dim, vb.pp("lin"))?;
        Ok(Self { lin })
    }
    ...
    fn forward(&self, vec_: &Tensor) -> Result<ModulationOut> {
        let ys = vec_
            ...
            .chunk(3, D::Minus1)?;
        if ys.len() != 3 {
            fuel::bail!("unexpected len from chunk {ys:?}")
        }
        Ok(ModulationOut {
            shift: ys[0].clone(),
            scale: ys[1].clone(),
...
impl Modulation2 {
    fn new(dim: usize, vb: VarBuilder) -> Result<Self> {
        let lin = fuel_nn::linear(dim, 6 * dim, vb.pp("lin"))?;
        Ok(Self { lin })
    }
...
/// Single-stream self-attention module with RoPE.
#[derive(Debug, Clone)]
...
impl SelfAttention {
    fn new(dim: usize, num_heads: usize, qkv_bias: bool, vb: VarBuilder) -> Result<Self> {
        ...
        let qkv = fuel_nn::linear_b(dim, dim * 3, qkv_bias, vb.pp("qkv"))?;
        let norm = QkNorm::new(head_dim, vb.pp("norm"))?;
        let proj = fuel_nn::linear(dim, dim, vb.pp("proj"))?;
        Ok(Self {
            qkv,
            norm,
            proj,
...
impl Mlp {
    fn new(in_sz: usize, mlp_sz: usize, vb: VarBuilder) -> Result<Self> {
        let lin1 = fuel_nn::linear(in_sz, mlp_sz, vb.pp("0"))?;
        let lin2 = fuel_nn::linear(mlp_sz, in_sz, vb.pp("2"))?;
        Ok(Self { lin1, lin2 })
    }
}

impl fuel::Module for Mlp {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        xs.apply(&self.lin1)?.gelu()?.apply(&self.lin2)
    }
}

/// Dual-stream (image + text) transformer block with independent modulation.
#[derive(Debug, Clone)]
...
/// Merged single-stream transformer block that processes concatenated image+text.
#[derive(Debug, Clone)]
...
impl SingleStreamBlock {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let linear1 = fuel_nn::linear(h_sz, h_sz * 3 + mlp_sz, vb.pp("linear1"))?;
        let linear2 = fuel_nn::linear(h_sz + mlp_sz, h_sz, vb.pp("linear2"))?;
        let norm = QkNorm::new(head_dim, vb.pp("norm"))?;
...
/// Final output projection layer with adaptive layer norm.
#[derive(Debug, Clone)]
...
impl LastLayer {
    fn new(h_sz: usize, p_sz: usize, out_c: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let linear = fuel_nn::linear(h_sz, p_sz * p_sz * out_c, vb.pp("linear"))?;
        let ada_ln_modulation = fuel_nn::linear(h_sz, 2 * h_sz, vb.pp("adaLN_modulation.1"))?;
        Ok(Self {
            norm_final,
            linear,
            ada_ln_modulation,
...
/// Full Flux rectified-flow transformer model.
#[derive(Debug, Clone)]
pub struct Flux {
    img_in: Linear,
    txt_in: Linear,
...
impl Flux {
    /// Build the Flux model from config.
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let img_in = fuel_nn::linear(cfg.in_channels, cfg.hidden_size, vb.pp("img_in"))?;
        let txt_in = fuel_nn::linear(cfg.context_in_dim, cfg.hidden_size, vb.pp("txt_in"))?;
...
impl super::WithForward for Flux {
    ...
    fn forward(
        ...
        if txt.rank() != 3 {
            fuel::bail!("unexpected shape for txt {:?}", txt.shape())
        }
        if img.rank() != 3 {
            fuel::bail!("unexpected shape for img {:?}", img.shape())
        }
        let dtype = img.dtype();
        let pe = {
