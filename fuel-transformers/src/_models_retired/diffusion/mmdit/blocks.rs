...
/// Intermediate tensors produced by adaptive layer norm modulation in a DiT block.
...
/// A Diffusion Transformer (DiT) block with adaptive layer normalization.
pub struct DiTBlock {
...
/// Layer normalization without learnable affine (scale/bias) parameters.
pub struct LayerNormNoAffine {
    eps: f64,
}

impl LayerNormNoAffine {
    /// Creates a new `LayerNormNoAffine` with the given epsilon for numerical stability.
...
impl DiTBlock {
    /// Creates a new `DiTBlock` with the specified hidden size and number of attention heads.
    ...
    /// Applies attention output and MLP with gating; call after joint attention.
...
/// Modulation intermediates for a DiT block with dual self-attention paths.
...
/// A DiT block with dual self-attention paths (MMDiT-X variant).
...
impl SelfAttnDiTBlock {
    /// Creates a new `SelfAttnDiTBlock` with dual attention paths.
    pub fn new(hidden_size: usize, num_heads: usize, vb: nn::VarBuilder) -> Result<Self> {
        let norm1 = LayerNormNoAffine::new(1e-6);
        let attn = AttnProjections::new(hidden_size, num_heads, vb.pp("attn"))?;
    ...
    /// Computes QKV projections and modulation intermediates; call before joint attention.    /// Computes QKV for both attention paths and modulation intermediates.
    ...
    /// Applies both attention outputs and MLP with gating.
    pub fn post_attention(
        &self,
        attn: &Tensor,
        attn2: &Tensor,
...
/// A DiT block that only computes QKV projections without an output projection.
...
impl QkvOnlyDiTBlock {
    ...
    /// Computes modulated QKV projections from input and conditioning.
...
/// Final output layer that applies adaptive layer norm and a linear projection.
...
impl FinalLayer {
    /// Creates a new `FinalLayer` that projects hidden states to patch-sized outputs.
    pub fn new(
        hidden_size: usize,
        patch_size: usize,
        out_channels: usize,
    ...
    /// Applies adaptive layer norm modulation followed by the linear output projection.
...
/// Trait for joint transformer blocks that process paired context and image streams.
pub trait JointBlock {
    /// Runs the joint block, returning updated `(context, x)` tensors.
    fn forward(&self, context: &Tensor, x: &Tensor, c: &Tensor) -> Result<(Tensor, Tensor)>;
}

/// A joint MMDiT block that processes context and image streams with shared attention.
...
impl MMDiTJointBlock {
    /// Creates a new `MMDiTJointBlock` with separate DiT blocks for context and image.
    pub fn new(
        hidden_size: usize,
        num_heads: usize,
        use_flash_attn: bool,
...
/// An MMDiT-X joint block where the image stream uses dual self-attention.
...
impl MMDiTXJointBlock {
    /// Creates a new `MMDiTXJointBlock` for the MMDiT-X variant (SD 3.5 medium).
    pub fn new(
        hidden_size: usize,
        num_heads: usize,
        use_flash_attn: bool,
...
/// A joint block where the context stream only contributes QKV (no output projection).
...
impl ContextQkvOnlyJointBlock {
    /// Creates a new `ContextQkvOnlyJointBlock` used as the final joint block.
    pub fn new(
        hidden_size: usize,
        num_heads: usize,
        use_flash_attn: bool,
    ...
    /// Runs the final joint block, returning only the updated image stream tensor.
