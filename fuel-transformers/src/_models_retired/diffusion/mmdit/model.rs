...
/// Configuration for the MMDiT model architecture.
#[derive(Debug, Clone)]
pub struct Config {
...
impl Config {
    /// Returns the configuration for Stable Diffusion 3 medium.
    ...
    /// Returns the configuration for Stable Diffusion 3.5 medium (with MMDiT-X blocks).
    ...
    /// Returns the configuration for Stable Diffusion 3.5 large.
...
/// Full MMDiT model including patch and positional embeddings, transformer core, and unpatchifier.
pub struct MMDiT {
    core: MMDiTCore,
...
impl MMDiT {
    /// Creates a new `MMDiT` model from the given configuration.
    ...
    /// Runs the forward pass, returning denoised latent output.
    ///
    /// - `x`: `(N, C, H, W)` noisy latent tensor
    /// - `t`: `(N,)` diffusion timestep tensor
    /// - `y`: `(N, adm_in_channels)` conditioning vector tensor
    /// - `context`: `(N, seq_len, context_embed_size)` text conditioning tensor
    /// - `skip_layers`: optional indices of joint blocks to skip (for distillation)
    pub fn forward(
        &self,
        x: &Tensor,
        t: &Tensor,
...
/// Core transformer of the MMDiT model, composed of joint blocks and a final layer.
...
impl MMDiTCore {
    /// Creates a new `MMDiTCore` with the given depth and hidden size.
    pub fn new(
        depth: usize,
        hidden_size: usize,
        num_heads: usize,
    ...
    /// Runs all joint blocks followed by the final output layer.
    pub fn forward(
        &self,
        context: &Tensor,
        x: &Tensor,
