...
/// Multi-head cross-attention with optional flash attention and sliced-attention support.
#[derive(Debug)]
...
impl CrossAttention {
    ...
    /// Creates a new `CrossAttention`; pass `context_dim = None` for self-attention.
    pub fn new(
        vs: nn::VarBuilder,
        query_dim: usize,
    ...
    /// Runs attention; `context=None` performs self-attention, `context=Some(...)` cross-attention.
...
/// Configuration for the spatial transformer (Transformer2DModel).
...
/// 2-D spatial transformer block that applies cross-attention in the spatial domain.
#[derive(Debug)]
...
impl SpatialTransformer {
    /// Creates a new `SpatialTransformer` with group norm, input/output projections, and stacked transformer blocks.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
        n_heads: usize,
    ...
    /// Applies the spatial transformer; returns a tensor with the same shape as the input.
    pub fn forward(&self, xs: &Tensor, context: Option<&Tensor>) -> Result<Tensor> {
        let _enter = self.span.enter();
...
/// Self-attention block using group normalization; used in VAE mid-blocks and plain UNet blocks.
#[derive(Debug)]
...
impl AttentionBlock {
    /// Creates a new `AttentionBlock`; handles weight-shape variants from SD3 safetensors.
