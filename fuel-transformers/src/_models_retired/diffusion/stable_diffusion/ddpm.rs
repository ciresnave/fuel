...
/// Variance schedule variant for the DDPM reverse diffusion step.
...
/// Configuration for the DDPM noise scheduler.
#[derive(Debug, Clone)]
...
/// DDPM noise scheduler holding the precomputed alpha-cumulative-product schedule.
...
impl DDPMScheduler {
    /// Creates a new `DDPMScheduler` precomputing beta/alpha schedules for `inference_steps` steps.
    ...
    /// Returns the ordered denoising timestep schedule.
    ...
    /// Performs one DDPM reverse step; adds variance noise when `timestep > 0`.
    ...
    /// Applies forward-process noise to `original_samples` at `timestep` using `alphas_cumprod`.
    pub fn add_noise(
        &self,
    ...
    /// Returns the initial noise standard deviation (always `1.0` for DDPM).
