...
/// A ResNet-style residual block with optional group norm, time-embedding projection, and conv shortcut.
#[derive(Debug)]
...
impl ResnetBlock2D {
    /// Creates a new `ResnetBlock2D` with the given input channels and configuration.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
        config: ResnetBlock2DConfig,
    ...
    /// Applies the residual block; `temb` is an optional time embedding added after the first conv.
