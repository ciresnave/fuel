...
/// Per-block channel and cross-attention configuration for a UNet2D model.
...
/// Build configuration for a `UNet2DConditionModel`.
#[derive(Debug, Clone)]
...
/// 2-D conditional UNet that takes a noisy image, timestep, and encoder context and predicts noise/velocity.
#[derive(Debug)]
...
impl UNet2DConditionModel {
    /// Creates a new `UNet2DConditionModel` loading weights with `in_channels` noisy-latent channels and `out_channels` prediction channels.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
        out_channels: usize,
    ...
    /// Runs a forward pass with `timestep` (float) and cross-attention context `encoder_hidden_states`.
    pub fn forward(
        &self,
        xs: &Tensor,
        timestep: f64,
    ...
    /// Like `forward` but injects additional residuals for ControlNet-style conditioning.
