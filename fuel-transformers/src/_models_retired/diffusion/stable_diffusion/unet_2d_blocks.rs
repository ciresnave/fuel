...
/// Configuration for a VAE encoder downsampling block.
...
/// A VAE encoder block that applies residual convolutions, optionally followed by downsampling.
#[derive(Debug)]
...
impl DownEncoderBlock2D {
    /// Creates a new `DownEncoderBlock2D` with stacked ResNets and an optional downsampling layer.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
        out_channels: usize,
...
/// Configuration for a VAE decoder upsampling block.
#[derive(Debug, Clone, Copy)]
...
/// A VAE decoder block that applies residual convolutions, optionally followed by upsampling.
#[derive(Debug)]
...
impl UpDecoderBlock2D {
    /// Creates a new `UpDecoderBlock2D` with stacked ResNets and an optional upsampling layer.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
        out_channels: usize,
...
/// Configuration for a UNet middle block without cross-attention.
#[derive(Debug, Clone, Copy)]
...
/// The UNet middle block: one ResNet followed by interleaved attention+ResNet pairs.
///
/// Used inside the VAE and the plain (non-cross-attention) UNet variant.
#[derive(Debug)]
...
impl UNetMidBlock2D {
    /// Creates a new `UNetMidBlock2D` with one ResNet followed by interleaved attention+ResNet pairs.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
    ...
    /// Applies the mid block; propagates an optional time embedding through each ResNet sub-block.
...
/// Configuration for the UNet middle block with cross-attention (used in conditional diffusion).
#[derive(Debug, Clone, Copy)]
...
/// The UNet middle block with cross-attention.
///
/// Interleaves spatial transformer (cross-attn) blocks with ResNet blocks;
/// used in text-conditioned Stable Diffusion UNets.
#[derive(Debug)]
...
impl UNetMidBlock2DCrossAttn {
    /// Creates a new `UNetMidBlock2DCrossAttn` with ResNets and spatial transformer cross-attention pairs.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
        temb_channels: Option<usize>,
    ...
    /// Applies the cross-attention mid block with optional time embedding and encoder hidden states.
    pub fn forward(
        &self,
        xs: &Tensor,
        temb: Option<&Tensor>,
...
/// Configuration for a plain (no cross-attention) UNet downsampling block.
#[derive(Debug, Clone, Copy)]
...
/// A plain UNet downsampling block: stacked ResNets with optional downsampling.
#[derive(Debug)]
...
impl DownBlock2D {
    /// Creates a new `DownBlock2D` with stacked ResNets and an optional downsampler.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
        out_channels: usize,
    ...
    /// Applies the down block; returns the output tensor and skip-connection tensors for the decoder.
...
/// Configuration for a cross-attention UNet downsampling block.
#[derive(Debug, Clone, Copy)]
...
/// A cross-attention UNet downsampling block: interleaves ResNets with spatial transformers.
#[derive(Debug)]
...
impl CrossAttnDownBlock2D {
    /// Creates a new `CrossAttnDownBlock2D` pairing each ResNet layer with a spatial transformer.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
        out_channels: usize,
    ...
    /// Applies the cross-attention down block; returns output and skip tensors for the decoder.
    pub fn forward(
        &self,
        xs: &Tensor,
        temb: Option<&Tensor>,
...
/// Configuration for a plain UNet upsampling block.
#[derive(Debug, Clone, Copy)]
...
/// A plain UNet upsampling block: stacked ResNets with optional bilinear upsampling.
#[derive(Debug)]
...
impl UpBlock2D {
    /// Creates a new `UpBlock2D` with stacked ResNets (each also fusing a skip-connection) and an optional upsampler.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
    ...
    /// Applies the up block, concatenating skip residuals before each ResNet layer.
    pub fn forward(
        &self,
        xs: &Tensor,
        res_xs: &[Tensor],
...
/// Configuration for a cross-attention UNet upsampling block.
#[derive(Debug, Clone, Copy)]
...
/// A cross-attention UNet upsampling block: interleaves ResNets with spatial transformers.
#[derive(Debug)]
...
impl CrossAttnUpBlock2D {
    /// Creates a new `CrossAttnUpBlock2D` pairing each ResNet layer with a spatial transformer.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
        prev_output_channels: usize,
    ...
    /// Applies the cross-attention up block, concatenating skip residuals and conditioning on encoder states.
    pub fn forward(
        &self,
        xs: &Tensor,
        res_xs: &[Tensor],
