...
/// Configuration for the Wuerstchen DDPM-W noise schedule.
#[derive(Debug, Clone)]
...
/// DDPM-W noise scheduler used for Wuerstchen image generation.
...
impl DDPMWScheduler {
    /// Creates a new scheduler with `inference_steps` linearly-spaced timesteps from 1 → 0.
    ...
    /// Returns the precomputed timestep schedule.
    ...
    /// Performs one DDPM reverse diffusion step given noise prediction and current timestep.
    ...
    /// Returns the initial noise standard deviation (always `1.0` for this scheduler).
