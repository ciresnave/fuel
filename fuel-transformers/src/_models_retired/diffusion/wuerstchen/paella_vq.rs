...
/// Mixing residual block with six learnable scalar gates used in the Paella VQ encoder/decoder.
#[derive(Debug)]
pub struct MixingResidualBlock {
    norm1: LayerNormNoWeights,
    depthwise_conv: fuel_nn::Conv2d,
...
impl MixingResidualBlock {
    /// Creates a new `MixingResidualBlock` with `inp` channels and `embed_dim` MLP width.
    pub fn new(inp: usize, embed_dim: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let depthwise_conv = fuel_nn::conv2d(inp, inp, 3, cfg, vb.pp("depthwise.1"))?;
        let channelwise_lin1 = fuel_nn::linear(inp, embed_dim, vb.pp("channelwise.0"))?;
        let channelwise_lin2 = fuel_nn::linear(embed_dim, inp, vb.pp("channelwise.2"))?;
...
impl Module for MixingResidualBlock {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let x_temp = fuel_nn::ops::replication_pad2d(&x_temp, 1)?;
...
/// Paella VQ-VAE used to encode and decode Wuerstchen Stage B latents.
#[derive(Debug)]
pub struct PaellaVQ {
    in_block_conv: fuel_nn::Conv2d,
    out_block_conv: fuel_nn::Conv2d,
    down_blocks: Vec<(Option<fuel_nn::Conv2d>, MixingResidualBlock)>,
    down_blocks_conv: fuel_nn::Conv2d,
    down_blocks_bn: fuel_nn::BatchNorm,
    up_blocks_conv: fuel_nn::Conv2d,
    up_blocks: Vec<(Vec<MixingResidualBlock>, Option<fuel_nn::ConvTranspose2d>)>,
}

impl PaellaVQ {
    /// Creates a new `PaellaVQ` with fixed architecture (`C_LEVELS=[192,384]`, 12 bottleneck blocks).
    pub fn new(vb: VarBuilder) -> Result<Self> {
        ...
        let in_block_conv = fuel_nn::conv2d(
            IN_CHANNELS * 4,
            C_LEVELS[0],
            1,
            Default::default(),
        ...
        let out_block_conv = fuel_nn::conv2d(
            C_LEVELS[0],
            OUT_CHANNELS * 4,
            1,
            Default::default(),
        ...
        for (i, &c_level) in C_LEVELS.iter().enumerate() {
            let conv_block = if i > 0 {
                ...
                let block = fuel_nn::conv2d(C_LEVELS[i - 1], c_level, 4, cfg, vb_d.pp(d_idx))?;
                d_idx += 1;
                Some(block)
            } else {
                None
        ...
        let down_blocks_conv = fuel_nn::conv2d_no_bias(
            C_LEVELS[1],
            LATENT_CHANNELS,
            1,
            Default::default(),
        ...
        let down_blocks_bn = fuel_nn::batch_norm(LATENT_CHANNELS, 1e-5, vb_d.pp(1))?;

        let mut up_blocks = Vec::new();
        let vb_u = vb.pp("up_blocks");
        let mut u_idx = 0;
        let up_blocks_conv = fuel_nn::conv2d(
            LATENT_CHANNELS,
            C_LEVELS[1],
            1,
            Default::default(),
        ...
        for (i, &c_level) in C_LEVELS.iter().rev().enumerate() {
            ...
            let conv_block = if i < C_LEVELS.len() - 1 {
                let cfg = fuel_nn::ConvTranspose2dConfig {
                    padding: 1,
                    stride: 2,
                    ..Default::default()
                };
                let block = fuel_nn::conv_transpose2d(
                    c_level,
    ...
    /// Encodes an RGB image tensor to VQ latents via pixel-unshuffle and down blocks.
    pub fn encode(&self, xs: &Tensor) -> Result<Tensor> {
        let mut xs = fuel_nn::ops::pixel_unshuffle(xs, 2)?.apply(&self.in_block_conv)?;
    ...
    /// Decodes VQ latents back to an RGB image tensor via up blocks and pixel-shuffle.
    pub fn decode(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        xs.apply(&self.out_block_conv)?
            .apply(&|xs: &_| fuel_nn::ops::pixel_shuffle(xs, 2))
    }
}
