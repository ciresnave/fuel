...
#[derive(Debug, Clone)]
struct Mlp {
    gate_proj: fuel_nn::Linear,
    up_proj: fuel_nn::Linear,
    down_proj: fuel_nn::Linear,
    act_fn: Activation,
}
...
impl Mlp {
    fn new(cfg: &TextEncoderConfig, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            gate_proj: fuel_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.intermediate_size,
                vb.pp("gate_proj"),
            )?,
            up_proj: fuel_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.intermediate_size,
                vb.pp("up_proj"),
            )?,
            down_proj: fuel_nn::linear_no_bias(
                cfg.intermediate_size,
                cfg.hidden_size,
                vb.pp("down_proj"),
            )?,
...
impl ZImageTextEncoder {
    /// Creates a new `ZImageTextEncoder` from config, loading weights under the `model.` prefix.
    pub fn new(cfg: &TextEncoderConfig, vb: VarBuilder) -> Result<Self> {
        // Note: weights have "model." prefix
        let vb_model = vb.pp("model");

        let embed_tokens =
            fuel_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb_model.pp("embed_tokens"))?;
    ...
    pub fn forward(&self, input_ids: &Tensor) -> Result<Tensor> {
        ...
        fuel::bail!("Layer index out of bounds")
    }
