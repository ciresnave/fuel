...
use fuel_nn::{conv2d, group_norm, Conv2d, Conv2dConfig, GroupNorm, VarBuilder};
...
impl Attention {
    fn new(channels: usize, num_groups: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let to_q = fuel_nn::linear(channels, channels, vb.pp("to_q"))?;
        let to_k = fuel_nn::linear(channels, channels, vb.pp("to_k"))?;
        let to_v = fuel_nn::linear(channels, channels, vb.pp("to_v"))?;
        let to_out = fuel_nn::linear(channels, channels, vb.pp("to_out").pp("0"))?;
        Ok(Self {
            group_norm,
            to_q,
            to_k,
...
impl Encoder {
    /// Creates a new VAE encoder from config, building down blocks from `block_out_channels`.
...
impl Decoder {
    /// Creates a new VAE decoder from config, building reversed up blocks from `block_out_channels`.
    pub fn new(cfg: &VaeConfig, vb: VarBuilder) -> Result<Self> {
        let conv_cfg = Conv2dConfig {
            padding: 1,
            ..Default::default()
...
impl DiagonalGaussian {
    /// Creates a new `DiagonalGaussian`. When `sample=true` uses the reparameterisation trick; otherwise returns the mean.
...
impl AutoEncoderKL {
    /// Creates a new `AutoEncoderKL` loading encoder and decoder from config.
    pub fn new(cfg: &VaeConfig, vb: VarBuilder) -> Result<Self> {
