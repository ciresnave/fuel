...
use fuel_nn::{ops::softmax_last_dim, LayerNormConfig, Module, VarBuilder};

// Geglu and feedforward from fuel-transformers/src/models/stable_diffusion/attention.rs
#[derive(Debug)]
struct GeGlu {
    proj: Linear,
    span: tracing::Span,
...
// CrossAttention from fuel-transformers/src/models/stable_diffusion/attention.rs
#[derive(Debug)]
...
/// NV-Embed v2 full model with latent attention for producing fixed-size embeddings.
#[derive(Debug)]
pub struct Model {
...
impl Model {
    /// Load the NV-Embed v2 model from a `VarBuilder`.
    pub fn new(vb: VarBuilder) -> Result<Self> {
        ...
        let cross_attn_context_norm = layer_norm(
            dim,
            fuel_nn::LayerNormConfig::default(),
    ...
    /// Run a forward pass with input ids, attention mask, and a pooling mask; returns mean-pooled embeddings.
    pub fn forward(
        &mut self,
        input_ids: &Tensor,
        attn_mask: &Tensor,
