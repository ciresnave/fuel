...
impl Model {
    /// Build the full Gemma 4 model from the given config.
    pub fn new(cfg: &Gemma4Config, vb: fuel_nn::VarBuilder) -> Result<Self> {
        let vb = vb.pp("model");
    ...
    /// Reset the KV cache in the language model.
...
fn broadcast_embed_to_mask(embeds: &Tensor, mask: &Tensor) -> Result<Tensor> {
    ...
    // Since fuel doesn't have cumsum, we use a broadcast approach:
