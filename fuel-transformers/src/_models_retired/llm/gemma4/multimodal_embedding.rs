...
/// Projects encoder features into the language model embedding space.
#[derive(Debug, Clone)]
pub struct MultimodalEmbedder {
    embedding_projection: fuel_nn::Linear,
    embedding_pre_projection_norm: BareRmsNorm,
}

impl MultimodalEmbedder {
    /// Build the multimodal embedder with the given hidden sizes and norm epsilon.
    pub fn new(
        ...
        let embedding_projection = fuel_nn::linear_no_bias(
    ...
    /// Normalise and project soft encoder features to text embedding space.
