...
//! available, fuel implements the v5 and v6 versions and can be used with
...
/// Configuration for the RWKV v5 model.
...
/// The recurrent state tensors for a single transformer layer.
...
/// The full recurrent state of an RWKV model across all layers.
pub struct State {
...
impl State {
    /// Create a zeroed initial state for the given batch size and config.
...
impl SelfAttention {
    ...
    pub fn forward(&self, xs: &Tensor, state: &mut State) -> Result<Tensor> {
        ...
        let (receptance, key, value, gate) = {
            ...
            let gate = fuel_nn::ops::silu(&self.gate.forward(&gate)?)?;
...
impl FeedForward {
    ...
    pub fn forward(&self, xs: &Tensor, state: &mut State) -> Result<Tensor> {
        ...
        let receptance = fuel_nn::ops::sigmoid(&receptance.apply(&self.receptance)?)?;
...
impl Model {
    /// Create a new RWKV v5 model from config and variable store.
    ...
    /// Run a forward pass and return output logits, updating state in-place.
    pub fn forward(&self, xs: &Tensor, state: &mut State) -> Result<Tensor> {
...
impl Tokenizer {
    ...
    pub fn new<P: AsRef<std::path::Path>>(p: P) -> Result<Self> {
        ...
        let token2idx: HashMap<String, u32> =
            serde_json::from_reader(file).map_err(fuel::Error::wrap)?;
    ...
    pub fn decode(&self, tokens: &[u32]) -> Result<String> {
        ...
        String::from_utf8(bytes).map_err(fuel::Error::wrap)
    }
    ...
    pub fn encode_bytes(&self, bytes: &[u8]) -> Result<Vec<u32>> {
        ...
        while i < bytes.len() {
            ...
            let token = match self.token2idx.get(&s) {
                None => fuel::bail!("unexpected token '{}' {s:?}", String::from_utf8_lossy(&s)),
                Some(token) => *token,
            };
            tokens.push(token)
        }
