...
//!
//! References:
//! - ⚡ [Interactive Wasm Example](https://huggingface.co/spaces/radames/Fuel-T5-Generation-Wasm)
...
//!   --model-id "t5-small" \
//!   --prompt "translate to German: A beautiful fuel." \
//!   --decode
//! > ...
//! >  Eine schöne Kerze.
...
//! ```bash
//! cargo run --example t5 --release -- \
//!   --model-id "t5-small" --prompt "A beautiful fuel."
//! ...
...
use fuel::{DType, Device, Module, Result, Tensor, D};
use fuel_nn::kv_cache::KvCache;
use fuel_nn::{Activation, VarBuilder};
use serde::Deserialize;
use std::sync::Arc;

/// A weight-only linear layer (no bias) used in T5 attention and feed-forward blocks.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::t5::linear_no_bias;
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let vb: VarBuilder = unimplemented!();
/// let layer = linear_no_bias(512, 512, vb)?;
/// # Ok(())
/// # }
/// ```
#[derive(Debug, Clone)]
...
/// Create a bias-free linear layer with Kaiming-normal weight initialization.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::t5::linear_no_bias;
...
pub fn linear_no_bias(d1: usize, d2: usize, vb: VarBuilder) -> Result<Linear> {
    let init_ws = fuel_nn::init::DEFAULT_KAIMING_NORMAL;
...
/// Activation function configuration for the T5 feed-forward layer, with optional gating.
///
/// # Example
/// ```
/// use fuel_transformers::models::t5::ActivationWithOptionalGating;
/// use fuel_nn::Activation;
/// let act = ActivationWithOptionalGating { gated: true, activation: Activation::NewGelu };
/// assert!(act.gated);
/// ```
#[derive(Debug, Deserialize, Default, Clone, PartialEq)]
pub struct ActivationWithOptionalGating {
    pub gated: bool,
    pub activation: fuel_nn::Activation,
}

/// Deserialize the `feed_forward_proj` field from a T5 config JSON string.
///
/// Maps strings like `"gated-gelu"` or `"gated-silu"` to gated variants; anything
/// else is passed to [`serde_plain`] for the non-gated [`fuel_nn::Activation`].
///
/// # Example
/// ```no_run
/// # use serde::Deserialize;
/// # use fuel_transformers::models::t5::{ActivationWithOptionalGating, deserialize_feed_forward_proj_activation};
/// #[derive(Deserialize)]
/// struct Cfg {
///     #[serde(deserialize_with = "deserialize_feed_forward_proj_activation")]
    ...
    D: serde::de::Deserializer<'de>,
{
    match String::deserialize(deserializer)?.as_str() {
        "gated-gelu" => Ok(ActivationWithOptionalGating {
            gated: true,
            activation: fuel_nn::Activation::NewGelu,
        }),
        "gated-silu" => Ok(ActivationWithOptionalGating {
            gated: true,
            activation: fuel_nn::Activation::Silu,
        }),
        buf => {
...
/// # Example
/// ```
/// use fuel_transformers::models::t5::Config;
/// let cfg = Config::default();
/// assert_eq!(cfg.vocab_size, 32128);
...
impl Config {
    ...
    /// Return the configuration for the `facebook/musicgen-small` model.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::t5::Config;
    /// let cfg = Config::musicgen_small();
...
impl T5Attention {
    fn load(
        ...
        Ok(Self {
            ...
            kv_cache: KvCache::new(2, cfg.relative_attention_max_distance),
    ...
    fn forward(
        ...
        let (k, v) = if self.use_cache && key_value_states.is_none() {
        ...
        let attn_weights = {
            ...
            fuel_nn::ops::softmax_last_dim(&scores)?
        };
...
impl T5EncoderModel {
    /// Load a T5 encoder model from a variable store.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::t5::{Config, T5EncoderModel};
    /// # use fuel_nn::VarBuilder;
    /// # fn main() -> fuel::Result<()> {
    /// # let vb: VarBuilder = unimplemented!();
    /// let model = T5EncoderModel::load(vb, &Config::default())?;
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// Encode `input_ids` and return the encoder hidden states.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::t5::T5EncoderModel;
    ...
    /// Encode `input_ids` with an optional dtype override and return encoder hidden states.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::t5::T5EncoderModel;
    ...
    /// # use fuel_transformers::models::t5::T5EncoderModel;
    /// # fn main() {
    /// # let model: T5EncoderModel = unimplemented!();
    ...
    /// Clear the key-value cache of all encoder layers.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::t5::T5EncoderModel;
...
/// A T5 encoder-decoder model for conditional text generation.
#[derive(Debug, Clone)]
...
impl T5ForConditionalGeneration {
    /// Load a T5 encoder-decoder model from a variable store.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::t5::{Config, T5ForConditionalGeneration};
    /// # use fuel_nn::VarBuilder;
    /// # fn main() -> fuel::Result<()> {
    /// # let vb: VarBuilder = unimplemented!();
    /// let model = T5ForConditionalGeneration::load(vb, &Config::default())?;
    /// # Ok(())
    /// # }
    /// ```
    pub fn load(vb: VarBuilder, cfg: &Config) -> Result<Self> {
    ...
    /// Run the encoder on `input_ids` and return the encoder hidden states.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::t5::T5ForConditionalGeneration;
    /// # use fuel::{Device, DType, Tensor};
    /// # fn main() -> fuel::Result<()> {
    ...
    /// Run one decoder step, attending over `encoder_output`, and return the next-token logits.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::t5::T5ForConditionalGeneration;
    ...
    /// Run the full encoder-decoder forward pass and return next-token logits.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::t5::T5ForConditionalGeneration;
    ...
    /// # use fuel_transformers::models::t5::T5ForConditionalGeneration;
    /// # fn main() {
    /// # let model: T5ForConditionalGeneration = unimplemented!();
    /// let _device = model.device();
    /// # }
    /// ```
    pub fn device(&self) -> &Device {
    ...
    /// Clear the key-value cache of all encoder and decoder layers.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::t5::T5ForConditionalGeneration;
