...
/// CLIP vision-language model combining a text and vision encoder.
#[derive(Clone, Debug)]
pub struct ClipModel {
    text_model: ClipTextTransformer,
    vision_model: ClipVisionTransformer,
    visual_projection: fuel_nn::Linear,
    text_projection: fuel_nn::Linear,
    logit_scale: Tensor,
}

/// Wraps either a text or vision encoder config for shared parameter access.
#[derive(Clone, Debug)]
...
impl EncoderConfig {
    /// Return the embedding dimension of the wrapped encoder.
    ...
    /// Return the number of attention heads of the wrapped encoder.
    ...
    /// Return the FFN intermediate size of the wrapped encoder.
    ...
    /// Return the number of hidden layers of the wrapped encoder.
    ...
    /// Return the activation function of the wrapped encoder.
...
/// Top-level CLIP configuration combining text, vision, and projection settings.
#[derive(Clone, Debug)]
...
impl ClipConfig {
    ...
    /// ViT-B/32 configuration matching the `openai/clip-vit-base-patch32` checkpoint.
...
impl ClipModel {
    /// Load a CLIP model from a `VarBuilder` and config.
    pub fn new(vs: fuel_nn::VarBuilder, c: &ClipConfig) -> Result<Self> {
        ...
        let visual_projection = fuel_nn::linear_no_bias(
            c.vision_config.embed_dim,
            c.vision_config.projection_dim,
            vs.pp("visual_projection"),
        )?;
        let text_projection = fuel_nn::linear_no_bias(
    ...
    /// Encode text tokens and project into the shared embedding space.
    ...
    /// Encode pixel values and project into the shared embedding space.
    ...
    /// Run a forward pass returning per-image and per-text logit matrices.
