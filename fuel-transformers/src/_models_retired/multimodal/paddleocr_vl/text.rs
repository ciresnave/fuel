...
use fuel_nn::{embedding, linear_b, rms_norm, Embedding, Linear, Module, RmsNorm, VarBuilder};
...
impl Attention {
    ...
    pub fn forward_with_mrope_export(
        ...
        let key_states_repeated =
            crate::utils::repeat_kv(key_states.clone(), self.num_kv_groups)?;
        let value_states_repeated =
            crate::utils::repeat_kv(value_states.clone(), self.num_kv_groups)?;
        ...
        let attn_weights = if original_dtype != DType::F32 {
            ...
            fuel_nn::ops::softmax_last_dim(&attn_weights_masked)?
        };
