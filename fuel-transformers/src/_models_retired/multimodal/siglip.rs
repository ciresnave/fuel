...
use fuel::{IndexOp, Module, Result, Tensor, D};
use fuel_nn::{layer_norm, linear, LayerNorm, Linear, VarBuilder};
...
fn default_text_hidden_act() -> fuel_nn::Activation {
    fuel_nn::Activation::GeluPytorchTanh
}

/// SigLIP text encoder configuration.
///
/// Used to configure the text transformer portion of a SigLIP model.
/// All fields have sensible defaults that match the base SigLIP checkpoint.
///
/// # Example
/// ```
/// use fuel_transformers::models::siglip::TextConfig;
/// // Construct with explicit values (matches base SigLIP checkpoint defaults)
/// let cfg = TextConfig {
///     vocab_size: 32000, hidden_size: 768, intermediate_size: 3072,
///     num_hidden_layers: 12, num_attention_heads: 12,
///     max_position_embeddings: 64, layer_norm_eps: 1e-6,
///     hidden_act: fuel_nn::Activation::GeluPytorchTanh,
///     pad_token_id: 1, bos_token_id: 49406, eos_token_id: 49407,
/// };
...
fn default_vision_hidden_act() -> fuel_nn::Activation {
    fuel_nn::Activation::GeluPytorchTanh
}

/// SigLIP vision encoder configuration.
///
/// Used to configure the ViT portion of a SigLIP model.
/// All fields have sensible defaults matching the base SigLIP checkpoint.
///
/// # Example
/// ```
/// use fuel_transformers::models::siglip::VisionConfig;
/// let cfg = VisionConfig::paligemma_3b_224();
...
trait TransformerConfig {
    ...
    fn hidden_act(&self) -> fuel_nn::Activation;
}
...
impl VisionConfig {
    /// PaliGemma-3B vision config for 224×224 images (256 image tokens).
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::siglip::VisionConfig;
    ...
    /// PaliGemma-3B vision config for 448×448 images (1024 image tokens).
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::siglip::VisionConfig;
    /// let cfg = VisionConfig::paligemma_3b_448();
    ...
    /// PaliGemma-3B vision config for 896×896 images (4096 image tokens).
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::siglip::VisionConfig;
    ...
    /// assert_eq!(cfg.num_patches(), 4096); // (896/14)^2
    /// ```
    ...
    /// Number of image patch tokens produced by this config.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::siglip::VisionConfig;
...
/// Combined SigLIP model configuration (text + vision).
///
/// # Example
/// ```
/// use fuel_transformers::models::siglip::Config;
/// let cfg = Config::base_patch16_224();
...
impl Config {
    /// SigLIP-Base/16 configuration for 224×224 images.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::siglip::Config;
...
impl MultiheadAttention {
    ...
    fn forward(&self, q: &Tensor, k: &Tensor, v: &Tensor) -> Result<Tensor> {
        ...
        let attn = fuel_nn::ops::softmax_last_dim(&attn)?;
...
impl Attention {
    ...
    fn forward(&self, xs: &Tensor, attention_mask: Option<&Tensor>) -> Result<Tensor> {
        ...
        // The original implementation upcasts to f32 but fuel_nn::ops::softmax should handle this properly.
...
impl Mlp {
    fn new<C: TransformerConfig>(cfg: &C, vb: VarBuilder) -> Result<Self> {
        ...
        let fc1 = fuel_nn::linear(hidden_size, intermediate_size, vb.pp("fc1"))?;
        let fc2 = fuel_nn::linear(intermediate_size, hidden_size, vb.pp("fc2"))?;
        Ok(Self {
            fc1,
            fc2,
...
impl Module for Mlp {
    fn forward(&self, xs: &fuel::Tensor) -> Result<fuel::Tensor> {
        xs.apply(&self.fc1)?
...
struct VisionEmbeddings {
    patch_embedding: fuel_nn::Conv2d,
...
impl VisionEmbeddings {
    fn new(cfg: &VisionConfig, vb: VarBuilder) -> Result<Self> {
        let conv2d_cfg = fuel_nn::Conv2dConfig {
            stride: cfg.patch_size,
            ..Default::default()
        };
        let patch_embedding = fuel_nn::conv2d(
            cfg.num_channels,
            cfg.hidden_size,
            cfg.patch_size,
            conv2d_cfg,
        ...
        let embedder = fuel_nn::embedding(
...
/// SigLIP stand-alone vision encoder wrapping the ViT transformer.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::siglip::{VisionConfig, VisionModel};
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let vb: VarBuilder = unimplemented!();
/// let model = VisionModel::new(&VisionConfig::paligemma_3b_224(), true, vb)?;
/// # Ok(())
/// # }
/// ```
#[derive(Debug, Clone)]
...
impl VisionModel {
    /// Create a new vision model from `cfg`.
    ///
    /// Set `use_head` to `true` to include the final projection head.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::siglip::{VisionConfig, VisionModel};
...
struct TextEmbeddings {
    token_embedding: fuel_nn::Embedding,
    position_embedding: fuel_nn::Embedding,
    position_ids: Tensor,
}
...
impl TextEmbeddings {
    fn new(cfg: &TextConfig, vb: VarBuilder) -> Result<Self> {
        let token_embedding =
            fuel_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("token_embedding"))?;
        let position_embedding = fuel_nn::embedding(
...
/// SigLIP text transformer (embeddings + encoder + head).
///
/// The `head` field is public for weight-tying with downstream tasks (e.g. PaliGemma).
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::siglip::{TextConfig, TextTransformer};
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let _vb: VarBuilder = unimplemented!();
/// // TextTransformer is constructed internally via TextModel::new
/// # Ok(())
/// # }
/// ```
#[derive(Debug, Clone)]
...
/// SigLIP stand-alone text encoder wrapping [`TextTransformer`].
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::siglip::{TextConfig, TextModel};
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: TextConfig = unimplemented!();
/// let model = TextModel::new(&cfg, vb)?;
/// # Ok(())
/// # }
/// ```
#[derive(Debug, Clone)]
...
impl TextModel {
    /// Create a new text model from `cfg`.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::siglip::{TextConfig, TextModel};
...
/// Full SigLIP model (vision + text + sigmoid contrastive loss parameters).
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::siglip::{Config, Model};
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let vb: VarBuilder = unimplemented!();
/// let model = Model::new(&Config::base_patch16_224(), vb)?;
/// # Ok(())
/// # }
/// ```
#[derive(Clone, Debug)]
...
impl Model {
    /// Create a new SigLIP model from the combined `cfg`.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::siglip::{Config, Model};
    ...
    /// Encode token sequences into text feature vectors.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::siglip::Model;
    ...
    /// Encode image batches into vision feature vectors.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::siglip::Model;
