...
//!   --example eva2 \
//!   --release -- \
//!   --image fuel-examples/examples/yolo-v8/assets/bike.jpg
//!
...
//!
//! <div align=center>
//!   <img src="https://github.com/huggingface/fuel/raw/main/fuel-examples/examples/yolo-v8/assets/bike.jpg" alt="" width=640>
//! </div>
//!
use fuel::{IndexOp, Result, Tensor, D};
use fuel_nn::{layer_norm, LayerNorm, Linear, Module, VarBuilder};
...
fn linear(vb: VarBuilder, in_dim: usize, out_dim: usize, bias: bool) -> Result<Linear> {
    if bias {
        fuel_nn::linear(in_dim, out_dim, vb)
    } else {
        fuel_nn::linear_no_bias(in_dim, out_dim, vb)
    }
}
...
impl Module for Attention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let attn = fuel_nn::ops::softmax(attn, D::Minus1)?;
...
impl PatchEmbed {
    fn new(
        ...
        let config = fuel_nn::Conv2dConfig {
            stride: patch_size,
            ..Default::default()
        };
        let proj = fuel_nn::conv2d(in_chans, embed_dim, patch_size, config, vb.pp("proj"))?;
...
impl Module for PatchEmbed {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        if (h % patch_h) != 0 {
            fuel::bail!("image height {h} is not a multiple of patch height {patch_h}")
        }
        if (w % patch_w) != 0 {
            fuel::bail!("image width {w} is not a multiple of patch width {patch_w}")
        }
...
/// EVA-02 Vision Transformer backbone.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::eva2::EVA2VisionTransformer;
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let vb: VarBuilder = unimplemented!();
/// // ViT-Base/16: depth=12, embed_dim=768, heads=12
/// let model = EVA2VisionTransformer::new(vb, 12, 768, 12)?;
/// # Ok(())
/// # }
/// ```
#[derive(Debug)]
...
impl EVA2VisionTransformer {
    /// Construct an EVA-02 ViT with the given depth, embedding dimension, and head count.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::eva2::EVA2VisionTransformer;
    ...
    /// # fn main() -> fuel::Result<()> {
    /// # let vb: VarBuilder = unimplemented!();
    /// let model = EVA2VisionTransformer::new(vb, 12, 768, 12)?; // ViT-Base
    /// # Ok(())
    /// # }
    /// ```
    ...
    /// Return intermediate hidden states at the specified block indices.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::eva2::EVA2VisionTransformer;
...
/// Load an EVA-02 ViT-Base/16 model.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::eva2::vit_base;
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let vb: VarBuilder = unimplemented!();
/// let model = vit_base(vb)?; // depth=12, embed_dim=768, heads=12
/// # Ok(())
/// # }
/// ```
...
/// Load an EVA-02 ViT-Large/16 model.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::eva2::vit_large;
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let vb: VarBuilder = unimplemented!();
/// let model = vit_large(vb)?; // depth=24, embed_dim=1024, heads=16
/// # Ok(())
/// # }
/// ```
