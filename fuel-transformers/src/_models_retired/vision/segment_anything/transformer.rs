...
impl Attention {
    fn new(
        ...
        let q_proj = fuel_nn::linear(embedding_dim, internal_dim, vb.pp("q_proj"))?;
        let k_proj = fuel_nn::linear(embedding_dim, internal_dim, vb.pp("k_proj"))?;
        let v_proj = fuel_nn::linear(embedding_dim, internal_dim, vb.pp("v_proj"))?;
        let out_proj = fuel_nn::linear(internal_dim, embedding_dim, vb.pp("out_proj"))?;
        Ok(Self {
            q_proj,
            k_proj,
            v_proj,
...
/// Bidirectional cross-attention transformer used inside the SAM mask decoder.
#[derive(Debug)]
...
impl TwoWayTransformer {
    /// Build the two-way transformer with the given depth and dimensions.
    pub fn new(
        depth: usize,
        embedding_dim: usize,
        num_heads: usize,
    ...
    /// Run bidirectional attention between query tokens and image keys; returns `(queries, keys)`.
    pub fn forward(
        &self,
