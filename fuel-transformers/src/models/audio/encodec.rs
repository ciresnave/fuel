...
use fuel::{D, DType, IndexOp, Layout, Module, Result, Shape, Tensor};
use fuel_nn::{Conv1d, ConvTranspose1d, VarBuilder, conv1d};

// Encodec Model
// https://github.com/huggingface/transformers/blob/main/src/transformers/models/encodec/modeling_encodec.py

/// Weight-normalisation strategy applied to convolution layers.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::encodec::NormType;
/// let norm = NormType::WeightNorm;
/// ```
...
/// Padding mode for causal convolutions in EnCodec.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::encodec::PadMode;
/// let mode = PadMode::Replicate;
/// ```
#[derive(Debug, Copy, Clone, PartialEq, Eq, serde::Deserialize)]
pub enum PadMode {
    Constant,
...
///
/// Matches the HuggingFace `config.json` fields for
/// `facebook/encodec_24khz` and similar checkpoints.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::encodec::Config;
...
impl Default for Config {
    fn default() -> Self {
        Self {
            ...
            // This should be PadMode::Reflect which is currently unsupported in fuel.
...
fn pad1d(xs: &Tensor, pad_l: usize, pad_r: usize, mode: PadMode) -> Result<Tensor> {
    match mode {
        ...
        PadMode::Reflect => fuel::bail!("pad-mode 'reflect' is not supported"),
...
/// Load a [`Conv1d`] layer whose weights are stored in weight-norm form (`weight_g` / `weight_v`).
///
/// Weight normalisation is applied at load time so no runtime overhead remains.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::{Config, conv1d_weight_norm};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let layer = conv1d_weight_norm(1, 128, 7, Default::default(), vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
/// Load a bias-free [`Conv1d`] layer whose weights are stored in weight-norm form.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::conv1d_weight_norm_no_bias;
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let layer = conv1d_weight_norm_no_bias(1, 128, 7, Default::default(), vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
/// Load a [`ConvTranspose1d`] layer whose weights are stored in weight-norm form.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::conv_transpose1d_weight_norm;
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let layer = conv_transpose1d_weight_norm(128, 64, 8, true, Default::default(), vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
pub fn conv_transpose1d_weight_norm(
    ...
    kernel_size: usize,
    bias: bool,
    config: fuel_nn::ConvTranspose1dConfig,
    vb: VarBuilder,
...
impl fuel::CustomOp2 for CodebookEncode {
    ...
    fn fwd(
        ...
        if lhs_dim2 != rhs_dim2 {
            fuel::bail!("CodebookEncode, mismatch on last dim, {lhs_layout:?} {rhs_layout:?}");
        }
        if lhs_dim2 == 0 {
            fuel::bail!("CodebookEncode, empty last dim {lhs_layout:?}")
        }
        let lhs = match lhs_layout.contiguous_offsets() {
            None => fuel::bail!("CodebookEncode, lhs has to be contiguous, got {lhs_layout:?}"),
            Some((o1, o2)) => {
        ...
        let rhs = match rhs_layout.contiguous_offsets() {
            None => fuel::bail!("CodebookEncode, rhs has to be contiguous, got {rhs_layout:?}"),
            Some((o1, o2)) => {
...
/// Euclidean-distance vector codebook used by EnCodec's RVQ.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::{Config, EuclideanCodebook};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
...
pub struct EuclideanCodebook {
    inited: Tensor,
    cluster_size: Tensor,
    embed: fuel_nn::Embedding,
    embed_avg: Tensor,
    c2: Tensor,
}

impl EuclideanCodebook {
    /// Load the codebook embedding table from a [`VarBuilder`].
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            inited,
            cluster_size,
            embed: fuel_nn::Embedding::new(embed, cfg.codebook_dim()),
            embed_avg,
            c2,
        })
    }

    /// Encode `xs` to codebook indices using a reference (non-fused) matmul.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Encode `xs` to codebook indices using the fused `CodebookEncode` op.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Decode codebook indices back to embedding vectors.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// Single-codebook vector quantizer (wraps [`EuclideanCodebook`]).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::{Config, VectorQuantization};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
...
impl VectorQuantization {
    /// Load a single VQ layer from a [`VarBuilder`].
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Map continuous features to the nearest codebook index.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Decode codebook indices to quantized feature vectors.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// Residual vector quantizer: stacked [`VectorQuantization`] layers that
/// progressively encode the residual from all previous codebooks.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::{Config, ResidualVectorQuantizer};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
...
impl ResidualVectorQuantizer {
    /// Load all VQ layers from a [`VarBuilder`].
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Encode a feature tensor to a stack of codebook index tensors `(num_quantizers, batch, time)`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Decode a stack of codebook indices back to a quantized feature tensor.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// Bidirectional LSTM with a residual connection used in the EnCodec encoder/decoder.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::{Config, EncodecLSTM};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
/// let lstm = EncodecLSTM::new(128, &cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Clone, Debug)]
pub struct EncodecLSTM {
    layers: Vec<fuel_nn::LSTM>,
}

impl EncodecLSTM {
    /// Load the LSTM layers from a [`VarBuilder`].
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn new(dim: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        for layer_idx in 0..cfg.num_lstm_layers {
            let config = fuel_nn::LSTMConfig {
                layer_idx,
                ..Default::default()
            };
            let lstm = fuel_nn::lstm(dim, dim, config, vb.clone())?;
            layers.push(lstm)
        }
        Ok(Self { layers })
    }
...
/// Weight-normalised transposed convolution layer for the EnCodec decoder upsampling path.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::{Config, Model};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// // EncodecConvTranspose1d is constructed internally by Decoder / Model.
/// let cfg = Config::default();
...
impl EncodecConvTranspose1d {
    fn new(
        ...
        let cfg = fuel_nn::ConvTranspose1dConfig {
            stride,
            ..Default::default()
        };
...
/// Padded 1-D convolution with optional weight normalisation or group normalisation.
///
/// Handles both causal (left-aligned) and non-causal (symmetric) padding so
/// that the output length is deterministic regardless of the sequence length.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::{Config, EncodecConv1d};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
/// let conv = EncodecConv1d::new(1, 32, 7, 1, 1, &cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Clone, Debug)]
pub struct EncodecConv1d {
    causal: bool,
    conv: Conv1d,
    norm: Option<fuel_nn::GroupNorm>,
    pad_mode: PadMode,
}

impl EncodecConv1d {
    /// Create an [`EncodecConv1d`] layer, applying weight-norm or group-norm
    /// depending on `cfg.norm_type`, and causal or symmetric padding depending
    /// on `cfg.use_causal_conv`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn new(
        ...
        let norm = match cfg.norm_type {
            ...
            NormType::TimeGroupNorm => {
                let gn = fuel_nn::group_norm(1, out_c, 1e-5, vb.pp("norm"))?;
                Some(gn)
            }
        };
        Ok(Self {
...
/// Residual block used in both the EnCodec encoder and decoder.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::{Config, EncodecResnetBlock};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
/// let block = EncodecResnetBlock::new(128, (1, 1), &cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Clone, Debug)]
...
impl EncodecResnetBlock {
    /// Build an [`EncodecResnetBlock`] with two dilated convolutions and an
    /// optional pointwise shortcut convolution (`cfg.use_conv_shortcut`).
    ///
    /// # Example
    ///
    /// ```no_run
...
///
/// Maps a waveform `(batch, channels, time)` to a latent code sequence
/// of shape `(batch, hidden_size, frames)` at a reduced frame rate.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::{Config, Encoder};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
/// let encoder = Encoder::new(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Clone, Debug)]
pub struct Encoder {
...
impl Encoder {
    /// Build the encoder from a model config and weight store.
    ///
    /// # Example
    ///
    /// ```no_run
...
///
/// Upsamples quantized latent codes back to a waveform.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::encodec::{Config, Decoder};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
...
impl Decoder {
    /// Build the decoder from a model config and weight store.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// The complete EnCodec neural audio codec.
///
/// Bundles the [`Encoder`], [`Decoder`], and [`ResidualVectorQuantizer`].
/// Use [`encode`](Model::encode) to compress a waveform to discrete codes, and
/// [`decode`](Model::decode) to reconstruct audio from those codes.
///
/// # Example
///
/// ```no_run
...
impl Model {
    /// Build the complete EnCodec model from a config and weight store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Encode a waveform tensor `(batch, channels, samples)` to discrete codes
    /// of shape `(batch, num_codebooks, frames)`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Reconstruct audio from codes of shape `(batch, num_codebooks, frames)`.
    ///
    /// # Example
    ///
    /// ```no_run
