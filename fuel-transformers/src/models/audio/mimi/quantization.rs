...
use fuel::{IndexOp, Layout, Result, Shape, Tensor, D};
...
#[allow(unused)]
#[derive(Debug, Clone)]
/// Euclidean-distance codebook used by Mimi's residual vector quantizer.
...
impl EuclideanCodebook {
    /// Load the codebook from a [`VarBuilder`].
    ...
    /// Encode using the O(N²) reference path (avoids the fused kernel).
    ...
    /// Encode via fast-matmul distance trick (no fused kernel).
    ...
    /// Encode using the fused `CodebookEncode` custom op.
    ...
    /// Decode codebook indices to embedding vectors.
    pub fn decode(&self, indexes: &Tensor) -> Result<Tensor> {
        let _enter = self.span_decode.enter();
        // let ys = fuel_nn::Embedding::new(self.embedding.clone(), self.dim).forward(xs)?;
...
#[allow(unused)]
#[derive(Debug, Clone)]
/// Single-codebook VQ layer with optional input/output projections.
...
impl VectorQuantization {
    /// Build the VQ layer.
    ///
    /// If `codebook_dim` differs from `dim`, linear projections are added on both sides.
    pub fn new(
        dim: usize,
        codebook_size: usize,
    ...
    /// Encode `xs` to codebook indices.
    pub fn encode(&self, xs: &Tensor) -> Result<Tensor> {
        let xs = xs.t()?.apply(&self.project_in.as_ref())?;
        self.codebook.encode_slow(&xs)
    }

    /// Decode codebook indices to feature vectors.
...
/// Stack of [`VectorQuantization`] layers for residual coding.
#[derive(Debug, Clone)]
...
impl ResidualVectorQuantization {
    ...
    /// Encode `xs` to stacked codes of shape `(n_q, batch, time)`.
    pub fn encode(&self, xs: &Tensor) -> Result<Tensor> {
    ...
    /// Decode stacked codes of shape `(n_q, batch, time)` to feature tensor.
    pub fn decode(&self, xs: &Tensor) -> Result<Tensor> {
        if self.layers.is_empty() {
            fuel::bail!("empty layers in ResidualVectorQuantization")
        }
...
/// RVQ with optional input/output projections, used directly by the Mimi model.
#[allow(unused)]
#[derive(Debug, Clone)]
pub struct ResidualVectorQuantizer {
    vq: ResidualVectorQuantization,
    input_proj: Option<fuel_nn::Conv1d>,
    output_proj: Option<fuel_nn::Conv1d>,
}
...
impl ResidualVectorQuantizer {
    ...
    /// Encode a feature tensor to codes of shape `(batch, n_q, time)`.
    pub fn encode(&self, xs: &Tensor) -> Result<Tensor> {
        let codes = self.vq.encode(&xs.apply(&self.input_proj.as_ref())?)?;
        codes.transpose(0, 1)
    }

    /// Decode codes of shape `(batch, n_q, time)` to a feature tensor.
    pub fn decode(&self, codes: &Tensor) -> Result<Tensor> {
...
/// Split RVQ: one semantic quantizer followed by `n_q-1` acoustic quantizers.
///
/// This matches the Mimi architecture where the first codebook carries
/// semantic information and the rest carry acoustic detail.
#[derive(Debug, Clone)]
...
impl SplitResidualVectorQuantizer {
    /// Build a [`SplitResidualVectorQuantizer`] with `n_q` total codebooks
    /// (`1` semantic + `n_q - 1` acoustic).
    pub fn new(
        dim: usize,
        input_dim: Option<usize>,
        output_dim: Option<usize>,
    ...
    /// Encode `xs` to complete codes of shape `(batch, n_q, time)`.
    pub fn encode(&self, xs: &Tensor) -> Result<Tensor> {
        let _enter = self.span_encode.enter();
    ...
    /// Decode codes of shape `(batch, n_q, time)` via sum of semantic + acoustic paths.
    pub fn decode(&self, codes: &Tensor) -> Result<Tensor> {
