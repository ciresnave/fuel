...
use fuel::{streaming, Module, Result, StreamTensor, StreamingModule, Tensor};
use fuel_nn::VarBuilder;

use super::conv::{StreamableConv1d, StreamableConvTranspose1d};

/// Configuration for the SeaNet encoder/decoder used by Mimi.
#[derive(Debug, Clone)]
pub struct Config {
    pub dimension: usize,
    pub channels: usize,
    ...
    pub final_activation: Option<fuel_nn::Activation>,
}

/// Residual block used inside the SeaNet encoder and decoder.
#[derive(Debug, Clone)]
pub struct SeaNetResnetBlock {
    block: Vec<StreamableConv1d>,
    shortcut: Option<StreamableConv1d>,
    activation: fuel_nn::Activation,
    skip_op: fuel::StreamingBinOp,
    span: tracing::Span,
}

impl SeaNetResnetBlock {
    #[allow(clippy::too_many_arguments)]
    /// Build a [`SeaNetResnetBlock`] with the given dilated-conv stack.
    pub fn new(
        dim: usize,
        ...
        Ok(Self {
            ...
            shortcut,
            activation,
            skip_op: streaming::StreamingBinOp::new(streaming::BinOp::Add, fuel::D::Minus1),
...
#[derive(Debug, Clone)]
/// SeaNet convolutional encoder used by Mimi.
///
/// Progressively downsamples audio to a latent sequence using
/// strided convolutions separated by residual blocks.
...
impl SeaNetEncoder {
    /// Build from a [`Config`] and variable store.
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        if cfg.lstm > 0 {
            fuel::bail!("seanet lstm is not supported")
        }
...
#[derive(Debug, Clone)]
/// SeaNet convolutional decoder used by Mimi.
///
/// Progressively upsamples quantized latents back to audio.
pub struct SeaNetDecoder {
    init_conv1d: StreamableConv1d,
    activation: fuel_nn::Activation,
    layers: Vec<DecoderLayer>,
    final_conv1d: StreamableConv1d,
    final_activation: Option<fuel_nn::Activation>,
    span: tracing::Span,
}
