...
use fuel::{DType, Device, IndexOp, Module, Result, StreamTensor, StreamingModule, Tensor, D};
use fuel_nn::{linear_no_bias, Linear, VarBuilder};
use std::sync::Arc;

fn linear(in_d: usize, out_d: usize, bias: bool, vb: VarBuilder) -> Result<Linear> {
    if bias {
        fuel_nn::linear(in_d, out_d, vb)
    } else {
        linear_no_bias(in_d, out_d, vb)
    }
}

/// Positional encoding scheme for the streaming transformer.
...
/// Configuration for the Mimi streaming transformer.
#[derive(Debug, Clone)]
pub struct Config {
    pub d_model: usize,
    pub num_heads: usize,
    ...
    pub gating: Option<fuel_nn::Activation>,
...
/// Precomputed rotary position embeddings (RoPE) for multi-head attention.
#[derive(Debug, Clone)]
...
impl RotaryEmbedding {
    /// Build RoPE tables for `dim`-dimensional queries/keys up to `max_seq_len` tokens.
    ...
    /// Apply rotary embeddings to `qk` starting at `seqlen_offset`.
    pub fn apply_rotary_emb(&self, qk: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        ...
        fuel_nn::rotary_emb::rope_i(&qk.to_dtype(DType::F32)?, &c, &s)?.to_dtype(qk_dtype)
    }
}

/// Per-layer learnable scale applied after each sub-layer output.
#[derive(Debug, Clone)]
...
impl LayerScale {
    /// Load the scale parameter of size `d_model`.
...
/// Streaming multi-head self-attention with a rotating KV cache.
#[derive(Debug, Clone)]
pub struct StreamingMultiheadAttention {
    ...
    kv_cache: fuel_nn::kv_cache::RotatingKvCache,
    pos: usize,
    use_flash_attn: bool,
    span: tracing::Span,
}
...
impl StreamingMultiheadAttention {
    /// Build a self-attention layer from a config and variable store.
    pub fn new(rope: &Option<Arc<RotaryEmbedding>>, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            ...
            context: cfg.context,
            neg_inf,
            kv_cache: fuel_nn::kv_cache::RotatingKvCache::new(2, cfg.context),
            pos: 0,
    ...
    /// Run self-attention on `xs`, applying the optional causal `mask`.
    pub fn forward(&mut self, xs: &Tensor, mask: Option<&Tensor>) -> Result<Tensor> {
        let _enter = self.span.enter();
        if self.kv_repeat != 1 {
            fuel::bail!("only kv-repeat = 1 is supported")
        }
        let (b, t, hd) = xs.dims3()?;
        ...
        let xs = if q.dtype() == DType::BF16 && self.use_flash_attn {
            ...
            let ws = fuel_nn::ops::softmax_last_dim(&pre_ws)?; // b,h,t,k
            ws.matmul(&v)? // b,h,t,d
        };
        let xs = xs
    ...
    /// Clear the rotating KV cache.
    pub fn reset_kv_cache(&mut self) {
        self.kv_cache.reset()
    }

    /// Replace the rotating KV cache (e.g., to share state across instances).
    pub fn set_kv_cache(&mut self, kv_cache: fuel_nn::kv_cache::RotatingKvCache) {
        self.kv_cache = kv_cache
    }
}

/// Streaming cross-attention operating against a fixed-length context source.
#[derive(Debug, Clone)]
...
impl StreamingMultiheadCrossAttention {
    /// Build a cross-attention layer from a config and variable store.
    ...
    /// Compute cross-attended output: `xs` attends to `ca_src`.
...
/// Feed-forward block; supports vanilla (two linear layers) and gated variants.
#[derive(Debug, Clone)]
pub enum Mlp {
    NoGating {
        span1: tracing::Span,
    ...
    Gating {
        linear_in: Linear,
        linear_out: Linear,
        activation: fuel_nn::Activation,
        span: tracing::Span,
    },
}

impl Mlp {
    /// Build an [`Mlp`] block according to `cfg.gating`.
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
...
/// RMS normalisation layer (used when `cfg.norm == NormType::RmsNorm`).
#[derive(Debug, Clone)]
pub struct RmsNorm {
...
impl RmsNorm {
    /// Load RMS-norm scale weights for a `d_model`-dimensional layer.
...
impl Module for RmsNorm {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        fuel_nn::ops::rms_norm(xs, &self.alpha, self.eps)
    }
}

/// Normalisation variant that can be either a [`fuel_nn::LayerNorm`] or [`RmsNorm`].
#[derive(Debug, Clone)]
pub enum Norm {
...
/// One transformer layer: pre-norm self-attention + optional cross-attention + FFN.
#[derive(Debug, Clone)]
pub struct StreamingTransformerLayer {
    ...
    cross_attn: Option<(fuel_nn::LayerNorm, StreamingMultiheadCrossAttention)>,
    norm_first: bool,
    span: tracing::Span,
}

impl StreamingTransformerLayer {
    /// Build a transformer layer; optionally sharing `rope` across layers.
    pub fn new(rope: &Option<Arc<RotaryEmbedding>>, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        if cfg.use_conv_block {
            fuel::bail!("conv-block is not supported")
        }
        let d_model = cfg.d_model;
        let mlp = Mlp::new(cfg, vb.clone())?;
        let (norm1, norm2) = match cfg.norm {
            super::NormType::LayerNorm => {
                let norm1 = fuel_nn::layer_norm(d_model, 1e-5, vb.pp("input_layernorm"))?;
                let norm2 =
                    fuel_nn::layer_norm(d_model, 1e-5, vb.pp("post_attention_layernorm"))?;
        ...
        let cross_attn = if cfg.cross_attention {
            let norm_cross = fuel_nn::layer_norm(cfg.d_model, 1e-5, vb.pp("norm_cross"))?;
    ...
    /// Run the layer forward pass, optionally with cross-attention source `ca_src`.
    pub fn forward(
        &mut self,
        xs: &Tensor,
        ...
        let _enter = self.span.enter();
        if !self.norm_first {
            fuel::bail!("only norm_first = true is supported")
        }
    ...
    /// Replace the KV cache with an existing one.
    pub fn set_kv_cache(&mut self, kv_cache: fuel_nn::kv_cache::RotatingKvCache) {
        self.self_attn.set_kv_cache(kv_cache)
    }
}

/// Stack of [`StreamingTransformerLayer`]s with positional encoding.
#[derive(Debug, Clone)]
...
impl StreamingTransformer {
    /// Build a transformer stack from a config and variable store.
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
    ...
    /// Run the full forward pass (no cross-attention).
    pub fn forward(&mut self, xs: &Tensor) -> Result<Tensor> {
        self.forward_ca(xs, None)
    }

    /// Run the full forward pass with optional cross-attention source.
    ...
    /// Copy KV-cache state from another transformer instance.
    pub fn copy_state(&mut self, from: &Self) -> Result<()> {
        if self.layers.len() != from.layers.len() {
            fuel::bail!("cannot copy kv-caches as the transformers have different depths")
        }
        self.layers
            .iter_mut()
...
/// Transformer with optional input/output linear projections and conv-layout support.
#[derive(Debug, Clone)]
...
impl ProjectedTransformer {
    /// Build from an input dimension, a list of output dimensions, a config, and weights.
    pub fn new(
        input_dim: usize,
        output_dims: &[usize],
        cfg: &Config,
    ...
    /// Run the transformer and return projected outputs (one per `output_dims` entry).
