...
//!
//! See: [SNAC](https://github.com/hubertsiuzdak/snac) and the paper
//! [SNAC: Multi-Scale Neural Audio Codec](https://arxiv.org/abs/2410.14411).
//!
//! SNAC compresses audio to multi-resolution residual codes and reconstructs
//! high-fidelity audio from those codes with a single shared decoder.
...
/// Configuration for a SNAC model.
///
/// # Example
///
/// ```no_run
/// # let cfg: fuel_transformers::models::snac::Config = unimplemented!(); // Typically loaded from JSON
/// ```
#[derive(serde::Deserialize, Debug, Clone)]
pub struct Config {
    /// Audio sampling rate (in Hz).
    pub sampling_rate: usize,
    /// Base channel count for the encoder.
    pub encoder_dim: usize,
    /// Stride per encoder stage (controls temporal downsampling).
    pub encoder_rates: Vec<usize>,
    /// Base channel count for the decoder.
    pub decoder_dim: usize,
    /// Stride per decoder stage (controls temporal upsampling).
    pub decoder_rates: Vec<usize>,
    /// Local-attention window size; `None` disables attention.
    pub attn_window_size: Option<usize>,
    /// Number of entries in each codebook.
    pub codebook_size: usize,
    /// Embedding dimension of each codebook.
    pub codebook_dim: usize,
    /// Per-quantizer temporal stride for the residual VQ.
    pub vq_strides: Vec<usize>,
    /// Whether to inject noise in the decoder.
    pub noise: bool,
    /// Use depthwise-separable convolutions.
    pub depthwise: bool,
}

/// Repeat each slice of `img` along `dim` exactly `repeats` times.
///
/// This is the Fuel equivalent of `torch.repeat_interleave`.
///
/// # Example
///
/// ```
...
/// # Ok::<(), fuel::Error>(())
/// ```
pub fn repeat_interleave<D: fuel::shape::Dim>(
    img: &Tensor,
    repeats: usize,
    dim: D,
) -> Result<Tensor> {
...
/// Load a [`Conv1d`] layer from weight-norm tensors stored under
/// `parametrizations.weight.original{0,1}` (PyTorch 2.x layout).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::snac::conv1d_weight_norm;
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let layer = conv1d_weight_norm(1, 64, 7, Default::default(), vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
/// Load a bias-free [`Conv1d`] layer from weight-norm tensors.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::snac::conv1d_weight_norm_no_bias;
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let layer = conv1d_weight_norm_no_bias(1, 64, 7, Default::default(), vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
/// Load a [`ConvTranspose1d`] layer from weight-norm tensors.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::snac::conv_transpose1d_weight_norm;
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let layer = conv_transpose1d_weight_norm(64, 32, 16, false, Default::default(), vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
impl LocalMHA {
    fn new(
        ...
        let norm = fuel_nn::layer_norm(dim, 1e-5, vb.pp("norm"))?;
...
/// SNAC convolutional encoder.
///
/// Transforms a waveform `(batch, 1, samples)` to a latent tensor at a
/// downsampled frame rate controlled by `encoder_rates`.
///
/// # Example
///
/// ```no_run
...
/// // Encoder is constructed internally by Model::new.
...
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// // Decoder is constructed internally by Model::new.
/// let model = Model::new(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
...
/// Residual vector quantizer: a stack of per-stride `VectorQuantizer`s.
///
/// Each quantizer runs at a different time resolution set by its `stride`.
/// Calling encode returns one index tensor per stride level,
/// suitable for passing back to [`decode`](Model::decode).
///
/// # Example
///
/// ```no_run
...
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// // ResidualVectorQuantizer is constructed internally by Model::new.
/// let model = Model::new(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Clone, Debug)]
...
/// Complete SNAC model: [`Encoder`] + [`ResidualVectorQuantizer`] + [`Decoder`].
///
/// # Example
///
/// ```no_run
...
pub struct Model {
    ...
    pub decoder: Decoder,
    /// Samples per encoded frame (product of all encoder strides).
    pub hop_length: usize,
    pub config: Config,
}

impl Model {
    /// Build the SNAC model from a [`Config`] and weight store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Encode a waveform to per-codebook index tensors.
    ///
    /// Returns one integer tensor per quantizer level; each has shape
    /// `(batch, frames_at_level)`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Decode multi-scale code tensors back to a waveform.
    ///
    /// `audio_codes` must contain one integer tensor per codebook, in the same
    /// order as [`encode`](Model::encode) produced them.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// # let cfg: Config = unimplemented!();
    /// let model = Model::new(&cfg, vb)?;
    /// let codes: Vec<Tensor> = vec![]; // produced by encode
    /// let refs: Vec<&Tensor> = codes.iter().collect();
    /// let audio = model.decode(&refs)?;
    /// # Ok::<(), fuel::Error>(())
    /// ```
    ...
    /// Return a reference to the model config.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Return the number of residual codebooks.
    ///
    /// # Example
    ///
    /// ```no_run
