...
use fuel::utils::get_num_threads;
use std::sync::Arc;
use std::thread;
...
/// Compute a log-mel spectrogram from raw samples using a multi-threaded FFT pipeline.
...
/// Convert raw PCM samples to a mel-spectrogram vector using the model's config parameters.
