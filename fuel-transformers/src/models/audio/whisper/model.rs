...
use fuel::{Device, IndexOp, Result, Tensor, D};
use fuel_nn::{embedding, Conv1d, Conv1dConfig, Embedding, LayerNorm, Module, VarBuilder};
...
impl MultiHeadAttention {
    ...
    fn qkv_attention(
        ...
        let w = {
            ...
            fuel_nn::ops::softmax_last_dim(&qk)?
        };
        let wv = {
            let _enter = self.matmul_span.enter();
            w.matmul(&v)?
...
/// Whisper audio encoder (CNN + transformer) that converts mel spectrograms to hidden states.
#[derive(Debug, Clone)]
...
impl AudioEncoder {
    ...
    /// Run the audio encoder forward pass; optionally flush the KV cache.
...
/// Whisper autoregressive text decoder that attends on encoder outputs.
#[derive(Debug, Clone)]
...
impl TextDecoder {
    ...
    /// Run one decoder step; flushes the KV cache when `flush_kv_cache` is true.
    ...
    /// Project decoder hidden states to vocabulary logits using the token embedding weight.
...
/// Full Whisper model combining an audio encoder and a text decoder.
#[derive(Debug, Clone)]
pub struct Whisper {
...
impl Whisper {
    /// Load the Whisper model from a `VarBuilder` and config.
    ...
    /// Reset the KV cache in both encoder and decoder.
    pub fn reset_kv_cache(&mut self) {
        self.encoder
            .blocks
            .iter_mut()
