...
/// Generate a standard-normal latent noise tensor of shape `(num_samples, 16, h/8, w/8)`.
pub fn get_noise(
    num_samples: usize,
    height: usize,
    width: usize,
...
/// Sampling state for a single denoising trajectory (packed image + IDs + text/vector embeddings).
#[derive(Debug, Clone)]
pub struct State {
    pub img: Tensor,
    pub img_ids: Tensor,
...
impl State {
    /// Build a sampling state from T5 embeddings, CLIP embeddings, and an initial noise image.
...
/// Unpack a `(b, h*w, c*4)` packed patch tensor into `(b, c, H, W)` image layout.
...
/// Run iterative denoising over the given timestep schedule.
