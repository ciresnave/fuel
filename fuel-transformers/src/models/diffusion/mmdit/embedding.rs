use fuel::{bail, DType, Module, Result, Tensor};
use fuel_nn as nn;

/// Embeds image patches by projecting each patch to the hidden dimension via a strided convolution.
pub struct PatchEmbedder {
    proj: nn::Conv2d,
}

impl PatchEmbedder {
    /// Creates a new `PatchEmbedder` with the given patch size and channel dimensions.
    pub fn new(
        patch_size: usize,
        in_channels: usize,
        embed_dim: usize,
...
/// Reverses patch embedding to reconstruct a spatial tensor from a patch sequence.
...
impl Unpatchifier {
    /// Creates a new `Unpatchifier` for the given patch size and output channels.
    ...
    /// Reconstructs a `(B, C, H, W)` tensor from a patch-sequence tensor.
...
/// Learnable 2D positional embeddings used to represent spatial position of patches.
...
impl PositionEmbedder {
    /// Creates a new `PositionEmbedder` loading positional weights from the var builder.
    pub fn new(
        hidden_size: usize,
        patch_size: usize,
        pos_embed_max_size: usize,
    ...
    /// Returns a cropped region of positional embeddings matching the given image resolution.
...
/// Embeds diffusion timesteps as conditioning vectors using sinusoidal frequency embeddings.
...
impl TimestepEmbedder {
    /// Creates a new `TimestepEmbedder` with the given hidden size and frequency embedding size.
    pub fn new(
        hidden_size: usize,
        frequency_embedding_size: usize,
        vb: nn::VarBuilder,
    ...
    fn timestep_embedding(t: &Tensor, dim: usize, max_period: f64) -> Result<Tensor> {
        ...
        embedding.to_dtype(fuel::DType::F16)
    }
}
...
/// Embeds conditioning vectors (e.g. CLIP pooled text embeddings) into the hidden dimension.
pub struct VectorEmbedder {
    mlp: nn::Sequential,
}

impl VectorEmbedder {
    /// Creates a new `VectorEmbedder` projecting `input_dim` to `hidden_size`.
