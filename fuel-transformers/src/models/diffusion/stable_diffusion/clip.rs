...
use fuel::{DType, Device, Result, Tensor, D};
use fuel_nn as nn;
use fuel_nn::Module;

/// Activation function variants used in CLIP MLP layers.
...
/// CLIP text encoder configuration controlling architecture and tokeniser behaviour.
#[derive(Debug, Clone)]
pub struct Config {
    vocab_size: usize,
...
impl Config {
    ...
    /// Returns the CLIP config for Stable Diffusion 1.5 (ViT-L/14, 768-dim, 12 layers).
    pub fn v1_5() -> Self {
        Self {
            vocab_size: 49408,
            embed_dim: 768,
    ...
    /// Returns the CLIP config for Stable Diffusion 2.1 (OpenCLIP ViT-H, 1024-dim, 23 layers).
    pub fn v2_1() -> Self {
        Self {
            vocab_size: 49408,
            embed_dim: 1024,
    ...
    /// Returns the CLIP config for SDXL text-encoder-1 (768-dim, 12 layers, QuickGelu).
    pub fn sdxl() -> Self {
        Self {
            vocab_size: 49408,
            embed_dim: 768,
    ...
    /// Returns the CLIP config for SDXL text-encoder-2 (1280-dim, 32 layers, Gelu).
    pub fn sdxl2() -> Self {
        Self {
            vocab_size: 49408,
            embed_dim: 1280,
    ...
    /// Returns the CLIP config for SSD-1B first text encoder (same as SDXL).
    pub fn ssd1b() -> Self {
        Self::sdxl()
    }

    /// Returns the CLIP config for SSD-1B second text encoder (same as SDXL-2).
    pub fn ssd1b2() -> Self {
        Self::sdxl2()
    }

    // https://huggingface.co/warp-ai/wuerstchen/blob/main/text_encoder/config.json
    /// Returns the CLIP config for the Würstchen text encoder (1024-dim, 24 layers, GeluErf).
    ...
    /// Returns the CLIP config for the Würstchen prior text encoder (1280-dim, 32 layers, GeluErf).
...
impl ClipTextEmbeddings {
    fn new(vs: fuel_nn::VarBuilder, c: &Config) -> Result<Self> {
        let token_embedding =
            fuel_nn::embedding(c.vocab_size, c.embed_dim, vs.pp("token_embedding"))?;
...
struct ClipAttention {
    k_proj: fuel_nn::Linear,
    v_proj: fuel_nn::Linear,
    q_proj: fuel_nn::Linear,
    out_proj: fuel_nn::Linear,
    head_dim: usize,
    scale: f64,
    num_attention_heads: usize,
}
...
impl ClipAttention {
    fn new(vs: fuel_nn::VarBuilder, c: &Config) -> Result<Self> {
        ...
        let k_proj = fuel_nn::linear(embed_dim, embed_dim, vs.pp("k_proj"))?;
        let v_proj = fuel_nn::linear(embed_dim, embed_dim, vs.pp("v_proj"))?;
        let q_proj = fuel_nn::linear(embed_dim, embed_dim, vs.pp("q_proj"))?;
        let out_proj = fuel_nn::linear(embed_dim, embed_dim, vs.pp("out_proj"))?;
    ...
    fn forward(&self, xs: &Tensor, causal_attention_mask: &Tensor) -> Result<Tensor> {
        ...
        let attn_weights = fuel_nn::ops::softmax(&attn_weights, D::Minus1)?;
...
impl ClipMlp {
    fn new(vs: fuel_nn::VarBuilder, c: &Config) -> Result<Self> {
        let fc1 = fuel_nn::linear(c.embed_dim, c.intermediate_size, vs.pp("fc1"))?;
        let fc2 = fuel_nn::linear(c.intermediate_size, c.embed_dim, vs.pp("fc2"))?;
        Ok(ClipMlp {
            fc1,
            fc2,
...
struct ClipEncoderLayer {
    ...
    layer_norm1: fuel_nn::LayerNorm,
    mlp: ClipMlp,
    layer_norm2: fuel_nn::LayerNorm,
}

impl ClipEncoderLayer {
    fn new(vs: fuel_nn::VarBuilder, c: &Config) -> Result<Self> {
        let self_attn = ClipAttention::new(vs.pp("self_attn"), c)?;
        let layer_norm1 = fuel_nn::layer_norm(c.embed_dim, 1e-5, vs.pp("layer_norm1"))?;
        let mlp = ClipMlp::new(vs.pp("mlp"), c)?;
        let layer_norm2 = fuel_nn::layer_norm(c.embed_dim, 1e-5, vs.pp("layer_norm2"))?;
        Ok(ClipEncoderLayer {
            self_attn,
            layer_norm1,
            mlp,
...
pub struct ClipTextTransformer {
    ...
    encoder: ClipEncoder,
    final_layer_norm: fuel_nn::LayerNorm,
}

impl ClipTextTransformer {
    /// Creates a new `ClipTextTransformer`, loading embeddings and encoder layers from weights.
    pub fn new(vs: fuel_nn::VarBuilder, c: &Config) -> Result<Self> {
        ...
        let final_layer_norm = fuel_nn::layer_norm(c.embed_dim, 1e-5, vs.pp("final_layer_norm"))?;
    ...
    /// Encodes token ids with causal masking applied after position `mask_after`.
    ...
    /// Returns `(final_output, hidden_state_at_layer)`, where `until_layer` may be negative to count from the end.
