...
/// Two-layer SiLU MLP for projecting raw sinusoidal timestep features to a diffusion time embedding.
#[derive(Debug)]
...
impl TimestepEmbedding {
    // act_fn: "silu"
    /// Creates a new `TimestepEmbedding` projecting `channel` features to `time_embed_dim`.
...
/// Sinusoidal position-encoding layer that converts integer timesteps to frequency embeddings.
#[derive(Debug)]
...
impl Timesteps {
    /// Creates a new `Timesteps` with the given channel count, flip flag, and frequency downscale offset.
...
impl Module for Timesteps {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let exponent = (Tensor::arange(0, half_dim, xs.device())?.to_dtype(fuel::DType::F32)?
            * -f64::ln(10000.))?;
