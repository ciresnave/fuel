...
/// Factory trait for diffusion noise schedulers; implement to create a scheduler from config.
pub trait SchedulerConfig: std::fmt::Debug + Send + Sync {
    /// Builds a scheduler ready for the specified number of inference steps.
...
/// The quantity predicted by the UNet: noise (`Epsilon`), velocity (`VPrediction`), or clean sample (`Sample`).
#[derive(Debug, Clone, Copy)]
...
pub(crate) fn betas_for_alpha_bar(num_diffusion_timesteps: usize, max_beta: f64) -> Result<Tensor> {
    ...
    Tensor::from_vec(betas, betas_len, &fuel::Device::Cpu)
}
