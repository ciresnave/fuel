...
//! See: <https://arxiv.org/abs/2302.04867>
//!
...
use fuel::{Error, IndexOp, Result, Tensor};

/// Sigma-to-timestep mapping strategy; either Karras or exponential spacing.
...
/// Exponential sigma schedule distributing sigmas on a log-linear scale.
#[derive(Debug, Clone, Copy)]
...
/// Solver variant for the UniPC predictor; `Bh1` and `Bh2` differ in how the prediction is formed.
...
/// Diffusion algorithm variant; `DpmSolverPlusPlus` is the standard deterministic solver, `SdeDpmSolverPlusPlus` adds stochasticity.
#[derive(Debug, Default, Clone, Copy)]
...
/// How to set the last sigma value in the schedule: clamp to zero or keep at `sigma_min`.
#[derive(Debug, Default, Clone, Copy)]
...
/// How to space the inference timesteps across the full training-timestep range.
#[derive(Debug, Clone)]
...
/// Controls whether the UniC corrector step is applied, and for which steps it is skipped.
#[derive(Debug, Clone)]
...
impl CorrectorConfiguration {
    /// Creates a new enabled corrector that skips the listed step indices.
...
/// Full configuration for the UniPC/EDM DPM multistep scheduler.
#[derive(Debug, Clone)]
...
/// EDM-style DPM multistep scheduler implementing the UniPC predictor-corrector algorithm.
...
impl EdmDpmMultistepScheduler {
    /// Creates a new `EdmDpmMultistepScheduler` computing the sigma/timestep schedule for `num_inference_steps`.
...
mod linalg {
    use fuel::{IndexOp, Result, Shape, Tensor};
