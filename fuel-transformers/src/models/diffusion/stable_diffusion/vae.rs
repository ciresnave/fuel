...
/// Configuration for a KL-regularised autoencoder (VAE).
#[derive(Debug, Clone)]
...
/// A diagonal Gaussian distribution parameterised by mean and log-variance tensors.
...
impl DiagonalGaussianDistribution {
    /// Creates a distribution by splitting `parameters` along dim-1 into mean and log-variance.
    ...
    /// Draws a sample from the distribution using the reparameterisation trick.
...
/// KL-regularised VAE encoder-decoder used in Stable Diffusion for latent-space compression.
#[derive(Debug)]
...
impl AutoEncoderKL {
    /// Creates a new `AutoEncoderKL` with the given input/output channel counts and VAE configuration.
    pub fn new(
        vs: nn::VarBuilder,
        in_channels: usize,
        out_channels: usize,
