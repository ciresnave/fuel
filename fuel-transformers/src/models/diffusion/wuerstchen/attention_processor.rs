...
/// Cross-attention module used within Wuerstchen transformer blocks.
#[derive(Debug)]
...
impl Attention {
    /// Creates a new cross-attention module with the given query dim, number of heads, and head dim.
    pub fn new(
        query_dim: usize,
        heads: usize,
        dim_head: usize,
    ...
    fn get_attention_scores(&self, query: &Tensor, key: &Tensor) -> Result<Tensor> {
        ...
        fuel_nn::ops::softmax_last_dim(&attn_probs)
    }

    /// Runs cross-attention over a spatial feature map conditioned on encoder hidden states.
