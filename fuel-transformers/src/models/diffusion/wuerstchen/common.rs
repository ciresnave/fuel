...
/// Channel-last layer normalization for NCHW tensors (no learnable affine parameters).
#[derive(Debug)]
...
impl WLayerNorm {
    /// Creates a new `WLayerNorm` with epsilon `1e-6`.
...
/// Layer normalization without learnable weights, operating on the last dimension.
#[derive(Debug)]
...
impl LayerNormNoWeights {
    /// Creates a new `LayerNormNoWeights` with epsilon `1e-6`.
    pub fn new(_size: usize) -> Result<Self> {
        Ok(Self { eps: 1e-6 })
    }
}
...
#[derive(Debug)]
/// FiLM conditioning block that applies scale and shift to activations based on a timestep embedding.
pub struct TimestepBlock {
    mapper: fuel_nn::Linear,
}

impl TimestepBlock {
    /// Creates a new `TimestepBlock` mapping `c_timestep`-dim timestep embeddings to scale+shift over `c` channels.
    pub fn new(c: usize, c_timestep: usize, vb: VarBuilder) -> Result<Self> {
        let mapper = fuel_nn::linear(c_timestep, c * 2, vb.pp("mapper"))?;
        Ok(Self { mapper })
    }

    /// Applies FiLM (feature-wise linear modulation) using the timestep embedding `t`.
...
#[derive(Debug)]
/// Global Response Normalization layer with learnable affine parameters.
...
impl GlobalResponseNorm {
    /// Creates a new `GlobalResponseNorm` loading `gamma` and `beta` tensors of shape `(1,1,1,dim)`.
...
#[derive(Debug)]
/// Residual block combining a depthwise convolution, layer norm, channelwise MLP, and GRN.
pub struct ResBlock {
    depthwise: fuel_nn::Conv2d,
    norm: WLayerNorm,
    channelwise_lin1: fuel_nn::Linear,
    channelwise_grn: GlobalResponseNorm,
    channelwise_lin2: fuel_nn::Linear,
}

impl ResBlock {
    /// Creates a new `ResBlock`. `c_skip` channels from an optional skip tensor are concatenated at input.
    pub fn new(c: usize, c_skip: usize, ksize: usize, vb: VarBuilder) -> Result<Self> {
        let cfg = fuel_nn::Conv2dConfig {
            padding: ksize / 2,
            groups: c,
            ..Default::default()
        };
        let depthwise = fuel_nn::conv2d(c + c_skip, c, ksize, cfg, vb.pp("depthwise"))?;
        let norm = WLayerNorm::new(c)?;
        let channelwise_lin1 = fuel_nn::linear(c, c * 4, vb.pp("channelwise.0"))?;
        let channelwise_grn = GlobalResponseNorm::new(c * 4, vb.pp("channelwise.2"))?;
        let channelwise_lin2 = fuel_nn::linear(c * 4, c, vb.pp("channelwise.4"))?;
        Ok(Self {
            depthwise,
            norm,
            channelwise_lin1,
    ...
    /// Applies the residual block; concatenates `x_skip` before depthwise conv when given.
...
#[derive(Debug)]
/// Self- or cross-attention block with optional KV conditioning for Wuerstchen.
pub struct AttnBlock {
    self_attn: bool,
    norm: WLayerNorm,
    attention: Attention,
    kv_mapper_lin: fuel_nn::Linear,
}

impl AttnBlock {
    /// Creates a new `AttnBlock`. When `self_attn` is true, spatial tokens are prepended to the KV.
    pub fn new(
        c: usize,
        c_cond: usize,
        nhead: usize,
        ...
        let kv_mapper_lin = fuel_nn::linear(c_cond, c, vb.pp("kv_mapper.1"))?;
        Ok(Self {
            self_attn,
            norm,
            attention,
    ...
    /// Forward pass; `kv` is the conditioning tensor (e.g. CLIP embeddings).
    pub fn forward(&self, xs: &Tensor, kv: &Tensor) -> Result<Tensor> {
        let kv = fuel_nn::ops::silu(kv)?.apply(&self.kv_mapper_lin)?;
