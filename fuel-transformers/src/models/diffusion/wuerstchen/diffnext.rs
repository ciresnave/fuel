...
/// Stage B residual block for `WDiffNeXt` with skip-connection concatenated after depthwise+norm.
#[derive(Debug)]
...
impl ResBlockStageB {
    /// Creates a new Stage B residual block; `c_skip` channels optionally concatenated after norm.
    pub fn new(c: usize, c_skip: usize, ksize: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let depthwise = fuel_nn::conv2d(c, c, ksize, cfg, vb.pp("depthwise"))?;
        let norm = WLayerNorm::new(c)?;
        let channelwise_lin1 = fuel_nn::linear(c + c_skip, c * 4, vb.pp("channelwise.0"))?;
    ...
    /// Forward pass; `x_skip` is concatenated after the depthwise+norm step when provided.
...
struct UpBlock {
    sub_blocks: Vec<SubBlock>,
    layer_norm: Option<WLayerNorm>,
    conv: Option<fuel_nn::ConvTranspose2d>,
}

/// Wuerstchen Stage B decoder U-Net conditioned on EfficientNet and CLIP embeddings.
#[derive(Debug)]
pub struct WDiffNeXt {
    clip_mapper: fuel_nn::Linear,
    effnet_mappers: Vec<Option<fuel_nn::Conv2d>>,
    seq_norm: LayerNormNoWeights,
    embedding_conv: fuel_nn::Conv2d,
    embedding_ln: WLayerNorm,
    down_blocks: Vec<DownBlock>,
    up_blocks: Vec<UpBlock>,
    clf_ln: WLayerNorm,
    clf_conv: fuel_nn::Conv2d,
    c_r: usize,
    patch_size: usize,
}
...
impl WDiffNeXt {
    ...
    /// Creates a new `WDiffNeXt` Stage B decoder with a fixed 4-level U-Net architecture.
    pub fn new(
        c_in: usize,
        c_out: usize,
        c_r: usize,
        ...
        let clip_mapper = fuel_nn::linear(clip_embd, c_cond, vb.pp("clip_mapper"))?;
        ...
        let embedding_conv = fuel_nn::conv2d(
        ...
        for (i, &c_hidden) in C_HIDDEN.iter().enumerate() {
            ...
            let (layer_norm, conv, start_layer_i) = if i > 0 {
                ...
                let conv = fuel_nn::conv2d(C_HIDDEN[i - 1], c_hidden, 2, cfg, vb.pp("0.1"))?;
        ...
        for (i, &c_hidden) in C_HIDDEN.iter().enumerate().rev() {
            ...
            let (layer_norm, conv) = if i > 0 {
                ...
                let conv = fuel_nn::conv_transpose2d(
                    c_hidden,
                    C_HIDDEN[i - 1],
                    2,
                    cfg,
    ...
    /// Denoises latents `xs` given noise ratio `r`, EfficientNet conditioning `effnet`, and optional CLIP text `clip`.
    pub fn forward(
        &self,
        xs: &Tensor,
        r: &Tensor,
        ...
        let mut xs = xs
            .apply(&|xs: &_| fuel_nn::ops::pixel_unshuffle(xs, self.patch_size))?
        ...
        let ab = xs
            .apply(&self.clf_ln)?
            .apply(&self.clf_conv)?
            .apply(&|xs: &_| fuel_nn::ops::pixel_shuffle(xs, self.patch_size))?
            .chunk(2, 1)?;
        let b = ((fuel_nn::ops::sigmoid(&ab[1])? * (1. - EPS * 2.))? + EPS)?;
        (x_in - &ab[0])? / b
    }
}
