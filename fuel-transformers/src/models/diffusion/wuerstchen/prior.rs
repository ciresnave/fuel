...
/// Wuerstchen prior model that denoises CLIP latents conditioned on text embeddings.
#[derive(Debug)]
pub struct WPrior {
    projection: fuel_nn::Conv2d,
    cond_mapper_lin1: fuel_nn::Linear,
    cond_mapper_lin2: fuel_nn::Linear,
    blocks: Vec<Block>,
    out_ln: super::common::WLayerNorm,
    out_conv: fuel_nn::Conv2d,
    c_r: usize,
}

impl WPrior {
    #[allow(clippy::too_many_arguments)]
    /// Creates a new `WPrior`. `c_r` is the sinusoidal noise-ratio embedding dimension.
    pub fn new(
        c_in: usize,
        c: usize,
        c_cond: usize,
        ...
        let projection = fuel_nn::conv2d(c_in, c, 1, Default::default(), vb.pp("projection"))?;
        let cond_mapper_lin1 = fuel_nn::linear(c_cond, c, vb.pp("cond_mapper.0"))?;
        let cond_mapper_lin2 = fuel_nn::linear(c, c, vb.pp("cond_mapper.2"))?;
        let out_ln = super::common::WLayerNorm::new(c)?;
        let out_conv = fuel_nn::conv2d(c, c_in * 2, 1, Default::default(), vb.pp("out.1"))?;
    ...
    /// Generates sinusoidal positional encodings from a scalar noise ratio `r` in `[0,1]`.
    ...
    /// Denoises latents `xs` conditioned on noise ratio `r` and CLIP embeddings `c`.
    pub fn forward(&self, xs: &Tensor, r: &Tensor, c: &Tensor) -> Result<Tensor> {
        ...
        let c_embed = c
            ...
            .apply(&|xs: &_| fuel_nn::ops::leaky_relu(xs, 0.2))?
