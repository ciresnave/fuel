...
use fuel_nn::{linear, linear_no_bias, VarBuilder};
...
fn flash_attn(_: &Tensor, _: &Tensor, _: &Tensor, _: f32, _: bool) -> Result<Tensor> {
    fuel::bail!("flash-attn feature not enabled, compile with '--features flash-attn'")
}
...
pub struct TimestepEmbedder {
    linear1: fuel_nn::Linear,
    linear2: fuel_nn::Linear,
    frequency_embedding_size: usize,
}

impl TimestepEmbedder {
    /// Creates a new `TimestepEmbedder` with sinusoidal frequency encoding followed by a two-layer MLP.
    ...
    /// Embeds scalar timesteps into conditioning vectors via sinusoidal encoding.
...
impl FeedForward {
    /// Creates a new `FeedForward` SwiGLU MLP with input dim `dim` and hidden dim `hidden_dim`.
...
impl QkNorm {
    /// Creates a new `QkNorm` loading `norm_q` and `norm_k` RMSNorm weights for the given head dimension.
    ...
    /// Applies per-head RMSNorm to query and key tensors of shape `(B, seq_len, n_heads, head_dim)`.
...
impl RopeEmbedder {
    /// Creates a new `RopeEmbedder` precomputing cosine/sine caches for each spatial axis.
    pub fn new(
        theta: f64,
        axes_dims: Vec<usize>,
        axes_lens: Vec<usize>,
...
impl ZImageAttention {
    /// Creates a new `ZImageAttention` with optional per-head QK normalisation.
    ...
    /// Computes multi-head attention with rotary position embeddings.
    pub fn forward(
        &self,
    ...
    fn attention_metal(
        ...
        fuel_nn::ops::sdpa(q, k, v, sdpa_mask.as_ref(), false, scale as f32, 1.0)
    }
...
pub struct ZImageTransformerBlock {
    ...
    ffn_norm1: RmsNorm,
    ffn_norm2: RmsNorm,
    adaln_modulation: Option<fuel_nn::Linear>,
}

impl ZImageTransformerBlock {
    /// Creates a new transformer block; set `modulation=true` for noise/main layers, `false` for context layers.
    ...
    /// Forward pass; `adaln_input` is required when the block was created with `modulation=true`.
    pub fn forward(
        &self,
        x: &Tensor,
...
impl LayerNormNoParams {
    /// Creates a new `LayerNormNoParams` with the given epsilon.
...
pub struct FinalLayer {
    ...
    linear: fuel_nn::Linear,
    adaln_silu: fuel_nn::Linear,
}

impl FinalLayer {
    /// Creates a new `FinalLayer` projecting `hidden_size` hidden dim to `out_channels` patch outputs.
    pub fn new(hidden_size: usize, out_channels: usize, vb: VarBuilder) -> Result<Self> {
        let norm_final = LayerNormNoParams::new(1e-6);
        let linear = fuel_nn::linear(hidden_size, out_channels, vb.pp("linear"))?;
        let adaln_dim = hidden_size.min(ADALN_EMBED_DIM);
        let adaln_silu =
            fuel_nn::linear(adaln_dim, hidden_size, vb.pp("adaLN_modulation").pp("1"))?;
    ...
    /// Applies AdaLN scale modulation followed by the output linear projection.
...
pub struct ZImageTransformer2DModel {
    ...
    cap_embedder_linear: fuel_nn::Linear,
    x_embedder: fuel_nn::Linear,
...
impl ZImageTransformer2DModel {
    /// Creates a new `ZImageTransformer2DModel` from config.
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
