...
use fuel::{bail, Context, DType, Device, Module, Result, Tensor, D};
use fuel_nn::{
...
/// Activation function variants used in DeBERTa-v2 feed-forward layers.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::debertav2::HiddenAct;
...
///
/// ```no_run
/// # let cfg: fuel_transformers::models::debertav2::Config = unimplemented!(); // Typically loaded from JSON
/// ```
...
/// Stable dropout (no-op at inference time).
///
/// # Example
///
/// ```
...
impl StableDropout {
    ...
    /// Pass the tensor through (no-op at inference time).
    ///
    /// # Example
    ///
    /// ```
...
/// DeBERTa-v2 embedding layer (word + position + token-type embeddings).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2Embeddings};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// let embeddings = DebertaV2Embeddings::load(vb, &cfg)?;
/// # Ok::<(), fuel::Error>(())
/// ```
pub struct DebertaV2Embeddings {
    ...
    embed_proj: Option<fuel_nn::Linear>,
}

impl DebertaV2Embeddings {
    /// Load from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let embed_proj: Option<fuel_nn::Linear> = if embedding_size != config.hidden_size {
            Some(fuel_nn::linear_no_bias(
                embedding_size,
                config.hidden_size,
                vb.pp("embed_proj"),
            )?)
    ...
    /// Compute the combined embeddings for the given inputs.
    ///
    /// # Example
    ///
    /// ```no_run
...
impl XSoftmax {
    pub fn apply(input: &Tensor, mask: &Tensor, dim: D, device: &Device) -> Result<Tensor> {
        // NOTE: At the time of this writing, fuel does not have a logical-not operator.
        ...
        output = fuel_nn::ops::softmax(&output, dim)?;
...
/// Disentangled self-attention layer (DeBERTa-v2 style).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2DisentangledSelfAttention};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
pub struct DebertaV2DisentangledSelfAttention {
    ...
    query_proj: fuel_nn::Linear,
    key_proj: fuel_nn::Linear,
    value_proj: fuel_nn::Linear,
    dropout: StableDropout,
    device: Device,
    ...
    pos_ebd_size: isize,
    share_att_key: bool,
    pos_key_proj: Option<fuel_nn::Linear>,
    pos_query_proj: Option<fuel_nn::Linear>,
}
...
impl DebertaV2DisentangledSelfAttention {
    ...
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let query_proj = fuel_nn::linear(config.hidden_size, all_head_size, vb.pp("query_proj"))?;
        let key_proj = fuel_nn::linear(config.hidden_size, all_head_size, vb.pp("key_proj"))?;
        let value_proj = fuel_nn::linear(config.hidden_size, all_head_size, vb.pp("value_proj"))?;
        ...
        let mut pos_key_proj: Option<fuel_nn::Linear> = None;
        let mut pos_query_proj: Option<fuel_nn::Linear> = None;
        ...
        if relative_attention {
            ...
            if !share_att_key {
                if config.pos_att_type.iter().any(|s| s == "c2p") {
                    pos_key_proj = Some(fuel_nn::linear(
                        config.hidden_size,
                        all_head_size,
                        vb.pp("pos_key_proj"),
                    )?);
                ...
                if config.pos_att_type.iter().any(|s| s == "p2c") {
                    pos_query_proj = Some(fuel_nn::linear(
                        config.hidden_size,
                        all_head_size,
...
/// Combined disentangled attention + output projection.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2Attention};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// let attn = DebertaV2Attention::load(vb, &cfg)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
impl DebertaV2Attention {
    /// Load a `DebertaV2Attention` from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// Post-attention residual norm layer.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2SelfOutput};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// let layer = DebertaV2SelfOutput::load(vb, &cfg)?;
/// # Ok::<(), fuel::Error>(())
/// ```
pub struct DebertaV2SelfOutput {
    dense: fuel_nn::Linear,
    layer_norm: LayerNorm,
    dropout: StableDropout,
}
...
impl DebertaV2SelfOutput {
    ...
    /// ```
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let dense = fuel_nn::linear(config.hidden_size, config.hidden_size, vb.pp("dense"))?;
        let layer_norm = fuel_nn::layer_norm(
            config.hidden_size,
            config.layer_norm_eps,
            vb.pp("LayerNorm"),
        )?;
    ...
    /// Apply dense projection + dropout + layer norm (with residual).
...
/// Feed-forward intermediate (expansion) layer.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2Intermediate};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
impl DebertaV2Intermediate {
    ...
    /// Apply the intermediate projection + activation.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// Feed-forward output (contraction + norm) layer.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2Output};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// let layer = DebertaV2Output::load(vb, &cfg)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
/// Single DeBERTa-v2 transformer layer (attention + FFN).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2Layer};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// let layer = DebertaV2Layer::load(vb, &cfg)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
/// Optional convolutional layer applied after the first transformer layer.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, ConvLayer};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// let layer = ConvLayer::load(vb, &cfg)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
impl ConvLayer {
    ...
    /// Apply the convolutional layer (not yet implemented).
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// # let cfg: Config = unimplemented!();
    /// let layer = ConvLayer::load(vb, &cfg)?;
    /// // Note: forward is currently unimplemented (todo!)
    /// # Ok::<(), fuel::Error>(())
    /// ```
    pub fn forward(
        &self,
...
/// Stack of DeBERTa-v2 transformer layers.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2Encoder};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
impl DebertaV2Encoder {
    ...
    /// Run the encoder stack forward pass.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// DeBERTa-v2 base encoder model (embeddings + transformer).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2Model};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
impl DebertaV2Model {
    ...
    /// Run DeBERTa-v2 embeddings + encoder forward pass.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// A single NER prediction item.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::debertav2::NERItem;
/// let item = NERItem { entity: "PER".into(), word: "Alice".into(), score: 0.99, start: 0, end: 5, index: 1 };
/// assert_eq!(item.entity, "PER");
/// ```
#[derive(Debug)]
pub struct NERItem {
    pub entity: String,
...
/// A single text-classification prediction item (label + score).
///
/// # Example
///
/// ```
/// use fuel_transformers::models::debertav2::TextClassificationItem;
/// let item = TextClassificationItem { label: "positive".into(), score: 0.95 };
/// assert_eq!(item.label, "positive");
/// ```
#[derive(Debug)]
...
/// DeBERTa-v2 model for token-level NER classification.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2NERModel};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
pub struct DebertaV2NERModel {
    ...
    dropout: fuel_nn::Dropout,
    classifier: fuel_nn::Linear,
}
...
impl DebertaV2NERModel {
    /// Load the NER model from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn load(vb: VarBuilder, config: &Config, id2label: Option<Id2Label>) -> Result<Self> {
        ...
        let dropout = fuel_nn::Dropout::new(config.hidden_dropout_prob as f32);
        let classifier: fuel_nn::Linear = fuel_nn::linear_no_bias(
            config.hidden_size,
            id2label_len,
...
/// DeBERTa-v2 model for sequence-level text classification.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2SeqClassificationModel};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
impl DebertaV2SeqClassificationModel {
    /// Load the sequence classification model from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn load(vb: VarBuilder, config: &Config, id2label: Option<Id2Label>) -> Result<Self> {
        ...
        let classifier = fuel_nn::linear(output_dim, id2label_len, vb.root().pp("classifier"))?;
    ...
    /// Run a sequence classification forward pass.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// DeBERTa-v2 context pooler (used by sequence classifiers).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::debertav2::{Config, DebertaV2ContextPooler};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
impl DebertaV2ContextPooler {
    ...
    /// Return the output dimension of the pooler.
    ///
    /// # Example
    ///
    /// ```no_run
...
pub(crate) fn make_log_bucket_position(
    ...
    let condition = lt_mid
        ...
        .mul(&gt_neg_mid.to_dtype(fuel::DType::F32)?)?
        .to_dtype(DType::U8)?;
