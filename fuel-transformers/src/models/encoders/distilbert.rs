...
use fuel_nn::{Embedding, Module, VarBuilder};
use serde::Deserialize;

pub const DTYPE: DType = DType::F32;

/// DistilBERT activations.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::distilbert::HiddenAct;
...
/// Position embedding variant (currently only absolute is supported).
///
/// # Example
///
/// ```
/// use fuel_transformers::models::distilbert::PositionEmbeddingType;
/// let t = PositionEmbeddingType::Absolute;
/// ```
...
///
/// ```
/// use fuel_transformers::models::distilbert::Config;
...
impl Embeddings {
    fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        let word_embeddings =
            fuel_nn::embedding(config.vocab_size, config.dim, vb.pp("word_embeddings"))?;
        let position_embeddings = fuel_nn::embedding(
...
impl MultiHeadSelfAttention {
    fn forward(&self, hidden_states: &Tensor, attention_mask: &Tensor) -> Result<Tensor> {
        ...
        let weights = fuel_nn::ops::softmax(&scores, fuel::D::Minus1)?;
...
/// DistilBERT base model (embeddings + transformer encoder).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::distilbert::{Config, DistilBertModel};
...
impl DistilBertModel {
    /// Load a `DistilBertModel` from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run a forward pass and return the sequence of hidden states.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// LM prediction head for DistilBERT (transform + tied decoder projection).
...
///
/// ```no_run
/// # use fuel_transformers::models::distilbert::{Config, DistilBertLMPredictionHead};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
/// let head = DistilBertLMPredictionHead::load(vb, &cfg)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
impl DistilBertLMPredictionHead {
    /// Load the LM prediction head from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        let init_bs = fuel_nn::Init::Uniform {
            lo: -bound,
            up: bound,
        };
...
/// MLM head wrapping [`DistilBertLMPredictionHead`].
...
///
/// ```no_run
/// # use fuel_transformers::models::distilbert::{Config, DistilBertOnlyMLMHead};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
/// let head = DistilBertOnlyMLMHead::load(vb, &cfg)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
impl DistilBertOnlyMLMHead {
    /// Load the MLM head from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// Full DistilBERT model for masked language modelling.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::distilbert::{Config, DistilBertForMaskedLM};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::default();
...
impl DistilBertForMaskedLM {
    /// Load the full masked LM model from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run masked-LM forward pass and return per-token logits.
    ///
    /// # Example
    ///
    /// ```no_run
