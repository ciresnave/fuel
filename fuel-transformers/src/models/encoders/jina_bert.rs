...
use fuel_nn::{layer_norm, LayerNorm, Module, VarBuilder};
use serde::Deserialize;

pub const DTYPE: DType = DType::F32;

/// Position embedding strategy used by Jina-BERT.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::jina_bert::PositionEmbeddingType;
/// let t = PositionEmbeddingType::Alibi;
/// ```
...
/// Configuration for Jina-BERT encoder models.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::jina_bert::Config;
/// let cfg = Config::v2_base();
...
impl Config {
    /// Return a default `jina-embeddings-v2-base-en` configuration.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// Construct a config from individual parameters.
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use fuel_transformers::models::jina_bert::{Config, PositionEmbeddingType};
    /// let cfg = Config::new(
    ///     30528, 768, 12, 12, 3072,
    ///     fuel_nn::Activation::Gelu, 8192, 2, 0.02, 1e-12, 0,
...
impl BertSelfAttention {
    ...
    fn forward(&self, xs: &Tensor, bias: &Tensor) -> Result<Tensor> {
        ...
        let attention_probs = {
            ...
            fuel_nn::ops::softmax_last_dim(&attention_scores)?
        };
...
impl BertGLUMLP {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        ...
        let act = fuel_nn::Activation::Gelu; // geglu
...
impl BertEncoder {
    fn new(vb: VarBuilder, cfg: &Config) -> Result<Self> {
        if cfg.position_embedding_type != PositionEmbeddingType::Alibi {
            fuel::bail!("only alibi is supported as a position-embedding-type")
        }
...
/// Jina-BERT base encoder model.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::jina_bert::{BertModel, Config};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::v2_base();
/// let model = BertModel::new(vb, &cfg)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Clone, Debug)]
...
impl BertModel {
    /// Load a `BertModel` from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
