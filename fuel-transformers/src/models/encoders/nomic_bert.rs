...
///
/// Fields match the `nomic-ai/nomic-embed-text-v1.5` `config.json`.
/// The [`Default`] implementation returns the standard NomicBERT-Base values.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::nomic_bert::Config;
...
impl RotaryEmbedding {
    ...
    fn apply(&self, x: &Tensor) -> Result<Tensor> {
        ...
        if self.interleaved {
            fuel_nn::rotary_emb::rope_i(x, &cos, &sin)
        } else {
...
impl NomicBertAttention {
    ...
    fn forward(
        ...
        let attn_probs = fuel_nn::ops::softmax_last_dim(&attn_scores)?;
...
impl NomicBertModel {
    /// Load a `NomicBertModel` from a variable store.
    ///
    /// Tries loading with no prefix first, then falls back to a `model_type`
    /// prefix (e.g. `"nomic_bert"`) if the direct load fails.
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use fuel_transformers::models::nomic_bert::{Config, NomicBertModel};
    ...
    /// Run a forward pass and return token-level hidden states.
    ///
    /// Returns a tensor of shape `(batch, seq_len, n_embd)`.
    /// Pass `attention_mask` with shape `(batch, seq_len)` (1 = attend, 0 = pad);
    /// if omitted, all tokens are attended to.
    ///
    /// # Example
    ///
    /// ```no_run
