...
/// NV-Embed v2 decoder-only embedding model (LLM backbone without LM head).
#[derive(Debug, Clone)]
pub struct Model {
...
impl Model {
    /// Load the embedding backbone from a `VarBuilder` and config.
    ...
    /// Run the backbone forward pass with an attention mask; returns hidden states.
    pub fn forward(
        &mut self,
        attn_mask: &Tensor,
        input_ids: &Tensor,
