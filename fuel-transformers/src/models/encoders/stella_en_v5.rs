...
use fuel::{DType, Device, Error, IndexOp, Module, Result, Tensor, D};
use fuel_nn::{layer_norm, Activation, LayerNorm, VarBuilder};
use std::sync::Arc;

/// Identifies which size variant of the Stella EN v5 model to use.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::stella_en_v5::ModelVariant;
/// let variant = ModelVariant::Large;
/// assert_eq!(variant, ModelVariant::default());
/// ```
...
/// Configuration for the Stella EN v5 embedding model.
///
/// Based on the Qwen2 architecture with the output causal LM head replaced by a learned embed head.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::stella_en_v5::{Config, EmbedDim};
/// let cfg = Config::new_1_5_b_v5(EmbedDim::Dim1024);
...
/// The configuration for a Stella embedding projection head (input and output feature dimensions).
///
/// # Example
///
/// ```
/// use fuel_transformers::models::stella_en_v5::EmbedHead;
/// let head = EmbedHead { in_features: 1536, out_features: 1024 };
/// assert_eq!(head.out_features, 1024);
/// ```
...
///
/// ```
/// use fuel_transformers::models::stella_en_v5::{EmbedDim, EmbedHead};
/// let dim = EmbedDim::Dim1024;
/// let head = dim.config(1536);
/// assert_eq!(head.out_features, 1024);
/// ```
...
impl EmbedDim {
    /// Return an `EmbedHead` config for this dimension given the input feature size.
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::stella_en_v5::EmbedDim;
    /// let head = EmbedDim::Dim1024.config(1536);
    /// assert_eq!(head.out_features, 1024);
    /// assert_eq!(head.in_features, 1536);
    /// ```
...
impl Config {
    ...
    pub fn new_1_5_b_v5(embed_dim: EmbedDim) -> Self {
        ...
        Self {
            ...
            activation_fn: fuel_nn::Activation::Silu,
            vocab_size: 151646,
            hidden_size: 1536,
    ...
    /// ```
    /// use fuel_transformers::models::stella_en_v5::{Config, EmbedDim};
    /// let cfg = Config::new_400_m_v5(EmbedDim::Dim1024);
...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb_qkv(&self, q: &Tensor, k: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let q_embed = fuel_nn::rotary_emb::rope(&q.contiguous()?, &cos, &sin)?;
        let k_embed = fuel_nn::rotary_emb::rope(&k.contiguous()?, &cos, &sin)?;
        Ok((q_embed, k_embed))
    }
}
...
impl Attention {
    ...
    fn forward(&mut self, xs: &Tensor, attention_mask: Option<&Tensor>) -> Result<Tensor> {
        ...
        let (key_states, value_states) = if self.variant == ModelVariant::Large {
            (
                crate::utils::repeat_kv(key_states, self.num_kv_groups)?,
                crate::utils::repeat_kv(value_states, self.num_kv_groups)?,
            )
        } else {
        ...
        match self.variant {
            ModelVariant::Large => {
                    ...
                    (&self.post_attention_layernorm, &self.layernorm)
                {
                    ...
                    return Err(fuel::error::Error::Msg(
...
/// Input embeddings for the Stella EN v5 model, supporting both the 1.5B and 400M variants.
///
/// # Example
///
/// ```no_run
/// use fuel_transformers::models::stella_en_v5::{Config, EmbedDim, Embeddings};
/// # let vb: fuel_nn::VarBuilder = unimplemented!();
/// let cfg = Config::new_1_5_b_v5(EmbedDim::Dim1024);
/// let embeddings = Embeddings::new(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
pub struct Embeddings {
    ...
    embeddings: fuel_nn::Embedding,
    // following are specifically for 400M
    token_type_embeddings: Option<fuel_nn::Embedding>,
    layer_norm: Option<LayerNorm>,
    position_ids: Option<Tensor>,
}

impl Embeddings {
    /// Create a new `Embeddings` instance from config and variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let (embeddings, token_type_embeddings, layer_norm, position_ids) = match cfg.variant {
            ModelVariant::Large => (
                fuel_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("embed_tokens"))?,
                None,
                None,
                None,
            ),
            ModelVariant::Small => {
                ...
                let weight = vb.pp("LayerNorm").get_with_hints(
                    cfg.hidden_size,
                    "weight",
                    fuel_nn::Init::Const(1.0),
                )?;
                let bias = vb.pp("LayerNorm").get_with_hints(
                    cfg.hidden_size,
                    "bias",
                    fuel_nn::Init::Const(0.0),
                )?;
                let dev = bias.device().clone();

                let layer_norm = fuel_nn::LayerNorm::new(weight, bias, cfg.norm_eps);
...
/// The Stella EN v5 transformer backbone model.
///
/// # Example
///
/// ```no_run
/// use fuel_transformers::models::stella_en_v5::{Config, EmbedDim, Model};
/// # let vb: fuel_nn::VarBuilder = unimplemented!();
/// let cfg = Config::new_1_5_b_v5(EmbedDim::Dim1024);
...
impl Model {
    /// Create a new Stella EN v5 backbone model.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run the encoder forward pass and return hidden states.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// # let mask: fuel::Tensor = unimplemented!();
...
/// Full Stella EN v5 embedding model combining the backbone with a learned projection head.
///
/// # Example
///
/// ```no_run
/// use fuel_transformers::models::stella_en_v5::{Config, EmbedDim, EmbeddingModel};
/// # let base_vb: fuel_nn::VarBuilder = unimplemented!();
/// # let embed_vb: fuel_nn::VarBuilder = unimplemented!();
/// let cfg = Config::new_1_5_b_v5(EmbedDim::Dim1024);
...
impl EmbeddingModel {
    /// Create a new `EmbeddingModel` from config, backbone variable store, and projection head variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run the full forward pass: encode, mean-pool, project, and return embeddings.
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use fuel_transformers::models::stella_en_v5::EmbeddingModel;
    /// # let mut model: EmbeddingModel = unimplemented!();
    /// # let input_ids: fuel::Tensor = unimplemented!();
    /// # let mask: fuel::Tensor = unimplemented!();
    ...
    /// # let input_ids: fuel::Tensor = unimplemented!();
    /// # let mask: fuel::Tensor = unimplemented!();
    /// let embeddings = model.forward_norm(&input_ids, &mask)?;
    /// # Ok::<(), fuel::Error>(())
    /// ```
