...
use fuel::{DType, Module, Result, Tensor};
use fuel_nn::{
    embedding, layer_norm, ops::softmax_last_dim, Activation, Embedding, LayerNorm, VarBuilder,
};

/// Configuration for an XLM-RoBERTa model.
///
/// # Example
///
/// ```no_run
/// # let cfg: fuel_transformers::models::xlm_roberta::Config = unimplemented!(); // Typically loaded from JSON
/// ```
...
impl XLMRobertaSelfOutput {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let layernorm =
            fuel_nn::layer_norm(cfg.hidden_size, cfg.layer_norm_eps, vb.pp("LayerNorm"))?;
...
/// Base XLM-RoBERTa encoder returning contextualized hidden states.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::xlm_roberta::{Config, XLMRobertaModel};
...
impl XLMRobertaModel {
    /// Load an XLM-RoBERTa model from config and variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run the encoder forward pass and return the final hidden states.
    ///
    /// # Example
    ///
    /// ```no_run
...
impl XLMRobertaLMHead {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let dense = linear(cfg.hidden_size, cfg.hidden_size, vb.pp("dense"))?;
        let layer_norm =
            fuel_nn::layer_norm(cfg.hidden_size, cfg.layer_norm_eps, vb.pp("layer_norm"))?;
        Ok(Self { dense, layer_norm })
    }

    fn forward(&self, hidden_states: &Tensor, shared_embeddings: &Tensor) -> Result<Tensor> {
        let hidden_states = self.dense.forward(hidden_states)?;
        let hidden_states = fuel_nn::Activation::Gelu.forward(&hidden_states)?;
...
/// XLM-RoBERTa model with a masked language modeling head.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::xlm_roberta::{Config, XLMRobertaForMaskedLM};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
impl XLMRobertaForMaskedLM {
    /// Load an XLM-RoBERTa MLM model from config and variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run the forward pass and return masked LM logits over the vocabulary.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// XLM-RoBERTa model with a sequence classification head.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::xlm_roberta::{Config, XLMRobertaForSequenceClassification};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
impl XLMRobertaForSequenceClassification {
    /// Load an XLM-RoBERTa sequence classification model from config and variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run the forward pass and return per-label classification logits.
    ///
    /// # Example
    ///
    /// ```no_run
