...
//!      [`crate::models::lazy_glm4::apply_interleaved_partial_rope`]
...
impl ChatGlmModel {
    ...
    fn apply_lm_head(&self, h_post: &Tensor) -> Result<Tensor> {
        ...
        self.weights
            .output_layer
            .apply_linear(h_post, cfg.hidden_size, cfg.padded_vocab_size)
    }
    ...
    fn run_backbone_embeds(&self, embeds: &Tensor, start_pos: usize) -> Result<Tensor> {
        ...
        if dims.len() != 3 || dims[0] != 1 || dims[2] != cfg.hidden_size {
            return Err(fuel_core::Error::Msg(format!(
                "ChatGlmModel::forward_embeds: expected embeds shape (1, seq, hidden_size={}), got {:?}",
        ...
        if !cfg
            .num_attention_heads
            .is_multiple_of(cfg.multi_query_group_num)
        {
            return Err(fuel_core::Error::Msg(format!(
    ...
    fn apply_mlp(&self, x: &Tensor, layer: &ChatGlmLayerWeights) -> Result<Tensor> {
        let cfg = &self.config;
        let h = cfg.hidden_size;
