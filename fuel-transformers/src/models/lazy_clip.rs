...
impl ClipVisionModel {
    ...
    pub fn forward(&self, pixel_values: &Tensor) -> Result<Tensor> {
        ...
        pixel_values
            .shape()
            .dims4()
            .map_err(|e| e.context("ClipVisionModel::forward: pixel_values"))?;
        let batch = dims[0];
        assert_eq!(batch, 1, "v1 supports batch == 1");
        if dims[2] != cfg.image_size || dims[3] != cfg.image_size {
            return Err(fuel_core::Error::Msg(format!(
                "clip vision: input spatial dims ({}, {}) must equal image_size {}; this model is \
    ...
    pub fn forward_intermediate_layers(
        ...
        pixel_values
            .shape()
            .dims4()
            .map_err(|e| e.context("ClipVisionModel::forward_intermediate_layers: pixel_values"))?;
        let batch = dims[0];
        assert_eq!(batch, 1, "v1 supports batch == 1");
        if dims[2] != cfg.image_size || dims[3] != cfg.image_size {
            return Err(fuel_core::Error::Msg(format!(
