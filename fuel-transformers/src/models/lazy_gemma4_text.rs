...
//!
//! The text decoder extends [`crate::models::lazy_gemma3`] with several
...
pub use crate::models::lazy_gemma::GemmaActivation as Gemma4Activation;
...
impl Gemma4TextModel {
    ...
    /// concatenation of vision/audio-projected embeddings + text
    /// token embeddings).
    ///
    ...
    /// retrieval / embedding consumers that project hidden states
    ...
    /// the multimodal composition to obtain text-side embeddings that
    /// will be spliced with vision/audio features before
    /// [`Self::forward_embeds`] is called.
    ///
    /// Returns shape `(1, seq, hidden_size)`. The caller is responsible
    /// for the `sqrt(hidden_size)` scaling (matches PaliGemma/LLaVA
    /// convention so the composition owns the splice ordering).
    ///
    ...
    fn decode_from_scaled_embeds(
        ...
        if dims.len() != 3 || dims[0] != 1 || dims[2] != cfg.hidden_size {
            return Err(fuel_core::Error::Msg(format!(
                "Gemma4TextModel::forward_embeds: expected scaled_embeds shape \
        ...
        if seq == 0 {
            return Err(fuel_core::Error::Msg(
                "Gemma4TextModel::forward_embeds: seq must be > 0".into(),
            )
            .bt());
        }
        if cfg.num_hidden_layers == 0 {
            return Err(fuel_core::Error::Msg(
                "Gemma4TextModel::forward_embeds: num_hidden_layers must be > 0".into(),
            )
            .bt());
        }
        if cfg.layer_types.len() != cfg.num_hidden_layers {
            return Err(fuel_core::Error::Msg(format!(
                "Gemma4TextModel::forward_embeds: layer_types length ({}) \
                 must match num_hidden_layers ({})",
                cfg.layer_types.len(),
        ...
        let (rope_cos_g, rope_sin_g) =
            h.rope_tables_const(cfg.rope_theta, start_pos, seq, rope_dim_global);
...
impl Gemma4TextWeights {
    /// Load Gemma 4 text-decoder weights from a `MmapedSafetensors`
    /// file using the standard HuggingFace naming. Gemma 4 ties
    /// `lm_head.weight` to `model.embed_tokens.weight` — there is no
    /// separate output projection field on `Gemma4TextWeights` (the
    /// `apply_lm_head` path reuses the embedding table directly).
    ///
    /// Tensor names mirrored from `fuel_transformers::models::llm::gemma4::text`:
    ///   - `model.embed_tokens.weight` → [`Gemma4TextWeights::token_embedding`]
    ...
    ///
    /// Per-layer Q/K/V/O dims are layer-type dependent: sliding layers
    /// use `cfg.head_dim` + `cfg.num_key_value_heads`; full ("global")
    ...
    pub fn load_from_mmapped(
        ...
        if cfg.layer_types.len() != cfg.num_hidden_layers {
            fuel_core::bail!(
                "Gemma4TextWeights::load_from_mmapped: layer_types length ({}) \
                 must match num_hidden_layers ({})",
                cfg.layer_types.len(),
                cfg.num_hidden_layers,
            );
        ...
        let mut layers: Vec<Gemma4LayerWeights> = Vec::with_capacity(cfg.num_hidden_layers);
        for li in 0..cfg.num_hidden_layers {
            let kind = cfg.layer_type(li);
...
mod tests {
    ...
    fn per_layer_head_dim_alters_output() {
        ...
        let m_a = Gemma4TextModel {
            config: cfg_a,
            weights: weights_a,
        };
        let m_b = Gemma4TextModel {
            config: cfg_b,
            weights: weights_b,
        };
        let toks = [1_u32, 2, 3];
    ...
    fn final_softcap_changes_output() {
        let cfg_no = Gemma4TextConfig {
            final_logit_softcapping: None,
            ..tiny_config()
        };
        let cfg_yes = Gemma4TextConfig {
            final_logit_softcapping: Some(5.0),
            ..tiny_config()
        };
        let weights = tiny_weights(&cfg_no);
        let m_no = Gemma4TextModel {
            config: cfg_no,
            weights: weights.clone(),
        };
        let m_yes = Gemma4TextModel {
            config: cfg_yes,
            weights,
        };
        let toks = [3_u32, 7, 11];
    ...
    fn forward_embeds_matches_forward_after_token_lookup() {
        // forward_embeds(scaled embed_tokens(tokens), start_pos) must
        // produce the same logits as forward(tokens, start_pos) when the
        // caller does the sqrt scaling externally — this is the contract
        // PaliGemma / LLaVA / Voxtral all rely on.
        let cfg = tiny_config();
        let model = Gemma4TextModel {
            config: cfg.clone(),
            weights: tiny_weights(&cfg),
        ...
        assert!(
            max_diff < 1e-5,
            "forward vs forward_embeds (post-scale) must agree (max diff {max_diff})"
        );
    }
    ...
    #[test]
    fn forward_hidden_embeds_returns_pre_lm_head_states() {
        // forward_hidden_embeds should produce the same hidden states as
        // forward_hidden(tokens) when fed the post-scale embeddings.
        let cfg = tiny_config();
        let model = Gemma4TextModel {
            config: cfg.clone(),
            weights: tiny_weights(&cfg),
        ...
        assert_eq!(h_ref.len(), h_via_embeds.len());
        let max_diff = h_ref
            .iter()
        ...
        assert!(
            max_diff < 1e-5,
            "forward_hidden vs forward_hidden_embeds (post-scale) must agree (max diff {max_diff})"
        );
    }
}
