...
//! no MoE, no QK-norm.
//!
//! Reuses `fuel_core::lazy::LayerWeights`. Forward is essentially
...
impl HeliumModel {
    ...
    /// over pre-embedded inputs. Helium does NOT scale embeddings.
    ...
    fn run_backbone_embeds(&self, embeds: &Tensor, start_pos: usize) -> Result<Tensor> {
        ...
        if dims.len() != 3 || dims[0] != 1 || dims[2] != cfg.hidden_size {
            return Err(fuel_core::Error::Msg(format!(
                "HeliumModel::forward_embeds: expected embeds shape (1, seq, hidden_size={}), got {:?}",
        ...
        if seq == 0 {
            return Err(fuel_core::Error::Msg(
                "HeliumModel::forward_embeds: seq must be > 0".into(),
            )
            .bt());
        }
        if cfg.num_attention_heads * cfg.head_dim != cfg.hidden_size {
            return Err(fuel_core::Error::Msg(
                "HeliumConfig: num_attention_heads * head_dim must equal hidden_size".into(),
            )
            .bt());
        }
...
mod tests {
    ...
    fn forward_embeds_test_cfg() -> HeliumConfig {
        HeliumConfig {
            vocab_size: 32,
            hidden_size: 16,
            intermediate_size: 32,
    ...
    fn forward_embeds_matches_forward_after_token_lookup() {
        ...
        assert!(
            max_diff < 1e-5,
            "Helium forward vs forward_embeds must agree (max diff {max_diff})"
        );
    }
    ...
    fn forward_hidden_embeds_matches_forward_hidden() {
        ...
        assert!(
            max_diff < 1e-5,
            "Helium forward_hidden vs forward_hidden_embeds must agree (max diff {max_diff})"
        );
    }
}
