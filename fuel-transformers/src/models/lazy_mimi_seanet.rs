...
//! taps (via [`crate::models::lazy_dac::expand_conv1d_weight_for_dilation_if_needed`])
...
use crate::models::lazy_encodec::{PadMode, pad1d};
...
impl SeaNetEncoderWeights {
    ...
    pub fn load_from_mmapped(
        ...
        let init_conv = load_mimi_conv1d(
            st,
            &format!("{layers_pp}.{layer_idx}.conv"),
            cfg.channels,
            cfg.n_filters,
            cfg.kernel_size,
            1,
        ...
        for ratio in cfg.ratios.iter().rev() {
            ...
            let downsample = load_mimi_conv1d(
                st,
                &format!("{layers_pp}.{}.conv", layer_idx + 1),
                dim,
                dim * 2,
                ratio * 2,
                *ratio,
