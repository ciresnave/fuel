...
impl MptModel {
    ...
    fn run_backbone_embeds(&self, embeds: &Tensor) -> Result<Tensor> {
        ...
        if seq == 0 {
            return Err(
                fuel_core::Error::Msg("MptModel::forward_embeds: seq must be > 0".into()).bt(),
            );
        }
    ...
    fn apply_layer(&self, x: &Tensor, layer: &MptLayerWeights, mask: &Tensor) -> Result<Tensor> {
        ...
        let q = layer
            .attn_q
            .apply_linear(&x_norm, cfg.d_model, cfg.d_model)?;
        ...
        let attn_out = layer
            .attn_o
            .apply_linear(&merged, cfg.d_model, cfg.d_model)?;
...
impl MptWeights {
    ...
    pub fn load_from_mmapped(
        ...
        for i in 0..cfg.n_layers {
            ...
            let norm1_bias = load_tensor_as_f32(st, &format!("{p}.norm_1.bias"))
                .ok()
                .map(Arc::from)
                .unwrap_or_else(|| Arc::from(vec![0.0; d]));
            let norm2_gain = Arc::from(load_tensor_as_f32(st, &format!("{p}.norm_2.weight"))?);
            let norm2_bias = load_tensor_as_f32(st, &format!("{p}.norm_2.bias"))
                .ok()
                .map(Arc::from)
                .unwrap_or_else(|| Arc::from(vec![0.0; d]));
            ...
            let attn_o = fuel_core::lazy::load_transposed_matrix_preserve_dtype(
                st,
                &format!("{p}.attn.out_proj.weight"),
                d,
                d,
            )?;
            let mlp_up = fuel_core::lazy::load_transposed_matrix_preserve_dtype(
                st,
                &format!("{p}.ffn.up_proj.weight"),
                ffn,
                d,
            )?;
            let mlp_down = fuel_core::lazy::load_transposed_matrix_preserve_dtype(
                st,
        ...
        let output = match fuel_core::lazy::load_transposed_matrix_preserve_dtype(
            st,
            "lm_head.weight",
            cfg.vocab_size,
            d,
...
mod tests {
    ...
    fn tiny_weights(cfg: &MptConfig) -> MptWeights {
        ...
        let layers: Vec<MptLayerWeights> = (0..cfg.n_layers)
            .map(|_| MptLayerWeights {
    ...
    fn alibi_slopes_power_of_two_heads() {
        ...
        for (i, expected) in [0.25_f32, 0.0625, 0.015_625, 0.003_906_25]
            .iter()
            .enumerate()
        {
            assert!(
                (slopes[i] - *expected).abs() < 1e-6,
                "slopes[{i}] = {} vs {expected}",
                slopes[i]
            );
        }
    }
    ...
    fn alibi_mask_differs_from_zero_lower_triangle() {
        ...
        let any_diff = causal_mask
            .iter()
            .zip(alibi_mask.iter())
            .any(|(&a, &b)| (a - b).abs() > 1e-7 && a.is_finite() && b.is_finite());
        assert!(
            any_diff,
            "ALiBi mask must differ from zero-slope causal mask on j < i positions"
        );
    }
