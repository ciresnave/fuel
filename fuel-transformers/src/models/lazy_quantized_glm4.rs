...
//! identical to [`crate::models::lazy_glm4::Glm4Model`]; only the weight storage
...
use crate::models::lazy_glm4::{Glm4Config, Glm4LayerWeights, Glm4Model, Glm4Weights};
#[cfg(test)]
...
impl QuantizedGlm4Model {
    ...
    /// (attn_q/k/v, ffn_gate_up, lm_head), `cfg.intermediate_size`
    /// (ffn_down), and `cfg.num_attention_heads * cfg.head_dim`
    /// (attn_o). Out-features carry no divisibility requirement —
    /// neither `num_key_value_heads * head_dim` (attn_k / attn_v
    /// outputs) nor the fused `ffn_gate_up`'s
    /// `2 * intermediate_size`. Source weights follow the same
    ...
    pub fn from_f32_bake(cfg: Glm4Config, src: Glm4Weights) -> Result<Self> {
        ...
        check_q4_0_divisible(
            "hidden_size (attn_q/k/v, ffn_gate_up, lm_head in-features)",
            h,
        )?;
        ...
        let quantize_linear =
            |w: &WeightStorage, in_features: usize, out_features: usize| -> Result<WeightStorage> {
                ...
                _ => return Err(fuel_core::Error::Msg(
                    "QuantizedGlm4Model::from_f32_bake: source weights must be WeightStorage::F32"
                        .into(),
                )
                .bt()),
            };
        ...
        for (idx, layer) in src.layers.into_iter().enumerate() {
            ...
            let ffn_gate_up = quantize_linear(&layer.ffn_gate_up, h, 2 * inter)
                .map_err(|e| layer_err(idx, "ffn_gate_up", e))?;
            let ffn_down = quantize_linear(&layer.ffn_down, inter, h)
        ...
        let lm_head = match src.lm_head {
            Some(w) => Some(
                quantize_linear(&w, h, cfg.vocab_size)
                    .map_err(|e| fuel_core::Error::Msg(format!("lm_head: {e}")).bt())?,
            ),
    ...
    pub fn from_gguf<P: AsRef<std::path::Path>>(path: P, cfg: &Glm4Config) -> Result<Self> {
        ...
        for idx in 0..cfg.num_hidden_layers {
            ...
            let post_self_attn_norm_gain: Arc<[f32]> =
            ...
            let post_mlp_norm_gain: Arc<[f32]> =
...
fn layer_err(idx: usize, name: &str, e: fuel_core::Error) -> fuel_core::Error {
    fuel_core::Error::Msg(format!("layer {idx} {name}: {e}")).bt()
}
...
mod tests {
    use super::*;
    use crate::models::lazy_glm4::Glm4Activation;
    ...
    fn forward_shape_finite_with_q4_0_weights() {
        ...
        assert!(matches!(
            model.inner().weights.lm_head.as_ref().unwrap(),
            WeightStorage::Q4_0 { .. }
        ));
