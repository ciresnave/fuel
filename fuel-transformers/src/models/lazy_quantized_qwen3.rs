...
//! identical to [`crate::models::lazy_qwen3::Qwen3Model`]; only the weight
...
use crate::models::lazy_qwen3::{Qwen3Config, Qwen3LayerExtras, Qwen3Model, Qwen3Weights};
#[cfg(test)]
...
impl QuantizedQwen3Model {
    ...
    /// `Qwen3Model` is a leaf (`config` + `weights`). Recorded because the sibling
    ...
    /// row-major layout as [`Qwen3Weights`].
    pub fn from_f32_bake(cfg: Qwen3Config, src: Qwen3Weights) -> Result<Self> {
        ...
        let quantize_linear =
            |w: &WeightStorage, in_features: usize, out_features: usize| -> Result<WeightStorage> {
                ...
                _ => return Err(fuel_core::Error::Msg(
                    "QuantizedQwen3Model::from_f32_bake: source weights must be WeightStorage::F32"
                        .into(),
                )
                .bt()),
            };
        ...
        let output = quantize_linear(&src.output, h, cfg.vocab_size)
            .map_err(|e| fuel_core::Error::Msg(format!("output: {e}")).bt())?;
    ...
    let vocab_size = match md.get("qwen3.vocab_size") {
        ...
        None => {
            let info = content
                ...
                .ok_or_else(|| {
                    fuel_core::Error::Msg("gguf: missing token_embd.weight".into()).bt()
                })?;
...
mod tests {
    ...
    /// so this overrides it to a MIXED window: a quantized decode test on the
    /// dense config would exercise the delegation without ever touching the
    /// N=2 stacked-mask path. Three decode steps, so the assertions land on the
    /// per-token REBIND path rather than the held-graph build.
    #[test]
    fn quantized_qwen3_windowed_decode_matches_quantized_forward() {
        use fuel_core::Device;
        ...
        let model = QuantizedQwen3Model::from_f32_bake(cfg.clone(), tiny_weights(&cfg)).unwrap();
        ...
        for pos in prefill..tokens.len() {
            ...
            assert!(
                worst < 1e-5,
                "quantized windowed decode at position {pos} diverged by {worst}; a \
                 single-mask port shows ~7e-3 here",
            );
        }
