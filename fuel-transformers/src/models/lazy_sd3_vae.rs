...
//! [`crate::models::lazy_sd_vae`] — see that file for op-level commentary.
...
impl SdVae3Decoder {
    ...
    pub fn decode(&self, latent: &Tensor) -> fuel_core::Result<Tensor> {
        let cfg = &self.config;
        ...
        let x = latent
            .mul_scalar(1.0 / cfg.scale_factor)
...
impl SdVae3DecoderWeights {
    pub fn load_from_mmapped(
        ...
        for si in 0..4 {
            ...
            up_blocks.push(UpBlock3Weights {
                resnets,
                upsample_conv,
            });
        }
...
fn load_resnet(
    ...
    c_in: usize,
    c_out: usize,
) -> fuel_core::Result<Resnet3Weights> {
...
fn load_f32(
    ...
    let view = st
        .get(name)
        .map_err(|e| fuel_core::Error::Msg(format!("sd3 vae load_f32 {name:?}: {e}")).bt())?;
    let bytes = view.data();
    match view.dtype() {
        Dtype::F32 => {
        ...
        other => fuel_core::bail!("sd3 vae load_f32: unsupported dtype {other:?} for {name:?}"),
    }
}
...
impl SdVae3Decoder {
    ...
    pub fn from_hub_with_config(repo_id: &str, config: SdVae3Config) -> fuel_core::Result<Self> {
