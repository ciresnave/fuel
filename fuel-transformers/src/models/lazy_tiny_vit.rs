...
//! plugs into [`crate::models::lazy_sam::SamMaskDecoder`] interchangeably with
//! the standard SAM ViT encoder output.

use crate::models::lazy_convmixer::BatchNormParams;
use crate::models::lazy_sam::SamLayerNormWeights;
...
fn layer_norm_2d(x: &Tensor, w: &LayerNorm2dWeights, c: usize, eps: f64) -> Result<Tensor> {
    crate::models::lazy_sam::layer_norm_2d(
        x,
        &SamLayerNormWeights {
...
fn apply_tiny_vit_block(x: &Tensor, w: &TinyVitBlockWeights) -> Result<Tensor> {
    ...
    if c != w.dim {
        return Err(fuel_core::Error::Msg(format!(
            "TinyViTBlock: input C={c} != block dim={}",
            w.dim,
        ))
        .bt());
    }
