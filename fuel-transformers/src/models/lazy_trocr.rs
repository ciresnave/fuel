...
use crate::models::lazy_vit::{VitConfig, VitLayerWeights, VitModel, VitWeights};
...
impl TrocrModel {
    ...
    pub fn forward_encoder(&self, image: &Tensor) -> Result<Tensor> {
        ...
        let vit = VitModel {
            config: self.encoder_config.clone(),
            weights,
        };
        vit.forward(image)
    }
...
fn apply_decoder_layer(
    ...
    let h1 = h1.layer_norm_affine(
        std::sync::Arc::clone(&w.self_attn_ln_gain),
        std::sync::Arc::clone(&w.self_attn_ln_bias),
        1e-5,
    )?;
    ...
    let h2 = h2.layer_norm_affine(
        std::sync::Arc::clone(&w.encoder_attn_ln_gain),
        std::sync::Arc::clone(&w.encoder_attn_ln_bias),
        1e-5,
    )?;
...
pub fn load_vit_weights(
    ...
    let position_embeddings =
        load_tensor_as_f32(st, &format!("{prefix}embeddings.position_embeddings"))?;
    ...
    for i in 0..cfg.num_hidden_layers {
        ...
        let (q_proj_bias, k_proj_bias, v_proj_bias) = if cfg.qkv_bias {
            (
                load_tensor_as_f32(st, &format!("{p}.attention.attention.query.bias"))
                    .ok()
                    .map(Arc::from),
                load_tensor_as_f32(st, &format!("{p}.attention.attention.key.bias"))
                    .ok()
                    .map(Arc::from),
                load_tensor_as_f32(st, &format!("{p}.attention.attention.value.bias"))
                    .ok()
                    .map(Arc::from),
            )
        } else {
        ...
        let attn_output_proj_bias =
            load_tensor_as_f32(st, &format!("{p}.attention.output.dense.bias"))?;
        ...
        let intermediate_proj_bias =
            load_tensor_as_f32(st, &format!("{p}.intermediate.dense.bias"))?;
        ...
        let mlp_output_proj_bias = load_tensor_as_f32(st, &format!("{p}.output.dense.bias"))?;
...
pub fn load_trocr_decoder_weights(
    ...
    let d = cfg.d_model;
    let kv_in = cfg.cross_attention_hidden_size;
    let embed_tokens = load_tensor_as_f32(st, &format!("{pfx}embed_tokens.weight"))?;
    let embed_positions = load_tensor_as_f32(st, &format!("{pfx}embed_positions.weight"))?;
    ...
    for i in 0..cfg.decoder_layers {
        ...
        let self_attn_ln_bias = load_tensor_as_f32(st, &format!("{p}.self_attn_layer_norm.bias"))?;
        ...
        let encoder_attn_ln_gain =
            load_tensor_as_f32(st, &format!("{p}.encoder_attn_layer_norm.weight"))?;
        let encoder_attn_ln_bias =
            load_tensor_as_f32(st, &format!("{p}.encoder_attn_layer_norm.bias"))?;
...
mod tests {
    use super::*;
    use crate::models::lazy_vit::{VitActivation, VitConfig, VitLayerWeights, VitWeights};
    ...
    fn tiny_vit_weights(cfg: &VitConfig) -> VitWeights {
        ...
        let layers: Vec<VitLayerWeights> = (0..cfg.num_hidden_layers)
            .map(|_| VitLayerWeights {
                ...
                q_proj: ws(h * h, &mut nb),
                q_proj_bias: Some(vec_of(h, &mut nb)),
                k_proj: ws(h * h, &mut nb),
                k_proj_bias: Some(vec_of(h, &mut nb)),
                v_proj: ws(h * h, &mut nb),
                v_proj_bias: Some(vec_of(h, &mut nb)),
    ...
    fn tiny_trocr_weights(dcfg: &TrocrDecoderConfig) -> TrocrDecoderWeights {
        ...
        let layers: Vec<TrocrDecoderLayerWeights> = (0..dcfg.decoder_layers)
            .map(|_| TrocrDecoderLayerWeights {
        ...
        TrocrDecoderWeights {
            ...
            embed_positions: vec_of(
                (dcfg.max_position_embeddings + dcfg.learned_pos_offset) * d,
                &mut nb,
            ),
            layers,
    ...
    mod load {
        ...
        fn build_tiny_safetensors(
            ...
            put(
                &mut map,
                &format!("{ep}embeddings.patch_embeddings.projection.weight"),
                &[h, v_cfg.num_channels, v_cfg.patch_size, v_cfg.patch_size],
                &vec_n(h * v_cfg.num_channels * v_cfg.patch_size * v_cfg.patch_size),
            );
            put(
                &mut map,
                &format!("{ep}embeddings.patch_embeddings.projection.bias"),
                &[h],
                &vec_n(h),
            );
            put(
                &mut map,
                &format!("{ep}embeddings.cls_token"),
                &[1, 1, h],
                &vec_n(h),
            );
            put(
                &mut map,
                &format!("{ep}embeddings.position_embeddings"),
                &[1, np + 1, h],
                &vec_n((np + 1) * h),
            );
            ...
            for i in 0..v_cfg.num_hidden_layers {
                ...
                for kn in &["query", "key", "value"] {
                    put(
                        &mut map,
                        &format!("{p}.attention.attention.{kn}.weight"),
                        &[h, h],
                        &vec_n(h * h),
                    );
                    if v_cfg.qkv_bias {
                        put(
                            &mut map,
                            &format!("{p}.attention.attention.{kn}.bias"),
                            &[h],
                            &vec_n(h),
                        );
                    }
                ...
                put(&mut map, &format!("{p}.output.dense.bias"), &[h], &vec_n(h));
            }
            ...
            put(
                ...
                &[d_cfg.vocab_size, d],
                &vec_n(d_cfg.vocab_size * d),
            );
            put(
                &mut map,
                &format!("{pfx}embed_positions.weight"),
            ...
            for i in 0..d_cfg.decoder_layers {
                ...
                put(
                    &mut map,
                    &format!("{p}.fc1.weight"),
                    &[d_cfg.decoder_ffn_dim, d],
                    &vec_n(d_cfg.decoder_ffn_dim * d),
                );
                put(
                    &mut map,
                    &format!("{p}.fc2.weight"),
                    &[d, d_cfg.decoder_ffn_dim],
                    &vec_n(d * d_cfg.decoder_ffn_dim),
                );
                put(
                    &mut map,
        ...
        fn round_trip_synthetic_safetensors() {
            ...
            assert!(
                model.decoder_weights.output_projection.is_none(),
