...
/// Configuration for the CodeGeeX4-9B model.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::codegeex4_9b::Config;
/// let cfg = Config::codegeex4();
...
impl Config {
    /// Return the default configuration for CodeGeeX4-9B.
    ///
    /// # Example
    ///
    /// ```
...
impl CoreAttention {
    fn new(layer_number: usize, cfg: &Config, _dtype: DType) -> Result<Self> {
...
impl SelfAttention {
    fn new(layer_number: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        Ok(Self {
            ...
            kv_cache: KvCache::new(0, cfg.seq_length),
        })
    }
    ...
    fn forward(
        ...
        if !self.multi_query_attention {
            fuel::bail!("only multi_query_attention=true is supported")
        }
        ...
        // KV cache.
        let (key_layer, value_layer) =
            self.kv_cache.append(&key_layer.contiguous()?, &value_layer.contiguous()?)?;
...
impl Module for MLP {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        xs.apply(&self.dense_h_to_4h)?
            .apply(&fuel_nn::Activation::Swiglu)?
...
#[derive(Debug, Clone)]
struct Block {
    input_layernorm: fuel_nn::LayerNorm,
    self_attention: SelfAttention,
    post_attention_layernorm: fuel_nn::LayerNorm,
    mlp: MLP,
...
struct Transformer {
    layers: Vec<Block>,
    final_layernorm: Option<fuel_nn::LayerNorm>,
    rotary_emb: RotaryEmbedding,
}
...
#[derive(Debug, Clone)]
struct Embedding {
    word_embeddings: fuel_nn::Embedding,
    fp32_residual_connection: bool,
}

impl Embedding {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let word_embeddings = fuel_nn::embedding(
...
/// The CodeGeeX4-9B language model.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::codegeex4_9b::{Config, Model};
...
impl Model {
    /// Create a new model from the given configuration and variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Reset the key-value cache for all attention layers.
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use fuel_transformers::models::codegeex4_9b::Model;
