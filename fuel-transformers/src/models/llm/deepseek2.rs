...
/// Extension trait: return non-zero element indices.
pub trait NonZeroOp {
...
/// Output of a top-k operation.
///
/// # Example
///
/// ```no_run
/// // TopKOutput is produced by TopKLastDimOp::topk or topk_unsorted.
/// use fuel_transformers::models::deepseek2::TopKLastDimOp;
/// # let tensor: fuel::Tensor = unimplemented!();
/// let out = tensor.topk(3)?;
/// let _values = out.values;
/// let _indices = out.indices;
/// # Ok::<(), fuel::Error>(())
/// ```
...
/// Extension trait: top-k along the last dimension.
...
/// Extension trait: split a tensor along a dimension.
pub trait SplitOp {
...
/// Extension trait: compute the bincount of integer values.
...
///
/// ```no_run
/// use fuel_transformers::models::deepseek2::DeepSeekV2Config;
/// // DeepSeekV2Config is typically deserialized from a JSON config file.
/// # let _cfg: DeepSeekV2Config = unimplemented!();
/// ```
#[derive(Deserialize, Clone, Debug)]
...
/// The type of scaled rotary positional embedding used by DeepSeek-V2.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::deepseek2::ScaledRopeType;
/// let rope_type = ScaledRopeType::Su;
/// ```
...
/// Precomputed rotary embedding cosine/sine cache for DeepSeek-V2.
///
/// # Example
///
/// ```no_run
/// use fuel_transformers::models::deepseek2::{DeepSeekV2RopeConfig, DeepSeekV2RotaryEmbedding};
/// use fuel::DType;
/// # let dev: fuel::Device = unimplemented!();
...
///     qk_rope_head_dim: 64,
/// };
/// let rope = DeepSeekV2RotaryEmbedding::new(&rope_cfg, DType::F32, &dev)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
...
/// RoPE scaling configuration for DeepSeek-V2 (Yarn or linear/dynamic variants).
///
/// # Example
///
/// ```no_run
/// use fuel_transformers::models::deepseek2::{DeepSeekV2RopeScaling, ScaledRopeType};
/// let scaling = DeepSeekV2RopeScaling::LinearOrDynamic {
///     scaling_type: ScaledRopeType::Linear,
///     factor: 2.0,
/// };
/// ```
#[derive(Debug, Clone, Deserialize)]
...
/// Rotary-embedding configuration for DeepSeek-V2.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::deepseek2::DeepSeekV2RopeConfig;
...
impl DeepSeekV2RotaryEmbedding {
    ...
    /// Create a new rotary embedding from a config.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn new(cfg: &DeepSeekV2RopeConfig, dtype: DType, dev: &Device) -> Result<Self> {
        match &cfg.rope_scaling {
            Some(DeepSeekV2RopeScaling::LinearOrDynamic {
                scaling_type: _,
                factor: _,
            }) => fuel::bail!("linear and dynamic rope are not implemented yet!"),
    ...
    ///
    /// ```no_run
    /// # use fuel_transformers::models::deepseek2::DeepSeekV2RotaryEmbedding;
    /// # let rope: DeepSeekV2RotaryEmbedding = unimplemented!();
    /// # let q: fuel::Tensor = unimplemented!();
    /// # let k: fuel::Tensor = unimplemented!();
    ...
    pub fn forward(
        ...
        let q_embed = fuel_nn::rotary_emb::rope_i(&q.contiguous()?, &cos, &sin)?;
        let k_embed = fuel_nn::rotary_emb::rope_i(&k.contiguous()?, &cos, &sin)?;
...
impl Attention {
    fn new(
        ...
        let q = match cfg.q_lora_rank {
            ...
            None => QProj::Plain(fuel_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.num_attention_heads * q_head_dim,
                vb.pp("q_proj"),
            )?),
        ...
        let kv_a_proj_with_mqa = fuel_nn::linear_b(
            cfg.hidden_size,
...
impl Mlp {
    fn new(
        ...
        Ok(Self {
            gate: fuel_nn::linear_no_bias(hidden_size, intermediate_size, vb.pp("gate_proj"))?,
            up: fuel_nn::linear_no_bias(hidden_size, intermediate_size, vb.pp("up_proj"))?,
            down: fuel_nn::linear_no_bias(intermediate_size, hidden_size, vb.pp("down_proj"))?,
            act: cfg.hidden_act,
        })
    }
...
impl MoeGate {
    ...
    fn forward(&self, xs: &Tensor) -> Result<(Tensor, Tensor)> {
        ...
        let scores = match self.cfg.scoring_func {
            ScoringFunc::Softmax => fuel_nn::ops::softmax_last_dim(&logits)?,
        };
...
/// DeepSeek-V2 MoE/MLA causal language model.
///
/// # Example
///
/// ```no_run
/// use fuel_transformers::models::deepseek2::{DeepSeekV2, DeepSeekV2Config};
/// # let cfg: DeepSeekV2Config = unimplemented!();
/// # let vb: fuel_nn::VarBuilder = unimplemented!();
/// let model = DeepSeekV2::new(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
impl DeepSeekV2 {
    ...
    pub fn new(cfg: &DeepSeekV2Config, vb: VarBuilder) -> Result<Self> {
        ...
        let lm_head = if !cfg.tie_word_embeddings {
            fuel_nn::linear_no_bias(cfg.hidden_size, cfg.vocab_size, vb.pp("lm_head"))?
        } else {
            fuel_nn::Linear::new(embed_tokens.embeddings().clone(), None)
        };
    ...
    /// Run a forward pass and return next-token logits.
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use fuel_transformers::models::deepseek2::DeepSeekV2;
    /// # let mut model: DeepSeekV2 = unimplemented!();
    /// # let input_ids: fuel::Tensor = unimplemented!();
    /// let logits = model.forward(&input_ids, 0)?;
    /// # Ok::<(), fuel::Error>(())
    /// ```
    pub fn forward(&mut self, input_ids: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        let (_bs, seq_len) = input_ids.dims2()?;
