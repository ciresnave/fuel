...
use fuel_nn::{
    embedding, kv_cache::KvCache, linear_b as linear, Embedding, LayerNorm, Linear, Module,
    VarBuilder,
};
...
/// Falcon configuration deserialized from HuggingFace `config.json`.
///
/// # Example
/// ```
/// use fuel_transformers::models::falcon::Config;
/// let cfg = Config::falcon7b();
/// assert_eq!(cfg.hidden_size, 4544);
...
impl Config {
    /// Validates the config, returning an error for unsupported features.
    ///
    /// Currently unsupported: `alibi`, `new_decoder_architecture`, and `n_head_kv`.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::falcon::Config;
    /// # fn main() -> fuel::Result<()> {
    /// let cfg = Config::falcon7b();
    /// cfg.validate()?; // standard config is always valid
    /// # Ok(())
    /// # }
    /// ```
    pub fn validate(&self) -> Result<()> {
        if self.alibi {
            fuel::bail!("alibi is not supported");
        }
        if self.new_decoder_architecture {
            fuel::bail!("new_decoder_architecture is not supported");
        }
        if self.n_head_kv.is_some() {
            fuel::bail!("n_head_kv is not supported");
        }
        Ok(())
    }

    /// Returns the preset configuration for Falcon-7B.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::falcon::Config;
...
impl FalconAttention {
    ...
    fn forward(&mut self, x: &Tensor, mask: Option<&Tensor>, past_kv_len: usize) -> Result<Tensor> {
        ...
        if self.use_cache {
            let (k, v) = self.kv_cache.append(&key.contiguous()?, &value.contiguous()?)?;
            key = k;
            value = v;
        }
        ...
        let attention_scores =
            fuel_nn::ops::softmax(&attention_scores.to_dtype(DType::F32)?, D::Minus1)?
                .to_dtype(x.dtype())?;
...
/// The Falcon causal language model.
///
/// # Example
/// ```no_run
...
/// let model = Falcon::load(vb, Config::falcon7b())?;
/// # Ok(())
/// # }
/// ```
#[derive(Debug, Clone)]
...
impl Falcon {
    ...
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::falcon::Falcon;
    /// # fn main() {
    /// # let model: Falcon = unimplemented!();
    ...
    /// Runs the forward pass on `input_ids` of shape `(batch, seq_len)`.
    ///
    ...
    pub fn forward(&mut self, input_ids: &Tensor) -> Result<Tensor> {
        ...
        let past_kv_len = self.blocks[0].self_attention.kv_cache.current_seq_len();
