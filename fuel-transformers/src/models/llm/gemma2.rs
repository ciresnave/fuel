...
impl Attention {
    ...
    fn forward(
        ...
        let softcapping = self.attn_logit_softcapping.map(|v| v as f32).unwrap_or(1.);
        let attn_output =
            fuel_nn::ops::sdpa(&query_states, &key_states, &value_states, attention_mask, false, scale, softcapping)?;
        attn_output
            .transpose(1, 2)?
            .reshape((b_sz, q_len, ()))?
            .apply(&self.o_proj)
...
/// Gemma 2 transformer language model.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::gemma2::{Config, Model};
...
impl Model {
    ...
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::gemma2::Model;
