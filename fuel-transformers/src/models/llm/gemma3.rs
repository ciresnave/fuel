...
use fuel_nn::{linear_b as linear, Activation, Linear, VarBuilder};
...
impl Attention {
    fn new(
        ...
        let kv_cache = if let Some(sliding_window) = sliding_window {
            KvCache::Rotating(fuel_nn::kv_cache::RotatingKvCache::new(2, sliding_window))
        } else {
...
/// Gemma 3 transformer language model.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::gemma3::{Config, Model};
...
impl Model {
    ...
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::gemma3::Model;
    ...
    pub fn forward(&mut self, input_ids: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        ...
        let (attention_mask, sliding_attention_mask) =
            self.create_attention_masks(1, seq_len, seqlen_offset)?;
    ...
    /// Clear the KV cache.
    ///
    /// Call this between independent generation requests to reset the cache.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::gemma3::Model;
