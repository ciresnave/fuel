...
use fuel_nn::{Conv1d, Conv2d, Conv2dConfig, VarBuilder};
...
impl SSCPConvBlock {
    fn new(
        ...
        let conv = fuel_nn::conv2d_no_bias(
            in_channels,
            out_channels,
...
struct SubSampleConvProjection {
    ...
    input_proj_linear: fuel_nn::Linear,
}
...
impl SubSampleConvProjection {
    fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let input_proj_linear = fuel_nn::linear_no_bias(
...
impl RelativePositionEmbedding {
    fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let pos_proj = fuel_nn::linear_no_bias(
            channels,
            num_heads * head_dim,
...
impl ConformerAttention {
    fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let q_proj = fuel_nn::linear_no_bias(hidden_size, num_heads * head_dim, attn_vb.pp("q_proj"))?;
        let k_proj = fuel_nn::linear_no_bias(hidden_size, num_heads * head_dim, attn_vb.pp("k_proj"))?;
        let v_proj = fuel_nn::linear_no_bias(hidden_size, num_heads * head_dim, attn_vb.pp("v_proj"))?;
        let post = fuel_nn::linear_no_bias(hidden_size, hidden_size, attn_vb.pp("post"))?;
    ...
    fn forward(&self, x: &Tensor, mask: &Tensor) -> Result<Tensor> {
        ...
        let probabilities =
            fuel_nn::ops::softmax_last_dim(&masked_logits.to_dtype(DType::F32)?)?;
...
struct ConformerFeedForward {
    ...
    ffw_layer_1: fuel_nn::Linear,
    ffw_layer_2: fuel_nn::Linear,
...
impl ConformerFeedForward {
    fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            ...
            ffw_layer_1: fuel_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.hidden_size * 4,
                vb.pp("ffw_layer_1"),
            )?,
            ffw_layer_2: fuel_nn::linear_no_bias(
                cfg.hidden_size * 4,
                cfg.hidden_size,
                vb.pp("ffw_layer_2"),
            )?,
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let x = fuel_nn::ops::silu(&self.ffw_layer_1.forward(&x)?)?;
        let x = self
            .ffw_layer_2
            .forward(&x)?
            .clamp(-self.gradient_clipping, self.gradient_clipping)?;
...
struct ConformerLightConv1d {
    ...
    conv_norm: RmsNorm,
    linear_start: fuel_nn::Linear,
    linear_end: fuel_nn::Linear,
...
impl ConformerLightConv1d {
    fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        Ok(Self {
            pre_layer_norm: RmsNorm::new(cfg.hidden_size, cfg.rms_norm_eps, vb.pp("pre_layer_norm"))?,
            linear_start: fuel_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.hidden_size * 2,
                vb.pp("linear_start"),
            )?,
            depthwise_conv1d: fuel_nn::conv1d_no_bias(
                cfg.hidden_size,
                cfg.hidden_size,
            ...
            linear_end: fuel_nn::linear_no_bias(
                cfg.hidden_size,
                cfg.hidden_size,
                vb.pp("linear_end"),
            )?,
    ...
    fn forward(&self, audio_encodings: &Tensor) -> Result<Tensor> {
        ...
        let x = (x1 * fuel_nn::ops::sigmoid(&x2)?)?;
...
/// Conformer-based audio encoder for Gemma 4.
#[derive(Debug, Clone)]
pub struct AudioModel {
    subsample_conv_projection: SubSampleConvProjection,
    conformer: Vec<ConformerBlock>,
    conf_reduction_factor: usize,
    output_proj: Option<fuel_nn::Linear>,
}

impl AudioModel {
    /// Build the audio model from the given config.
    pub fn new(cfg: &Gemma4AudioConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let output_proj = if let Some(output_dim) = cfg.output_proj_dims {
            Some(fuel_nn::linear(cfg.hidden_size, output_dim, vb.pp("output_proj"))?)
        } else {
            None
        };
        Ok(Self {
    ...
    /// Encode a mel-spectrogram tensor; returns `(encodings, mask)`.
    pub fn forward(
        &self,
        audio_mel: &Tensor,
