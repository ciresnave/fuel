...
//!
//! and following the fuel gemma3.rs patterns.

use std::collections::HashMap;
use std::sync::Arc;

use fuel::{DType, Device, Module, Result, Tensor, D};
use fuel_nn::{linear_b as linear_bias, Activation, Linear, VarBuilder};
...
#[derive(Debug, Clone)]
enum KvCache {
    Normal(fuel_nn::kv_cache::KvCache),
    Rotating(fuel_nn::kv_cache::RotatingKvCache),
}
...
impl Attention {
    ...
    fn new(
        ...
        let kv_cache = if is_sliding {
            KvCache::Rotating(fuel_nn::kv_cache::RotatingKvCache::new(
                2,
                cfg.effective_sliding_window(),
            ))
        } else {
            KvCache::Normal(fuel_nn::kv_cache::KvCache::new(
                2,
    ...
    fn forward(
        ...
        let k = crate::utils::repeat_kv(k, self.num_kv_groups)?;
        let v = crate::utils::repeat_kv(v, self.num_kv_groups)?;
...
/// Gemma 4 causal language model (decoder only).
#[derive(Debug, Clone)]
pub struct TextModel {
    ...
    masks: HashMap<(usize, usize), (Tensor, Tensor)>,
}

impl TextModel {
    /// Build the text model from the given config.
    ...
    fn create_attention_masks(
        ...
        let key = (seq_len, seqlen_offset);
        if let Some((mask, sliding_mask)) = self.masks.get(&key) {
            return Ok((Some(mask.clone()), Some(sliding_mask.clone())));
        }
        ...
        self.masks
            .insert(key, (mask.clone(), sliding_mask.clone()));
        Ok((Some(mask), Some(sliding_mask)))
    }

    /// Scaled token embedding lookup.
    pub fn embed_tokens(&self, input_ids: &Tensor) -> Result<Tensor> {
        let xs = self.embed_tokens.forward(input_ids)?;
        xs * (self.hidden_size as f64).sqrt()
    }

    /// Run the text forward pass; returns logits for the last token.
    ...
    /// Run the forward pass from pre-computed embeddings.
    ...
    /// Reset the KV cache in all decoder layers.
