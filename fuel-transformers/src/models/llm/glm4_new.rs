...
use fuel_nn::{kv_cache::KvCache, Activation, VarBuilder};
use std::sync::Arc;

/// Configuration for the GLM-4 (new architecture) model.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::glm4_new::Config;
...
impl RotaryEmbedding {
    ...
    pub(crate) fn apply(&self, xs: &Tensor, offset: usize) -> Result<Tensor> {
        ...
        let xs_rot = fuel_nn::rotary_emb::rope_i(&xs_rot, &cos, &sin).unwrap();
...
/// The GLM-4 base transformer model.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::glm4_new::{Config, Model};
...
impl Model {
    /// Create a new GLM-4 base model from config and variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let embed_tokens =
            fuel_nn::embedding(cfg.vocab_size, cfg.hidden_size, vb.pp("model.embed_tokens"))?;
    ...
    /// Run the base transformer forward pass and return hidden states.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// The GLM-4 model with a causal language modelling head.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::glm4_new::{Config, ModelForCausalLM};
/// # use fuel_nn::VarBuilder;
/// # let cfg: Config = unimplemented!();
/// # let vb: VarBuilder = unimplemented!();
...
impl ModelForCausalLM {
    /// Create a new causal LM model from config and variable store.
    ///
    /// # Example
    ///
    /// ```no_run
