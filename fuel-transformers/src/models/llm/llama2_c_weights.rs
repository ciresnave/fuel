...
use fuel::{DType, Device, IndexOp, Result, Shape, Tensor};
use fuel_nn::VarBuilder;

use super::llama2_c::Config;

/// Raw transformer weight tensors for the llama2.c binary format.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::llama2_c_weights::TransformerWeights;
/// # use fuel_transformers::models::llama2_c::Config;
/// # use fuel::Device;
/// # use std::io::Cursor;
...
impl Config {
    /// Read a `Config` from a llama2.c binary file.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// # let data: Vec<u8> = unimplemented!();
    /// let mut reader = Cursor::new(data);
    /// let cfg = Config::from_reader(&mut reader)?;
    /// # Ok::<(), fuel::Error>(())
    /// ```
    ...
    /// Return the size of each attention head.
    ///
    /// # Example
    ///
    /// ```
...
impl TransformerWeights {
    /// Read `TransformerWeights` from a llama2.c binary file.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Build a `VarBuilder` backed by these weight tensors.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn var_builder(&self, cfg: &Config, device: &Device) -> Result<VarBuilder<'static>> {
        ...
        let tr = device.is_cpu() && !fuel::utils::has_mkl();
