...
/// Mamba-2 model configuration, deserialized from a HuggingFace `config.json`.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::mamba2::Config;
/// let cfg = Config { d_model: 512, n_layer: 24, vocab_size: 50280,
///     d_state: 128, expand: 2, headdim: 64, ngroups: 1, pad_vocab_size_multiple: 16 };
...
/// Mutable recurrent state for Mamba-2 autoregressive inference.
///
/// Holds per-layer SSM hidden states (`hs`) and convolution ring buffers
/// (`conv_states`), plus a position counter incremented on each forward call.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::mamba2::{Config, State};
...
impl State {
    /// Allocate a zeroed `State` for `batch_size` sequences.
    ///
    /// # Example
    ///
    /// ```
...
/// Single Mamba-2 SSM layer: input projection, depthwise conv-1d, SSD recurrence, and output projection.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::mamba2::{Config, Mamba2Block};
...
impl Mamba2Block {
    /// Construct a `Mamba2Block` for the given `layer_idx`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn new(layer_idx: usize, cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let norm = fuel_nn::rms_norm(d_inner, 1e-5, vb.pp("norm"))?;
    ...
    /// Single-token autoregressive forward pass; updates `state` in-place.
    ///
    /// `xs` must have shape `(batch, d_model)`. Returns output with the same shape.
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use fuel_transformers::models::mamba2::{Config, Mamba2Block, State};
    ...
    pub fn forward(&self, xs: &Tensor, state: &mut State) -> Result<Tensor> {
        ...
        let xbc_conv = fuel_nn::ops::silu(&xbc_conv)?;
    ...
    /// Chunked parallel prefill; updates `state` in-place and returns the full-sequence output.
    ///
    /// `xs` has shape `(batch, seq_len, d_model)`. The sequence is split into
    /// non-overlapping chunks of `chunk_size` tokens processed via the SSD algorithm.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// A Mamba-2 residual block: pre-RMS-norm followed by a [`Mamba2Block`] mixer with a skip connection.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::mamba2::{Config, ResidualBlock};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
impl ResidualBlock {
    /// Construct a `ResidualBlock` for the given `layer_idx`.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// Full Mamba-2 causal language model: token embedding, stacked [`ResidualBlock`]s,
/// a final RMS-norm, and a weight-tied LM head.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::mamba2::{Config, Model};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
impl Model {
    /// Construct a `Model` from the given configuration and variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let embedding = fuel_nn::embedding(cfg.vocab_size(), cfg.d_model, vb.pp("embeddings"))?;
    ...
    /// Single-token autoregressive forward pass; updates `state` and returns logits.
    ///
    /// `input_ids` must have shape `(batch, 1)`. Returns logits `(batch, vocab_size)`.
    ///
    /// # Example
    ///
    /// ```no_run
    /// # use fuel_transformers::models::mamba2::{Config, Model, State};
    /// # use fuel::{Device, DType, Tensor};
    /// # use fuel_nn::VarBuilder;
    /// # let vb: VarBuilder = unimplemented!();
    /// # let cfg: Config = unimplemented!();
    ...
    /// Chunked parallel prefill; updates `state` and returns per-token logits.
    ///
