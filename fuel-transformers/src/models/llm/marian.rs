...
/// Marian NMT model configuration (matches HuggingFace `config.json`).
///
/// # Example
///
/// ```
/// use fuel_transformers::models::marian::Config;
...
impl Config {
    ...
    /// ```
    /// use fuel_transformers::models::marian::Config;
    /// let cfg = Config::opus_mt_tc_big_fr_en();
    /// assert_eq!(cfg.d_model, 1024);
    /// ```
    pub fn opus_mt_tc_big_fr_en() -> Self {
        Self {
            activation_function: fuel_nn::Activation::Relu,
            d_model: 1024,
    ...
    pub fn opus_mt_fr_en() -> Self {
        Self {
            activation_function: fuel_nn::Activation::Swish,
            d_model: 512,
...
/// The Marian encoder: embeds source tokens and stacks `encoder_layers` self-attention blocks.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::marian::{Config, MTModel};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::opus_mt_fr_en();
/// let mut model = MTModel::new(&cfg, vb)?;
/// let _encoder = model.encoder();
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
pub struct Encoder {
...
impl Encoder {
    ...
    /// Encodes `xs` (token ids of shape `(batch, src_len)`) into contextualised hidden states.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Resets the KV cache of every encoder layer.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// The Marian decoder: attends over both target tokens and encoder output.
///
/// # Example
///
/// ```no_run
...
impl Decoder {
    ...
    /// Decodes `xs` (target token ids) conditioned on `encoder_xs`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Resets the KV cache of every decoder layer.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// The complete Marian machine-translation model with an LM head.
///
/// Wraps a shared encoder-decoder with sinusoidal positional embeddings and
/// a final linear projection onto the target vocabulary.
///
/// # Example
///
/// ```no_run
...
impl MTModel {
    /// Constructs the `MTModel` from a HuggingFace-style [`VarBuilder`].
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Runs a full decoding step: decoder forward pass followed by the LM head.
    ///
    /// Returns logits of shape `(batch, tgt_len, target_vocab_size)`.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Resets the KV cache in both encoder and decoder.
    ///
    /// # Example
    ///
    /// ```no_run
