...
/// MixFormer (Phi-1 / Phi-1.5 / Phi-2-preview) model configuration.
///
/// Deserializable from a HuggingFace-style `config.json`. Use one of the
/// preset constructors (e.g. [`Config::v1_5`]) for the standard checkpoints.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::mixformer::Config;
/// let cfg = Config::v1_5();
/// ```
...
impl Config {
    /// Return the configuration for the Phi-1 (v1) model.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// Return the configuration for the Phi-1.5 (v1.5) model.
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::mixformer::Config;
    /// let _cfg = Config::v1_5();
    /// ```
    pub fn v1_5() -> Self {
        Self {
            vocab_size: 51200,
    ...
    /// Return the configuration for the Phi-2 preview (v2) model.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// Return the configuration for the Puffin-Phi-v2 fine-tune.
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::mixformer::Config;
    /// let _cfg = Config::puffin_phi_v2();
    /// ```
    ...
    /// Return the configuration for the Phi-Hermes-1.3B fine-tune.
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::mixformer::Config;
    /// let _cfg = Config::phi_hermes_1_3b();
    /// ```
...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb_qkv(
        ...
        if three != 3 {
            fuel::bail!("unexpected shape for qkv {:?}", qkv.shape())
        }
        ...
        let q_rot = fuel_nn::rotary_emb::rope_thd(&q_rot, &c, &s)?;
        let k_rot = fuel_nn::rotary_emb::rope_thd(&k_rot, &c, &s)?;
...
#[derive(Debug, Clone)]
struct CausalLMHead {
    ln: fuel_nn::LayerNorm,
    linear: Linear,
}

impl CausalLMHead {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        let ln = fuel_nn::layer_norm(cfg.n_embd, cfg.layer_norm_epsilon, vb.pp("ln"))?;
...
impl MHA {
    ...
    fn forward(&mut self, xs: &Tensor, mask: Option<&Tensor>) -> Result<Tensor> {
        ...
        let seqlen_offset = self.kv_cache.k_cache().current_seq_len();
        ...
        let attn_weights = {
            ...
            fuel_nn::ops::softmax_last_dim(&attn_weights)?
        };
...
/// MixFormer sequential causal language model (Phi-1 / Phi-1.5 family).
///
/// A decoder-only transformer with parallel attention + MLP blocks and
/// rotary position embeddings. Use [`new_v2`](Self::new_v2) for Phi-2
/// checkpoints and [`new`](Self::new) for Phi-1/Phi-1.5 checkpoints.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::mixformer::{Config, MixFormerSequentialForCausalLM};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::v1_5();
/// let model = MixFormerSequentialForCausalLM::new(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
...
impl MixFormerSequentialForCausalLM {
    /// Construct the model for a Phi-2 (`v2`) checkpoint.
    ///
    /// Uses the `transformer.` prefix layout found in the HuggingFace Phi-2 weights.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// # let vb: VarBuilder = unimplemented!();
    /// let cfg = Config::v2();
    /// let model = MixFormerSequentialForCausalLM::new_v2(&cfg, vb)?;
    /// # Ok::<(), fuel::Error>(())
    /// ```
    ...
    /// Construct the model for a Phi-1 / Phi-1.5 (`v1` / `v1_5`) checkpoint.
    ///
    /// Uses the flat `layers.` prefix layout found in the original Phi weights.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run a multimodal forward pass interleaving a BOS token, image embeddings, and text tokens.
    ///
    /// Sequence order follows the Moondream convention:
    /// `<bos token> <image embeddings> <text tokens>`.
    /// Returns logits of shape `(batch, vocab_size)` for the last position.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Clear the KV cache across all transformer blocks.
    ///
    /// # Example
    ///
    /// ```no_run
