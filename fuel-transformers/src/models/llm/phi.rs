...
//! - QK normalization
//!
//! - ⚡ [Interactive Wasm Example](https://huggingface.co/spaces/radames/Fuel-phi1-phi2-wasm-demo)
...
/// Phi-2 model configuration deserializable from a HuggingFace `config.json`.
///
/// See <https://huggingface.co/microsoft/phi-2/blob/main/configuration_phi.py>.
...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb(&self, xs: &Tensor, seqlen_offset: usize) -> Result<Tensor> {
        ...
        let xs_rot = fuel_nn::rotary_emb::rope(&xs_rot, &c, &s)?;
...
impl Attention {
    ...
    fn forward(&mut self, xs: &Tensor, mask: Option<&Tensor>) -> Result<Tensor> {
        ...
        let attn_weights =
            fuel_nn::ops::softmax_last_dim(&attn_weights)?.to_dtype(value_states.dtype())?;
...
/// Phi-2 causal language model.
///
/// A decoder-only transformer using parallel attention + MLP blocks and
/// rotary position embeddings. Compatible with the `microsoft/phi-2` checkpoint.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::phi::{Config, Model};
...
impl Model {
    /// Construct a new Phi-2 model, loading weights from `vb`.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::phi::{Config, Model};
    ...
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::phi::Model;
