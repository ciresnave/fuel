...
//! See: <https://github.com/huggingface/transformers/blob/b109257f4fb8b1166e7c53cc5418632014ed53a5/src/transformers/models/recurrent_gemma/modeling_recurrent_gemma.py#L2>
//!
use fuel::{DType, Device, IndexOp, Module, Result, Tensor, D};
use fuel_nn::{kv_cache::KvCache, linear_b as linear, Linear, VarBuilder};
use std::collections::HashMap;
use std::sync::Arc;

/// Specifies whether a transformer layer uses recurrent or attention blocks.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::recurrent_gemma::TemporalBlockType;
/// let t = TemporalBlockType::Attention;
/// ```
...
/// Configuration for the Recurrent Gemma model.
///
/// # Example
///
/// ```no_run
/// # let cfg: fuel_transformers::models::recurrent_gemma::Config = unimplemented!(); // Typically loaded from JSON
/// ```
...
pub struct Config {
    ...
    pub hidden_activation: fuel_nn::Activation,
...
impl RotaryEmbedding {
    pub(crate) fn new(dtype: DType, cfg: &Config, dev: &Device) -> Result<Self> {
        if cfg.partial_rotary_factor != 0.5 {
            fuel::bail!("partial-rotary-factor {} <> 0.5", cfg.partial_rotary_factor)
        }
...
impl Rglru {
    ...
    pub(crate) fn forward(&mut self, xs: &Tensor, pos: usize) -> Result<Tensor> {
        ...
        let input_gate = fuel_nn::ops::sigmoid(&input_gate)?;
        let res = baddbmm(
        ...
        let recurrent_gate = fuel_nn::ops::sigmoid(&recurrent_gate)?;
    ...
    pub fn forward(&mut self, xs: &Tensor, pos: usize) -> Result<Tensor> {
        ...
        let x_branch = if pos == 0 {
            ...
            let conv_state = match self.conv1d_state.as_ref() {
                None => fuel::bail!("empty cache despite pos > 0"),
...
/// The Recurrent Gemma language model.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::recurrent_gemma::{Config, Model};
...
impl Model {
    /// Create a new Recurrent Gemma model from config and variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run the forward pass and return logits for the last sequence position.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn forward(&mut self, xs: &Tensor, pos: usize) -> Result<Tensor> {
        ...
        let attention_mask = if seq_len <= 1 {
            ...
            let mask = self.prepare_decoder_attention_mask(seq_len, pos)?;
            Some(mask)
        };
