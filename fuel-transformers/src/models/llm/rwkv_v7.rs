...
use fuel_nn::{embedding, Embedding, VarBuilder};
...
///
/// ```
/// use fuel_transformers::models::rwkv_v7::ModelVersion;
/// let v = ModelVersion::V7;
/// ```
...
///
/// ```no_run
/// # let cfg: fuel_transformers::models::rwkv_v7::Config = unimplemented!(); // Typically loaded from JSON
/// ```
...
fn layer_norm(xs: &Tensor, weight: &Tensor, bias: &Tensor, eps: f64) -> Result<Tensor> {
    ...
    let dim = xs_f32.dim(fuel::D::Minus1)?;
    let mean = (xs_f32.sum_keepdim(fuel::D::Minus1)? / dim as f64)?;
    let centered = xs_f32.broadcast_sub(&mean)?;
    let var = (centered.sqr()?.sum_keepdim(fuel::D::Minus1)? / dim as f64)?;
    ...
    fn forward(
        ...
        let (v, v_first) = if self.layer_id == 0 {
            ...
            if let (Some(v0), Some(v1), Some(v2)) = (&self.v0, &self.v1, &self.v2) {
                let gate = fuel_nn::ops::sigmoid(&(v0 + mm!(mm!(xv, v1), v2))?)?;
        ...
        let a = fuel_nn::ops::sigmoid(&(&self.a0 + mm!(mm!(xa, &self.a1), &self.a2))?)?;

        // 6. Gate: g = sigmoid(xg @ g1) @ g2
        let g = mm!(fuel_nn::ops::sigmoid(&mm!(xg, &self.g1))?, &self.g2);
...
impl DeaAttention {
    ...
    fn forward(&self, x: &Tensor, dea_state: &mut DeaState, token_ids: &[u32]) -> Result<Tensor> {
        ...
        let k_full = if seq_len > 1 {
            ...
            // Note: Fuel doesn't support scalar - tensor directly, use neg + scalar
...
/// RWKV v7 language model.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::rwkv_v7::{Config, Model};
...
impl Model {
    /// Load the RWKV v7 model weights from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    ///
    /// ```no_run
    /// # use fuel_transformers::models::rwkv_v7::{Config, Model, State};
    ...
    pub fn forward_seq(&self, token_ids: &[u32], state: &mut State) -> Result<Tensor> {
        if token_ids.is_empty() {
            fuel::bail!("token_ids cannot be empty");
        }
        ...
        last_logits.ok_or_else(|| fuel::Error::Msg("No tokens processed".to_string()))
    }
}
