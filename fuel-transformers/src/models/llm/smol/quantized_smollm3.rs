...
/// GGUF model configuration for quantised SmolLM3.
#[derive(Debug, Clone)]
...
impl QuantizedConfig {
    ...
    pub fn from_gguf(ct: &gguf_file::Content) -> Result<Self> {
        ...
        let get_u32 = |key: &str| -> Result<usize> {
            metadata
                ...
                .map(|v| v as usize)
                .ok_or_else(|| {
                    fuel::Error::Msg(format!("Missing or invalid metadata key: {}", key))
                })
        };
    ...
    /// Whether this layer index should skip RoPE (no-rope-layer interval).
    ...
    /// Return the per-head dimension derived from the RoPE dimension count.
...
impl RmsNorm {
    ...
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let hidden_size = x.dim(fuel::D::Minus1)?;
        let x = x.to_dtype(internal_dtype)?;
        let norm_x = (x.sqr()?.sum_keepdim(fuel::D::Minus1)? / hidden_size as f64)?;
...
/// Pre-computed RoPE sin/cos tables for GGUF-quantised SmolLM3.
#[derive(Debug, Clone)]
...
impl RotaryEmbedding {
    /// Pre-compute RoPE sin/cos tables from the given config.
    ...
    /// Apply RoPE to query and key tensors at the given sequence offset.
...
impl QuantizedAttention {
    fn new(
        ...
        use fuel::quantized::{GgmlDType, QTensor};
    ...
    fn forward(&mut self, x: &Tensor, mask: Option<&Tensor>, offset: usize) -> Result<Tensor> {
        ...
        // can remove this continguous call if using ConcatKV-Cache https://github.com/huggingface/fuel/pull/3143
        ...
        attn_weights = fuel_nn::ops::softmax_last_dim(&attn_weights)?;
...
/// GGUF-quantised SmolLM3 causal language model.
#[derive(Debug, Clone)]
...
impl QuantizedModelForCausalLM {
    /// Load a GGUF-quantised SmolLM3 model from a file path.
    ...
    /// Reset the KV cache across all decoder layers.
