...
use fuel_nn::{layer_norm, Conv2dConfig, LayerNorm, VarBuilder};
use serde::Deserialize;

/// Configuration for the BLIP vision encoder.
///
/// # Example
///
/// ```
...
/// Top-level BLIP model configuration.
///
/// # Example
///
/// ```
...
impl Config {
    /// Return the config for BLIP `large` image-captioning.
    ///
    /// # Example
    ///
    /// ```
...
/// BLIP vision encoder (patch-embedding + transformer).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::blip::{BlipForConditionalGeneration, Config};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::image_captioning_large();
/// let model = BlipForConditionalGeneration::new(&cfg, vb)?;
/// let _vision = model.vision_model();
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
...
/// BLIP image-captioning model (vision encoder + text decoder).
///
/// # Example
///
/// ```no_run
...
impl BlipForConditionalGeneration {
    /// Create the BLIP model from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// # let vb: VarBuilder = unimplemented!();
    /// let cfg = Config::image_captioning_large();
    /// let mut model = BlipForConditionalGeneration::new(&cfg, vb)?;
    /// let _decoder = model.text_decoder();
    /// # Ok::<(), fuel::Error>(())
    /// ```
    ...
    /// Clear the KV cache in the text decoder.
    ///
    /// # Example
    ///
    /// ```no_run
