...
//!
//! - ⚡ [Interactive Wasm Example](https://huggingface.co/spaces/radames/Fuel-BLIP-Image-Captioning)
...
/// Configuration for the BLIP text encoder / decoder.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::blip::Config;
/// let cfg = Config::image_captioning_large();
/// assert_eq!(cfg.text_config.vocab_size, 30524);
/// ```
    ...
    fn forward(
        ...
        let (key, value) = match encoder_hidden_states {
            None => {
                ...
                let (key, value) =
                    self.kv_cache.append(&key.contiguous()?, &value.contiguous()?)?;
                (key, value)
            }
            Some(xs) => {
                let key = self.transpose_for_scores(&self.key.forward(xs)?)?;
...
impl TextLayer {
    ...
    fn forward(
        ...
        let attention_output = match &mut self.cross_attention {
            ...
            None => fuel::bail!("expected some cross-attn"),
        };
...
/// Pooler that extracts the first-token representation.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::blip_text::{Config, TextPooler};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// let pooler = TextPooler::new(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
...
impl TextPooler {
    /// Load a `TextPooler` from a variable store.
    ///
    /// # Example
    ///
    /// ```no_run
...
struct TextPredictionHeadTransform {
    dense: Linear,
    transform_act_fn: fuel_nn::Activation,
    layer_norm: LayerNorm,
}
...
/// BLIP causal language-model head (encoder–decoder text model).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::blip_text::{Config, TextLMHeadModel};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// let model = TextLMHeadModel::new(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
...
impl TextLMHeadModel {
    ...
    /// Clear the KV cache (for restarting generation).
    ///
    /// # Example
    ///
    /// ```no_run
