...
use fuel::{DType, Device, IndexOp, Module, Result, Tensor};
use fuel_nn as nn;
...
/// Configuration for the Chinese CLIP text encoder (BERT-based).
///
/// # Example
/// ```
/// use fuel_transformers::models::chinese_clip::text_model::ChineseClipTextConfig;
/// let config = ChineseClipTextConfig::clip_vit_base_patch16();
/// assert_eq!(config.vocab_size, 21128);
/// ```
#[derive(Clone, Debug)]
...
impl ChineseClipTextConfig {
    ...
    /// Returns the Chinese CLIP ViT-B/16 text encoder configuration (vocab_size = 21128).
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::chinese_clip::text_model::ChineseClipTextConfig;
    /// let cfg = ChineseClipTextConfig::clip_vit_base_patch16();
...
/// Word, position, and token-type embeddings for the Chinese CLIP text encoder.
#[derive(Clone, Debug)]
...
impl ChineseClipTextEmbeddings {
    /// Creates word, position, and token-type embedding tables from weights in `var`.
    ///
    /// # Example
    /// ```no_run
    /// use fuel::Device;
...
impl ChineseClipTextSelfAttention {
    ...
    fn forward(&self, hidden_states: &Tensor, attention_mask: &Tensor) -> Result<Tensor> {
        ...
        let attention_probs = {
            ...
            nn::ops::softmax(&attention_scores, fuel::D::Minus1)?
        };
...
/// BERT-style text encoder that produces pooled `[CLS]` representations for Chinese CLIP.
#[derive(Clone, Debug)]
...
impl ChineseClipTextTransformer {
    /// Creates a new `ChineseClipTextTransformer`, loading embeddings, encoder layers, and optional pooler.
    ///
    /// # Example
    /// ```no_run
    /// use fuel::Device;
    ...
    /// Encodes `input_ids` and returns pooled `[CLS]` token representations.
    ///
    /// `token_type_ids` and `attention_mask` are optional and default to zeros/ones respectively.
    ///
    /// # Example
    /// ```no_run
    /// # fn main() -> fuel::Result<()> {
