...
//! - 💻 [HF Transformers](<https://github.com/huggingface/transformers/blob/5af7d41e49bbfc8319f462eb45253dcb3863dfb7/src/transformers/models/chinese_clip/modeling_chinese_clip.py>)

use fuel::{Context, DType, IndexOp, Module, Result, Shape, Tensor, D};
use fuel_nn as nn;

use super::{Activation, EncoderConfig};

/// Configuration for the Chinese CLIP vision encoder (ViT-based).
///
/// # Example
/// ```
/// use fuel_transformers::models::chinese_clip::vision_model::ChineseClipVisionConfig;
...
impl ChineseClipVisionConfig {
    ...
    /// Returns the Chinese CLIP ViT-B/16 vision encoder configuration.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::chinese_clip::vision_model::ChineseClipVisionConfig;
    /// let cfg = ChineseClipVisionConfig::clip_vit_base_patch16();
...
/// Patch + position + CLS embeddings for the Chinese CLIP vision transformer.
#[derive(Clone, Debug)]
...
impl ChineseClipVisionEmbeddings {
    /// Creates patch-embedding convolution and positional embedding table from weights in `var`.
    ///
    /// # Example
    /// ```no_run
    /// use fuel::Device;
...
/// Stacked transformer layers for the Chinese CLIP vision encoder.
#[derive(Clone, Debug)]
...
impl ChineseClipVisionEncoder {
    /// Creates the encoder by stacking `config.num_hidden_layers()` transformer layers.
    ///
    /// # Example
    /// ```no_run
    /// use fuel::Device;
    ...
    /// Passes `xs` through all encoder layers; returns the output of the final layer.
    ...
    // required by LLaVA
    /// Passes `xs` through all encoder layers and returns every intermediate hidden state.
    ///
    /// Used by LLaVA-style multimodal models that need intermediate vision features.
...
/// Full Chinese CLIP vision transformer: patch embeddings → pre-LN → encoder → post-LN.
#[derive(Clone, Debug)]
...
impl ChineseClipVisionTransformer {
    /// Creates a new `ChineseClipVisionTransformer` loading weights from `var`.
    ///
    /// # Example
    /// ```no_run
    /// use fuel::Device;
    ...
    // required by LLaVA
    /// Returns every intermediate hidden state plus the final pooled `[CLS]` output.
    ///
    /// Used by LLaVA-style models that need intermediate patch features.
