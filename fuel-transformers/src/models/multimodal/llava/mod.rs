...
use fuel::{bail, Context, Device, IndexOp, Result, Tensor};
use fuel_nn::{seq, Activation, Module, Sequential, VarBuilder};
...
/// Identity passthrough module (returns its input unchanged).
...
/// Multimodal projection head that maps vision features to language embedding space.
pub struct MMProjector {
    pub modules: Sequential,
}

impl MMProjector {
    /// Load the MM projector weights according to the projector type in `config`.
    ...
    /// Project vision features through the MLP.
    pub fn forward(&self, x: &Tensor) -> Result<Tensor> {
        self.modules.forward(x)
    }
}

/// CLIP-based vision tower that extracts patch features from images.
...
impl ClipVisionTower {
    /// Build the vision tower, selecting the CLIP hidden state at `select_layer`.
    pub fn new(
        vb: VarBuilder,
        select_layer: isize,
    ...
    /// Encode an image and return the selected hidden states.
    pub fn forward(&self, x: &Tensor) -> Result<Tensor> {
    ...
    /// Return the number of image patches along one spatial dimension.
    pub fn num_patches_per_side(&self) -> usize {
        self.config.image_size / self.config.patch_size
    }
}

/// Full LLaVA model combining CLIP vision tower and Llama language model.
pub struct LLaVA {
...
impl LLaVA {
    /// Load the full LLaVA model weights from a `VarBuilder`.
    pub fn load(
        vb: VarBuilder,
        config: &LLaVAConfig,
    ...
    /// Encode a batch of images through the vision tower and MM projector.
    ...
    /// Merge image embeddings into the token embedding sequence.
    pub fn prepare_inputs_labels_for_multimodal(
        ...
        // can easily be replaced by nonzero if it is implemented in fuel
        ...
        // can be replace by split if it is implemented in fuel
    ...
    /// Run the language model forward pass from input embeddings.
    pub fn forward(
        &self,
        input_embeds: &Tensor,
        position_id: usize,
