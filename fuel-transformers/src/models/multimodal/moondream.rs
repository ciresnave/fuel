...
//!
//! # Now you can run Moondream from the `fuel-examples` crate:
...
/// Configuration for a Moondream multimodal model.
...
impl Config {
    /// Return the Moondream-2 model configuration.
    pub fn v2() -> Self {
        Self {
...
/// Configuration for the Moondream vision encoder.
...
pub struct VisionConfig {
    ...
    pub(crate) act: fuel_nn::Activation,
}

impl VisionConfig {
    /// Return the Moondream-2 vision encoder configuration.
    pub fn v2() -> Self {
        Self {
            ...
            num_blocks: 27,
            num_heads: 16,
            act: fuel_nn::Activation::GeluPytorchTanh,
        }
    }
}
...
/// The vision transformer backbone for Moondream.
#[derive(Debug, Clone)]
pub struct Encoder {
...
/// Vision encoder combining the ViT backbone with a projection head.
#[derive(Debug, Clone)]
...
impl VisionEncoder {
    /// Create a new vision encoder from config and variable store.
...
/// The Moondream multimodal model (vision + language).
#[derive(Debug, Clone)]
pub struct Model {
...
impl Model {
    /// Create a new Moondream model from config and variable store.
    ...
    /// Return a mutable reference to the language model.
