...
/// Configuration for an OpenCLIP text encoder.
#[derive(Debug, Clone)]
pub struct Config {
...
impl Config {
    /// ViT-B/32 text encoder configuration from the OpenCLIP ViT-B-32 checkpoint.
...
/// Transformer encoder stack for the OpenCLIP text model.
#[derive(Clone, Debug)]
pub struct Encoder {
...
impl Encoder {
    /// Load the encoder stack from a `VarBuilder` and config.
    ...
    /// Run the encoder forward pass, returning contextualized hidden states.
...
impl OpenClipTextTransformer {
    /// Load the text transformer from a `VarBuilder` and config.
    pub fn new(vs: VarBuilder, c: &Config) -> Result<Self> {
    ...
    /// Run a forward pass returning the full sequence hidden states.
