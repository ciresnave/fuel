...
impl Default for VisionConfig {
    /// Returns the default vision encoder configuration for PaddleOCR-VL.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::paddleocr_vl::config::VisionConfig;
...
impl Default for Config {
    /// Returns the default combined PaddleOCR-VL model configuration.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::paddleocr_vl::config::Config;
...
impl From<&Config> for TextConfig {
    /// Creates a [`TextConfig`] from the top-level [`Config`], extracting all LLM fields.
    ///
    /// The `mrope_section` is taken from `rope_scaling.mrope_section` if present,
    /// falling back to the default `[16, 24, 24]`.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::paddleocr_vl::config::{Config, TextConfig};
    /// let cfg = Config::default();
    /// let text_cfg = TextConfig::from(&cfg);
    /// assert_eq!(text_cfg.vocab_size, cfg.vocab_size);
    /// assert_eq!(text_cfg.mrope_section, vec![16, 24, 24]);
    /// ```
