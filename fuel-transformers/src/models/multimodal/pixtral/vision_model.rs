...
use fuel_nn::{linear_b, rms_norm, Linear, RmsNorm, VarBuilder};

fn default_act() -> fuel_nn::Activation {
    fuel_nn::Activation::Silu
}
...
/// Configuration for the Pixtral ViT vision encoder.
...
impl Config {
    /// Returns the preset config for the Pixtral-12B-2409 vision encoder.
...
impl RotaryEmbedding {
    ...
    fn apply_rotary_emb_qkv(
        ...
        let q_embed = fuel_nn::rotary_emb::rope(q, cos, sin)?;
        let k_embed = fuel_nn::rotary_emb::rope(k, cos, sin)?;
        Ok((q_embed, k_embed))
    }
}

/// Pixtral patch-based vision transformer model.
#[derive(Debug, Clone)]
pub struct Model {
    patch_conv: fuel_nn::Conv2d,
    ln_pre: RmsNorm,
...
impl Model {
    ...
    pub fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let patch_conv = fuel_nn::conv2d_no_bias(
            cfg.num_channels,
            cfg.hidden_size,
            cfg.patch_size,
            conv2d_cfg,
        ...
        let ln_pre = fuel_nn::rms_norm(cfg.hidden_size, 1e-5, vb.pp("ln_pre"))?;
    ...
    /// Compute flat position IDs for an h×w patch meshgrid.
