...
/// Full Qwen3-VL multimodal model (text + vision).
...
impl Qwen3VLModel {
    /// Build the full Qwen3-VL model from config.
    ...
    /// Run the full multimodal forward pass; optionally inject vision embeddings.
    ...
    pub fn forward(
        ...
        if let Some(pixel_values) = &pixel_values {
            let Some(image_grid_thw_ref) = image_grid_thw.as_ref() else {
                fuel::bail!("pixel_values require image_grid_thw");
            };
        ...
        if let Some(pixel_values_videos) = &pixel_values_videos {
            let Some(video_grid_thw_ref) = video_grid_thw.as_ref() else {
                fuel::bail!("pixel_values_videos require video_grid_thw");
            };
