...
/// Cached cosine/sine tables for rotary positional embeddings.
#[derive(Debug, Clone)]
...
impl RotaryEmbedding {
    /// Build RoPE tables for the given head dimension and max sequence length.
    pub fn new(
        base: f32,
        head_dim: usize,
    ...
    pub fn forward(
        ...
        let rope = fuel_nn::rotary_emb::rope;
...
impl Attention {
    ...
    fn forward(
        ...
        let k = crate::utils::repeat_kv(k, self.n_kv_groups)?;
        let v = crate::utils::repeat_kv(v, self.n_kv_groups)?;
...
/// Single transformer decoder layer for Qwen3-VL.
...
/// Qwen3-VL text decoder (causal language model).
...
impl Qwen3VLTextModel {
    /// Build the text decoder from config.
    ...
    /// Run the decoder from pre-computed embeddings; optionally inject DeepStack visual features.
