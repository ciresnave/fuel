...
/// Qwen3-VL vision encoder (patch embedding + transformer blocks + merger).
...
impl Qwen3VLVisionModel {
    /// Build the vision encoder from config.
    ...
    /// Encode visual patches; returns `(merged_features, deepstack_features_per_layer)`.
