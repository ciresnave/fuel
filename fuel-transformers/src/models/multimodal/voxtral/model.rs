...
/// Configuration for the Voxtral audio encoder (Whisper-compatible).
#[derive(Debug, Clone)]
...
/// Combined configuration for the Voxtral audio-language model.
#[derive(Debug, Clone)]
...
/// Generation-time configuration and optional KV cache for Voxtral.
#[derive(Debug, Clone)]
...
impl VoxtralGenerationConfig {
    /// Create a generation config with default parameters for the given device.
...
impl VoxtralCache {
    ...
    /// Reset the audio cache state (does not reset the LLM KV cache).
...
pub fn find_audio_token_positions(
    ...
    let input_ids = if input_ids.dtype() == fuel::DType::U32 {
        input_ids.to_dtype(fuel::DType::I64)?
    } else {
        input_ids.clone()
    };
...
impl Module for VoxtralAttention {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let attn_weights = fuel_nn::ops::softmax_last_dim(&scores)?;
...
impl VoxtralEncoderLayer {
    fn new(cfg: &VoxtralEncoderConfig, vb: VarBuilder) -> Result<Self> {
        ...
        let activation = match cfg.activation_function.as_str() {
            "gelu" => fuel_nn::Activation::Gelu,
            "relu" => fuel_nn::Activation::Relu,
            _ => fuel::bail!(
...
/// Voxtral Whisper-compatible audio encoder (CNN + transformer stack).
#[derive(Debug, Clone)]
...
impl VoxtralEncoder {
    /// Load the audio encoder from a `VarBuilder` and encoder config.
    ...
    /// Run inference-mode forward pass on mel-spectrogram features.
    ...
    pub fn forward_with_training(&self, input_features: &Tensor, training: bool) -> Result<Tensor> {
        ...
        let x = if false {
            ...
            let x_f32 = x.to_dtype(fuel::DType::F32)?;
            let positions_f32 = positions.to_dtype(fuel::DType::F32)?;
...
/// Linear projection bridge connecting the audio encoder to the language model.
#[derive(Debug, Clone)]
...
impl VoxtralMultiModalProjector {
    /// Load the projector from a `VarBuilder` and Voxtral config.
    ...
    /// Project audio features into the text embedding space.
...
/// Full Voxtral speech-language model combining the audio tower with a causal LLM.
#[derive(Debug, Clone)]
...
impl VoxtralForConditionalGeneration {
    /// Load the full Voxtral model from a `VarBuilder` and config.
    pub fn new(cfg: &VoxtralConfig, vb: VarBuilder) -> Result<Self> {
    ...
    pub fn get_audio_embeds(&self, input_features: &Tensor) -> Result<Tensor> {
        ...
        if total_elements % self.audio_config.intermediate_size != 0 {
            return Err(fuel::Error::DimOutOfRange {
                shape: fuel::Shape::from_dims(&[batch_size, seq_len, hidden_size]),
                dim: 0,
                op: "reshape",
            });
        }
    ...
    pub fn generate(
        ...
        if let Some(p) = config.top_p {
            if !(0.0..=1.0).contains(&p) {
                fuel::bail!("top_p must be between 0 and 1, got {}", p);
            }
        }
        ...
        for idx in 0..config.max_new_tokens {
            ...
            let next_token = if config.temperature > 0.0 {
                ...
                let prs = fuel_nn::ops::softmax_last_dim(&prs)?;
                ...
                let argmax_result = match logits.argmax(D::Minus1) {
                    Ok(result) => result,
                    Err(e) => {
                        return Err(fuel::Error::Msg(format!("Argmax failed: {e}")));
                    }
                };
                ...
                if argmax_result.dims().is_empty() {
                    ...
                    match argmax_result.to_scalar::<u32>() {
                        Ok(token) => token,
                        Err(e) => {
                            return Err(fuel::Error::Msg(format!("to_scalar failed: {e}")));
                        }
                    }
