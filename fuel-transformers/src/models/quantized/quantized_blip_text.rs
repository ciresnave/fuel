...
/// Pooler layer that projects the first-token hidden state for classification.
///
/// # Example
///
/// ```no_run
/// use fuel_transformers::models::quantized_blip_text::{TextPooler, Config};
/// use fuel_transformers::models::quantized_blip_text::VarBuilder;
...
impl TextPooler {
    /// Load the pooler from a `VarBuilder` and config.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// Quantised BLIP text LM head combining a BERT-style encoder with a masked-LM prediction head.
///
/// # Example
///
/// ```no_run
/// use fuel_transformers::models::quantized_blip_text::{TextLMHeadModel, Config};
/// use fuel_transformers::models::quantized_blip_text::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
...
impl TextLMHeadModel {
    /// Load the text LM head from a `VarBuilder` and config.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run the text decoder over token ids conditioned on encoder hidden states; returns vocabulary logits.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Reset the KV cache and sequence position counter.
    ///
    /// # Example
    ///
    /// ```no_run
