...
/// Maximum supported sequence length for GGUF-quantised Gemma-3 inference (128 K context).
pub const MAX_SEQ_LEN: usize = 131072; // Gemma 3 supports 128K context window
/// Default sliding-window attention type index for Gemma-3.
pub const DEFAULT_SLIDING_WINDOW_TYPE: usize = 6;
/// Default global RoPE base frequency for Gemma-3.
pub const DEFAULT_ROPE_FREQUENCY: f32 = 1_000_000.;
/// Default sliding-window RoPE base frequency for Gemma-3.
pub const DEFAULT_ROPE_FREQUENCY_SLIDING: f32 = 10_000.;
/// Default RoPE frequency scale factor for Gemma-3.
...
impl Module for Mlp {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let silu = fuel_nn::ops::silu(&gate)?;
        let gated = (silu * up)?;
...
/// GGUF-quantised Gemma-3 model weights for inference.
///
/// # Example
///
/// ```no_run
...
impl ModelWeights {
    /// Load quantised Gemma-3 weights from a GGUF file.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::quantized_gemma3::ModelWeights;
    ...
    pub fn from_gguf<R: std::io::Seek + std::io::Read>(
        ...
        let md_get = |s: &str| {
            ...
            match ct.metadata.get(&key) {
                None => fuel::bail!("cannot find {key} in metadata"),
                Some(v) => Ok(v),
            }
        };
