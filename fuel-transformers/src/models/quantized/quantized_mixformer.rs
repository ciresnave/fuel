...
/// Quantised MixFormer causal language model for text (and vision-language) generation.
///
/// # Example
///
/// ```no_run
/// use fuel_transformers::models::quantized_mixformer::{MixFormerSequentialForCausalLM, Config};
...
impl MixFormerSequentialForCausalLM {
    /// Load the model from a `VarBuilder` using the v2 (Phi-2) checkpoint layout.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Load the model from a `VarBuilder` using the v1 checkpoint layout.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run a text-only forward pass; returns next-token logits for the last position.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run a vision-language forward pass prepending BOS and image embeddings to the token sequence.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Reset the KV cache in all transformer blocks.
    ///
    /// # Example
    ///
    /// ```no_run
