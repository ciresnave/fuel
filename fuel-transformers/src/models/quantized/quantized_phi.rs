...
use fuel_nn::{Embedding, LayerNorm};

/// Maximum supported sequence length for GGUF-quantised Phi-2 inference.
...
impl QLinear {
    fn new<R: std::io::Read + std::io::Seek>(
        ...
        let inner = fuel::quantized::QMatMul::from_qtensor(w)?;
...
impl LayerWeights {
    fn apply_rotary_emb(&self, xs: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        let xs_rot = fuel_nn::rotary_emb::rope(&xs_rot.contiguous()?, &cos, &sin)?;
...
/// GGUF-quantised Phi-2 model weights for inference.
#[derive(Debug, Clone)]
...
impl ModelWeights {
    /// Load quantised Phi-2 weights from a GGUF file.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::quantized_phi::ModelWeights;
    /// # use fuel::Device;
    /// # fn main() -> fuel::Result<()> {
