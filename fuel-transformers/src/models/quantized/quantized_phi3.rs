...
use fuel_nn::{kv_cache::KvCache, Embedding, RmsNorm};
...
impl LayerWeights {
    fn apply_rotary_emb(&self, xs: &Tensor, index_pos: usize) -> Result<Tensor> {
        ...
        fuel_nn::rotary_emb::rope(&xs.contiguous()?, &cos, &sin)
    }
...
/// GGUF-quantised Phi-3 model weights for inference.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::quantized_phi3::ModelWeights;
/// # use fuel::Device;
/// # fn main() -> fuel::Result<()> {
/// # let ct: fuel::quantized::gguf_file::Content = unimplemented!();
/// # let mut reader: std::fs::File = unimplemented!();
/// let model = ModelWeights::from_gguf(false, ct, &mut reader, &Device::cpu())?;
/// # Ok(())
/// # }
/// ```
#[derive(Debug, Clone)]
...
impl ModelWeights {
    /// Load quantised Phi-3 weights from a GGUF file.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::quantized_phi3::ModelWeights;
