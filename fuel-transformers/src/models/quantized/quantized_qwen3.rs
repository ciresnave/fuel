...
use fuel::quantized::{gguf_file, QTensor};
use fuel::{DType, Device, Result, Tensor};
use fuel_nn::{kv_cache::ConcatKvCache, Activation, Embedding, Module};
use std::io::{Read, Seek};
use std::sync::Arc;

/// GGUF file handle exposing typed accessor methods for tensors and metadata.
///
/// # Example
///
/// ```no_run
/// use fuel_transformers::models::quantized_qwen3::Gguf;
/// use fuel::Device;
...
impl<R: Read + Seek> Gguf<R> {
    /// Wrap a GGUF file content with a reader and target device.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Load a quantised matrix-multiply layer by tensor name.
    ///
    /// # Example
    ///
    /// ```no_run
    /// use fuel_transformers::models::quantized_qwen3::Gguf;
    /// use std::fs::File;
    /// # let mut gg: Gguf<File> = unimplemented!();
    /// let lm_head = gg.qmatmul("output.weight")?;
    /// # Ok::<_, fuel::Error>(())
    /// ```
    ...
    /// Load an RMS normalisation layer by tensor name.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// use std::fs::File;
    /// # let mut gg: Gguf<File> = unimplemented!();
    /// let norm = gg.rms_norm("output_norm.weight", 1e-6)?;
    /// # Ok::<_, fuel::Error>(())
    /// ```
    ...
    /// Return the full GGUF metadata map.
    ///
    /// # Example
    ///
    /// ```no_run
    /// use fuel_transformers::models::quantized_qwen3::Gguf;
    /// use std::fs::File;
    /// # let gg: Gguf<File> = unimplemented!();
    /// let heads = gg.metadata().get("qwen3.attention.head_count");
    /// ```
    ...
    /// use std::fs::File;
    /// # let mut gg: Gguf<File> = unimplemented!();
    /// let t = gg.tensor("token_embd.weight")?;
    /// # Ok::<_, fuel::Error>(())
    /// ```
...
/// Pre-computed RoPE sin/cos tables for GGUF-quantised Qwen3.
///
/// # Example
///
/// ```no_run
...
impl RotaryEmbedding {
    /// Pre-compute RoPE sin/cos tables for the given head dimension and context length.
    ///
    /// # Example
    ///
    /// ```no_run
...
impl AttentionWeights {
    ...
    fn forward(&mut self, x: &Tensor, attn_mask: Option<&Tensor>, offset: usize) -> Result<Tensor> {
        ...
        let probs = fuel_nn::ops::softmax_last_dim(&scores)?;
...
/// GGUF-quantised Qwen3 model weights for inference.
///
/// # Example
///
/// ```no_run
...
impl ModelWeights {
    /// Load quantised Qwen3 weights from an open GGUF file.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    pub fn from_gguf<R: Read + Seek>(
        ...
        let md_get = |s: &str| match gg.metadata().get(s) {
            None => fuel::bail!("cannot find {s} in metadata"),
            Some(v) => Ok(v),
        };
    ...
    /// Run a forward pass; returns next-token logits for the last position.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Reset the KV cache across all layers.
    ///
    /// # Example
    ///
    /// ```no_run
