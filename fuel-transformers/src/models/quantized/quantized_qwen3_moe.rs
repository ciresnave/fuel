...
use fuel_nn::kv_cache::ConcatKvCache;
use fuel_nn::Linear;
...
/// Quantised multi-head grouped-query attention with KV cache for Qwen3-MoE.
///
/// # Example
///
/// ```no_run
...
impl QuantizedAttention {
    /// Construct from a `Gguf` loader, reading weights in `prefix.*` namespace.
    ///
    /// # Example
    ///
    /// ```no_run
    ...
    /// Run one attention step, returning the output tensor.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// GGUF-quantised Qwen3-MoE causal language model.
///
/// # Example
///
/// ```no_run
...
impl GGUFQWenMoE {
    /// Load quantised Qwen3-MoE weights from an open GGUF file.
    ///
    /// # Example
    ///
    /// ```no_run
