...
//!
use fuel::Result;
use fuel_nn::{batch_norm, Conv2dConfig, VarBuilder};
...
fn conv2d_same(
    ...
    c: Conv2dConfig,
    vb: VarBuilder,
) -> Result<fuel_nn::Func<'static>> {
    let conv2d = fuel_nn::conv2d(i, o, k, c, vb)?;
    let s = c.stride;
    let module = fuel_nn::func(move |xs| {
        let ih = xs.dim(2)?;
        let iw = xs.dim(3)?;
...
fn block(dim: usize, kernel_size: usize, vb: VarBuilder) -> Result<fuel_nn::Func<'static>> {
    ...
    let conv2 = fuel_nn::conv2d(dim, dim, 1, Default::default(), vb.pp(1))?;
    let bn2 = batch_norm(dim, 1e-5, vb.pp(3))?;
    Ok(fuel_nn::func(move |xs| {
...
/// Load a ConvMixer-1536/20 model.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::convmixer;
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let func = convmixer::c1536_20(1000, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
pub fn c1536_20(nclasses: usize, vb: VarBuilder) -> Result<fuel_nn::Func<'static>> {
    convmixer(nclasses, 1536, 20, 9, 7, vb)
}

/// Load a ConvMixer-1024/20 model.
///
/// # Example
///
/// ```no_run
...
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let func = convmixer::c1024_20(1000, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
pub fn c1024_20(nclasses: usize, vb: VarBuilder) -> Result<fuel_nn::Func<'static>> {
