...
//!
//! This fuel implementation uses a pre-trained ConvNeXt network for inference. The
...
use fuel::shape::ShapeWithOneHole;
use fuel::{Result, D};
use fuel_nn::{conv2d, layer_norm, linear, Conv2dConfig, Func, VarBuilder};

/// ConvNeXt model configuration.
///
/// Specifies the number of residual blocks per stage and the channel widths.
/// Use the preset constructors (e.g. [`Config::tiny`]) to obtain standard
/// ImageNet-pretrained configurations.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::convnext::Config;
/// let cfg = Config::tiny();
/// ```
#[derive(Clone)]
pub struct Config {
...
impl Config {
    ...
    /// ConvNeXt-Femto (5.2 M params).
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::convnext::Config;
    /// let cfg = Config::femto();
    /// ```
    pub fn femto() -> Self {
        Self {
            blocks: [2, 2, 6, 2],
    ...
    /// ConvNeXt-XLarge (350 M params).
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::convnext::Config;
    /// let cfg = Config::xlarge();
    /// ```
    pub fn xlarge() -> Self {
        Self {
            blocks: [3, 3, 27, 3],
...
/// Build a ConvNeXt model with a classification head.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::convnext::{self, Config};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::tiny();
/// let model = convnext::convnext(&cfg, 1000, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
/// Build a ConvNeXt model without the final classification layer.
///
/// Returns the feature-map tensor after global average pooling.
///
/// # Example
///
/// ```no_run
...
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::tiny();
/// let model = convnext::convnext_no_final_layer(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
