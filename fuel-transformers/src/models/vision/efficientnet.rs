...
//!
use fuel::{Context, Result, Tensor, D};
use fuel_nn as nn;
...
/// Per-stage MobileNet-style inverted-residual block configuration.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::efficientnet::MBConvConfig;
...
impl MBConvConfig {
    /// Layer configs for EfficientNet-B0 (width×1.0, depth×1.0).
    ///
    /// # Example
    ///
    /// ```
    ...
    /// Layer configs for EfficientNet-B1 (width×1.0, depth×1.1).
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::efficientnet::MBConvConfig;
    /// let configs = MBConvConfig::b1();
    /// ```
    pub fn b1() -> Vec<Self> {
        bneck_confs(1.0, 1.1)
    }
    /// Layer configs for EfficientNet-B2 (width×1.1, depth×1.2).
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::efficientnet::MBConvConfig;
    /// let configs = MBConvConfig::b2();
    /// ```
    pub fn b2() -> Vec<Self> {
        bneck_confs(1.1, 1.2)
    }
    /// Layer configs for EfficientNet-B3 (width×1.2, depth×1.4).
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::efficientnet::MBConvConfig;
    /// let configs = MBConvConfig::b3();
    /// ```
    pub fn b3() -> Vec<Self> {
        bneck_confs(1.2, 1.4)
    }
    /// Layer configs for EfficientNet-B4 (width×1.4, depth×1.8).
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::efficientnet::MBConvConfig;
    /// let configs = MBConvConfig::b4();
    /// ```
    pub fn b4() -> Vec<Self> {
        bneck_confs(1.4, 1.8)
    }
    /// Layer configs for EfficientNet-B5 (width×1.6, depth×2.2).
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::efficientnet::MBConvConfig;
    /// let configs = MBConvConfig::b5();
    /// ```
    pub fn b5() -> Vec<Self> {
        bneck_confs(1.6, 2.2)
    }
    /// Layer configs for EfficientNet-B6 (width×1.8, depth×2.6).
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::efficientnet::MBConvConfig;
    /// let configs = MBConvConfig::b6();
    /// ```
    pub fn b6() -> Vec<Self> {
        bneck_confs(1.8, 2.6)
    }
    /// Layer configs for EfficientNet-B7 (width×2.0, depth×3.1).
    ///
    /// # Example
    ///
    /// ```
    /// use fuel_transformers::models::efficientnet::MBConvConfig;
    /// let configs = MBConvConfig::b7();
    /// ```
    pub fn b7() -> Vec<Self> {
        bneck_confs(2.0, 3.1)
    }
...
///
/// Build with `EfficientNet::new(vb, MBConvConfig::b0(), nclasses)` (or any
/// B0–B7 config), then apply the [`Module`] trait on a
/// `(batch, 3, height, width)` input tensor.
///
/// # Example
///
/// ```no_run
...
/// let model = EfficientNet::new(vb, MBConvConfig::b0(), 1000)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug)]
...
impl EfficientNet {
    /// Construct an [`EfficientNet`] from a list of block configs and a weight store.
    ///
    /// # Example
    ///
    /// ```no_run
