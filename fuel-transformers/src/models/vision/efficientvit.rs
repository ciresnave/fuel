...
//! # Example Usage
//!
//! This fuel implementation uses a pre-trained EfficientViT (from Microsoft Research Asia) network for inference.
...
//!   --release -- \
//!   --image fuel-examples/examples/yolo-v8/assets/bike.jpg --which m1
//!
...
/// EfficientViT-MSRA model configuration.
#[derive(Clone)]
pub struct Config {
    channels: [usize; 3],
    blocks: [usize; 3],
...
impl Config {
    /// EfficientViT-M0 (smallest variant).
    pub fn m0() -> Self {
        Self {
            channels: [64, 128, 192],
            blocks: [1, 2, 3],
    ...
    /// EfficientViT-M5 (largest standard variant).
    pub fn m5() -> Self {
        Self {
...
/// Build an EfficientViT-MSRA model with a classification head.
pub fn efficientvit(cfg: &Config, nclasses: usize, vb: VarBuilder) -> Result<Func<'static>> {
    efficientvit_model(cfg, Some(nclasses), vb)
}

/// Build an EfficientViT-MSRA model without the final classification layer.
