...
use fuel::{Context, DType, Result, Tensor, D};
use fuel_nn::{
...
///
/// ```
/// use fuel_transformers::models::fastvit::Config;
/// let cfg = Config::t8();
/// assert_eq!(cfg.exp_ratio, 3);
/// assert_eq!(cfg.in_channels, 48);
/// ```
#[derive(serde::Serialize, serde::Deserialize, Clone, Debug)]
pub struct Config {
    /// Channel expansion ratio used in each MLP block.
    pub exp_ratio: usize,
    /// Base channel count for the first stage.
    pub in_channels: usize,
    /// Number of blocks in each of the four stages.
    pub blocks: [usize; 4],
    /// Whether stage-4 uses self-attention blocks instead of repMixer.
    pub attn: bool,
    /// Whether the large-kernel convolution in patch-embed uses an activation.
    pub lkc_use_act: bool,
}

impl Config {
    /// FastViT-T8 (training-optimised tiny, no attention).
    ///
    /// # Example
    ///
    /// ```
    ...
    /// FastViT-SA12 (with attention in last stage).
    ///
    /// # Example
    ///
    /// ```
    ...
    /// FastViT-MA36 (MobileNet-style attention, 36 blocks).
    ///
    /// # Example
    ///
    /// ```
    ...
    /// FastViT-MCI0, tuned for MobileCLIP S1.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// FastViT-MCI1, tuned for MobileCLIP S2.
    ///
    /// # Example
    ///
    /// ```
    ...
    /// FastViT-MCI2, tuned for MobileCLIP S3.
    ///
    /// # Example
    ///
    /// ```
...
/// Build a FastViT model with a classification head.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::fastvit::{fastvit, Config};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::t8();
/// let model = fastvit(&cfg, 1000, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
/// Build a FastViT model without the final classification layer.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::fastvit::{fastvit_no_final_layer, Config};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::t8();
/// let model = fastvit_no_final_layer(&cfg, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
