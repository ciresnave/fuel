...
use fuel::{Context, Module, ModuleT, Result, Tensor, D};
use fuel_nn::{conv2d_no_bias, layer_norm, Activation, Conv2dConfig, VarBuilder};
use serde::Deserialize;
use std::collections::HashMap;

/// Configuration for a SegFormer model.
// https://github.com/huggingface/transformers/blob/main/src/transformers/models/segformer/configuration_segformer.py
///
/// # Example
///
/// ```no_run
/// # let cfg: fuel_transformers::models::segformer::Config = unimplemented!(); // Typically loaded from JSON
/// ```
...
impl SegformerOverlapPatchEmbeddings {
    fn new(
        ...
        let layer_norm =
            fuel_nn::layer_norm(hidden_size, config.layer_norm_eps, vb.pp("layer_norm"))?;
        Ok(Self {
            projection,
            layer_norm,
        })
...
impl Module for SegformerEfficientSelfAttention {
    fn forward(&self, x: &Tensor) -> Result<Tensor> {
        ...
        let attention_scores = fuel_nn::ops::softmax_last_dim(&attention_scores)?;
...
struct SegformerLayer {
    layer_norm_1: fuel_nn::LayerNorm,
    attention: SegformerAttention,
    layer_norm_2: fuel_nn::LayerNorm,
    mlp: SegformerMixFFN,
}
...
struct SegformerEncoder {
    ...
    layer_norms: Vec<fuel_nn::LayerNorm>,
}
...
struct SegformerDecodeHead {
    ...
    linear_fuse: fuel_nn::Conv2d,
    batch_norm: fuel_nn::BatchNorm,
    classifier: fuel_nn::Conv2d,
}
...
impl SegformerDecodeHead {
    fn new(config: &Config, num_labels: usize, vb: VarBuilder) -> Result<Self> {
        ...
        let batch_norm = fuel_nn::batch_norm(
            config.decoder_hidden_size,
            config.layer_norm_eps,
            vb.pp("batch_norm"),
        )?;
...
/// SegFormer model with a semantic segmentation head for pixel-level classification.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::segformer::{Config, SemanticSegmentationModel};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// let model = SemanticSegmentationModel::new(&cfg, 150, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
...
impl SemanticSegmentationModel {
    /// Create a new semantic segmentation model with the given number of output labels.
    ///
    /// # Example
    ///
    /// ```no_run
...
/// SegFormer model with an image classification head for whole-image labeling.
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::segformer::{Config, ImageClassificationModel};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// # let cfg: Config = unimplemented!();
/// let model = ImageClassificationModel::new(&cfg, 1000, vb)?;
/// # Ok::<(), fuel::Error>(())
/// ```
#[derive(Debug, Clone)]
...
impl ImageClassificationModel {
    /// Create a new image classification model with the given number of output labels.
    ///
    /// # Example
    ///
    /// ```no_run
