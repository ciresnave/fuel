use fuel::{DType, IndexOp, Result, Tensor};
use fuel_nn::{LayerNorm, Module, VarBuilder, layer_norm};
...
impl PatchEmbed {
    fn new(
        ...
        let proj = fuel_nn::conv2d(in_chans, embed_dim, k_size, cfg, vb.pp("proj"))?;
...
// Ideally we would perform this operation in place but this is not supported in fuel at the
// moment. We should also investigate using f16 rather than f32.
struct Add3(usize, usize, usize, usize, usize);
impl fuel::CustomOp3 for Add3 {
    ...
    fn fwd(
        ...
        let s2 = match l2.contiguous_offsets() {
            None => fuel::bail!("input2 has to be contiguous"),
            Some((o1, o2)) => &s2[o1..o2],
        };
        let s3 = s3.as_slice::<f32>()?;
        let s3 = match l3.contiguous_offsets() {
            None => fuel::bail!("input3 has to be contiguous"),
            Some((o1, o2)) => &s3[o1..o2],
        };
...
impl Block {
    fn new(
        ...
        let mlp = super::MlpBlock::new(dim, dim * 4, fuel_nn::Activation::Gelu, vb.pp("mlp"))?;
...
/// ViT-based image encoder used in Segment Anything.
#[derive(Debug)]
...
impl ImageEncoderViT {
    ...
    /// Build the image encoder with the given ViT architecture parameters.
    pub fn new(
        img_size: usize,
        patch_size: usize,
        in_chans: usize,
        ...
        let neck_conv1 = fuel_nn::conv2d_no_bias(
            embed_dim,
            out_chans,
            1,
            Default::default(),
        ...
        let neck_conv2 = fuel_nn::conv2d_no_bias(out_chans, out_chans, 3, cfg, vb.pp("neck.2"))?;
