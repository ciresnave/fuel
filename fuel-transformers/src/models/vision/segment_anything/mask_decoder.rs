...
/// Upsampling mask prediction head for SAM.
#[derive(Debug)]
pub struct MaskDecoder {
    iou_token: fuel_nn::Embedding,
    mask_tokens: fuel_nn::Embedding,
    iou_prediction_head: MlpMaskDecoder,
    output_upscaling_conv1: fuel_nn::ConvTranspose2d,
    output_upscaling_ln: super::LayerNorm2d,
    output_upscaling_conv2: fuel_nn::ConvTranspose2d,
...
impl MaskDecoder {
    /// Build the mask decoder; `num_multimask_outputs` is typically 3.
    pub fn new(
        ...
        let iou_token = fuel_nn::embedding(1, transformer_dim, vb.pp("iou_token"))?;
        let mask_tokens =
            fuel_nn::embedding(num_mask_tokens, transformer_dim, vb.pp("mask_tokens"))?;
        let cfg = fuel_nn::ConvTranspose2dConfig {
            stride: 2,
            ..Default::default()
        };
        let output_upscaling_conv1 = fuel_nn::conv_transpose2d(
            transformer_dim,
            transformer_dim / 4,
            2,
            cfg,
        ...
        let output_upscaling_conv2 = fuel_nn::conv_transpose2d(
            transformer_dim / 4,
            transformer_dim / 8,
            2,
            cfg,
    ...
    /// Predict masks and IoU scores from image embeddings and prompt embeddings.
    pub fn forward(
        &self,
