...
/// Standard input image side length expected by SAM.
...
/// Segment Anything Model (SAM) — combines image encoder, prompt encoder, and mask decoder.
#[derive(Debug)]
pub struct Sam {
...
impl Sam {
    /// Build a SAM model using the standard ViT image encoder.
    pub fn new(
    ...
    /// Build a SAM model using the lightweight TinyViT image encoder.
    ...
    /// Reverse image normalisation (undo pixel mean/std) and clamp to [0, 255].
    ...
    /// Normalise and pad an image to `IMAGE_SIZE × IMAGE_SIZE` for the encoder.
    pub fn preprocess(&self, img: &Tensor) -> Result<Tensor> {
        ...
        if h > IMAGE_SIZE || w > IMAGE_SIZE {
            fuel::bail!("image is too large ({w}, {h}), maximum size {IMAGE_SIZE}")
        }
    ...
    /// Generate segmentation masks over all crops using a point grid.
