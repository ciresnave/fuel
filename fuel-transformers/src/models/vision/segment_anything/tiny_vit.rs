...
use fuel_nn::{Conv2dConfig, Module, VarBuilder};
...
struct Conv2dBN {
    c: fuel_nn::Conv2d,
    bn: fuel_nn::BatchNorm,
    span: tracing::Span,
}

impl Conv2dBN {
    fn new(in_: usize, out: usize, ks: usize, cfg: Conv2dConfig, vb: VarBuilder) -> Result<Self> {
        let c = fuel_nn::conv2d_no_bias(in_, out, ks, cfg, vb.pp("c"))?;
        let bn = fuel_nn::batch_norm(out, 1e-5, vb.pp("bn"))?;
...
impl Mlp {
    fn new(in_: usize, hidden: usize, vb: VarBuilder) -> Result<Self> {
        let norm = fuel_nn::layer_norm(in_, 1e-5, vb.pp("norm"))?;
...
impl Module for Attention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let attn = {
            ...
            fuel_nn::ops::softmax_last_dim(&attn)?
        };
        let attn = {
            let _enter = self.span_matmul.enter();
            attn.matmul(&v)?
...
/// TinyViT vision backbone for lightweight SAM (mobile/SAM2).
#[derive(Debug)]
pub struct TinyViT {
    patch_embed: PatchEmbed,
    layer0: ConvLayer,
    layers: Vec<BasicLayer>,
    // norm_head: fuel_nn::LayerNorm,
    // head: fuel_nn::Linear,
    neck_conv1: fuel_nn::Conv2d,
    neck_ln1: super::LayerNorm2d,
    neck_conv2: fuel_nn::Conv2d,
...
impl TinyViT {
    /// Build a TinyViT backbone with the given per-stage embed dims, depths, heads, and window sizes.
    pub fn new(
        embed_dims: &[usize],
        depths: &[usize],
        num_heads: &[usize],
        ...
        // let norm_head = fuel_nn::layer_norm(last_embed_dim, 1e-5, vb.pp("norm_head"))?;
        // let head = fuel_nn::linear(last_embed_dim, num_classes, vb.pp("head"))?;
        let neck_conv1 =
            fuel_nn::conv2d_no_bias(last_embed_dim, 256, 1, Default::default(), vb.pp("neck.0"))?;
        let neck_ln1 = super::LayerNorm2d::new(256, 1e-6, vb.pp("neck.1"))?;
        let cfg = fuel_nn::Conv2dConfig {
            padding: 1,
            ..Default::default()
        };
        let neck_conv2 = fuel_nn::conv2d_no_bias(256, 256, 3, cfg, vb.pp("neck.2"))?;
...
/// Build the 5 M-parameter TinyViT variant used by MobileSAM.
