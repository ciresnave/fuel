...
use fuel::{ModuleT, Result, Tensor};
use fuel_nn::{FuncT, VarBuilder};

/// Selects which VGG variant to build.
///
/// # Example
///
/// ```
/// use fuel_transformers::models::vgg::Models;
/// let variant = Models::Vgg16;
/// ```
...
/// A VGG convolutional neural network (VGG-13, VGG-16, or VGG-19).
///
/// # Example
///
/// ```no_run
/// # use fuel_transformers::models::vgg::{Models, Vgg};
/// # use fuel_nn::VarBuilder;
/// # let vb: VarBuilder = unimplemented!();
/// let model = Vgg::new(vb, Models::Vgg16)?;
/// # Ok::<(), fuel::Error>(())
/// ```
...
impl<'a> Vgg<'a> {
    /// Build a VGG model for the specified variant.
    ///
    /// # Example
    ///
    /// ```no_run
...
fn fully_connected(
    ...
    let dropout1 = fuel_nn::Dropout::new(0.5);
    let dropout2 = fuel_nn::Dropout::new(0.5);
    let dropout3 = fuel_nn::Dropout::new(0.5);
    Ok(FuncT::new(move |xs, train| {
        ...
        let lin3 = fuel_nn::linear(4096, num_classes, vb.pp("head.fc"))?;
...
fn get_weights_and_biases(
    ...
    Ok(fuel_nn::Linear::new(ws, Some(bs)))
}
