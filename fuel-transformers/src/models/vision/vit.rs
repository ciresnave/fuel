...
use fuel_nn::{layer_norm, LayerNorm, VarBuilder};
...
/// let cfg = Config::vit_base_patch16_224();
...
impl Config {
    /// Pre-built configuration for google/vit-base-patch16-224.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::vit::Config;
    ...
    pub fn vit_base_patch16_224() -> Self {
        Self {
            ...
            hidden_act: fuel_nn::Activation::Gelu,
    ...
    /// Pre-built configuration for microsoft/trocr-base-handwritten.
    ///
    /// Uses a 384×384 input image with `qkv_bias = false`.
    ///
    /// # Example
    /// ```
    /// use fuel_transformers::models::vit::Config;
    /// let cfg = Config::microsoft_trocr_base_handwritten();
...
impl PatchEmbeddings {
    fn new(cfg: &Config, vb: VarBuilder) -> Result<Self> {
        ...
        let conv_cfg = fuel_nn::Conv2dConfig {
            stride: patch_size,
            ..Default::default()
        };
...
/// ViT patch + position embeddings.
///
/// Projects image patches to `hidden_size`, prepends a CLS token, and adds
/// learned 1D position embeddings.
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::vit::{Config, Embeddings};
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let vb: VarBuilder = unimplemented!();
/// let cfg = Config::vit_base_patch16_224();
/// let emb = Embeddings::new(&cfg, false, vb)?;
/// # Ok(())
/// # }
/// ```
#[derive(Debug, Clone)]
...
impl Embeddings {
    /// Create embeddings, optionally with a mask token for masked autoencoding.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::vit::{Config, Embeddings};
    ...
    /// Embed `pixel_values` into patch tokens with position encoding.
    ///
    /// # Arguments
    /// * `pixel_values` - Image tensor of shape `[batch, channels, height, width]`.
    /// * `bool_masked_pos` - Optional binary mask for masked autoencoding.
    /// * `interpolate_pos_encoding` - If `true`, interpolate positional encoding
    ///   for images that differ from the training resolution (not yet implemented).
    ///
    /// # Returns
    /// Token sequence of shape `[batch, num_patches + 1, hidden_size]`
    ...
    pub fn forward(
        ...
        let embeddings = match (bool_masked_pos, &self.mask_token) {
            (None, _) => embeddings,
            (Some(_), None) => fuel::bail!("bool_masked_pos set without mask_token"),
...
impl Module for SelfAttention {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        ...
        let attention_probs = fuel_nn::ops::softmax_last_dim(&attention_scores)?;
        attention_probs
            .matmul(&value)?
            .permute((0, 2, 1, 3))?
            .contiguous()?
...
struct Intermediate {
    dense: Linear,
    intermediate_act_fn: fuel_nn::Activation,
}
...
/// # Example
/// ```no_run
/// # use fuel_transformers::models::vit::{Config, Encoder};
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let vb: VarBuilder = unimplemented!();
/// let encoder = Encoder::new(&Config::vit_base_patch16_224(), vb)?;
/// # Ok(())
/// # }
/// ```
#[derive(Debug, Clone)]
...
impl Encoder {
    /// Create a new encoder with `cfg.num_hidden_layers` layers.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::vit::{Config, Encoder};
...
/// Full ViT image-classification model (embeddings + encoder + classifier head).
///
/// # Example
/// ```no_run
/// # use fuel_transformers::models::vit::{Config, Model};
/// # use fuel_nn::VarBuilder;
/// # fn main() -> fuel::Result<()> {
/// # let vb: VarBuilder = unimplemented!();
...
impl Model {
    /// Create a new ViT classification model.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::vit::{Config, Model};
    ...
    /// Run the model and return class logits.
    ///
    /// # Arguments
    /// * `xs` - Image batch of shape `[batch, channels, height, width]`.
    ///
    /// # Returns
    /// Class logits of shape `[batch, num_labels]`, using the CLS token output.
    ///
    /// # Example
    /// ```no_run
    /// # use fuel_transformers::models::vit::{Config, Model};
