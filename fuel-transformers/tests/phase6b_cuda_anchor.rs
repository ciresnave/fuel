...
use fuel_transformers::models::lazy_qwen2_moe::{
    ExpertWeights, Qwen2MoeConfig, Qwen2MoeLayerWeights, Qwen2MoeModel, Qwen2MoeWeights,
};
use fuel_transformers::models::lazy_sd_text_encoder::{
    ClipLayerWeights, ClipTextActivation, ClipTextConfig, ClipTextWeights, SdTextEncoder,
};
use fuel_transformers::models::lazy_whisper::WhisperModel;
use fuel_transformers::models::lazy_yolov8::{YoloV8Config, YoloV8Model, YoloV8Weights};
use std::sync::Arc;
...
fn cuda_executor() -> fuel_core::CudaDevice {
    fuel_core::CudaDevice::new(0).expect("cuda device 0 should be available")
}
...
fn single_matmul_cuda_matches_reference_within_tolerance() -> Result<()> {
    ...
    let cuda_device = fuel_core::CudaDevice::new(0).expect("cuda device 0 should be available");
...
fn whisper_decoder_cuda_matches_reference() -> Result<()> {
    require_cuda();
    let cfg = fuel_transformers::models::lazy_whisper::tiny_cfg();
    let weights = fuel_transformers::models::lazy_whisper::zero_weights(&cfg);
...
fn convnext_cuda_matches_reference() -> Result<()> {
    require_cuda();
    let cfg = fuel_transformers::models::lazy_convnext::tiny_cfg();
    let weights = fuel_transformers::models::lazy_convnext::zero_weights(&cfg);
