...
WHY THIS EXISTS
    ...
    tool that did NOT live in the repo, so nobody but its author could re-run it and
    it could not be a gate's foundation. That absence was the finding. A partition
...
def parse_args(src, open_idx):
    ...
    or a '(' inside a message string. This skips string and char literals.
    """
    j = open_idx + 1
    depth = 1
    args, cur = [], []
...
def classify(name, args):
    ...
    prod = "*" in blob
    if HEADDIM.search(blob) and (HEADCOUNT.search(blob) or WIDTH.search(blob)) and prod:
...
def print_report():
    ...
    print("total production panic sites: %d" % len(sites))
    print("  of which head_dim relation (GAP-281, excluded from crate census): %d"
          % by.get("HEADDIM_RELATION", 0))
    print("  CRATE CENSUS (excl. head_dim relation): %d" % len(crate))
    print("\nSTRUCTURAL PARTITION:")
    for b, c in Counter(s[3] for s in crate).most_common():
        print("  %-18s %d" % (b, c))
    buf = [s for s in crate if s[4]]
    print("\nbuffer-length control class (X.len() == product): %d sites in %d files"
          % (len(buf), len({s[0] for s in buf})))
...
def self_test():
    ...
    # ARM A (parser robustness): a ';' and a '(' inside a message string must NOT
    # split the args or unbalance the parens. A naive matcher fails this.
    probe = 'assert_eq!(a, "b;c(d)e", f);'
    open_idx = probe.index("(")
    args = parse_args(probe, open_idx)
    arm_a = args == ["a", '"b;c(d)e"', "f"]
    print("  ARM A (string-aware parse survives ';' and '(' in a message): %s  got=%r"
          % ("PASS" if arm_a else "FAIL", args))

    # ARM B (sees the corpus): the buffer-length control class exists and is found.
    sites = census()
    buf_files = {s[0] for s in sites if s[4]}
    missing = [f for f in CONTROL_FILES if f not in buf_files]
    arm_b = not missing and len([s for s in sites if s[4]]) >= 8
    print("  ARM B (>=8 buffer-length sites, all control files present):        %s%s"
          % ("PASS" if arm_b else "FAIL",
...
def gate():
    ...
    sites = census()
    head = [s for s in sites if s[3] == "HEADDIM_RELATION"]
    if head:
